...
function OnboardingFlow(_a) {
    ...
    return (<div className="flex h-full flex-col bg-white p-4">
      {step === 1 ? (<>
          ...
          <input value={phone} onChange={function (event) { return setPhone(event.target.value.replace(/\D/g, "").slice(0, 10)); }} placeholder="98XXXXXXXX" className="mt-1 h-8 w-full rounded-md border border-gray-300 px-2.5 text-[12px]"/>
          <button type="button" disabled={phone.length < 10} onClick={function () { return setStep(2); }} className="mt-4 h-8 rounded-md bg-[#1557b0] text-[12px] font-medium text-white disabled:opacity-50">
            Continue
          </button>
        </>) : (<>
          ...
          <button type="button" onClick={function () { return onComplete(phone); }} className="mt-6 h-8 rounded-md bg-[#1557b0] text-[12px] font-medium text-white">
            Start using Khata
          </button>
        </>)}
    </div>);
