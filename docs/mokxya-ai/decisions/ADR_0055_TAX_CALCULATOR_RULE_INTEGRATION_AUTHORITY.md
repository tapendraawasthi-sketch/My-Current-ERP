...
5. Slice 2: consume builds `calculator_rule_candidate` /
   `calculator_consume_mode` (`CANDIDATE_ONLY` default for
   POLICY_DECLARED / RULE_TABLE_PENDING; `BLOCKED` for fake authority;
   `SKIP` for non-pilot). Live path forces `allow_rule_table_load=false`
   and `allow_live_calculation=false` — does **not** load rule tables,
   compute amounts, or claim production eligibility. GAP-P2-008 stays OPEN.
...
| Alternative | Why |
|-------------|-----|
| Live calculation in slice 1–2 | Authority / honesty risk |
| Live rule-table load in slice 2 | Authority / honesty risk |
...
- `docs/mokxya-ai/baselines/MAI_38_SLICE2_BASELINE_SUMMARY.md`
- `erp_bot/src/oip/modules/conversation/application/tax_calculator_rule_integration_service.py`
- `erp_bot/src/oip/modules/conversation/application/tax_calculator_rule_integration_consume_service.py`
