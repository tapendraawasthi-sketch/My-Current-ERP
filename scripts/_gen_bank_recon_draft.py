...
def w(path: str, text: str) -> None:
    ...
    b = p.read_bytes()[:4]
    assert not (len(b) > 1 and b[1] == 0), f"utf16 {path}"
    print("wrote", path, len(p.read_bytes()))

BANK = r'''"""Bank reconciliation / treasury draft lifecycle (Phase 10).
...
_IMPORT = re.compile(
    ...
    r"statement\s+import|csv\s+import|bank\s+statement"
    r")\b",
    re.I,
)
_MATCH = re.compile(
...
_CLOSE = re.compile(
    r"\b(close\s+(?:bank\s+)?reconcil|reconcil(?:iation)?\s+close)\b",
    re.I,
)
...
def detect_bank_recon_kind(text: str) -> BankReconKind | None:
    ...
    if _ADJUST.search(text):
        lower = text.lower()
        if any(k in lower for k in ("statement", "reconcil", "match line", "from statement")):
            return "bank_adjustment"
        if "bank charge" in lower or "bank interest" in lower:
            # Prefer recon when phrased as statement bank charge; else None for settlement
            if "statement" in lower or "reconcil" in lower:
                return "bank_adjustment"
...
def merge_into_draft(draft: BankReconDraft, message: str) -> BankReconDraft:
    ...
    if amt is not None:
        draft.amount = amt
    ref = _REF.search(message or "")
    if ref:
        draft.reference = ref.group(1).upper()
        if draft.reference not in draft.erp_document_ids:
            draft.erp_document_ids = list({*draft.erp_document_ids, draft.reference})
...
def to_confirmation_card(draft: BankReconDraft) -> dict[str, Any] | None:
    if draft.kind == "treasury_query":
        return {
            ...
            "raw_text": " | ".join(draft.source_messages) if draft.source_messages else "",
            "draft_id": draft.draft_id,
            "preview_hash": draft.preview_hash,
            "preview_version": draft.draft_version,
            "idempotency_key": draft.idempotency_key,
            "tags": ["phase10_treasury", "read_only", "treasury_query"],
...
def preview_message(draft: BankReconDraft) -> str:
    if draft.kind == "treasury_query":
        return "Treasury position query ready (read-only — no posting)."
...
w("erp_bot/src/khata/bank_recon_draft.py", BANK)
print("bank_recon_draft done")