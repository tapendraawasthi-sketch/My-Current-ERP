"""MAI-43 slice 1 — continuous change intelligence (never production truth)."""
...
from src.oip.contracts.continuous_change_intelligence import (
...
from src.oip.modules.conversation.application.continuous_change_intelligence_service import (
    ...
    build_continuous_change_intelligence_bundle,
)
...
def test_runtime_version() -> None:
    assert RUNTIME_VERSION == "mai-43.0.1-slice1"


def test_gazette_amendment_policy_declared() -> None:
    req = _pipeline(
    ...
    assert (
        bundle.analysis_status == ContinuousChangeIntelligenceStatus.COMPLETE
    )
    assert (
        bundle.continuous_change_readiness
        == ContinuousChangeReadiness.POLICY_DECLARED
    )
    assert "GAZETTE" in bundle.in_scope_topics
    assert "AMENDMENT" in bundle.in_scope_topics
    assert bundle.pilot_scope == "CONTINUOUS_CHANGE_CANDIDATE_ONLY"
    assert bundle.release_status == "NOT_RELEASED"
    assert bundle.gold_questions_status == "NOT_RELEASED"
    assert bundle.continuous_change_authority_claimed is False
    assert bundle.unreviewed_as_production_truth is False
    assert bundle.cache_invalidated is False
    assert bundle.rates_changed_as_truth is False
    assert bundle.change_applied is False
    assert bundle.amendment_applied is False
    assert bundle.rollback_executed is False
    ...
    assert (
        "PILOT_SCOPE_CONTINUOUS_CHANGE_CANDIDATE_ONLY" in bundle.reason_codes
    )
    assert "LEGAL_EFFECTIVE_DATES_NOT_PROVEN" in bundle.reason_codes
    assert_continuous_change_intelligence_authority(bundle)


def test_circular_effective_and_rate() -> None:
    req = _pipeline(
        "map expense accounts for circular effective date change and rate change under NFRS"
    )
    bundle = req.continuous_change_intelligence_bundle
    assert bundle is not None
    assert (
    ...
    assert "CIRCULAR" in bundle.in_scope_topics
    assert "EFFECTIVE_DATE_CHANGE" in bundle.in_scope_topics
    assert "RATE_CHANGE" in bundle.in_scope_topics
...
def test_vat_skips() -> None:
    ...
    bundle = req.continuous_change_intelligence_bundle
    assert bundle is not None
    assert bundle.analysis_status == ContinuousChangeIntelligenceStatus.SKIP
    assert bundle.change_applied is False


def test_court_without_change_skips() -> None:
    req = _pipeline(
...
def test_no_research_skips() -> None:
    req = CanonicalAIRequestV1(
        ...
        conversation_id="conv-1",
        message_id="msg-1",
        raw_text="map expense accounts for gazette amendment under NFRS",
        mode=InteractionModeV1.ASK,
        created_at=datetime.now(timezone.utc),
        trusted_scope=TrustedScopeV1(
            principal_id="user-1",
    ...
    bundle = build_continuous_change_intelligence_bundle(req)
    assert bundle.analysis_status == ContinuousChangeIntelligenceStatus.SKIP
...
def test_adapter_metadata() -> None:
    ...
    assert meta.get("continuous_change_authority_claimed") is False
    assert meta.get("unreviewed_as_production_truth") is False
    assert meta.get("cache_invalidated") is False
...
def test_frozen_eval_fixtures() -> None:
    ...
    path = (
        ...
        / "mai43"
        / "frozen"
        / "continuous_change_intelligence_v1.jsonl"
    )
