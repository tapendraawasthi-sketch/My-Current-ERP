...
def test_registry_and_honesty() -> None:
    ...
    assert reg["honesty"]["staging_conflict_attested"] is False
    assert_sync_honesty_residual_honesty()
    ...
    with pytest.raises(RuntimeError, match="STAGING_CONFLICT_FALSE_ATTESTATION"):
...
def test_artifacts_and_narrative() -> None:
    ...
    assert "VOID" in residual_text or "NOT ACCEPTED" in residual_text
...
def test_gap_and_pointer_pr_b4() -> None:
    ...
    assert obs["staging_conflict_attested"] is False
