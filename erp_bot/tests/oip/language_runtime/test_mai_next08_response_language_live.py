...
def test_pointer_next09_and_artifacts() -> None:
    ...
    assert ledger["recommended_next_step"] == "NEXT-09"
    ...
    assert matrix["recommended_next_step"] == "NEXT-09"
    ...
    assert "recommended_next_step = PR-A3 / NEXT-09" in plan

    nxt = (ROOT / "MOKXYA_AI_WHAT_MUST_BE_DONE_NEXT_V1.txt").read_text(
        encoding="utf-8"
    )
    assert "recommended_next_step = NEXT-09" in nxt
    assert "last_shipped_step = NEXT-08" in nxt
