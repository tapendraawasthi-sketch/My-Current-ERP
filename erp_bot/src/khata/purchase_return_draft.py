"""Purchase return / supplier debit-note draft lifecycle for Accountant Mode.

Structured draft + confirmation card only. Never invent VAT, cost, or
journal lines — frontend posts via postPurchaseAdjustmentTransaction.

Adapted from sales_return_draft.py for the purchase side (Phase 8).
"""
...
AdjustmentType = Literal["inventory_purchase_return", "financial_supplier_debit_note"]
...
_INVOICE_NO = re.compile(r"\b(PI-[\w-]+)\b", re.I)
...
_SUPPLIER_LEAD = re.compile(
    r"^(?:we\s+)?(?:returned|return(?:ed|ing)?)\s+.*?\bto\s+"
    r"([A-Za-z\u0900-\u097F][A-Za-z\u0900-\u097F\s&.]{1,40}?)"
    r"(?:\s+(?:invoice|and|for|against|in|on)|[.,;!?]|$)",
    re.I,
)
_SUPPLIER_FROM = re.compile(
    r"\b(?:from|for|to)\s+([A-Za-z\u0900-\u097F][A-Za-z\u0900-\u097F\s&.]{1,40}?)"
    r"(?:\s+(?:invoice|and|for|against|in|on)|$)",
    re.I,
)
_CASH_REFUND = re.compile(
    r"\b(cash\s*refund|refund(?:\s+(?:from|by)\s+(?:the\s+)?supplier)?\s+in\s+cash|"
    r"refund\s+cash|nagad\s*refund)\b",
    re.I,
)
_BANK_REFUND = re.compile(
    r"\b(bank\s*refund|refund(?:\s+from\s+supplier)?\s+(?:by|via|in)\s+bank)\b", re.I
)
_REDUCE_PAY = re.compile(
    r"\b(reduce\s+(?:the\s+)?(?:outstanding|payable|balance)|against\s+(?:outstanding|payable)|"
    r"reduce\s+payable)\b",
    re.I,
)
_SUPPLIER_CREDIT = re.compile(r"\b(supplier\s+credit|store\s+credit|credit\s+note\s+from\s+supplier|goodwill)\b", re.I)
...
_RETURN_SIGNAL = re.compile(
    r"\b(purchase\s+return|return(?:ed|ing)?\s+to\s+(?:the\s+)?supplier|"
    r"supplier\s+return|returned|returning|firta|समान\s*फिर्ता)\b",
    re.I,
)
...
_DEBIT_NOTE_SIGNAL = re.compile(r"\bdebit\s*notes?\b", re.I)
...
class PurchaseReturnDraft:
    ...
    supplier: PartyRef = field(default_factory=PartyRef)
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> PurchaseReturnDraft:
        ...
        return cls(
            ...
            supplier=PartyRef(id=supplier.get("id"), name=supplier.get("name")),
...
def is_explanation_about_return(text: str) -> bool:
    ...
    if not _EXPLANATION_Q.search(t):
        return False
    return bool(
        re.search(r"\b(return|firta|debit\s*note|vat|input\s+vat)\b", t, re.I)
    )


def is_purchase_return_message(text: str) -> bool:
    if is_explanation_about_return(text):
        return False
    if is_financial_supplier_debit_note_message(text):
        return False
    t = text or ""
    has_return = bool(
        re.search(
            r"\b(return(?:ed|ing)?|firta|purchase\s+return|supplier\s+return)\b",
            t,
            re.I,
        )
    )
    if not has_return:
        return False
    if _INVOICE_NO.search(t):
        return True
    return bool(re.search(r"\b(purchase|supplier)\b", t, re.I))

def is_financial_supplier_debit_note_message(text: str) -> bool:
    if is_explanation_about_return(text):
        return False
    t = text or ""
    if not _DEBIT_NOTE_SIGNAL.search(t):
        return False
    # Prefer debit note when no-goods / pricing-error language is present, or when
    # a debit note is stated without an inventory return verb.
...
def extract_return_fields(text: str) -> dict[str, Any]:
    """Extract known purchase-return / debit-note fields. Unknown stay absent."""
    fields: dict[str, Any] = {}
    t = text or ""

    if is_financial_supplier_debit_note_message(t):
        fields["adjustment_type"] = "financial_supplier_debit_note"
    elif is_purchase_return_message(t) or _RETURN_SIGNAL.search(t):
        fields["adjustment_type"] = "inventory_purchase_return"
    ...
    if qty_of:
        ...
        if bare_qty:
            ...
            remaining_qty = re.search(r"\bremaining\s+(\d+(?:\.\d+)?)\b", t, re.I)
            if remaining_qty:
    ...
    if item_match:
        ...
        if raw.lower() not in _GENERIC_ITEMS and len(raw) > 1:
            if raw.lower() in {"bike", "bikes"}:
                name = "Bike"
            elif re.search(r"bike", raw, re.I) and re.search(r"e2e|test", t, re.I):
                name = "E2E Test Bike"
            else:
                name = raw.title() if raw.isascii() else raw
            fields["item"] = {"name": name, "raw_name": raw}

    m = _SUPPLIER_LEAD.search(t) or _SUPPLIER_FROM.search(t)
    if m:
        name = m.group(1).strip(" .,")
        name = re.sub(r"\s+(a|an|the|rs\.?|npr|invoice)\b.*$", "", name, flags=re.I).strip()
        if name and name.lower() not in {
            ...
            fields["supplier"] = {"name": name.title() if name.isascii() else name}

    if _CASH_REFUND.search(t) or (re.search(r"\brefund\b", t, re.I) and _BARE_CASH.search(t)):
        fields["settlement_method"] = "cash_refund_received"
    elif _BANK_REFUND.search(t) or (re.search(r"\brefund\b", t, re.I) and _BARE_BANK.search(t)):
        fields["settlement_method"] = "bank_refund_received"
    elif _SUPPLIER_CREDIT.search(t):
        fields["settlement_method"] = "supplier_credit"
    elif _REDUCE_PAY.search(t):
        fields["settlement_method"] = "reduce_payable"
...
def merge_fields(draft: PurchaseReturnDraft, fields: dict[str, Any], message: str) -> PurchaseReturnDraft:
...
def compute_missing_fields(draft: PurchaseReturnDraft) -> list[str]:
    missing: list[str] = []
...
def validate_and_complete(draft: PurchaseReturnDraft) -> PurchaseReturnDraft:
    draft.ambiguous_fields = []
    draft.validation_errors = []

    if not draft.adjustment_type:
        draft.adjustment_type = "inventory_purchase_return"

    if (
        draft.adjustment_type == "financial_supplier_debit_note"
        and draft.settlement_method is None
        and draft.financial_amount is not None
        and draft.original_invoice_no
    ):
        draft.settlement_method = "reduce_payable"
...
def build_preview(draft: PurchaseReturnDraft) -> dict[str, Any]:
    ...
    if adj == "financial_supplier_debit_note":
        ...
        tags = ["financial_supplier_debit_note", "no_goods"]
        effects = [
            f"Supplier debit note draft against {draft.original_invoice_no}",
            f"Amount: NPR {_money(draft.financial_amount)}",
            f"Settlement: {draft.settlement_method}",
            "No stock movement (no goods returned)",
            "Frontend will post via postPurchaseAdjustmentTransaction",
        ]
        narration = draft.narration or f"Supplier debit note against {draft.original_invoice_no}"
    else:
        intent = "khata_purchase_return"
        tags = ["inventory_purchase_return"]
        effects = [
            f"Inventory purchase return draft against {draft.original_invoice_no}",
        ...
        narration = draft.narration or (
            f"Purchase return of {draft.quantity} {item_name} against {draft.original_invoice_no}"
        )

    preview = {
        "draft_id": draft.draft_id,
        "transaction": "Purchase Return"
        if adj == "inventory_purchase_return"
        else "Debit Note",
...
def clarification_message(draft: PurchaseReturnDraft) -> str:
    adj = draft.adjustment_type or "inventory_purchase_return"
    label = "debit note" if adj == "financial_supplier_debit_note" else "purchase return"
    ...
    if draft.supplier and draft.supplier.name:
        parts.append(f"Supplier: {draft.supplier.name}.")

    asks: list[str] = []
    if "original_invoice_no" in draft.missing_fields:
        asks.append("the original purchase invoice number (e.g. PI-…)")
    ...
    if "financial_amount" in draft.missing_fields:
        asks.append("the debit note amount")
    if "settlement_method" in draft.missing_fields:
        asks.append(
            "settlement (cash refund, bank refund, reduce outstanding payable, or supplier credit)"
        )
...
def preview_message(draft: PurchaseReturnDraft) -> str:
    p = draft.preview or {}
    adj = p.get("adjustment_type") or draft.adjustment_type
    title = (
        "**Debit note preview**"
        if adj == "financial_supplier_debit_note"
        else "**Purchase return preview**"
    )
    lines = [
        title,
        "",
    ...
    if p.get("supplier"):
        lines.append(f"Supplier: {p.get('supplier')}")
    if adj == "financial_supplier_debit_note":
...
def start_or_merge_return(
    ...
    *,
    ...
    company_id: str = "",
    user_id: str = "",
    existing: PurchaseReturnDraft | None = None,
) -> PurchaseReturnDraft:
...
def to_confirmation_card(draft: PurchaseReturnDraft) -> dict[str, Any] | None:
    if draft.status != "previewed" or not draft.preview:
        return None
    adj = draft.adjustment_type or "inventory_purchase_return"
    raw_text = " | ".join(draft.source_messages)
    # CRITICAL: frontend parses PI-… from raw_text
    ...
    if adj == "financial_supplier_debit_note":
        ...
        # Ensure purchase context so the frontend never routes to sales adjustment.
        if "debit note" not in raw_text.lower():
            raw_text = f"{raw_text} | debit note"
    else:
        intent = "khata_purchase_return"
        tags = ["inventory_purchase_return"]
        if "purchase return" not in raw_text.lower() and "supplier" not in raw_text.lower():
            raw_text = f"{raw_text} | purchase return"

    amount = float(draft.financial_amount) if draft.financial_amount is not None else 0.0
    card: dict[str, Any] = {
        "intent": intent,
        "party": draft.supplier.name if draft.supplier else None,
        "amount": amount,
...
def _store_path() -> Path:
    ...
    return path / "purchase_return_drafts.json"
...
def save_draft(draft: PurchaseReturnDraft) -> None:
    with _LOCK:
        data = _load_all()
...
def load_pending_draft(
    *,
    ...
    with _LOCK:
        ...
        if not raw:
            return None
        draft = PurchaseReturnDraft.from_dict(raw)
        if draft.status in {"posted", "cancelled"}:
            return None
        return draft


def mark_posted(draft: PurchaseReturnDraft, result: dict[str, Any]) -> PurchaseReturnDraft:
    draft.status = "posted"
