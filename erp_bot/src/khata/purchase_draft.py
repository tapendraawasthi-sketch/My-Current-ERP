"""Purchase transaction draft lifecycle for Accountant Mode.
...
_SUPPLIER = re.compile(
    r"\b(?:from|bata|sanga)\s+([A-Za-z\u0900-\u097F][A-Za-z\u0900-\u097F\s&.]{1,40}?)(?:\s+at|\s+@|\s+for|\s+in|\s+on|\s*$)",
    re.I,
)
...
class PurchaseDraft:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> PurchaseDraft:
...
def extract_purchase_fields(text: str) -> dict[str, Any]:
    ...
    if total_match:
        ...
        # Only treat bare Rs amounts as total when no qty×rate context invents money from qty
...
def compute_missing_fields(draft: PurchaseDraft) -> list[str]:
    missing: list[str] = []
    ...
    if draft.payment_method == "credit" and not (draft.supplier and draft.supplier.name):
        missing.append("supplier")
    if draft.payment_method == "bank" and not draft.payment_account:
        # Soft requirement — ask only if we have no account hint
        missing.append("payment_account")
    return missing


def validate_and_complete(draft: PurchaseDraft) -> PurchaseDraft:
        ...
        if draft.total_amount is None:
            ...
            if abs(draft.total_amount - computed) > Decimal("0.01"):
                ...
                draft.validation_errors.append(
                    f"Quantity × rate = {_money(computed)} but stated total is {_money(draft.total_amount)}."
                )
...
def build_preview(draft: PurchaseDraft) -> dict[str, Any]:
    ...
    effects = [
        f"Purchase voucher draft — {item_name}",
        f"Inventory increase: {item_name} +{draft.quantity} {draft.unit}",
        "Stock-ledger inward movement",
        "Accounting journal",
    ]
    ...
    if payment == "cash":
        ...
        effects.append("Cash decrease")
        if supplier_name:
            effects.append(f"Supplier noted: {supplier_name} (no payable)")
    elif payment == "bank":
        journal.append(
            {
                "accountCode": "KH-BANK",
        ...
        effects.append(f"Payable to {supplier_name}")

    effects.append("Audit-event draft")

    intent = "khata_cash_purchase" if payment == "cash" else (
        "khata_credit_purchase" if payment == "credit" else "khata_cash_purchase"
    )
    ...
    preview = {
        ...
        "party": supplier_name,
        "amount": float(amount),
        "narration": f"Purchase of {draft.quantity} {draft.unit} {item_name}"
...
def clarification_message(draft: PurchaseDraft) -> str:
    if draft.ambiguous_fields and "total_amount" in draft.ambiguous_fields:
        return (
            "The stated quantity × rate does not match the total amount.\n\n"
    ...
    parts.append(f"I have started a purchase entry{item_part}{qty_part}.")
    ...
    if "supplier" in draft.missing_fields:
        asks.append("the supplier name")
...
def preview_message(draft: PurchaseDraft) -> str:
    p = draft.preview or {}
    lines = [
        "**Purchase preview**",
        "",
...
def to_confirmation_card(draft: PurchaseDraft) -> dict[str, Any] | None:
...
# ── Draft store (session-scoped, file-backed for single-instance durability) ──
...
def _store_path() -> Path:
    ...
    return path / "purchase_drafts.json"
...
def save_draft(draft: PurchaseDraft) -> None:
    with _LOCK:
        data = _load_all()
...
def load_pending_draft(
    *,
    ...
    with _LOCK:
        ...
        if not raw:
            return None
        draft = PurchaseDraft.from_dict(raw)
        if draft.status in {"posted", "cancelled"}:
            return None
        return draft


def mark_posted(draft: PurchaseDraft, result: dict[str, Any]) -> PurchaseDraft:
    draft.status = "posted"
