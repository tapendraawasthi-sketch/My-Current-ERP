"""MAI-07R3P-2 — authorized one-shot frozen V3 evaluation of R3N6 candidate.

Loads mai-07.1.11-r3n6-chaincomplete explicitly. Does not promote it.
Active default must remain mai-07.1.3-r3f-sealnew.
...
from .eval_c2_helpers import extract_challenge_produced, extract_primary_produced
...
OUT = REPO / "evals/mai07/r3p_frozen_v3"
REPORTS = OUT / "reports"
...
AUTHORIZATION = "EXPLICIT_USER_AUTHORIZATION_MAI_07R3P2_FROZEN_V3_ONE_SHOT"
PHASE_ID = "MAI-07R3P-2-AUTHORIZED-FROZEN-V3-ONE-SHOT"
ATTEMPT_ID = "MAI_07R3P_FROZEN_V3_ATTEMPT_001"
...
def prove_candidate_pack_load() -> dict[str, Any]:
    ...
    _write_json(REPORTS / "MAI_07R3P_CANDIDATE_PACK_LOAD_PROOF.json", proof)
    return proof
...
def preflight(repo: Path = REPO) -> dict[str, Any]:
    ...
    _write_json(REPORTS / "MAI_07R3P_V3_ONE_SHOT_PREFLIGHT.json", out)
    return out
...
def lock_attempt(repo: Path = REPO) -> dict[str, Any]:
    ...
    lock = {
        ...
        "prediction_path": f"evals/mai07/r3p_frozen_v3/reports/{ATTEMPT_ID}_PREDICTIONS.jsonl",
...
def run_predictions(cases: list[dict[str, Any]], resources: Any) -> list[dict[str, Any]]:
    ...
    for case in cases:
        ...
        bundle = transliterate_r3n6(case["input_text"], resources=resources)
        # Prefer span-aligned produced list when possible
        ...
        # Also try analyze+attach path if empty (should be rare)
        if not produced:
            ...
            if tb is not None:
                produced, source, err = extract_challenge_produced(
                    tb, preferred=span if span and " " not in span.strip() else None, acceptable=[span] if span else []
                )
                if not produced:
                    produced, source, err = extract_primary_produced(tb)
        ranked = [
            {
                "surface": p.surface,
        ...
        out.append(
            {
                ...
                "source_surface": source or span or case["input_text"],
                "ranked": ranked,
...
def score_predictions(
    ...
    return {
        "scorer": "mai-07.r3p.canonical.1.0.0",
...
def execute_one_shot(repo: Path = REPO) -> dict[str, Any]:
    ...
    # Independent audit: recompute same metrics (same function; separate report stamp)
    audit = dict(score)
    audit["scorer"] = "mai-07.r3p.audit.1.0.0"
    audit["canonical_match"] = audit["QUALITY_GATES_PASSED"] == score["QUALITY_GATES_PASSED"]

    _write_json(REPORTS / "MAI_07R3P_V3_CANONICAL_SCORE_REPORT.json", score)
    _write_json(REPORTS / "MAI_07R3P_V3_AUDIT_SCORE_REPORT.json", audit)
