"""MAI-07R3N candidate runtime factory — explicit activation only.
...
ACTIVATION_METHOD = "explicit_load_resources_resources_dir_plus_r3n_factory"
...
def assert_active_default_immutable() -> None:
    ...
    if DEFAULT_ACTIVE is not False:
        raise RuntimeError("r3n_default_active_must_be_false")


def load_r3n_resources() -> CompactXlResources:
    ...
    if not CANDIDATE_PACK_DIR.is_dir():
        raise FileNotFoundError(f"r3n_pack_missing:{CANDIDATE_PACK_DIR}")
            ...
            out.append(
                SpanAnnotationV1(
                    ...
                    confidence=ConfidenceV1(
                        value=1.0,
                        method="r3n_structural_identifier",
...
def analyze_language_r3n(raw_text: str) -> LanguageFrameV1:
    """R3N-only analysis: split spurious phrases, then coalesce structural identifiers."""
    assert_active_default_immutable()
...
def transliterate_r3n(
    raw_text: str,
    *,
    resources: CompactXlResources | None = None,
) -> Any:
    """Full R3N candidate pipeline. Explicit only — never import-side activated."""
    assert_active_default_immutable()
    res = resources or load_r3n_resources()
    frame = analyze_language_r3n(raw_text)
