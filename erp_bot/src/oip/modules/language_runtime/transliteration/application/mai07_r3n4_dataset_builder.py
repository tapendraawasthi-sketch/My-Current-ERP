"""MAI-07R3N4 fresh-holdout evaluation dataset builder.

Firewall:
- Blocks all R3N, R3N2, AND R3N3 holdout lineage (case IDs, normalized text hashes,
  template families, skeletons). R3N3 is loaded from its split jsonl files only
  (case_id + input_text for one-way hashing) — never from reports/*predictions*.jsonl.
...
- Does not read R3N2/R3N3 prediction jsonl or reports.
...
SEED_DEVELOPMENT = 20260724
SEED_HOLDOUT = 20260725
BUILDER_VERSION = "mai-07-r3n4-dataset.1.0.0"
SCHEMA = "mai07_r3n4_fresh_holdout_case_v1"
...
UID_RE = re.compile(r"r3n4uid\d+", re.IGNORECASE)
...
def _case_id_prefix(split: SplitName) -> str:
    return {
        "DEVELOPMENT": "R3N4-DEV",
        "HOLDOUT_VALIDATION": "R3N4-HLD",
        "SAFETY_CHALLENGE": "R3N4-SAF",
        "CONTEXT_COUNTERFACTUAL": "R3N4-CFX",
        "OOV_CHALLENGE": "R3N4-OOV",
        "MONOTONIC_REGRESSION": "R3N4-MON",
        "IDENTITY_ANCHOR_CHALLENGE": "R3N4-ANC",
    }[split]


def _template_prefix(split: SplitName) -> str:
    return "r3n4_dev" if split == "DEVELOPMENT" else "r3n4_hld"
...
def _load_split_jsonl_blocked_sets(base: Path) -> dict[str, Any]:
    """Load split manifest jsonl only (case_id + input_text hashes) — never reports/predictions."""
    all_rows: list[dict[str, Any]] = []
    sources: list[str] = []
    for path in sorted(base.glob("*.jsonl")):
        if path.parent.name == "reports" or "predictions" in path.name:
            continue
        rel = path.relative_to(REPO).as_posix()
        sources.append(rel)
        all_rows.extend(_load_jsonl(path))
...
def _load_r3n2_blocked_sets() -> dict[str, Any]:
    ...
    return _load_split_jsonl_blocked_sets(R3N2_OUT)


def _load_r3n3_blocked_sets() -> dict[str, Any]:
    """Load R3N3 split jsonl only (case_id + input_text hashes) — never reports/*predictions*.jsonl."""
    return _load_split_jsonl_blocked_sets(R3N3_OUT)
...
def _case(
    *,
    ...
    if not (
        template_family.startswith(f"{prefix}_")
        or template_family.startswith("r3n4_dev_")
        or template_family.startswith("r3n4_hld_")
    ):
        raise ValueError(f"invalid_template_family:{template_family}")
    for forbidden in ("r3n_dev_", "r3n_hld_", "r3n2_dev_", "r3n2_hld_", "r3n3_dev_", "r3n3_hld_"):
    ...
    return {
        ...
        "parent_lineage_blocked": ["R3N", "R3N2", "R3N3"],
    }
...
class _CaseBuilder:
    ...
    def add(
        ...
        *,
        ...
        if split == "HOLDOUT_VALIDATION" and "FINALIZER_IDEMPOTENCE_REQUIRED" not in pops:
            pops.append("FINALIZER_IDEMPOTENCE_REQUIRED")
        case = _case(
            split=split,
...
def _load_authorized_code_corrective_cases(builder: _CaseBuilder) -> None:
    ...
    for row in sorted(_load_jsonl(R3M_PRIVATE), key=lambda r: r["source_item_id"]):
        ...
        case = builder.add(
            ...
            template_family="r3n4_dev_authorized_code_corrective",
...
def _generate_development_synthetics(builder: _CaseBuilder, target: int) -> None:
    ...
    dev_specs: list[tuple[list[str], str, str, str, str]] = [
        (["ENGLISH_IDENTITY_REQUIRED"], "english_identity", "IDENTITY_TOP1", "{term}", "please review the {term} report for r3n4uid{uid} today"),
        (["ROMANIZED_NEPALI_REQUIRED"], "romanized_nepali", "ROMANIZED_SCRIPT_AT_5", "{term}", "aaja {term} ko record hernu r3n4uid{uid}"),
        (["IDENTITY_RETENTION_REQUIRED", "EXACT_RAW_IDENTITY_REQUIRED", "EXACTLY_ONE_IDENTITY_REQUIRED"], "identity_retention", "IDENTITY_RETAINED", "{term}", "staff asked about {term} context r3n4uid{uid}"),
        (["ACRONYM_IDENTITY_REQUIRED"], "acronym", "ACRONYM_IDENTITY_TOP1", "{acr}", "please confirm {acr} code r3n4uid{uid}"),
        (["IDENTIFIER_PROTECTION_REQUIRED"], "identifier", "PROTECTED_IDENTITY", "SKU-{sku}", "lookup order SKU-{sku} in ledger r3n4uid{uid}"),
        (["PROTECTED_IDENTITY_REQUIRED"], "protected", "PROTECTED_IDENTITY", "PAN-{pan}", "verify PAN PAN-{pan} on invoice r3n4uid{uid}"),
        (["SHARED_OR_AMBIGUOUS"], "shared_ambiguous", "SHARED_CONSERVATIVE", "balance", "check balance r3n4uid{uid} in daily report"),
        (
            ["CANDIDATE_CAP_PRESSURE"],
            "cap_pressure",
            "CAP_OK",
            "ledger",
            "please review ledger voucher payment supplier customer discount commission statement reconcile opening closing export import r3n4uid{uid}",
        ),
        (["ENGLISH_GUARD_ANALOGUE"], "english_guard_analogue", "IDENTITY_TOP1", "{term}", "export the {term} summary before audit r3n4uid{uid}"),
        (["IDENTITY_INVARIANT_ANALOGUE"], "identity_invariant_analogue", "IDENTITY_RETAINED", "{term}", "optional transliteration for {term} may vary r3n4uid{uid}"),
        (["ACRONYM_IDENTIFIER_ANALOGUE"], "acronym_identifier_analogue", "ACRONYM_IDENTITY_TOP1", "{acr}", "validate {acr} column on gst worksheet r3n4uid{uid}"),
        (["MONOTONIC_PARENT_INCORRECT"], "monotonic_incorrect", "IDENTITY_TOP1", "ledger", "branch ledger: ledger entry ra aaja baaki milaaune r3n4uid{uid}"),
        (["MULTI_TOKEN_IDENTITY"], "multi_token_identity", "IDENTITY_RETAINED", "REF-{ref}-A", "protected reference REF-{ref}-A must stay intact r3n4uid{uid}"),
        (["REFINED_SPAN_IDENTITY"], "refined_span_identity", "IDENTITY_RETAINED", "ORD/{ord}-B", "track shipment ORD/{ord}-B without rewriting r3n4uid{uid}"),
        (["COALESCED_SPAN_IDENTITY"], "coalesced_span_identity", "IDENTITY_RETAINED", "SKU-{sku}-X1", "lookup coalesced order SKU-{sku}-X1 in ledger r3n4uid{uid}"),
        (["SERIALIZATION_ROUNDTRIP"], "serialization_roundtrip", "IDENTITY_RETAINED", "{term}", "confirm the {term} entry serializes cleanly r3n4uid{uid}"),
        (["UNICODE_IDENTITY"], "unicode_identity", "IDENTITY_RETAINED", "{term}", f"please review the {{term}} rep{COMBINING_ACUTE}ort for r3n4uid{{uid}} today"),
    ]
    idx = 0
    while len([c for c in builder.cases if c["split"] == split]) < target:
        ...
        ref = f"2026-{uid:05d}"
        ordid = f"{8800 + uid}/X"
        span = span_tpl.format(term=term, acr=acr, sku=sku, pan=pan, ref=ref, ord=ordid)
        text = text_tpl.format(term=term, acr=acr, sku=sku, pan=pan, ref=ref, ord=ordid, uid=f"{uid:05d}")
        ...
        builder.add(
            ...
            template_family=f"r3n4_dev_{suffix}",
        )
        idx += 1
...
def _generate_holdout_validation(builder: _CaseBuilder, target: int) -> None:
    ...
    for i in range(200):
        ...
        text = f"please review the {term} report for r3n4uid{uid:05d} today"
        builder.add(
            split=split,
            population_ids=pops,
            input_text=text,
            ...
            expected_behavior="IDENTITY_TOP1",
            template_family="r3n4_hld_english_identity",
        )
    ...
    for i in range(200):
        ...
        text = f"{particle} {term} ko record hernu r3n4uid{uid:05d}"
        builder.add(
            split=split,
            ...
            template_family="r3n4_hld_romanized_nepali",
        )

    # Core identity-retention block: satisfies IDENTITY_RETENTION_REQUIRED,
    # EXACT_RAW_IDENTITY_REQUIRED, EXACTLY_ONE_IDENTITY_REQUIRED, SERIALIZATION_ROUNDTRIP.
    for i in range(500):
        uid = builder._next_uid()
        term = _pick(seed, split, i, "retention_core", ENGLISH_TERMS)
        text = f"staff asked about {term} context r3n4uid{uid:05d}"
        builder.add(
            split=split,
            population_ids=[
            ...
            template_family="r3n4_hld_identity_retention_core",
        )

    # Cap-pressure block: also feeds identity-retention totals + invariant analogue.
    for i in range(350):
        uid = builder._next_uid()
        term = _pick(seed, split, i, "cap_pressure_core", ENGLISH_TERMS)
        text = f"please review {CAP_PRESSURE_FILLERS_EN} for {term} r3n4uid{uid:05d}"
        builder.add(
            split=split,
            population_ids=[
                "IDENTITY_RETENTION_REQUIRED",
            ...
            template_family="r3n4_hld_cap_pressure_core",
        )

    # Multi-token identifier block (letter+digit fragments joined by separators).
    for i in range(300):
        uid = builder._next_uid()
        ref = f"2026-{uid:05d}-A"
        text = f"protected reference REF-{ref} must stay intact r3n4uid{uid:05d}"
        builder.add(
            split=split,
            population_ids=["MULTI_TOKEN_IDENTITY"],
            input_text=text,
            highlighted_span=f"REF-{ref}",
            expected_behavior="IDENTITY_RETAINED",
            template_family="r3n4_hld_multi_token_identity",
        )

    # Refined-span identifier block (slash+hyphen multi-separator structural identifiers).
    for i in range(200):
        uid = builder._next_uid()
        ordid = f"{8800 + uid}/X-{i % 97}"
        text = f"track shipment ORD/{ordid} without rewriting r3n4uid{uid:05d}"
        builder.add(
            split=split,
            population_ids=["REFINED_SPAN_IDENTITY"],
            input_text=text,
            highlighted_span=f"ORD/{ordid}",
            expected_behavior="IDENTITY_RETAINED",
            template_family="r3n4_hld_refined_span_identity",
        )

    # Coalesced-span identifier block (adjacent letter+digit fragments merged).
    for i in range(200):
        uid = builder._next_uid()
        sku = 54100 + uid
        text = f"lookup coalesced order SKU-{sku}-X{i % 47} in ledger r3n4uid{uid:05d}"
        builder.add(
            split=split,
            population_ids=["COALESCED_SPAN_IDENTITY"],
            input_text=text,
            highlighted_span=f"SKU-{sku}-X{i % 47}",
            expected_behavior="IDENTITY_RETAINED",
            template_family="r3n4_hld_coalesced_span_identity",
        )

    # Unicode-adjacency block: NBSP / combining mark / emoji-adjacent / ZWJ neighbors.
    unicode_variants = (
        lambda term, uid: f"please{NBSP}review the {term} report for r3n4uid{uid:05d} today",
        lambda term, uid: f"please review the {term} report {CHECK_EMOJI} for r3n4uid{uid:05d} today",
        lambda term, uid: f"please review the {term} rep{COMBINING_ACUTE}ort for r3n4uid{uid:05d} today",
        lambda term, uid: f"please{ZWJ} review the {term} report for r3n4uid{uid:05d} today",
    )
    for i in range(150):
        uid = builder._next_uid()
        term = _pick(seed, split, i, "unicode_identity", ENGLISH_TERMS)
        text = unicode_variants[i % len(unicode_variants)](term, uid)
        builder.add(
            split=split,
            population_ids=["UNICODE_IDENTITY"],
            input_text=text,
            highlighted_span=term,
            expected_behavior="IDENTITY_RETAINED",
            template_family="r3n4_hld_unicode_identity",
        )
    ...
    for i in range(75):
        ...
        sku = 54100 + uid
        text = f"please confirm {acr} code SKU-{sku} r3n4uid{uid:05d}"
        builder.add(
            split=split,
            population_ids=[
            ...
            template_family="r3n4_hld_acronym_identifier_analogue",
        )

    for i in range(25):
        uid = builder._next_uid()
        acr = ACRONYMS[(i + 5) % len(ACRONYMS)]
        text = f"please confirm {acr} code r3n4uid{uid:05d}"
        builder.add(
            split=split,
            ...
            highlighted_span=acr,
            expected_behavior="ACRONYM_IDENTITY_TOP1",
            template_family="r3n4_hld_acronym",
        )

    for i in range(25):
        uid = builder._next_uid()
        sku = 55000 + uid
        text = f"lookup order SKU-{sku} in ledger r3n4uid{uid:05d}"
        builder.add(
            split=split,
            ...
            template_family="r3n4_hld_identifier",
        )

    for i in range(100):
        uid = builder._next_uid()
        pan = 8800000000 + uid
        text = f"verify PAN PAN-{pan} on invoice r3n4uid{uid:05d}"
        builder.add(
            split=split,
            ...
            expected_behavior="PROTECTED_IDENTITY",
            template_family="r3n4_hld_protected",
        )

    for i in range(150):
        uid = builder._next_uid()
        if i % 2 == 0:
            text = f"check balance r3n4uid{uid:05d} in daily report"
            span = "balance"
        else:
            term = builder.romanized_terms[i % len(builder.romanized_terms)]
            text = f"aaja {term} ko hisab hernu r3n4uid{uid:05d}"
            span = term
        builder.add(
            split=split,
            ...
            template_family="r3n4_hld_shared_ambiguous",
        )
    ...
    while len([c for c in builder.cases if c["split"] == split]) < target:
        ...
        builder.add(
            ...
            input_text=f"please audit the {term} worksheet r3n4uid{uid:05d} today",
            highlighted_span=term,
            expected_behavior="IDENTITY_TOP1",
            template_family="r3n4_hld_english_identity",
        )
...
def _generate_safety_challenge(builder: _CaseBuilder, target: int) -> None:
    ...
    seed = _seed_for_split(split)
    kinds = (
        ("protected", ["PROTECTED_IDENTITY_REQUIRED"], "NO_RAW_MUTATION", "REF-{ref}", "protected reference REF-{ref} must stay intact r3n4uid{uid}"),
        ("english", ["ENGLISH_IDENTITY_REQUIRED"], "IDENTITY_TOP1", "{term}", "please print the {term} copy for supplier r3n4uid{uid}"),
        ("identifier", ["IDENTIFIER_PROTECTION_REQUIRED"], "PROTECTED_IDENTITY", "ORD/{ord}", "track shipment ORD/{ord} without rewriting r3n4uid{uid}"),
        ("acronym", ["ACRONYM_IDENTITY_REQUIRED"], "ACRONYM_IDENTITY_TOP1", "{acr}", "validate {acr} column on gst worksheet r3n4uid{uid}"),
    )
    idx = 0
    while len([c for c in builder.cases if c["split"] == split]) < target:
        ...
        builder.add(
            ...
            highlighted_span=span if span in text else term,
            expected_behavior=behavior,
            template_family=f"r3n4_hld_safety_{kind}",
        )
        idx += 1
...
def _generate_counterfactuals(builder: _CaseBuilder, target: int) -> None:
    ...
    for i, tok in enumerate(words):
        pair_id = f"r3n4_cfx_pair_{i:03d}"
        uid_en = builder._next_uid()
        uid_np = builder._next_uid()
        english_text = f"please verify the {tok} total in english report r3n4uid{uid_en:05d}"
        nepali_text = f"aaja {tok} ko hisab hernu r3n4uid{uid_np:05d}"
        builder.add(
            split=split,
            ...
            expected_behavior="IDENTITY_TOP1",
            template_family="r3n4_hld_counterfactual_english",
            pair_id=pair_id,
        ...
        builder.add(
            ...
            highlighted_span=tok,
            expected_behavior="ROMANIZED_SCRIPT_AT_5",
            template_family="r3n4_hld_counterfactual_nepali",
            pair_id=pair_id,
...
def _generate_oov(builder: _CaseBuilder, target: int) -> None:
    ...
    while len([c for c in builder.cases if c["split"] == split]) < target:
        ...
        if idx % 3 == 0:
            text = f"customer asked about {morph} balance today r3n4uid{uid:05d}"
        elif idx % 3 == 1:
            text = f"aaja {morph} ko record hernu r3n4uid{uid:05d}"
        else:
            text = f"please verify {morph} export status r3n4uid{uid:05d}"
        builder.add(
            split=split,
            population_ids=["OOV"],
            input_text=text,
            ...
            template_family="r3n4_hld_oov_generalization",
        )
        idx += 1
...
def _generate_monotonic(builder: _CaseBuilder, target: int) -> None:
    ...
    for i in range(target):
        ...
        if i % 2 == 0:
            term = ENGLISH_TERMS[i % len(ENGLISH_TERMS)]
            text = f"monotonic parent english check for {term} r3n4uid{uid:05d}"
            span = term
            behavior = "IDENTITY_TOP1"
            family = "r3n4_hld_monotonic_correct"
        else:
            term = builder.romanized_terms[i % len(builder.romanized_terms)]
            text = f"aaja {term} ko monotonic parent check r3n4uid{uid:05d}"
            span = term
...
def _generate_identity_anchor_challenge(builder: _CaseBuilder, target: int) -> None:
    """Identity-anchor stress: multi-token, punctuation, NBSP, combining marks,
    emoji-adjacent, refined/coalesced identifiers, cap pressure, duplicate
    identity-like alternatives, and repeated finalization markers."""
    split: SplitName = "IDENTITY_ANCHOR_CHALLENGE"
    seed = _seed_for_split(split)
    kinds = (
        "cap_pressure_en",
        "cap_pressure_np",
        ...
        "refined_ord",
        "nbsp_adjacent",
        "combining_mark_adjacent",
        "emoji_adjacent",
        "duplicate_identity_like",
        "repeated_finalization_marker",
    )
    idx = 0
    while len([c for c in builder.cases if c["split"] == split]) < target:
        ...
        uid = builder._next_uid()
        seq = idx + 1
        if kind == "cap_pressure_en":
            term = _pick(seed, split, seq, kind, ENGLISH_TERMS)
            text = (
                f"please review {CAP_PRESSURE_FILLERS_EN} and also {CAP_PRESSURE_FILLERS_NP} "
                f"while checking {term} totals r3n4uid{uid:05d}"
            )
            span = term
            behavior = "CAP_OK" if idx % 4 == 0 else "IDENTITY_RETAINED"
        elif kind == "cap_pressure_np":
            term = _pick(seed, split, seq, kind, builder.romanized_terms)
            text = (
                f"aaja {CAP_PRESSURE_FILLERS_NP} ra {CAP_PRESSURE_FILLERS_EN} "
                f"ko hisab ma {term} hernu r3n4uid{uid:05d}"
            )
            span = term
            behavior = "IDENTITY_RETAINED"
        elif kind == "multi_token_ref":
            ref = f"2026-{uid:05d}-C"
            text = f"anchor challenge reference REF-{ref} must persist r3n4uid{uid:05d}"
            span = f"REF-{ref}"
            behavior = "IDENTITY_RETAINED"
        elif kind == "coalesced_sku":
            sku = 56000 + uid
            text = f"anchor challenge coalesced order SKU-{sku}-Y{idx % 31} in ledger r3n4uid{uid:05d}"
            span = f"SKU-{sku}-Y{idx % 31}"
            behavior = "IDENTITY_RETAINED"
        elif kind == "refined_ord":
            ordid = f"{8900 + uid}/Z-{idx % 89}"
            text = f"anchor challenge shipment ORD/{ordid} without rewriting r3n4uid{uid:05d}"
            span = f"ORD/{ordid}"
            behavior = "IDENTITY_RETAINED"
        elif kind == "nbsp_adjacent":
            term = _pick(seed, split, seq, kind, ENGLISH_TERMS)
            text = f"please{NBSP}confirm the {term} anchor stays intact r3n4uid{uid:05d}"
            span = term
            behavior = "IDENTITY_RETAINED"
        elif kind == "combining_mark_adjacent":
            term = _pick(seed, split, seq, kind, ENGLISH_TERMS)
            text = f"please confirm the {term} anch{COMBINING_ACUTE}or stays intact r3n4uid{uid:05d}"
            span = term
            behavior = "IDENTITY_RETAINED"
        elif kind == "emoji_adjacent":
            term = _pick(seed, split, seq, kind, ENGLISH_TERMS)
            text = f"please confirm the {term} anchor {CHECK_EMOJI} stays intact r3n4uid{uid:05d}"
            span = term
            behavior = "IDENTITY_RETAINED"
        elif kind == "duplicate_identity_like":
            term = _pick(seed, split, seq, kind, ENGLISH_TERMS)
            text = f"{term} {term} anchor duplicate check r3n4uid{uid:05d}"
            span = term
            behavior = "IDENTITY_RETAINED"
        else:  # repeated_finalization_marker
            term = _pick(seed, split, seq, kind, ENGLISH_TERMS)
            text = (
                f"finalize finalize the {term} anchor twice for idempotence r3n4uid{uid:05d}"
            )
            span = term
            behavior = "IDENTITY_RETAINED"
        builder.add(
            split=split,
            population_ids=["IDENTITY_ANCHOR_CHALLENGE"],
            input_text=text,
            highlighted_span=span if span in text else term,
            expected_behavior=behavior,
            template_family=f"r3n4_hld_identity_anchor_challenge_{kind}",
            notes="Identity-anchor stress case; exact raw identity must survive anchor finalization",
        )
        idx += 1
...
def build_all_cases() -> dict[SplitName, list[dict[str, Any]]]:
    ...
    r3n3_blocked = _load_r3n3_blocked_sets()
    blocked = {
    ...
    _generate_identity_anchor_challenge(builder, SPLIT_SIZE_MINIMA["IDENTITY_ANCHOR_CHALLENGE"])
...
def difficulty_distribution(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "mai07_r3n4_difficulty_distribution_v1",
        "builder_version": BUILDER_VERSION,
...
def aggregate_only_input_proof() -> dict[str, Any]:
    """Prove builder/eval never opened R3N2/R3N3 prediction jsonl; list allowed aggregate inputs."""
    r3n = _load_r3n_blocked_sets()
    r3n2 = _load_r3n2_blocked_sets()
    r3n3 = _load_r3n3_blocked_sets()
    prohibited_patterns = [
        "evals/mai07_r3n2_fresh_holdout/reports/*.jsonl",
        "evals/mai07_r3n2_fresh_holdout/**/*predictions*.jsonl",
        "evals/mai07_r3n3_fresh_holdout/reports/*.jsonl",
        "evals/mai07_r3n3_fresh_holdout/**/*predictions*.jsonl",
    ]
    allowed_aggregate_inputs = [
        ...
        "evals/mai07_r3n3_fresh_holdout/*.jsonl (split manifests only; excludes reports/ and *predictions*)",
        "docs/mokxya-ai/reviews/mai07_v3_ai_assisted/policy_mismatch_triage/closure/R3M_CODE_CORRECTIVE_AUTHORITY.json",
        "docs/mokxya-ai/reviews/mai07_v3_ai_assisted/policy_mismatch_triage/closure/R3M_CODE_CORRECTIVE_PRIVATE_CASES.jsonl",
        "docs/mokxya-ai/reviews/mai07_v3_ai_assisted/runtime_conformance_diagnostic/BEHAVIOR_EXPECTATIONS.jsonl",
        "evals/mai07_r3n_integrity_closure/SEMANTIC_HASH.json",
        "evals/mai07_r3n4_fresh_holdout/PREEDIT_BASELINE.json",
        "evals/mai07_r3n4_fresh_holdout/MAI_07R3N4_THRESHOLDS.json",
    ]
    return {
        "schema_version": "mai07_r3n4_aggregate_only_input_proof_v1",
        "builder_version": BUILDER_VERSION,
        "prediction_jsonl_opened": False,
        "r3n2_reports_opened": False,
        "r3n3_reports_opened": False,
        ...
        "r3n3_blocked_row_count": r3n3["row_count"],
        "r3n3_blocked_case_id_count": len(r3n3["case_ids"]),
        "r3n3_blocked_text_hash_count": len(r3n3["text_hashes"]),
...
def freshness_firewall(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    r3n3_id_overlap = _overlap_count(hld_ids, r3n3["case_ids"])
    ...
    r3n3_text_overlap = _overlap_count(hld_text_hashes, r3n3["text_hashes"])
    ...
    r3n3_family_overlap = _overlap_count(hld_families, r3n3["template_families"])
    ...
    r3n3_skel_overlap = _overlap_count(hld_skeleton_hashes, r3n3["skeleton_hashes"])
    ...
    dev_r3n3_text_overlap = _overlap_count(dev_text_hashes, r3n3["text_hashes"])
    ...
    return {
        "schema_version": "mai07_r3n4_freshness_firewall_v1",
        "builder_version": BUILDER_VERSION,
        "r3n_sources": r3n["sources"],
        "r3n2_sources": r3n2["sources"],
        "r3n3_sources": r3n3["sources"],
        ...
        "zero_overlap_proofs": {
            ...
            "r3n3_case_id_intersection_count": r3n3_id_overlap,
...
def _split_integrity(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "mai07_r3n4_split_integrity_v1",
...
def write_datasets(*, write: bool = False, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "mai07_r3n4_manifest_v1",
        "manifest_id": "MANIFEST",
        "builder_version": BUILDER_VERSION,
        "candidate_policy_version": "mai-07-r3n4.1.0.0",
        "parent_failed_r3n3_runtime": "mai-07.1.8-r3n3-identityinv",
    ...
    (dest / "R3N4_AGGREGATE_ONLY_INPUT_PROOF.json").write_text(aggregate_body, encoding="utf-8", newline="\n")

    denominators = {
        "schema_version": "mai07_r3n4_population_denominators_v1",
        "builder_version": BUILDER_VERSION,
        "minimum_denominators": dict(MINIMUM_DENOMINATORS),
        "split_size_minima": dict(SPLIT_SIZE_MINIMA),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="MAI-07R3N4 fresh-holdout dataset builder")
    parser.add_argument(
        "--write",
        action="store_true",
