"""MAI-07R3F-SEAL-NEW — fresh non-frozen holdout datasets (not a reuse of R3F holdout).
...
SEED = 20260717
SCHEMA = "mai07r3f_seal_new_case_v1"
BUILDER_VERSION = "mai-07-r3f-sealnew-dataset.1.0.0"
...
def _cid(split: str, kind: str, n: int) -> str:
    ...
    return f"sn_{split[:3].lower()}_{kind[:12]}_{n:04d}_{h}"
...
def _case(
    *,
    ...
    return {
        ...
        "pair_id": pair_id,
        "pair_role": pair_role,
        "provenance": "R3F_SEALNEW_SYNTHETIC_OR_RESOURCE_BACKED_NON_FROZEN",
...
def build_cases(repo: Path = REPO) -> list[dict[str, Any]]:
    ...
    for i, w in enumerate(ordinary_verbs + function_words + eng[:40]):
        for variant, ctx in (
            (f"please {w} sn0e{i:04d}", "english_instruction"),
            (f"the {w} report sn0e{i:04d}", "english_np"),
            (f"{w} and confirm sn0e{i:04d}", "english_coord"),
        ):
            split = _bucket(n)
            if add(
                _case(
    ...
    for i, term in enumerate(tech_terms * 12):
        for variant in (
            f"post the {term} entry sn0ta{i:04d}",
            f"accounting {term} schedule sn0ta{i:04d}",
            f"banking {term} statement sn0ta{i:04d}",
            f"inventory {term} count sn0ta{i:04d}",
        ):
            split = _bucket(n)
            if add(
                _case(
    ...
    for i, term in enumerate(shared * 6):
        text = f"please review {term} totals sn0sb{i:04d}"
        split = _bucket(n)
        if add(
            _case(
                split=split,
    ...
    for i, term in enumerate(shared[:24]):
        for j, particle in enumerate(("aaja", "mero", "hamro", "tyaha")):
            idx = i * 4 + j
            text = f"{particle} {term} pathau sn0sr{idx:04d}"
    ...
    for i, (rom, devs) in enumerate(gen_lex):
        for variant, kind in (
            (f"{rom} sn0rn{i:04d}", "romanized_core"),
            (f"aaja {rom} sn0rn{i:04d}", "romanized_phrase"),
            (f"{rom} kharcha sn0rn{i:04d}", "romanized_phrase"),
            (f"mero {rom} pathau sn0rn{i:04d}", "romanized_phrase"),
        ):
            split = _bucket(n)
            if add(
                _case(
    ...
    for i, (rom, devs) in enumerate(gen_domain):
        for variant in (f"{rom} hisaab sn0rd{i:04d}", f"mero {rom} pathau sn0rd{i:04d}"):
            split = _bucket(n)
            if add(
                _case(
                    split=split,
    ...
    for stem, stem_devs in gen_lex:
        ...
        for suf in morph_suf[:6]:
            ...
            if not targets:
                continue
            text = f"{composed} sn0rm{morph_n:04d}"
            split = _bucket(n)
            if add(
                _case(
                    split=split,
    ...
    for i, name in enumerate(names):
        for variant in (f"{name} sn0nm{i:04d}", f"party {name} sn0nm{i:04d}", f"{name} traders sn0nm{i:04d}"):
            split = _bucket(n)
            if add(
                _case(
                    split=split,
    ...
    for i in range(60):
        for tpl, kind in protected_samples:
            amt = 1000 + i
            text = tpl.format(i=i, amt=amt) + f" sn0pr{i:04d}"
            split = _bucket(n, safety=True)
            if add(
                _case(
                    split=split,
    ...
    cm = [
        ("mero payment pathau sn0cm{n:04d}", "payment", True, []),
        ("please kharcha hernu sn0cm{n:04d}", "please", True, []),
        ("aaja sales report sn0cm{n:04d}", "aaja", False, list(res.lexicon.get("aaja", ["आज"])[:2])),
        ("bank bata cash sn0cm{n:04d}", "bank", True, []),
        ("hello mera sn0cm{n:04d}", "hello", True, []),
        ("paisa pathau sn0cm{n:04d}", "paisa", False, list(res.lexicon.get("paisa", ["पैसा"])[:2])),
        ("balance hernu sn0cm{n:04d}", "balance", True, []),
        ("tax tira sn0cm{n:04d}", "tax", True, []),
    ]
    for i in range(200):
    ...
    for surf in pair_surfaces_eng_rom:
        ...
        for k in range(5):
            ...
            eng_text = f"please confirm {surf} amount sn0cfe{pair_n:04d}"
            rom_text = f"aaja {surf} pathau sn0cfr{pair_n:04d}"
            ...
            amb_text = f"{amb_tok} sn0cfa{pair_n:04d}"
            split = "CONTEXT_COUNTERFACTUAL" if (pair_n % 5) else "DEVELOPMENT"
            if pair_n % 11 == 0:
    ...
    for i, term in enumerate(eng[:30]):
        for variant in (
            f"{term.upper()} sn0uc{i:04d}",
            f"{term.capitalize()} sn0uc{i:04d}",
            f"{term}! sn0uc{i:04d}",
            f"({term}) sn0uc{i:04d}",
        ):
            split = _bucket(n, safety=True)
...
def write_datasets(repo: Path = REPO) -> dict[str, Any]:
    ...
    holdout_manifest = {
        ...
        "manifest_id": "MAI_07R3F_SEAL_NEW_DATASET_MANIFEST",
    ...
    (OUT / "MAI_07R3F_SEAL_NEW_DATASET_MANIFEST.json").write_text(hm_body, encoding="utf-8", newline="\n")

    thresholds = {
        "schema_version": "1.0.0",
        "threshold_id": "MAI_07R3F_SEAL_NEW_HOLDOUT_THRESHOLDS_V1",
    ...
    (OUT / "MAI_07R3F_SEAL_NEW_HOLDOUT_THRESHOLDS.json").write_text(th_body, encoding="utf-8", newline="\n")

    readme = """# MAI-07R3F-SEAL-NEW fresh datasets (non-frozen)
