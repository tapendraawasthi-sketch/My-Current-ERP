"""MAI-07R3N3 fresh-holdout policy-conformance corrective evaluation runner.

Uses the R3N3 sealed pack explicitly via load_r3n3_resources / transliterate_r3n3.
Never mutates ACTIVE_PACK_VERSION or enables the overlay.
Does not read R3N2 prediction jsonl.
"""
...
from .build_mai07r3n3_pack import PACK_VERSION, check_existing as check_pack
from .eval_mai07_r3n3_audit_scorer import compare_canonical_audit, observe_case_audit, score_observations_audit
from .eval_mai07_r3n3_canonical_scorer import observe_case, score_observations
from .mai07_r3n3_candidate_runtime import (
    CANDIDATE_POLICY_VERSION,
    CANDIDATE_RUNTIME_VERSION,
    PARENT_FAILED_R3N2_LOCK_SEMANTIC,
    PARENT_FAILED_R3N2_PACK_HASH,
    PARENT_FAILED_R3N2_RUNTIME_SEMANTIC,
...
from .r3n3_candidate_finalization import FINALIZER_VERSION
...
REPORTS = OUT / "reports"
RC_ID = "MAI_07R3N3_FRESH_HOLDOUT_RELEASE_CANDIDATE_001"
ATTEMPT_ID = "MAI_07R3N3_HOLDOUT_ATTEMPT_001"
...
THRESHOLDS_PATH = OUT / "MAI_07R3N3_THRESHOLDS.json"
POLICY_CONFIG_PATH = XL / "resources" / "r3n3_identity_invariant_policy.json"
...
def _require_write() -> None:
    if os.environ.get(AUTHORIZE_ENV) != "1":
        raise PermissionError(f"Set {AUTHORIZE_ENV}=1 to authorize R3N3 evaluation writes")
...
def _source_hashes() -> dict[str, str]:
    return {
        "scorer_canonical_source_sha256": sha256_file(APP / "eval_mai07_r3n3_canonical_scorer.py"),
        "scorer_audit_source_sha256": sha256_file(APP / "eval_mai07_r3n3_audit_scorer.py"),
        "scoring_contract_sha256": sha256_file(APP / "r3n3_scoring_contracts.py"),
        "evaluator_source_sha256": sha256_file(Path(__file__)),
        "policy_config_sha256": sha256_file(POLICY_CONFIG_PATH),
        "finalizer_source_sha256": sha256_file(APP / "r3n3_candidate_finalization.py"),
        "runtime_semantic_hash": sha256_file(APP / "mai07_r3n3_candidate_runtime.py"),
    }
...
def score_split(
    ...
    *,
    ...
    candidate_res = load_r3n3_resources()
    parent_res = load_resources() if include_parent and split in PARENT_COMPARISON_SPLITS else None
    observations = []
    audit_obs = []
    bundles: dict[str, Any] = {}
    for case in cases:
        bundle = transliterate_r3n3(case["input_text"], resources=candidate_res)
    ...
    for case in cases:
        b2 = transliterate_r3n3(case["input_text"], resources=candidate_res)
...
def lock_rc(*, write: bool = True) -> dict[str, Any]:
    ...
    body = {
        ...
        "parent_failed_r3n2_lineage": {
            "runtime_version": PARENT_FAILED_R3N2_RUNTIME_VERSION,
            "pack_hash": PARENT_FAILED_R3N2_PACK_HASH,
            "runtime_semantic_sha256": PARENT_FAILED_R3N2_RUNTIME_SEMANTIC,
            "lock_semantic_sha256": PARENT_FAILED_R3N2_LOCK_SEMANTIC,
            "verdict": PARENT_FAILED_R3N2_VERDICT,
            "consumed": True,
    ...
    record = create_lock_record(
        rc_id=RC_ID, locked_path=LOCKED_PATH, locked_body=locked_body, provenance="MAI_07R3N3"
    )
...
def one_shot_holdout() -> dict[str, Any]:
    ...
    attempt = create_attempt(
        ...
        command="python -m src.oip.modules.language_runtime.transliteration.application.eval_mai07_r3n3 --one-shot",
    ...
    qual["MAI-07"] = "NEEDS_CORRECTIVE_WORK" if not holdout_ok else "PASSED_CORRECTIVE_RC"
    ...
    imm = {
        ...
        "candidate_promoted": False,
        "parent_failed_r3n2_runtime_version": PARENT_FAILED_R3N2_RUNTIME_VERSION,
        "r3n_integrity_closure_semantic": R3N_INTEGRITY_CLOSURE_SEMANTIC,
    }
