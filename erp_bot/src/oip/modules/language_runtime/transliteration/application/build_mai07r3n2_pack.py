"""Build immutable versioned resource pack mai-07.1.7-r3n2-freshholdout.
...
PACK_VERSION = "mai-07.1.7-r3n2-freshholdout"
PACK_ID = "mai07_transliteration_pack_r3n2_fresh_holdout"
DEST = XL / "sealed_packs" / PACK_VERSION
POLICY_SOURCE = XL / "resources" / "r3n2_fresh_holdout_policy.json"
    ...
    man.update(
        {
            ...
            "provenance": (
                "MAI-07R3N2: new sealed pack derived from mai-07.1.3-r3f-sealnew; "
                "identity-disposition policy = mai-07-r3n2.1.0.0 fresh-holdout corrective; parent invalidated mai-07.1.6-r3n-policyconf; "
...
def _dual_build() -> tuple[dict, dict]:
    ...
    with tempfile.TemporaryDirectory(prefix="mai07_r3n2_pack_a_") as ta, tempfile.TemporaryDirectory(
        prefix="mai07_r3n2_pack_b_"
    ) as tb:
        a = Path(ta)
        b = Path(tb)
        ra = _materialize(a)
...
def build_pack(*, promote: bool = True) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="mai07_r3n2_pack_promote_") as tp:
        p = Path(tp)
        _materialize(p)
