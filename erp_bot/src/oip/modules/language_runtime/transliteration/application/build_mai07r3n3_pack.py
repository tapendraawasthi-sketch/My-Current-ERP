"""Build immutable versioned resource pack mai-07.1.8-r3n3-identityinv.

Derives from the active sealed pack (mai-07.1.3-r3f-sealnew), replacing only the
identity-disposition policy config with the R3N policy-conformance corrective.
...
PACK_VERSION = "mai-07.1.8-r3n3-identityinv"
PACK_ID = "mai07_transliteration_pack_r3n3_identity_invariant"
DEST = XL / "sealed_packs" / PACK_VERSION
POLICY_SOURCE = XL / "resources" / "r3n3_identity_invariant_policy.json"
    ...
    man.update(
        {
            ...
            "provenance": (
                "MAI-07R3N3: new sealed pack derived from mai-07.1.3-r3f-sealnew; "
                "identity-disposition policy = mai-07-r3n3.1.0.0 identity-invariant corrective; parent failed mai-07.1.7-r3n2-freshholdout; "
...
def _dual_build() -> tuple[dict, dict]:
    ...
    with tempfile.TemporaryDirectory(prefix="mai07_r3n3_pack_a_") as ta, tempfile.TemporaryDirectory(
        prefix="mai07_r3n3_pack_b_"
    ) as tb:
        a = Path(ta)
        b = Path(tb)
        ra = _materialize(a)
...
def build_pack(*, promote: bool = True) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="mai07_r3n3_pack_promote_") as tp:
        p = Path(tp)
        _materialize(p)
