"""MAI-07R3N4 fresh-holdout identity-anchor evaluation runner.

Uses the R3N4 sealed pack explicitly via load_r3n4_resources / transliterate_r3n4.
Never mutates ACTIVE_PACK_VERSION or enables the overlay.
Does not read R3N3/R3N2 prediction jsonl — only aggregate lock/attempt/chain
metadata is read from evals/mai07_r3n3_fresh_holdout/ for failed-parent authority
verification (verdict, consumed flag, lock semantic hash). Case-level R3N3
prediction bodies are never opened.
"""
...
from .build_mai07r3n4_pack import PACK_VERSION, check_existing as check_pack
from .eval_mai07_r3n4_audit_scorer import compare_canonical_audit, observe_case_audit, score_observations_audit
from .eval_mai07_r3n4_canonical_scorer import observe_case, score_observations
from .mai07_r3n3_candidate_runtime import load_r3n3_resources, transliterate_r3n3
from .mai07_r3n4_candidate_runtime import (
    ...
    PARENT_FAILED_R3N3_ATTEMPT,
    PARENT_FAILED_R3N3_LOCK_SEMANTIC,
    PARENT_FAILED_R3N3_PACK_HASH,
    PARENT_FAILED_R3N3_POLICY_VERSION,
    PARENT_FAILED_R3N3_RUNTIME_SEMANTIC,
...
from .r3n4_candidate_finalization import FINALIZER_VERSION
...
REPORTS = OUT / "reports"
RC_ID = "MAI_07R3N4_FRESH_HOLDOUT_RELEASE_CANDIDATE_001"
ATTEMPT_ID = "MAI_07R3N4_HOLDOUT_ATTEMPT_001"
...
THRESHOLDS_PATH = OUT / "MAI_07R3N4_THRESHOLDS.json"
POLICY_CONFIG_PATH = XL / "resources" / "r3n4_identity_anchor_policy.json"
...
# Aggregate-only failed R3N3 parent authority (case-level R3N3 prediction jsonl bodies
# are never read; only this chain manifest's verdict/consumed/lock-semantic fields).
R3N3_OUT = REPO / "evals" / "mai07_r3n3_fresh_holdout"
PARENT_FAILED_R3N3_RC_ID = "MAI_07R3N3_FRESH_HOLDOUT_RELEASE_CANDIDATE_001"
PARENT_FAILED_R3N3_CHAIN_PATH = R3N3_OUT / f"{PARENT_FAILED_R3N3_RC_ID}.CHAIN_MANIFEST.json"
...
def _require_write() -> None:
    if os.environ.get(AUTHORIZE_ENV) != "1":
        raise PermissionError(f"Set {AUTHORIZE_ENV}=1 to authorize R3N4 evaluation writes")
...
def _runtime_semantic_hash() -> str:
    """Semantic hash spanning the candidate runtime + finalizer + identity-anchor sources."""
    parts = [
        sha256_file(APP / "mai07_r3n4_candidate_runtime.py"),
        sha256_file(APP / "r3n4_candidate_finalization.py"),
        sha256_file(APP / "r3n4_identity_anchor.py"),
        sha256_file(APP / "r3n4_finalization_path_registry.py"),
    ]
...
def _source_hashes() -> dict[str, str]:
    return {
        "scorer_canonical_source_sha256": sha256_file(APP / "eval_mai07_r3n4_canonical_scorer.py"),
        "scorer_audit_source_sha256": sha256_file(APP / "eval_mai07_r3n4_audit_scorer.py"),
        "scoring_contract_sha256": sha256_file(APP / "r3n4_scoring_contracts.py"),
        "evaluator_source_sha256": sha256_file(Path(__file__)),
        "policy_config_sha256": sha256_file(POLICY_CONFIG_PATH),
        "finalizer_source_sha256": sha256_file(APP / "r3n4_candidate_finalization.py"),
        "identity_anchor_source_sha256": sha256_file(APP / "r3n4_identity_anchor.py"),
        "path_registry_source_sha256": sha256_file(APP / "r3n4_finalization_path_registry.py"),
        "runtime_semantic_hash": _runtime_semantic_hash(),
    }
...
def _verify_r3n3_failed_parent() -> dict[str, Any]:
    """Aggregate-only authority check: verdict/consumed/lock-semantic from the R3N3
    chain manifest. Never opens R3N3 reports/*predictions*.jsonl case bodies."""
    if not PARENT_FAILED_R3N3_CHAIN_PATH.is_file():
        return {"ok": False, "error": "missing_r3n3_chain_manifest", "path": str(PARENT_FAILED_R3N3_CHAIN_PATH)}
    chain = json.loads(PARENT_FAILED_R3N3_CHAIN_PATH.read_text(encoding="utf-8"))
    if chain.get("verdict") != PARENT_FAILED_R3N3_VERDICT:
        return {"ok": False, "error": "r3n3_verdict_mismatch", "observed": chain.get("verdict")}
    if not chain.get("consumed"):
        return {"ok": False, "error": "r3n3_attempt_not_consumed"}
    if chain.get("locked_semantic_sha256") != PARENT_FAILED_R3N3_LOCK_SEMANTIC:
        return {"ok": False, "error": "r3n3_lock_semantic_mismatch"}
    return {
        "ok": True,
        "chain_path": str(PARENT_FAILED_R3N3_CHAIN_PATH),
        "verdict": chain.get("verdict"),
        "consumed": chain.get("consumed"),
        "locked_semantic_sha256": chain.get("locked_semantic_sha256"),
    }


def _transliterate_parent(raw_text: str, *, resources: Any) -> Any:
    """Active R3F parent (grandparent lineage) for differential comparison."""
...
def _transliterate_failed_r3n3_parent(raw_text: str, *, resources: Any) -> Any:
    """Failed R3N3 immediate parent for differential comparison (runtime-only; no
    R3N3 prediction jsonl is read)."""
    return transliterate_r3n3(raw_text, resources=resources)


def _bundle_identity_top1(bundle: Any, highlighted: str) -> bool | None:
    matches = [s for s in bundle.span_results if s.raw_span.original_text.lower() == highlighted.lower()]
    if not matches:
        return None
    cands = list(matches[0].candidates or ())
    return bool(cands and cands[0].is_identity)
    ...
    for case, obs in zip(cases, observations, strict=True):
        ...
        if parent_id_top1 is not None:
            ...
            if "MONOTONIC_PARENT_INCORRECT" in pops and cand_id_top1 and not parent_id_top1:
                monotonic_fix += 1
        r3n3_id_top1 = obs.get("parent_r3n3_identity_top1")
        if r3n3_id_top1 is not None:
            r3n3_comparable += 1
            if cand_id_top1 and not r3n3_id_top1:
                r3n3_improved += 1
            elif r3n3_id_top1 and not cand_id_top1:
                r3n3_regressed += 1
            else:
                r3n3_unchanged += 1
    return {
        ...
        "parent_r3n3_differential": {
            "comparable_cases": r3n3_comparable,
            "candidate_improved_identity_top1": r3n3_improved,
            "candidate_regressed_identity_top1": r3n3_regressed,
            "unchanged_identity_top1": r3n3_unchanged,
        },
    }
...
def score_split(
    ...
    *,
    ...
    candidate_res = load_r3n4_resources()
    do_parent = include_parent and split in PARENT_COMPARISON_SPLITS
    parent_res = load_resources() if do_parent else None
    r3n3_res = load_r3n3_resources() if do_parent else None
    observations = []
    audit_obs = []
    bundles: dict[str, Any] = {}
    for case in cases:
        bundle = transliterate_r3n4(case["input_text"], resources=candidate_res)
        bundles[case["case_id"]] = bundle
        parent_bundle = None
        if parent_res is not None:
            parent_bundle = _transliterate_parent(case["input_text"], resources=parent_res)
        obs = observe_case(case, bundle, parent_bundle=parent_bundle)
        if r3n3_res is not None:
            r3n3_bundle = _transliterate_failed_r3n3_parent(case["input_text"], resources=r3n3_res)
            obs["parent_r3n3_identity_top1"] = _bundle_identity_top1(r3n3_bundle, case["highlighted_span"])
        observations.append(obs)
        audit_obs.append(observe_case_audit(case, bundle))

    for case in cases:
        b2 = transliterate_r3n4(case["input_text"], resources=candidate_res)
        o1 = observe_case(case, bundles[case["case_id"]])
        o2 = observe_case(case, b2)
        if (o1["identity_top1"], o1["candidate_count"], o1["exact_raw_identity"]) != (
            o2["identity_top1"],
            o2["candidate_count"],
            o2["exact_raw_identity"],
        ):
...
def lock_rc(*, write: bool = True) -> dict[str, Any]:
    ...
    r3n3_parent = _verify_r3n3_failed_parent()
    if not r3n3_parent["ok"]:
        raise RuntimeError(f"BLOCKED_PRECONDITION_FAILED:r3n3_failed_parent:{r3n3_parent}")
    ...
    body = {
        ...
        "parent_failed_r3n3_lineage": {
            "runtime_version": PARENT_FAILED_R3N3_RUNTIME_VERSION,
            "policy_version": PARENT_FAILED_R3N3_POLICY_VERSION,
            "pack_hash": PARENT_FAILED_R3N3_PACK_HASH,
            "runtime_semantic_sha256": PARENT_FAILED_R3N3_RUNTIME_SEMANTIC,
            "lock_semantic_sha256": PARENT_FAILED_R3N3_LOCK_SEMANTIC,
            "verdict": PARENT_FAILED_R3N3_VERDICT,
            "attempt_id": PARENT_FAILED_R3N3_ATTEMPT,
            "consumed": True,
            "candidate_promoted": False,
        },
        "preserved_failed_r3n2_lineage": {
            "runtime_version": PARENT_FAILED_R3N2_RUNTIME_VERSION,
            "pack_hash": PARENT_FAILED_R3N2_PACK_HASH,
            "consumed": True,
            "candidate_promoted": False,
        },
        "invalidated_r3n_lineage": {
            "runtime_version": "mai-07.1.6-r3n-policyconf",
        ...
        "no_r3n2_prediction_use": True,
        "no_r3n3_prediction_use": True,
    ...
    record = create_lock_record(
        rc_id=RC_ID, locked_path=LOCKED_PATH, locked_body=locked_body, provenance="MAI_07R3N4"
    )
...
def _write_r3n3_parent_holdout_predictions() -> str:
    """Runtime-only comparison predictions for the failed R3N3 parent (never
    reads R3N3's own prediction jsonl)."""
    cases = load_split("HOLDOUT_VALIDATION")
    r3n3_res = load_r3n3_resources()
    rows: list[dict[str, Any]] = []
    for case in cases:
        bundle = _transliterate_failed_r3n3_parent(case["input_text"], resources=r3n3_res)
        obs = observe_case(case, bundle)
        rows.append(_prediction_row(case, obs, runtime=PARENT_FAILED_R3N3_RUNTIME_VERSION))
    return _write_jsonl(REPORTS / "parent_r3n3_holdout_predictions.jsonl", rows)
...
def one_shot_holdout() -> dict[str, Any]:
    ...
    attempt = create_attempt(
        ...
        command="python -m src.oip.modules.language_runtime.transliteration.application.eval_mai07_r3n4 --one-shot",
        split="HOLDOUT_VALIDATION",
    )

    REPORTS.mkdir(parents=True, exist_ok=True)
    parent_holdout_sha = _write_parent_holdout_predictions()
    parent_r3n3_holdout_sha = _write_r3n3_parent_holdout_predictions()
    ...
    verdict = (
        "PASSED_FRESH_HOLDOUT_CORRECTIVE_RC" if holdout_ok else "FAILED_HOLDOUT_QUALITY"
    )
    ...
    # Phase authority: governance flags remain false until later linguist/production phases.
    ...
    _write_json(
        CHAIN_PATH,
        {
            ...
            "parent_r3n3_holdout_predictions_path": str(
                (REPORTS / "parent_r3n3_holdout_predictions.jsonl").relative_to(REPO)
            ).replace("\\", "/"),
            "parent_r3n3_holdout_predictions_sha256": parent_r3n3_holdout_sha,
    ...
    imm = {
        ...
        "candidate_promoted": False,
        "parent_failed_r3n3_runtime_version": PARENT_FAILED_R3N3_RUNTIME_VERSION,
        "r3n_integrity_closure_semantic": R3N_INTEGRITY_CLOSURE_SEMANTIC,
    }
    ...
    if args.check_preflight:
        ...
        if (OUT / SPLIT_FILES["HOLDOUT_VALIDATION"]).is_file():
            try:
                ...
                pop_error = str(exc)
        payload = {
            "ok": bool(pack.get("ok")) and integrity["ok"] and r3n3_parent["ok"] and (pop is None or pop.get("ok")),
            "pack": pack,
            "integrity_closure": integrity,
            "r3n3_failed_parent": r3n3_parent,
            "population_gate": pop,
