"""Build immutable versioned resource pack mai-07.1.6-r3n-policyconf.
...
PACK_VERSION = "mai-07.1.6-r3n-policyconf"
PACK_ID = "mai07_transliteration_pack_r3n_policy_conformance"
DEST = XL / "sealed_packs" / PACK_VERSION
POLICY_SOURCE = XL / "resources" / "r3n_policy_conformance_policy.json"
    ...
    man.update(
        {
            ...
            "provenance": (
                "MAI-07R3N: new sealed pack derived from mai-07.1.3-r3f-sealnew; "
                "identity-disposition policy = mai-07-r3n.1.0.0 policy-conformance corrective; "
...
def _dual_build() -> tuple[dict, dict]:
    ...
    with tempfile.TemporaryDirectory(prefix="mai07_r3n_pack_a_") as ta, tempfile.TemporaryDirectory(
        prefix="mai07_r3n_pack_b_"
    ) as tb:
        a = Path(ta)
        b = Path(tb)
        ra = _materialize(a)
...
def build_pack(*, promote: bool = True) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="mai07_r3n_pack_promote_") as tp:
        p = Path(tp)
        _materialize(p)
