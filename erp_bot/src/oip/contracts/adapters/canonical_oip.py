    ...
    def to_intelligence_dto(
        ...
        *,
        ...
        if canonical.load_latency_failover_bundle is not None:
            try:
                from ...modules.conversation.application.load_latency_failover_consume_service import (
                    enrich_llf_metadata_with_consume,
                )
                from ...modules.conversation.application.load_latency_failover_service import (
                    load_latency_failover_to_metadata,
                )

                base_meta = load_latency_failover_to_metadata(
                    canonical.load_latency_failover_bundle
                )
                metadata["load_latency_failover"] = (
                    enrich_llf_metadata_with_consume(base_meta, canonical)
                )
            except Exception:  # noqa: BLE001
                ...
                metadata["load_latency_failover"] = {
                    ...
                    "gap_p2_008_status": "OPEN",
                    "load_latency_failover_consume_mode": "UNCHANGED",
        ...
        if canonical.human_review_pilot_operations_bundle is not None:
            try:
                from ...modules.conversation.application.human_review_pilot_operations_consume_service import (
                    enrich_hrpo_metadata_with_consume,
                )
                ...
                base_meta = human_review_pilot_operations_to_metadata(
                    canonical.human_review_pilot_operations_bundle
                )
                metadata["human_review_pilot_operations"] = (
                    enrich_hrpo_metadata_with_consume(base_meta, canonical)
                )
            except Exception:  # noqa: BLE001
                ...
                metadata["human_review_pilot_operations"] = {
                    ...
                    "gap_p2_008_status": "OPEN",
                    "human_review_pilot_operations_consume_mode": "UNCHANGED",
        ...
        if canonical.production_capability_release_bundle is not None:
            try:
                from ...modules.conversation.application.production_capability_release_consume_service import (
                    enrich_pcr_metadata_with_consume,
                )
                ...
                base_meta = production_capability_release_to_metadata(
                    canonical.production_capability_release_bundle
                )
                metadata["production_capability_release"] = (
                    enrich_pcr_metadata_with_consume(base_meta, canonical)
                )
            except Exception:  # noqa: BLE001
        ...
        if canonical.nepali_english_speech_channel_bundle is not None:
            try:
                from ...modules.conversation.application.nepali_english_speech_channel_consume_service import (
                    enrich_speech_metadata_with_consume,
                )
                ...
                base_meta = nepali_english_speech_channel_to_metadata(
                    canonical.nepali_english_speech_channel_bundle
                )
                metadata["nepali_english_speech_channel"] = (
                    enrich_speech_metadata_with_consume(base_meta, canonical)
                )
            except Exception:  # noqa: BLE001
        ...
        if canonical.compliance_obligation_calendar_bundle is not None:
            try:
                ...
                metadata["compliance_obligation_calendar"] = {
                    "analysis_status": coc.analysis_status.value,
                    "runtime_version": coc.runtime_version,
                    "compliance_obligation_calendar_readiness": (
                        coc.compliance_obligation_calendar_readiness.value
                    ),
                    "release_status": "NOT_RELEASED",
