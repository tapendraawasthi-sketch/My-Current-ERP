...
class ComplianceObligationCalendarStatus(str, Enum):
    NOT_RUN = "NOT_RUN"
    COMPLETE = "COMPLETE"
    SKIP = "SKIP"
    FAILED = "FAILED"
...
class ComplianceObligationCalendarReadiness(str, Enum):
...
class ComplianceObligationCalendarBundleV1(ContractBase):
    ...
    pilot_scope: str = "COMPLIANCE_OBLIGATION_CALENDAR_CANDIDATE_ONLY"
    ...
    calendar_authority_claimed: bool = False
    compliance_calendar_enabled: bool = False
    obligation_created: bool = False
    deadline_scheduled: bool = False
    reminder_sent: bool = False
    automation_armed: bool = False
    calendar_synced: bool = False
    filing_submitted: bool = False
    obligation_closed: bool = False
    ...
    def _scope(cls, v: str) -> str:
        if v != "COMPLIANCE_OBLIGATION_CALENDAR_CANDIDATE_ONLY":
    ...
    def _mut(cls, v: bool) -> bool:
        if v:
            raise ValueError(
                "COMPLIANCE_OBLIGATION_CALENDAR_MUST_NOT_ALLOW_MUTATION"
            )
        return v

    @field_validator(
        "calendar_authority_claimed",
        "compliance_calendar_enabled",
        "obligation_created",
        "deadline_scheduled",
        "reminder_sent",
        "automation_armed",
    ...
    def _never_claim(cls, v: bool) -> bool:
        if v:
            raise ValueError(
                "COMPLIANCE_OBLIGATION_CALENDAR_MUST_NOT_CLAIM_PASS"
            )
        return v
    ...
    def _zero(cls, v: int) -> int:
        if v != 0:
            raise ValueError(
                "COMPLIANCE_OBLIGATION_CALENDAR_MUST_NOT_MUTATE_OR_RETRIEVE"
            )
        return v
