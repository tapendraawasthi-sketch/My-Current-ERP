...
class BroaderNepalBusinessLawDomainReleaseStatus(str, Enum):
    NOT_RUN = "NOT_RUN"
    COMPLETE = "COMPLETE"
    SKIP = "SKIP"
    FAILED = "FAILED"
...
class DomainReleaseReadiness(str, Enum):
...
class BusinessLawDomainTopic(str, Enum):
    COMPANY_LAW = "COMPANY_LAW"
    LABOR_LAW = "LABOR_LAW"
    CONTRACT_LAW = "CONTRACT_LAW"
    BUSINESS_LAW_DOMAIN = "BUSINESS_LAW_DOMAIN"
    DOMAIN_RELEASE = "DOMAIN_RELEASE"
    UNSUPPORTED = "UNSUPPORTED"


class BroaderNepalBusinessLawDomainReleaseBundleV1(ContractBase):
    ...
    pilot_scope: str = "BROADER_NEPAL_BUSINESS_LAW_CANDIDATE_ONLY"
    ...
    domain_authority_claimed: bool = False
    domain_released: bool = False
    production_domain_eligible: bool = False
    ...
    def _scope(cls, v: str) -> str:
        if v != "BROADER_NEPAL_BUSINESS_LAW_CANDIDATE_ONLY":
    ...
    def _mut(cls, v: bool) -> bool:
        if v:
            raise ValueError("DOMAIN_RELEASE_MUST_NOT_ALLOW_MUTATION")
        return v

    @field_validator(
        "domain_authority_claimed",
        "domain_released",
        "production_domain_eligible",
    ...
    def _never_claim(cls, v: bool) -> bool:
        if v:
            raise ValueError("DOMAIN_RELEASE_MUST_NOT_CLAIM_OR_RELEASE")
        return v
    ...
    def _zero(cls, v: int) -> int:
        if v != 0:
            raise ValueError("DOMAIN_RELEASE_MUST_NOT_MUTATE_OR_RETRIEVE")
        return v
