...
_USE_PROVIDER_RUNTIME = provider_runtime_active()
if _USE_PROVIDER_RUNTIME:
    print("[START] Provider Runtime active — skipping Ollama warmup/fatal checks for chat ingress")
else:
    try:
...
def _keep_models_warm() -> None:
    """Ping Ollama every 5 minutes so legacy offline models stay loaded."""
    while True:
        time.sleep(300)
        try:
            httpx.post(
...
if not _USE_PROVIDER_RUNTIME:
    ...
    print("[START] Ollama keep-alive disabled — Provider Runtime handles chat ingress")
