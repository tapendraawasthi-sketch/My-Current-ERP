...
export function detectBankFormat(csv: string): NepalBankFormat {
  ...
  if (
    (header.includes("narration") || header.includes("ref")) &&
    header.includes("dr") &&
    header.includes("cr")
  ) {
  ...
  if (
    /shrawan|baisakh|jestha|ashadh|bhadra|ashwin|kartik|mangsir|poush|magh|falgun|chaitra/i.test(
      fullText,
    )
  )
    return "SBL";
  ...
  if (
    header.includes("ref no") ||
    header.includes("withdrawal (dr.)") ||
  ...
  for (let i = 0; i < Math.min(lines.length, 15); i++) {
    ...
    if (lower.includes("date") && (lower.includes("debit") || lower.includes("dr"))) {
      ...
      descIdx = headers.findIndex(
        (h) => h.includes("particular") || h.includes("narration") || h.includes("description"),
      );
...
export function parseNepalBankStatement(csv: string): ParseResult {
  ...
  try {
    switch (format) {
      case "NMB":
        entries = parseNMBStatement(csv);
        break;
      case "NABIL":
        entries = parseNabilStatement(csv);
        break;
      case "EVEREST":
        entries = parseEverestStatement(csv);
        break;
      case "SBL":
        entries = parseSBLStatement(csv);
        break;
      case "HIMALAYAN":
        entries = parseHimalayanStatement(csv);
        break;
      case "KUMARI":
        entries = parseKumariStatement(csv);
        break;
      case "NEPALSBI":
        entries = parseNepalSBIStatement(csv);
        break;
      case "CONNECTIPS":
        entries = parseConnectIPSStatement(csv);
        break;
      case "ESEWA":
        entries = parseESewaStatement(csv);
        break;
      case "KHALTI":
        entries = parseKhaltiStatement(csv);
        break;
      default:
        // Generic fallback: auto-detect columns by header keywords
        entries = parseGenericCSV(csv);
        errors.push(
          "Unknown format — using generic column detection. Please verify the imported data.",
        );
    }
  } catch (err: any) {
