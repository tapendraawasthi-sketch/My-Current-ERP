...
export function classifyQuestion(query: string, currentRoute?: string): QuestionCategory {
  ...
  if (
    /how|what is|explain|where/i.test(lowerQuery) &&
    /invoice|voucher|ledger|party|item|stock|report/i.test(lowerQuery)
  ) {
    return "erp-how-to";
  }
  if (
    /debit|credit|ledger|voucher|vat|tds|balance sheet|profit|loss|depreciation|fifo/i.test(
      lowerQuery,
    )
  ) {
  ...
  if (
    currentRoute &&
    !["dashboard", "home"].includes(currentRoute) &&
    /how|do i|create/i.test(lowerQuery)
  ) {
    return "erp-how-to";
  }
...
export function shouldSearchWeb(query: string, category: QuestionCategory): boolean {
  return (
    ["current-events", "web-search-needed"].includes(category) ||
...
export function extractSearchQuery(originalQuery: string, category: QuestionCategory): string {
  ...
  query = query.replace(
    /^(what is|what's|how is|tell me about|search for|find me|can you check|show me) /i,
    "",
  );
  query = query.replace(/\?$/, "");
  return query.trim();
...
export function generateFollowUps(
  ...
  if (category === "erp-how-to" || category === "erp-troubleshoot") {
    if (route === "sales-invoice")
      return ["How do I print this invoice?", "How to apply a discount?"];
    if (route === "parties")
      return ["How to check a customer ledger?", "How to set a credit limit?"];
...
export function analyzeERPContext(route: string): {
  moduleName: string;
  likelyIntent: string;
  suggestions: string[];
} {
  ...
  if (safeRoute.includes("invoice")) {
    ...
    safeRoute.includes("reports") ||
    safeRoute.includes("sheet") ||
    safeRoute.includes("balance")
  ) {
