  ...
  for (const m of sorted) {
    ...
    if (isInward) {
      ...
      rows.push({
        ...
        outQty:  0, outRate: 0, outValue: 0,
        balQty,  balRate, balValue: r2(balValue),
      });
    } else if (m.qty < 0) {
      ...
      if (balQty <= 0) { balQty = 0; balValue = 0; }
...
export function computeAllItemSummaries(
  ...
  return items.map((item) => {
    ...
    const periodMovements = allMovements.filter(
      (m) => m.itemId === item.id && m.date >= startDate && m.date <= endDate
...
export function computeStockAlerts(
  allMovements: RawMovement[],
  items: { id: string; name?: string; reorderLevel?: number; minimumStock?: number; unit?: string }[]
): StockAlert[] {
