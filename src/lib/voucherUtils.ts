...
export function getVoucherGroupForType(voucherType: string): string {
  const accountingTypes = ["contra", "payment", "receipt", "journal", "journal-voucher", "sales-invoice", "purchase-invoice", "credit-note", "debit-note"];
  const inventoryTypes = ["stock-journal", "physical-stock", "delivery-note", "receipt-note", "rejection-in", "rejection-out", "material-in", "material-out", "production", "unassemble", "material-issued", "material-received"];
...
export function getDefaultVoucherMode(voucherType: string): string {
  const itemInvoiceTypes = [
    "sales-invoice", "purchase-invoice", "delivery-note", "receipt-note", 
...
export function isInventoryVoucher(voucherType: string): boolean {
  return [
    ...
    "rejection-in", "rejection-out", "material-in", "material-out", 
    "production", "unassemble", "material-issued", "material-received"
  ].includes(voucherType);
}
...
export function requiresPartyLedger(voucherType: string): boolean {
  return [
    "sales-invoice", "purchase-invoice", "credit-note", "debit-note", 
    "payment", "receipt", "delivery-note", "receipt-note", 
...
export function calculateVoucherTotals(lines: Array<{debit?: number; credit?: number; amount?: number; taxAmount?: number}>) {
  let totalDebit = 0;
  let totalCredit = 0;
  let totalAmount = 0;
  let taxAmount = 0;
...
export function validateVoucherDate(date: string, fiscalYear?: {startDate: string; endDate: string}) {
...
export function generateVoucherSummaryLine(voucher: {voucherTypeName?: string; voucherNumber?: string; narration?: string; partyName?: string; totalAmount?: number; totalDebit?: number}): string {
  const parts = [];
