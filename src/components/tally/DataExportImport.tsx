...
import { Database, Upload, Download, FileSpreadsheet, CheckSquare, AlertTriangle, CheckCircle, XCircle, RefreshCw } from "lucide-react";
...
export default function DataExportImport() {
  ...
  const parseTallyXML = (xmlText) => {
    ...
    ledgerNodes.forEach(l => {
      ...
      if (name) {
        ...
        if (parent === "Sundry Debtors" || parent === "Sundry Creditors") {
          result.parties.push({ name, type: parent === "Sundry Debtors" ? "customer" : "vendor", openingBalance: openingBal, address, pan });
        }
      }
    });
    ...
    stockNodes.forEach(s => {
      ...
      const rate = s.querySelector("STANDARDCOST RATEOFVAT, BASELCVALUE")?.textContent?.trim() || "0";
      ...
      reader.onload = (event) => {
        try {
          ...
          [...parsed.ledgers, ...parsed.stockItems].forEach(item => {
            const existingAcc = accounts.some(acc => acc.name.toLowerCase() === item.name.toLowerCase());
            const existingItem = items.some(it => it.name.toLowerCase() === item.name.toLowerCase());
  ...
  const runImport = async () => {
    ...
    for (const unit of parsedData.units) {
      try {
        ...
        if (existingUnit && conflicts.some(c => c.name === unit.name && c.resolution === "skip")) { 
          skipped++; 
          continue; 
        }
        await db.units.put({ 
    ...
    setStatusLog(prev => [...prev, `Import completed. Imported: ${imported}, Skipped: ${skipped}, Errors: ${errors.length}`]);
  };
  ...
  const renderTallyMigration = () => {
      ...
      return (
        <div>
          ...
          >
            ...
            <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#000000", marginBottom: "10px" }}>
          ...
          <div style={{ marginBottom: "20px", padding: "15px", backgroundColor: "#f0f0f0", borderRadius: "6px", border: BORDER }}>
            <h4 style={{ fontSize: "14px", fontWeight: "bold", color: "#000000", marginBottom: "10px" }}>HOW TO EXPORT FROM TALLY PRIME:</h4>
          ...
          {tallyFile && (
            <div style={{ marginBottom: "20px", padding: "10px", backgroundColor: "#dcfce7", borderRadius: "6px", border: "1px solid #059669", display: "flex", alignItems: "center", gap: "10px" }}>
    ...
    if (tallyStep === 2) {
      return (
        <div>
          <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#000000", marginBottom: "15px" }}>Parse & Preview</h3>
          
          {parsedData ? (
            <div style={{ marginBottom: "20px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "15px", marginBottom: "20px" }}>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px", border: BORDER }}>
                  <div style={{ fontSize: "12px", color: "#000000" }}>Groups Found</div>
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: "#000000" }}>{parsedData.groups.length}</div>
                </div>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px", border: BORDER }}>
                  <div style={{ fontSize: "12px", color: "#000000" }}>Ledgers Found</div>
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: "#000000" }}>{parsedData.ledgers.length}</div>
                </div>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px", border: BORDER }}>
                  <div style={{ fontSize: "12px", color: "#000000" }}>Stock Items Found</div>
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: "#000000" }}>{parsedData.stockItems.length}</div>
                </div>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px", border: BORDER }}>
                  <div style={{ fontSize: "12px", color: "#000000" }}>Units Found</div>
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: "#000000" }}>{parsedData.units.length}</div>
                </div>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px", border: BORDER }}>
                  <div style={{ fontSize: "12px", color: "#000000" }}>Parties Found</div>
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: "#000000" }}>{parsedData.parties.length}</div>
                </div>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px", border: BORDER }}>
              ...
              {conflicts.length > 0 && (
                <div style={{ marginBottom: "20px" }}>
                  <h4 style={{ fontSize: "14px", fontWeight: "bold", color: "#000000", marginBottom: "10px" }}>Detected Conflicts</h4>
                  <div style={{ border: BORDER, borderRadius: "4px", maxHeight: "300px", overflowY: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
                        <tr style={{ backgroundColor: BG_HEADER }}>
                          ...
                          <th style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>Conflict</th>
                          <th style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>Resolution</th>
                        </tr>
                      </thead>
                      <tbody>
                        {conflicts.map((conflict, index) => (
                          <tr key={index}>
                            <td style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>{conflict.name}</td>
                            <td style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>{conflict.type}</td>
                            <td style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>{conflict.conflict}</td>
                            <td style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>
                              <select
                                ...
                                style={{ padding: "4px", fontSize: "12px", border: BORDER, borderRadius: "4px" }}
                              >
    ...
    if (tallyStep === 3) {
      return (
        <div>
          <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#000000", marginBottom: "15px" }}>Account Group Mapping</h3>
          ...
          <div style={{ border: BORDER, borderRadius: "4px", maxHeight: "400px", overflowY: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: BG_HEADER }}>
                  <th style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>Tally Group</th>
                  <th style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>Your System Nature</th>
                </tr>
              </thead>
              <tbody>
                {parsedData?.groups.map((group, index) => {
                  const suggestedType = 
                    group.parent?.includes("Debtors") ? "asset" :
                    group.parent?.includes("Creditors") ? "liability" :
                    group.parent?.includes("Bank") ? "asset" :
                    group.parent?.includes("Cash") ? "asset" :
                    group.parent?.includes("Sales") ? "income" :
                    group.parent?.includes("Purchase") ? "expense" :
                    group.parent?.includes("Capital") ? "equity" : "asset";
                  
                  return (
                    <tr key={index}>
                      <td style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>{group.name}</td>
                      <td style={{ border: BORDER, padding: "6px", fontSize: "12px" }}>
                        <select
                          ...
                          style={{ width: "100%", padding: "4px", fontSize: "12px", border: BORDER, borderRadius: "4px" }}
                        >
    ...
    if (tallyStep === 4) {
      return (
        <div>
          <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#000000", marginBottom: "15px" }}>Import Progress</h3>
          
          <div style={{ marginBottom: "20px" }}>
            <div style={{ height: "20px", border: BORDER, borderRadius: "10px", overflow: "hidden", backgroundColor: "#e0e0e0" }}>
              <div 
                style={{ 
                  height: "100%", 
                  width: `${progress}%`, 
            ...
            <div style={{ textAlign: "center", marginTop: "5px", color: "#000000" }}>{progress}%</div>
          </div>
          
          <div style={{ border: BORDER, borderRadius: "4px", maxHeight: "300px", overflowY: "auto", marginBottom: "20px" }}>
            {statusLog.map((log, index) => (
              <div key={index} style={{ padding: "6px", borderBottom: index < statusLog.length - 1 ? BORDER : "none", fontSize: "12px", color: "#000000" }}>
                {log}
              </div>
            ))}
          </div>
          ...
          {importResult && (
            <div style={{ marginBottom: "20px", padding: "15px", borderRadius: "6px", border: BORDER, backgroundColor: importResult.errors.length > 0 ? "#fee2e2" : "#dcfce7" }}>
              <h4 style={{ fontSize: "14px", fontWeight: "bold", color: "#000000", marginBottom: "10px" }}>Import Summary</h4>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "15px", marginBottom: "15px" }}>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px" }}>
                  ...
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: "#000000" }}>{importResult.imported}</div>
                </div>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px" }}>
                  <div style={{ fontSize: "12px", color: "#000000" }}>Skipped</div>
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: "#000000" }}>{importResult.skipped}</div>
                </div>
                <div style={{ padding: "10px", backgroundColor: BG_HEADER, borderRadius: "4px" }}>
                  <div style={{ fontSize: "12px", color: "#000000" }}>Errors</div>
                  <div style={{ fontSize: "18px", fontWeight: "bold", color: "#000000" }}>{importResult.errors.length}</div>
                </div>
              </div>
              
              {importResult.errors.length > 0 && (
                <div style={{ marginTop: "15px" }}>
                  <h5 style={{ fontSize: "12px", fontWeight: "bold", color: "#dc2626", marginBottom: "5px" }}>Errors:</h5>
                  <div style={{ maxHeight: "150px", overflowY: "auto", fontSize: "11px", color: "#dc2626" }}>
  ...
  const renderExportMasters = () => (
    <div style={{ padding: "20px" }}>
      <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#000000", marginBottom: "15px" }}>Export Masters</h3>
      <p style={{ color: "#666", marginBottom: "20px" }}>Select masters to export to Excel</p>
      
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "15px", marginBottom: "20px" }}>
        {["Account Masters", "Item Masters", "Party Masters", "Units", "Cost Centers"].map((master) => (
          <label key={master} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px", border: BORDER, borderRadius: "4px" }}>
  ...
  const renderImportFromExcel = () => (
    <div style={{ padding: "20px" }}>
      <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#000000", marginBottom: "15px" }}>Import from Excel</h3>
      
      <div style={{ marginBottom: "20px" }}>
        <label style={{ display: "block", marginBottom: "5px", fontWeight: "bold" }}>Import Type</label>
        <select
      ...
      <div style={{ marginBottom: "20px" }}>
        <label style={{ display: "block", marginBottom: "5px", fontWeight: "bold" }}>Excel File</label>
        <input type="file" accept=".xlsx,.csv" style={{ width: "100%", padding: "8px", border: BORDER, borderRadius: "4px" }} />
      </div>
  ...
  const renderImportVouchers = () => (
    <div style={{ padding: "20px" }}>
      <h3 style={{ fontSize: "16px", fontWeight: "bold", color: "#000000", marginBottom: "15px" }}>Import Vouchers from Excel</h3>
  ...
  return (
    <div style={{ backgroundColor: "#f5f6fa", minHeight: "500px", padding: "20px" }}>
      <div style={{ backgroundColor: "#ffffff", borderRadius: "8px", border: BORDER, overflow: "hidden" }}>
        {/* Tab Navigation */}
