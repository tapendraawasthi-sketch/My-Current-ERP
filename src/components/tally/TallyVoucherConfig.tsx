...
const ROWS: ConfigRow[] = [
  {
    ...
    hint: "Show Ref / Cheque No field",
    isBool: true,
  },
  {
    key: "showNarrationPerEntry",
    label: "Narration per Entry",
    hint: "Allow narration on each ledger line",
    isBool: true,
  },
  {
    key: "showBillWise",
    label: "Bill-wise Details",
    hint: "Allocate against outstanding invoices",
    isBool: true,
    types: ["payment", "receipt", "journal"],
  },
  {
    key: "showBankAllocation",
    label: "Bank Allocation",
    hint: "Cheque / NEFT / UPI details popup",
    isBool: true,
    types: ["payment", "receipt", "contra"],
  },
  {
    key: "showCashDenomination",
    label: "Cash Denominations",
    hint: "Note-wise breakdown for cash entries",
    isBool: true,
    types: ["contra", "payment", "receipt"],
  },
  {
    key: "showCostCenter",
    label: "Cost Centre",
    hint: "Enable cost-centre allocation per line",
    isBool: true,
  },
  {
    key: "showGST",
    label: "GST / Tax Details",
    hint: "Show Place of Supply, HSN/SAC fields",
    isBool: true,
    types: ["sales", "purchase"],
  },
  {
    key: "showDispatchDetails",
    label: "Dispatch Details",
    hint: "Transporter, vehicle, LR number",
    isBool: true,
    types: ["sales"],
  },
  {
    ...
    label: "Order Details",
    hint: "Link sales/purchase to order reference",
    isBool: true,
    types: ["sales", "purchase"],
  },
];
...
export const TallyVoucherConfig: React.FC<Props> = ({ voucherType, config, onChange, onClose }) => {
  const set = (key: keyof F12VoucherConfig, value: any) => onChange({ ...config, [key]: value });

  const toggle = (key: keyof F12VoucherConfig) => set(key, !config[key]);

  const visibleRows = ROWS.filter((r) => !r.types || r.types.includes(voucherType));
            ...
            <div className="tally-f12-row">
              <div>
                ...
                <div className="text-[11px] text-gray-600">
                  Single Entry (one primary + many) or Double Entry (Dr/Cr grid)
                </div>
              </div>
              <select
          ...
          {["sales", "purchase"].includes(voucherType) && (
            <div className="tally-f12-row">
              <div>
                ...
                <div className="text-[11px] text-gray-600">
                  Item Invoice (stock tracking) or Accounting Invoice (services)
                </div>
              </div>
              <select
                className="tally-input"
          ...
          {voucherType === "journal" && (
            <div className="tally-f12-row">
              <div>
                ...
                <div className="text-[11px] text-gray-600">
                  Show By/To (traditional) or Dr/Cr (accounting) labels
                </div>
              </div>
              <select
                className="tally-input"
          ...
          {visibleRows
            ...
            .map((r) => (
              <div key={r.key} className="tally-f12-row">
                ...
                <Toggle on={!!config[r.key]} onToggle={() => toggle(r.key)} />
              </div>
            ))}
