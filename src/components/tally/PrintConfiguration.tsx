...
const DEFAULT_CONFIG: PrintConfig = {
  ...
  logoHeight: 60,
  companyNameFont: "Arial Bold",
  companNameFontSize: 16,
  printAddress: true,
  printGSTIN: true,
  showItemSerialNos: true,
  showHSN: true,
  showTaxBreakup: true,
...
const VOUCHER_TYPES = [
  ...
  { id: "purchase-order", label: "Purchase Order", formats: ["Purchase Order"] },
  {
    ...
    formats: ["Delivery Challan", "Packing Slip"],
  },
...
const PrintConfiguration = () => {
  ...
  const renderSection = (id: string, title: string, content: React.ReactNode) => (
    <div
      className={`animate-in fade-in duration-200 ${expandedSection === id ? "block" : "hidden"}`}
    >
      <div className="mb-4">
    ...
    <div className="space-y-1">
      ...
      <div className="flex gap-4 pt-3 mt-1 border-t border-gray-100">
        <div className="flex-1">
          ...
          >
            {["Arial Bold", "Times New Roman", "Helvetica", "Calibri Bold", "Georgia"].map(
              (font) => (
  ...
  const renderWatermark = () => (
    <div className="space-y-1">
      ...
      <div className="pt-2 border-t border-gray-100">
        ...
        <div className="text-[10px] text-gray-500 ml-1 italic">
          (Suppresses company name/address from printing)
        </div>
      </div>
    </div>
  );
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#f5f6fa] border-b border-gray-200">
                ...
                <th className="px-3 py-2 text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-right">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {currentVoucher.formats.map((format) => (
                <tr
                  key={format}
                  className={`hover:bg-gray-50 ${selectedFormat === format ? "bg-blue-50/30" : ""}`}
                >
    ...
    <div className="max-w-[1000px] mx-auto p-5 font-sans">
      ...
      <div className="bg-gray-50 border-x border-b border-gray-200 p-3 shadow-sm flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <div className="text-[11px] font-bold text-gray-700 uppercase tracking-wide whitespace-nowrap">
            Configure for:
          </div>
        ...
        <div className="bg-gray-100/50 p-4 border-l border-gray-200 flex flex-col h-[550px]">
          ...
          <div className="flex-1 bg-white border border-gray-300 shadow-md rounded-sm p-4 relative overflow-hidden flex flex-col mx-auto w-full max-w-[280px]">
            ...
            <div className="mb-2 relative z-10">
              ...
              {!config.usingPrePrintedLetterhead && (
                <>
                  ...
                  {config.printAddress && (
                    <div
                      className={`text-[7px] text-gray-600 mt-0.5 ${config.logoPosition === "top-center" ? "text-center" : "text-left"}`}
                    >
                  ...
                  {config.printGSTIN && (
                    <div
                      className={`text-[7px] text-gray-600 font-medium mt-0.5 ${config.logoPosition === "top-center" ? "text-center" : "text-left"}`}
                    >
            ...
            <div className="flex justify-between text-[7px] text-gray-700 mb-2 relative z-10">
              <div>
                <span className="font-semibold">Invoice No:</span> INV/24-25/001
                <br />
                <span className="font-semibold">Date:</span> 24-Oct-2024
              </div>
              <div className="text-right">
                <span className="font-semibold">Billed To:</span> Acme Corp
                <br />
            ...
            <table className="w-full text-left border-collapse text-[6px] mb-2 relative z-10">
              <thead>
                <tr className="bg-gray-100 border-y border-gray-300">
                  ...
                  {config.showTaxBreakup && (
                    <th className="py-1 px-1 font-semibold text-gray-700 text-right">CGST</th>
                  )}
                  {config.showTaxBreakup && (
                    <th className="py-1 px-1 font-semibold text-gray-700 text-right">SGST</th>
                  )}
              ...
              <tbody className="divide-y divide-gray-100">
                <tr>
                  ...
                  <td className="py-1 px-1 text-gray-800 font-medium">
                    Premium Widget{" "}
                    {config.showItemSerialNos && (
                      <span className="text-[5px] text-gray-400 block">SN: WID-9912</span>
                    )}
                  </td>
                  ...
                  {config.showTaxBreakup && (
                    <td className="py-1 px-1 text-gray-600 text-right">9%</td>
                  )}
                  {config.showTaxBreakup && (
                    <td className="py-1 px-1 text-gray-600 text-right">9%</td>
                  )}
                ...
                <tr>
                  ...
                  <td className="py-1 px-1 text-gray-800 font-medium">
                    Basic Gadget{" "}
                    {config.showItemSerialNos && (
                      <span className="text-[5px] text-gray-400 block">SN: GAD-1011</span>
                    )}
                  </td>
                ...
                <tr className="border-t border-gray-300">
                  <td
                    colSpan={
                      config.showHSN && config.showTaxBreakup
                        ? 7
                        : config.showHSN || config.showTaxBreakup
                          ? 6
                          : 5
                    }
                    className="py-1 px-1 font-bold text-gray-800 text-right"
                  >
                    Total ₹
                  </td>
                ...
                <div className="shrink-0 border border-gray-200 p-1 flex flex-col items-center justify-center">
                  <div className="w-8 h-8 grid grid-cols-5 gap-px bg-white">
                    {Array.from({ length: 25 }).map((_, i) => (
                      <div
                        key={i}
                        className={i % 2 === 0 || i % 7 === 0 ? "bg-black" : "bg-transparent"}
                      />
                    ))}
                  </div>
            ...
            {config.showSignatureLine && (
              <div className="mt-auto pt-4 text-right relative z-10">
                ...
                <div className="text-[5px] text-gray-500 mt-6 border-t border-dashed border-gray-400 inline-block pt-0.5">
