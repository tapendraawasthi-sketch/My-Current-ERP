...
const SecurityControl = () => {
  ...
  const MOCK_AUDIT = [
    { timestamp: '2024-04-05 10:32:15', user: 'accounts1', action: 'CREATE' as const, voucher: 'SI-001', details: 'Sales Invoice ₹11,800 — ABC Traders' },
    { timestamp: '2024-04-05 14:15:30', user: 'accounts1', action: 'ALTER' as const, voucher: 'SI-001', details: 'Changed amount from ₹10,000 to ₹11,800' },
    { timestamp: '2024-04-05 14:20:00', user: 'manager', action: 'APPROVE' as const, voucher: 'SI-001', details: 'Marked approved' },
    { timestamp: '2024-04-06 09:00:00', user: 'admin', action: 'DELETE' as const, voucher: 'SI-001', details: 'Deleted — Reason: Duplicate entry' },
    { timestamp: '2024-04-06 09:30:00', user: 'accounts1', action: 'CREATE' as const, voucher: 'PB-001', details: 'Purchase Bill ₹47,200 — XYZ Suppliers' },
    { timestamp: '2024-04-06 11:00:00', user: 'salesperson', action: 'CREATE' as const, voucher: 'SO-001', details: 'Sales Order ₹25,000 — New Customer' },
    { timestamp: '2024-04-06 11:05:00', user: 'salesperson', action: 'LOGIN' as const, voucher: '—', details: 'Logged in from 192.168.1.15' },
    { timestamp: '2024-04-06 17:30:00', user: 'salesperson', action: 'LOGOUT' as const, voucher: '—', details: 'Session ended — idle timeout' },
  ];
  ...
  const filteredAudit = MOCK_AUDIT.filter(row => {
    if (auditFilter.user && !row.user.toLowerCase().includes(auditFilter.user.toLowerCase())) return false;
    if (auditFilter.action && auditFilter.action !== 'All Actions' && row.action !== auditFilter.action) return false;
    return true;
  });
    ...
    <div className="max-w-[850px] mx-auto p-5 font-sans">
      ...
      <div className="bg-[#1e2433] px-4 py-3 rounded-t-lg flex justify-between items-center border-b border-gray-700 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[14px] font-semibold text-white tracking-wide">Security Control</span>
        </div>
        <div className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider text-white shadow-sm flex items-center gap-1.5 ${
      ...
      {showSetupForm && !localStorage.getItem('adminSetup') && (
        <div className="bg-amber-50 border-x border-b border-amber-200 p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="text-[13px] font-bold text-amber-800">Configure Administrator Account</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 ml-6 max-w-lg">
            <div className="col-span-2">
              <label className="block text-[11px] font-medium text-amber-900 mb-1">Name of Administrator</label>
              <input
                type="text"
                value={adminName}
            ...
            <div>
              <label className="block text-[11px] font-medium text-amber-900 mb-1">Confirm Password</label>
              <div className="relative">
                <input
                  type={showAdminPass ? 'text' : 'password'}
      ...
      {/* Main Content */}
      <div className={`bg-[#f5f6fa] border-x border-b border-gray-200 p-5 shadow-sm ${!showSetupForm && securityEnabled ? 'rounded-b-lg' : ''}`}>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100">
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Administrator</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">Full access — cannot be restricted</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Owner</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">Full access except Security Control</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Accountant</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">All accounting + inventory; no payroll/security</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Data Entry Operator</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">Create vouchers only; no reports</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Sales Person</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">Sales Invoice + Order; view Sales Register</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Purchase Manager</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">Purchase Bill + Order; no Sales access</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Inventory Manager</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">Full inventory; no accounting vouchers</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Payroll Officer</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">Full payroll; no accounting/inventory</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Auditor</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">View-only; cannot create/alter/delete</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-[11px] font-medium text-gray-800">Report Viewer</td>
                    <td className="px-3 py-2 text-[11px] text-gray-600">Reports only; no voucher access</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        {tab === 'admin' && securityEnabled && (
          <div className="bg-white border border-gray-200 rounded-md p-5 shadow-sm max-w-md mx-auto">
            ...
            <div className="mb-4">
              ...
              <div className="text-[10px] text-amber-600 font-medium mt-1">Password Strength: Fair</div>
            </div>
            ...
            <button
              onClick={() => toast.success('Admin password changed successfully. Keep it safe — there is no recovery option.')}
            ...
            <div className="mt-5 p-3 bg-blue-50 border border-blue-100 rounded-md">
              <div className="text-[11px] text-blue-800 leading-snug">
                <span className="font-semibold">Note:</span> The administrator account has unrestricted access and cannot be deleted. Last login was simulated at {new Date().toLocaleString()}.
              </div>
            </div>
          </div>
        )}
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm">
            ...
            {!auditEnabled && (
              <div className="p-8 text-center border-t border-gray-100">
                ...
                <div className="text-[11px] text-gray-500 mt-1">Enable above to track all voucher changes and user actions.</div>
              </div>
            )}
            ...
            {auditEnabled && (
              <div className="p-4">
                ...
                <div className="flex gap-2 mb-4 flex-wrap">
                  ...
                  />
                  <button
                    onClick={() => setAuditFilter({ user: '', action: '', dateFrom: '', dateTo: '' })}
                ...
                <div className="border border-gray-200 rounded-md overflow-hidden">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="px-3 py-2 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Timestamp</th>
                        <th className="px-3 py-2 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">User</th>
                        <th className="px-3 py-2 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Action</th>
                        <th className="px-3 py-2 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Voucher</th>
                        <th className="px-3 py-2 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Details</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 bg-white">
                      {filteredAudit.map((row, index) => (
                        <tr key={index} className="hover:bg-gray-50/50">
                          <td className="px-3 py-2.5 text-[11px] text-gray-600 font-mono">{row.timestamp}</td>
                          <td className="px-3 py-2.5 text-[11px] text-gray-800 font-medium">{row.user}</td>
                          <td className="px-3 py-2.5">
                            <span 
                              className={`px-1.5 py-0.5 rounded-[3px] text-[9px] font-bold uppercase tracking-wider border ${
                                row.action === 'CREATE' ? 'bg-green-50 text-green-700 border-green-200' :
                                row.action === 'ALTER' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                                row.action === 'DELETE' ? 'bg-red-50 text-red-700 border-red-200' :
                                row.action === 'APPROVE' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                ...
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-md flex gap-2">
                  ...
                  <div className="text-[11px] text-amber-800 leading-snug">
                    <span className="font-semibold">Security Note:</span> Audit logs are append-only. No user (including the Administrator) can delete or edit them through the application. TallyVault encryption protects audit logs from physical file access tampering.
                  </div>
                </div>
              </div>
            )}
