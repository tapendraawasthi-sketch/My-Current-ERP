...
export const TallyBillWiseAllocation: React.FC<Props> = ({
  ...
  return (
    <div className="tally-modal-overlay fixed inset-0 z-[100] flex items-center justify-center bg-black/30" onClick={onClose}>
      <div className="tally-modal bg-[var(--t-card)] border-2 border-[#2E5B1E] rounded-md max-w-[640px] w-full" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="tally-modal-body p-3">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="border border-[#9DC07A] bg-[var(--t-muted)] text-[11px] uppercase p-1">Method</th>
                <th className="border border-[#9DC07A] bg-[var(--t-muted)] text-[11px] uppercase p-1">Reference</th>
                <th className="border border-[#9DC07A] bg-[var(--t-muted)] text-[11px] uppercase p-1">Date</th>
                <th className="border border-[#9DC07A] bg-[var(--t-muted)] text-[11px] uppercase p-1 text-right">Amount</th>
              ...
              {rows.map((r, i) => (
                <tr key={r.id}>
                  <td className="border border-[#9DC07A] p-1">
                    <select className="tally-input w-full p-1" value={r.method} onChange={(e) => set(i, 'method', e.target.value as any)}>
                  ...
                  <td className="border border-[#9DC07A] p-1">
                    {r.method === 'Against Reference' && partyInvoices.length ? (
                      <select className="tally-input w-full p-1" value={r.refNo} onChange={(e) => set(i, 'refNo', e.target.value)}>
                      ...
                      <input className="tally-input w-full p-1" value={r.refNo} onChange={(e) => set(i, 'refNo', e.target.value)} />
                    )}
                  </td>
                  <td className="border border-[#9DC07A] p-1">
                    <input type="date" className="tally-input w-full p-1" value={r.refDate} onChange={(e) => set(i, 'refDate', e.target.value)} />
                  </td>
                  <td className="border border-[#9DC07A] p-1">
                    <input type="number" className="tally-input w-full p-1 text-right" value={r.amount} onChange={(e) => set(i, 'amount', parseMoney(e.target.value))} />
                  </td>
          ...
          <div className="flex items-center justify-between mt-2">
            <button className="tally-btn flex items-center gap-1" onClick={() => setRows((p) => [...p, { id: cryptoRandomId(), method: 'New Reference', refNo: '', refDate: todayAD(), amount: diff > 0 ? diff : 0 }])}>
              <Plus size={12} /> Add Bill
            </button>
            <span className={`tally-total-box ${Math.abs(diff) > 0.001 ? 'tally-cr text-[#8B1E1E]' : 'tally-dr text-[#1B5E20]'}`}>
        ...
        <div className="flex justify-end gap-2 p-3 no-print">
          ...
          <button className="tally-btn tally-btn-primary flex items-center gap-1" onClick={() => { onSave(rows); onClose(); }}>
            <Save size={13} /> Accept
          </button>
        </div>
      </div>
