...
import {
  ...
  TRANSACTION_TYPES,
  DENOMINATIONS,
  type F12VoucherConfig,
  DEFAULT_F12_CONFIG,
  type InvoiceItemLine,
  type BillWiseAllocation,
...
const TallySalesVoucher: React.FC<Props> = ({ onSwitchType }) => {
  ...
  const [taxLines, setTaxLines] = useState<
    Array<{ id: string; accountId: string; accountName: string; rate: number; amount: number }>
  >([]);

  // ── Accounting Invoice mode ──
  const [acctLines, setAcctLines] = useState([
    { id: cryptoRandomId(), accountId: "", accountName: "", amount: 0, narration: "" },
  ]);
  ...
  const taxTotal = useMemo(() => taxLines.reduce((s, t) => s + (t.amount || 0), 0), [taxLines]);
  const grandTotal = round2(itemSubTotal + taxTotal);

  const acctTotal = useMemo(() => acctLines.reduce((s, l) => s + (l.amount || 0), 0), [acctLines]);
  ...
  const handleAccept = useCallback(async () => {
    ...
    if (invoiceMode === "item") {
      if (!partyId) {
        toast.error("Select party / customer.");
        return;
      }
      if (grandTotal <= 0) {
        toast.error("Grand total must be > 0.");
        return;
      }
  ...
  React.useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "F8") {
        ...
        e.preventDefault();
        setShowF12((v) => !v);
      } else if (alt && e.key.toLowerCase() === "i") {
        e.preventDefault();
        setInvoiceMode((m) => (m === "item" ? "accounting" : "item"));
      } else if (ctrl && e.key.toLowerCase() === "v") {
        e.preventDefault();
        setInvoiceMode((m) => (m === "voucher" ? "item" : "voucher"));
        ...
        e.preventDefault();
        setIsOptional((v) => !v);
      } else if (ctrl && (e.key.toLowerCase() === "a" || e.key === "Enter")) {
        e.preventDefault();
        handleAccept();
      } else if (e.key === "Escape") {
        e.preventDefault();
        resetForm();
      } else if (alt && e.key.toLowerCase() === "d") {
        e.preventDefault();
        toast("Delete not available for new vouchers.");
      }
    };
    ...
    <div className="tally-shell flex flex-col h-full w-full overflow-hidden">
      ...
      <div className="tally-header flex flex-wrap items-center gap-2 px-3 py-1">
        {[
          ...
          <button
            key={k}
            className={`tally-hint ${l === "Sales" ? "tally-btn-primary" : ""}`}
        ...
        <button
          className="tally-hint"
          onClick={() => setInvoiceMode((m) => (m === "item" ? "accounting" : "item"))}
        >
          Alt+I: {invoiceMode === "item" ? "Item→Acctg" : "Acctg→Item"}
        </button>
        <button
          className="tally-hint"
          onClick={() => setInvoiceMode((m) => (m === "voucher" ? "item" : "voucher"))}
        >
          Ctrl+V: {invoiceMode === "voucher" ? "Invoice" : "Voucher"}
        </button>
        <button className="tally-hint" onClick={() => setShowF12(true)}>
          F12: Config
        </button>
      </div>
      ...
      <div className="tally-mode-tabs">
        <button
          className={`tally-mode-tab ${invoiceMode === "item" ? "active" : ""}`}
          onClick={() => setInvoiceMode("item")}
        >
          Item Invoice (Alt+I)
        </button>
        <button
          className={`tally-mode-tab ${invoiceMode === "accounting" ? "active" : ""}`}
          onClick={() => setInvoiceMode("accounting")}
        >
          Accounting Invoice
        </button>
        <button
          className={`tally-mode-tab ${invoiceMode === "voucher" ? "active" : ""}`}
          onClick={() => setInvoiceMode("voucher")}
        >
          As Voucher (Ctrl+V)
        </button>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
              ...
              {f12Config.showDispatchDetails && (
                <div className="grid grid-cols-3 gap-3 mt-2">
                  <label className="flex flex-col gap-0.5">
                    ...
                    <input
                      className="tally-input"
                      value={dispatchDetails.transporter}
                      onChange={(e) =>
                        setDispatchDetails((p) => ({ ...p, transporter: e.target.value }))
                      }
                    />
                  </label>
                  <label className="flex flex-col gap-0.5">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDispatchDetails((p) => ({ ...p, vehicleNo: e.target.value }))
                      }
                    />
                  </label>
                  <label className="flex flex-col gap-0.5">
                    ...
                    <input
                      className="tally-input"
                      value={dispatchDetails.lrNo}
                      onChange={(e) => setDispatchDetails((p) => ({ ...p, lrNo: e.target.value }))}
                    />
                  </label>
                </div>
              )}
          ...
          {invoiceMode === "item" && (
            <div className="flex-1 overflow-auto px-3 py-2">
              <table className="tally-grid tally-item-grid">
                <thead>
                  <tr>
                    ...
                    <th className="col-qty" style={{ width: 72 }}>
                      Qty
                    </th>
                    <th style={{ width: 64 }}>Unit</th>
                    <th className="col-rate" style={{ width: 100 }}>
                      Rate
                    </th>
                    ...
                    <tr key={l.id}>
                      ...
                      <td>
                        <input
                          ...
                          className="tally-input text-right"
                          value={l.qty || ""}
                          onChange={(e) => updateItemLine(i, { qty: parseMoney(e.target.value) })}
                        />
                      </td>
                      <td>
                        <input
                          className="tally-input"
                          value={l.unit}
                          onChange={(e) => updateItemLine(i, { unit: e.target.value })}
                          style={{ width: 56 }}
                        />
                      </td>
                      <td>
                        <input
                          ...
                          className="tally-input text-right"
                          value={l.rate || ""}
                          onChange={(e) => updateItemLine(i, { rate: parseMoney(e.target.value) })}
                        />
                      </td>
                      <td>
                        <input
                          className="tally-input"
                          value={l.perUnit}
                          onChange={(e) => updateItemLine(i, { perUnit: e.target.value })}
                          style={{ width: 48 }}
                        />
                      </td>
                      <td className="cell-dr">
                        <input
                          type="number"
                          className="tally-input text-right"
                          value={l.amount || ""}
              ...
              <button
                className="tally-btn text-xs mt-2"
                onClick={() => setItemLines((p) => [...p, blankItemLine()])}
              >
              ...
              <div className="mt-3 border-t border-[var(--t-line-soft)] pt-2">
                ...
                {taxLines.map((t, i) => (
                  <div key={t.id} className="flex gap-3 items-end mt-1">
                    <label className="flex flex-col gap-0.5 w-48">
                      ...
                      <LedgerSelect
                        ...
                        onChange={(id, name) =>
                          setTaxLines((p) =>
                            p.map((r, idx) =>
                              idx === i ? { ...r, accountId: id, accountName: name } : r,
                            ),
                          )
                        }
                      />
                          ...
                          setTaxLines((p) =>
                            p.map((r, idx) =>
                              idx === i
                                ? {
                                    ...r,
                                    rate: parseMoney(e.target.value),
                                    amount: round2(
                                      (itemSubTotal * parseMoney(e.target.value)) / 100,
                                    ),
                                  }
                                : r,
                            ),
                    ...
                    <button
                      className="tally-btn py-0.5 px-1"
                      onClick={() => setTaxLines((p) => p.filter((_, idx) => idx !== i))}
                    >
                      <Trash2 size={11} />
                    </button>
                  </div>
                ...
                <button
                  className="tally-btn text-xs mt-1"
                  onClick={() =>
                    setTaxLines((p) => [
                      ...p,
                      {
                        ...
                        accountName: "",
                        rate: 13,
                        amount: round2(itemSubTotal * 0.13),
                      },
                    ])
                  }
                >
              ...
              <div className="flex justify-end mt-3">
                <div className="tally-totals-panel">
                  <div className="row">
                    <span>Sub Total</span>
                    <span className="tally-dr">{formatMoney(itemSubTotal)}</span>
                  </div>
                  <div className="row">
                    <span>Tax Amount</span>
                    <span className="tally-dr">{formatMoney(taxTotal)}</span>
                  </div>
                  <div className="row grand">
                    <span>Grand Total</span>
                    <span className="tally-dr">{formatMoney(grandTotal)}</span>
                  </div>
                </div>
              </div>
              ...
              {f12Config.showBillWise && partyId && grandTotal > 0 && (
                <div className="mt-2">
                  <button
                    className="tally-btn text-xs"
                    onClick={() => {
                      setBillWisePartyId(partyId);
                      setBillWiseAmount(grandTotal);
                    }}
                  >
                    <FileText size={11} className="inline mr-1" />
                    Bill-wise Details
                    {billWiseData.length > 0 && (
                      <span className="ml-1 tally-badge-post-dated">
                        {billWiseData.length} bills
                      </span>
                    )}
                  </button>
                </div>
            ...
            <div className="flex-1 overflow-auto px-3 py-2">
                    ...
                    <tr key={l.id}>
                      ...
                      <td className="cell-cr">
                        <input
                          ...
                          onChange={(e) =>
                            setAcctLines((p) =>
                              p.map((r, idx) =>
                                idx === i ? { ...r, amount: parseMoney(e.target.value) } : r,
                              ),
                            )
                          }
                        />
                        ...
                        {f12Config.showNarrationPerEntry && (
                          <input
                            ...
                            onChange={(e) =>
                              setAcctLines((p) =>
                                p.map((r, idx) =>
                                  idx === i ? { ...r, narration: e.target.value } : r,
                                ),
                              )
                            }
                      ...
                      <td className="text-center">
                        {acctLines.length > 1 && (
                          <button
                            className="tally-btn py-0.5 px-1"
                            onClick={() => setAcctLines((p) => p.filter((_, idx) => idx !== i))}
                          >
                            <Trash2 size={11} />
                          </button>
                        )}
              ...
              <div className="flex justify-end mt-3">
                <div className="tally-totals-panel">
                  <div className="row grand">
                    <span>Total</span>
                    <span className="tally-dr">{formatMoney(acctTotal)}</span>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <tr key={l.id}>
                      ...
                      <td>
                        <select
                          ...
                          onChange={(e) =>
                            setVLines((p) =>
                              p.map((r, idx) => (idx === i ? { ...r, drcr: e.target.value } : r)),
                            )
                          }
                        >
                      ...
                      <td className="text-center">
                        {vLines.length > 1 && (
                          <button
                            className="tally-btn py-0.5 px-1"
                            onClick={() => setVLines((p) => p.filter((_, idx) => idx !== i))}
                          >
                            <Trash2 size={11} />
                          </button>
                        )}
          ...
          <div className="tally-footer px-3 py-2 flex items-center justify-between no-print">
            ...
            <div className="flex gap-3 text-sm">
              <div className="flex flex-col items-end">
                ...
                <span className="tally-dr">
                  {formatMoney(
                    invoiceMode === "item"
                      ? grandTotal
                      : invoiceMode === "accounting"
                        ? acctTotal
                        : vLines.filter((l) => l.drcr === "dr").reduce((s, l) => s + l.amount, 0),
                  )}
                </span>
              </div>
              <div className="flex flex-col items-end">
                <span className="tally-label text-[10px]">Total Cr</span>
                <span className="tally-cr">
                  {formatMoney(
                    invoiceMode === "item"
                      ? grandTotal
                      : invoiceMode === "accounting"
                        ? acctTotal
                        : vLines.filter((l) => l.drcr === "cr").reduce((s, l) => s + l.amount, 0),
                  )}
                </span>
              </div>
            </div>
        ...
        <div className="tally-right-bar w-[130px] p-2 flex flex-col gap-2 no-print">
          <button className="tally-btn tally-btn-primary" onClick={handleAccept}>
            <Save size={12} /> Ctrl+A: Save
          </button>
          <button className="tally-btn" onClick={() => setShowF12(true)}>
            <Settings size={12} /> F12: Config
          </button>
          <button
            className={`tally-btn ${isPostDated ? "tally-btn-primary" : ""}`}
            onClick={() => setIsPostDated((v) => !v)}
          >
            Ctrl+T: Post-Date
          </button>
          <button
            className={`tally-btn ${isOptional ? "tally-btn-primary" : ""}`}
            onClick={() => setIsOptional((v) => !v)}
          >
            Ctrl+L: Optional
          </button>
          <button className="tally-btn" onClick={resetForm}>
            Esc: Clear
          </button>
        </div>
      </div>
      ...
      {showF12 && (
        <TallyVoucherConfig
          ...
          onChange={(cfg) => {
            setF12Config(cfg);
            setInvoiceMode(cfg.invoiceMode as InvoiceMode);
          }}
      ...
      {billWisePartyId && (
        <BillWiseModal
          ...
          invoices={invoices}
          onSave={(rows) => {
            setBillWiseData(rows);
            setBillWisePartyId(null);
          }}
