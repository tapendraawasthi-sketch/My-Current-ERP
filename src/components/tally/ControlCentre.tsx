...
import { Monitor, Cloud, RefreshCw, Users, Wifi, WifiOff, Download, AlertTriangle, CheckCircle, Settings, Globe, Key, CreditCard, Clock, Shield } from 'lucide-react';
import toast from 'react-hot-toast';
import { useStore } from '../../store/useStore';

const ControlCentre = () => {
  const { companySettings } = useStore();
  const [tab, setTab] = useState<'license' | 'account' | 'services' | 'remote' | 'updates'>('license');
  ...
  const [remoteEnabled, setRemoteEnabled] = useState(() => localStorage.getItem('remoteEnabled') === 'true');
      ...
      <div className="bg-[#f5f6fa] border border-gray-200 border-t-0 p-5 rounded-b-lg shadow-sm">
              ...
              <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center text-[12px]">
                    ...
                    <span className="font-semibold text-gray-800">Gold (Multi-User, 10 licenses)</span>
                  </div>
                  <div className="flex justify-between items-center text-[12px]">
                    <span className="text-gray-500 font-medium">Serial Number:</span>
                    <span className="font-mono font-semibold text-gray-800 bg-gray-50 px-1.5 py-0.5 rounded border border-gray-100">SUTRA-2024-NPL-XXXXXX</span>
                  </div>
                  <div className="flex justify-between items-center text-[12px]">
                  ...
                  <div className="flex justify-between items-center text-[12px] pt-2 border-t border-gray-100">
                    ...
                    <span className="font-semibold text-gray-600 text-[11px]">Today, 09:45 AM ✓ (Online)</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm">
                ...
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden mb-1.5">
                  <div className="bg-[#059669] h-full rounded-full transition-all" style={{ width: '30%' }} />
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="px-2 py-1.5 text-[9px] font-semibold text-gray-500 uppercase tracking-wide">User</th>
                        <th className="px-2 py-1.5 text-[9px] font-semibold text-gray-500 uppercase tracking-wide">Machine</th>
                        <th className="px-2 py-1.5 text-[9px] font-semibold text-gray-500 uppercase tracking-wide">Since</th>
                        <th className="px-2 py-1.5 text-[9px] font-semibold text-gray-500 uppercase tracking-wide text-right">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {activeSessions.map((session, index) => (
                        <tr key={index} className="hover:bg-gray-50/50">
                          <td className="px-2 py-2 text-[11px] font-medium text-gray-800">{session.user}</td>
                      ...
                      {activeSessions.length === 0 && (
                        <tr>
                          <td colSpan={4} className="px-2 py-4 text-center text-[11px] text-gray-500 italic">No active sessions</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
              ...
              <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm">
                ...
                <div className="flex flex-col gap-2 mb-3">
                  <label className="flex items-center gap-2 text-[11px] font-medium text-gray-700 cursor-pointer">
                    <input type="radio" name="transfer" defaultChecked className="text-[#1557b0] focus:ring-[#1557b0]" /> 
                    Deactivate on this machine
                  </label>
                  <label className="flex items-center gap-2 text-[11px] font-medium text-gray-700 cursor-pointer">
                    <input type="radio" name="transfer" className="text-[#1557b0] focus:ring-[#1557b0]" /> 
          ...
          <div className="max-w-2xl mx-auto flex flex-col gap-5">
            ...
            <div className="bg-white border border-gray-200 rounded-md p-5 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8 mb-5">
                <div>
                  <div className="text-[10px] text-gray-500 font-medium uppercase tracking-wide mb-1">Account Email</div>
                  <div className="text-[13px] font-semibold text-gray-800">owner@company.com</div>
                </div>
                <div>
                  <div className="text-[10px] text-gray-500 font-medium uppercase tracking-wide mb-1">Account ID</div>
                  <div className="text-[13px] font-semibold text-gray-800">ACC-000123456</div>
                </div>
                <div className="sm:col-span-2">
                  <div className="text-[10px] text-gray-500 font-medium uppercase tracking-wide mb-1">Account Name</div>
                  <div className="text-[13px] font-semibold text-gray-800">{companySettings?.name || 'Company'}</div>
                </div>
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <div className="text-[11px] font-bold text-gray-700 uppercase tracking-wide">Linked Companies</div>
              </div>
              
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-white border-b border-gray-100">
                    <th className="px-4 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Company</th>
                    <th className="px-4 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Status</th>
                    <th className="px-4 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-right">Active Users</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  <tr className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-[12px] font-semibold text-gray-800">Current Company</td>
                    <td className="px-4 py-3">
                      <span className="bg-green-50 text-green-700 border border-green-200 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">Primary</span>
                    </td>
                    <td className="px-4 py-3 text-[11px] text-gray-600 font-medium text-right">3 users</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-[12px] font-medium text-gray-800">Branch Office</td>
                    <td className="px-4 py-3">
                      <span className="bg-green-50 text-green-700 border border-green-200 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">Active</span>
                    </td>
                    <td className="px-4 py-3 text-[11px] text-gray-600 font-medium text-right">1 user</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-[12px] font-medium text-gray-800">Demo Company</td>
                    <td className="px-4 py-3">
                      <span className="bg-gray-100 text-gray-600 border border-gray-200 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">Inactive</span>
                    </td>
                    <td className="px-4 py-3 text-[11px] text-gray-600 font-medium text-right">0 users</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        {tab === 'services' && (
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <div className="text-[11px] font-bold text-gray-700 uppercase tracking-wide">Connected Services Status</div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div className="divide-y divide-gray-100">
                  {[
                    { name: 'Cloud Backup', status: 'connected', detail: 'Last backup: Today 03:00 AM' },
                    { name: 'Remote Access', status: 'connected', detail: '2 remote sessions' },
                    { name: 'e-Way Bill Integration', status: 'connected', detail: 'NIC API: Connected' },
                    { name: 'e-Invoice Integration', status: 'connected', detail: 'IRP API: Connected (via GSP)' },
                  ].map((service, index) => (
                    <div key={index} className="px-4 py-3 flex justify-between items-center hover:bg-gray-50/50">
                      <div>
                ...
                <div className="divide-y divide-gray-100">
                  {[
                    { name: 'GST Filing Integration', status: 'connected', detail: 'GSTN API: Connected' },
                    { name: 'Bank Reconciliation Feed', status: 'disconnected', detail: 'Not configured' },
                    { name: 'WhatsApp/Email Sharing', status: 'disconnected', detail: 'Not configured' }
                  ].map((service, index) => (
                    <div key={index} className="px-4 py-3 flex justify-between items-center hover:bg-gray-50/50">
                      <div>
                        <div className="text-[12px] font-medium text-gray-800">{service.name}</div>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2 text-[12px] text-amber-900 pb-2 border-b border-amber-200/40">
                  ...
                  <div>
                    ...
                    <button className="ml-1 text-[#1557b0] hover:underline font-medium">View Release Notes</button>
                    <span className="mx-1.5 text-amber-300">•</span>
                    <button className="text-[#1557b0] hover:underline font-medium">Update Now</button>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2 text-[12px] text-amber-900">
                  <AlertTriangle size={14} className="shrink-0 mt-0.5 text-amber-600" />
                  <div>
                    <span className="font-semibold">Compliance Alert:</span> GST API update required by 31st May — 
                    <button className="ml-1 text-[#1557b0] hover:underline font-medium">Read More</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-md p-5 shadow-sm max-w-2xl mx-auto">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-100 mb-5">
              <div>
                ...
                <div className="text-[11px] text-gray-500 mt-0.5">Allow users to access this company data remotely over the internet.</div>
              </div>
            ...
            {remoteEnabled && (
              <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-top-2 duration-200">
                {/* Mode Selection */}
                <div>
                  <label className="block text-[11px] font-semibold text-gray-700 mb-2 uppercase tracking-wide">Connection Mode</label>
                  <div className="flex flex-col gap-2.5">
                    <label className="flex items-start gap-2.5 text-[12px] font-medium text-gray-700 cursor-pointer">
                      ...
                      />
                      <div>
                        <div className="text-gray-800">Cloud Relay <span className="text-[10px] font-normal text-green-600 bg-green-50 px-1.5 py-0.5 rounded ml-1">Easiest Setup</span></div>
                        <div className="text-[11px] text-gray-500 font-normal mt-0.5">Connects via vendor's secure relay server. No router configuration required.</div>
                      </div>
                    </label>
                    <label className="flex items-start gap-2.5 text-[12px] font-medium text-gray-700 cursor-pointer">
                      <input
                      ...
                      />
                      <div>
                        <div className="text-gray-800">Direct Tunnel (TLS/SSL) <span className="text-[10px] font-normal text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded ml-1">Best Performance</span></div>
                        <div className="text-[11px] text-gray-500 font-normal mt-0.5">Direct connection to this machine. Requires port forwarding in your router.</div>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-5">
                  {/* Port */}
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Port</label>
                    <input
                      type="number"
                      value={odbcPort}
                  ...
                  {/* Max Connections */}
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Max Concurrent Sessions</label>
                    <input
                      type="number"
                ...
                {remoteMode === 'tunnel' && (
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-700 mb-2 uppercase tracking-wide">SSL Certificate</label>
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 text-[12px] font-medium text-gray-700 cursor-pointer">
                        <input type="radio" name="ssl" defaultChecked className="text-[#1557b0] focus:ring-[#1557b0]" /> 
                        Auto (Let's Encrypt — auto-renews)
                      </label>
                      <label className="flex items-center gap-2 text-[12px] font-medium text-gray-700 cursor-pointer">
                        <input type="radio" name="ssl" className="text-[#1557b0] focus:ring-[#1557b0]" /> 
                        Custom certificate
                      </label>
                    </div>
                  </div>
                ...
                {/* Whitelist IPs */}
                <div>
                  <label className="block text-[11px] font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Whitelist IPs</label>
                  <textarea
                    value={whitelistIPs}
        ...
        {tab === 'updates' && (
          <div className="max-w-2xl mx-auto flex flex-col gap-5">
            ...
            <div className="bg-white border border-gray-200 rounded-md shadow-sm p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                <div className="text-[10px] text-gray-500 font-semibold uppercase tracking-wide mb-1">Current Version</div>
                <div className="text-[14px] font-bold text-gray-800">v4.1.0 <span className="text-[11px] font-medium text-gray-500 ml-1">(Build 2024.04.01)</span></div>
              </div>
              
              {updateAvailable && (
                <div className="bg-blue-50 border border-blue-200 rounded-md px-3 py-2 flex items-center gap-3">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-bold text-blue-600 uppercase tracking-wide">Update Available</span>
            ...
            <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <div className="text-[11px] font-bold text-gray-700 uppercase tracking-wide">Update Settings</div>
              </div>
              <div className="p-5 flex flex-col gap-5">
                {/* Update Channel */}
                <div>
                  <label className="block text-[11px] font-semibold text-gray-700 mb-2 uppercase tracking-wide">Release Channel</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[12px] font-semibold text-gray-800">Auto-Download</span>
                        <div className="flex rounded shadow-sm">
                          <button
                            onClick={() => setAutoDownload(true)}
                            className={`px-3 py-1 text-[10px] font-bold border transition-colors rounded-l ${
                              autoDownload ? 'bg-[#059669] text-white border-[#059669] z-10' : 'bg-white text-gray-700 border-gray-300'
                            }`}
                          >Yes</button>
                          <button
                            onClick={() => setAutoDownload(false)}
                            className={`px-3 py-1 text-[10px] font-bold border-y border-r transition-colors rounded-r -ml-px ${
                              !autoDownload ? 'bg-[#dc2626] text-white border-[#dc2626] z-10' : 'bg-white text-gray-700 border-gray-300'
                            }`}
                          >No</button>
                        </div>
                      </div>
                      <div className="text-[11px] text-gray-500">Downloads in background during idle time.</div>
                    </div>
                    
                    {/* Auto-Install */}
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[12px] font-semibold text-gray-800">Auto-Install</span>
                        <div className="flex rounded shadow-sm">
                          <button
                            onClick={() => setAutoInstall(true)}
                            className={`px-3 py-1 text-[10px] font-bold border transition-colors rounded-l ${
                              autoInstall ? 'bg-[#059669] text-white border-[#059669] z-10' : 'bg-white text-gray-700 border-gray-300'
                            }`}
                          >Yes</button>
                          <button
                            onClick={() => setAutoInstall(false)}
                            className={`px-3 py-1 text-[10px] font-bold border-y border-r transition-colors rounded-r -ml-px ${
                              !autoInstall ? 'bg-[#dc2626] text-white border-[#dc2626] z-10' : 'bg-white text-gray-700 border-gray-300'
                            }`}
                          >No</button>
                        </div>
                      </div>
                      <div className="text-[11px] text-gray-500">Installs at next startup without prompting.</div>
                    </div>
                  </div>
                  
                  <div className="mt-4 pt-4 border-t border-gray-100 flex items-center gap-3">
                    <span className="text-[12px] font-semibold text-gray-800">Scheduled Update Window:</span>
                    <input
                      type="text"
            ...
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-md flex items-start gap-2 mt-2">
              ...
              <div className="text-[11px] text-amber-800 leading-snug">
                <span className="font-semibold">Important:</span> Tax rate changes (GST, TDS, PF) are pushed as mandatory updates with a strict deadline. If not updated by the deadline, statutory voucher creation for the affected category will be blocked.
              </div>
            </div>
          </div>
        )}
