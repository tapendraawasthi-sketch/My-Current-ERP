  ...
  return (
    <div className="fixed inset-0 z-[200] overflow-auto bg-white p-4 tally-print">
      <div className="no-print flex justify-end gap-2 mb-4">
        <button className="tally-btn tally-btn-primary flex items-center gap-1" onClick={handlePrint}>
          <Printer size={14} /> Print
        </button>
        {onClose && <button className="tally-btn" onClick={onClose}>Close</button>}
      </div>
      ...
      <div className="mx-auto max-w-[210mm] border border-black p-6 text-sm">
        <div className="text-center mb-4">
          ...
          <h3 className="text-lg font-bold mt-2 underline">{voucherTypeLabel[voucher.voucherType]}</h3>
        </div>

        <div className="flex justify-between mb-4">
          <div>
            <p><strong>Voucher No:</strong> {voucher.voucherNumber || 'Auto'}</p>
            <p><strong>Reference:</strong> {voucher.reference || '-'}</p>
          </div>
          <div className="text-right">
            <p><strong>Date (AD):</strong> {formatDate(voucher.date)}</p>
            <p><strong>Date (BS):</strong> {formatDateBS(voucher.date)}</p>
          </div>
        </div>
            ...
            {voucher.lines.map((line, idx) => (
              <tr key={line.id}>
                ...
                <td className="border border-black p-1">
                  {line.accountName}
                  {line.narration && <div className="text-xs italic text-gray-700 ml-4">{line.narration}</div>}
                </td>
                <td className="border border-black p-1 text-right">{line.debit ? formatMoney(line.debit) : ''}</td>
                <td className="border border-black p-1 text-right">{line.credit ? formatMoney(line.credit) : ''}</td>
              </tr>
            ))}
            <tr className="font-bold">
              <td className="border border-black p-1" colSpan={2}>Total</td>
              <td className="border border-black p-1 text-right">{formatMoney(voucher.totalDebit)}</td>
              <td className="border border-black p-1 text-right">{formatMoney(voucher.totalCredit)}</td>
            </tr>
          </tbody>
        </table>

        <div className="mb-4">
          <strong>Amount in words:</strong> {amountInWords(voucher.totalDebit || voucher.totalCredit)}
        </div>
