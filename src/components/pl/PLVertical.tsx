    ...
    >
      ...
      <td className="px-3 py-2 text-right font-mono text-[12px]">
        {isZero ? (
          ...
          <span className={isNeg ? "text-red-600" : bold || isTotal ? "text-gray-800 font-bold" : "text-gray-700"}>
            {isNeg ? "-" : ""}
            {fmt(Math.abs(amount))}
          </span>
        )}
      ...
      {lines.map((line) => {
        ...
        return (
          <VRow
            ...
            sign={1}
            clickable
            onClick={() => onDrillDown({ level: 2, selectedAccountId: line.accountId, selectedAccountName: line.accountName, selectedGroupId: groupId, selectedGroupLabel: groupLabel })}
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm max-w-4xl mx-auto">
      ...
      <table className="w-full">
        ...
        <tbody>
          {/* Revenue */}
          <VRow label="Revenue from Operations (Sales)" amount={pl.sales.total} bold clickable onClick={() => onDrillDown({ level: 1, selectedGroupId: "sales", selectedGroupLabel: "Sales Accounts" })} pct={options.showPercentage ? 100 : undefined} />
          <AccountGroup lines={pl.sales.lines} options={options} onDrillDown={onDrillDown} sign={1} groupId="sales" groupLabel="Sales Accounts" />

          <VRow label="Add: Direct Income" amount={pl.directIncome.total} indent={1} clickable onClick={() => onDrillDown({ level: 1, selectedGroupId: "direct-income", selectedGroupLabel: "Direct Income" })} pct={options.showPercentage ? (pl.directIncome.total / pl.revenueBase) * 100 : undefined} />
          <AccountGroup lines={pl.directIncome.lines} options={options} onDrillDown={onDrillDown} sign={1} groupId="direct-income" groupLabel="Direct Income" />

          <VRow label="Total Revenue" amount={totalRevenue} bold isTotal />

          {/* COGS */}
          <tr>
            <td colSpan={colCount} className="px-3 py-2 text-[12px] font-bold text-gray-800 bg-gray-50 border-y border-gray-100">
              Less: Cost of Goods Sold (COGS)
            </td>
          </tr>
          <VRow label="Opening Stock" amount={pl.openingStock} indent={1} sign={-1} clickable onClick={() => onDrillDown({ level: 1, selectedGroupId: "opening-stock", selectedGroupLabel: "Opening Stock" })} />
          
          <VRow label="Add: Purchases" amount={pl.purchases.total} indent={1} sign={-1} clickable onClick={() => onDrillDown({ level: 1, selectedGroupId: "purchases", selectedGroupLabel: "Purchases" })} pct={options.showPercentage ? (pl.purchases.total / pl.revenueBase) * 100 : undefined} />
          <AccountGroup lines={pl.purchases.lines} options={options} onDrillDown={onDrillDown} sign={-1} groupId="purchases" groupLabel="Purchases" />
          
          <VRow label="Add: Direct Expenses" amount={pl.directExpenses.total} indent={1} sign={-1} clickable onClick={() => onDrillDown({ level: 1, selectedGroupId: "direct-expenses", selectedGroupLabel: "Direct Expenses" })} pct={options.showPercentage ? (pl.directExpenses.total / pl.revenueBase) * 100 : undefined} />
          <AccountGroup lines={pl.directExpenses.lines} options={options} onDrillDown={onDrillDown} sign={-1} groupId="direct-expenses" groupLabel="Direct Expenses" />
          ...
          <AccountGroup lines={pl.indirectIncome.lines} options={options} onDrillDown={onDrillDown} sign={1} groupId="indirect-income" groupLabel="Indirect Income" />
          
          <VRow label="Less: Indirect Expenses" amount={pl.indirectExpenses.total} indent={1} sign={-1} clickable onClick={() => onDrillDown({ level: 1, selectedGroupId: "indirect-expenses", selectedGroupLabel: "Indirect Expenses" })} pct={options.showPercentage ? (pl.indirectExpenses.total / pl.revenueBase) * 100 : undefined} />
