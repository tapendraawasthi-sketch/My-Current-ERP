...
const BatchSerialSelector: React.FC<BatchSerialSelectorProps> = ({ 
  ...
  useEffect(() => {
    if (mode === "serial" && itemId) {
      const db = getDB();
      db.serialNumbers.where("itemId").equals(itemId)
              ...
              {itemBatches.map(batch => {
                ...
                return (
                  <option key={batch.id} value={batch.id}>
                    {batch.batchNo} | Qty: {batch.currentQty} | Exp: {batch.expiryDate} | Rate: {batch.saleRate}
                  </option>
                );
              })}
            </select>
          ) : (
            <div className="text-[12px] text-gray-500 py-1 italic">No active batches available for this item.</div>
          )}
          
          {itemBatches.length > 0 && selectedBatch && calculateDaysToExpiry(itemBatches.find(b => b.id === selectedBatch)?.expiryDate) <= 30 && (
      ...
      {mode === "serial" && (
        <div>
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className={`text-[10px] font-medium ${selectedSerials.length === quantity ? 'text-green-600' : 'text-gray-500'}`}>
