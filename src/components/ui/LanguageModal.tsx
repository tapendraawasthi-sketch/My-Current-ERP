...
export const DisplayLanguageModal: React.FC<{
  ...
  const [selected, setSelected] = useState(localStorage.getItem("displayLanguage") || "ne-NP");

  const DISPLAY_LANGUAGES = [
    { code: "ne-NP", label: "नेपाली (Nepali)", script: "Devanagari", region: "Nepal" },
  ...
  const handleApply = () => {
    ...
    toast.success(
      `Display language changed to ${selectedLanguage?.label}. App restart may be needed for full effect.`,
    );
    onClose();
  };
  ...
  return (
    <div
      className="fixed inset-0 bg-black/50 z-[var(--ds-z-modal)] flex items-center justify-center font-sans"
      onClick={onClose}
    >
      <div
      ...
      >
        {/* Header */}
        <div className="bg-[var(--ds-surface-inverse)] px-4 py-3 flex justify-between items-center border-b border-white/10">
          <div>
            <div className="text-[14px] font-semibold text-[var(--ds-text-inverse)]">
              Display language
            </div>
            <div className="text-[12px] text-[var(--ds-text-inverse)]/60 mt-0.5">
              Menus and labels only
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-4 py-2 bg-gray-50 border-b border-gray-200 flex items-center gap-2">
          <span className="text-[12px] text-[var(--ds-text-muted)] font-medium">Current:</span>
          <span className="text-[13px] font-medium text-[var(--ds-text-strong)]">
            {selectedLanguage?.label}
          </span>
        </div>

        {/* Note Band */}
        <div className="px-4 py-2 bg-[var(--ds-status-info-surface)] border-b border-[var(--ds-border-default)] flex items-start gap-2">
          <div className="text-[13px] text-[var(--ds-status-info)] leading-snug">
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`text-[13px] truncate ${
                      isSelected
                        ? "font-semibold text-[var(--ds-action-primary)]"
                        : "font-medium text-gray-800"
                    }`}
                  >
                    {lang.label}
                  </div>
                  <div className="text-[12px] text-gray-500 truncate mt-0.5">{lang.region}</div>
                </div>

                <div className="text-[12px] bg-gray-100 border border-gray-200 rounded px-1.5 py-0.5 text-gray-600 font-medium">
                  {lang.script}
                </div>
              </div>
            );
        ...
        {selected === "ar" && (
          <div className="px-4 py-2 bg-amber-50 border-t border-amber-100 flex items-start gap-2">
            ...
            <div className="text-[13px] text-amber-800 leading-snug">
              Arabic is RTL — layout will mirror. Menus appear on the right, text flows
              right-to-left.
            </div>
          </div>
        )}
        ...
        <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 flex justify-end gap-2">
          <button
            onClick={onClose}
            className="h-8 px-4 bg-white border border-gray-300 text-gray-700 text-[13px] font-medium rounded-md hover:bg-gray-100 transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={handleApply}
            className="h-8 px-4 bg-[var(--ds-action-primary)] hover:bg-[var(--ds-action-primary-hover)] text-white text-[13px] font-medium rounded-md transition-colors"
          >
            Apply
          </button>
        </div>
...
export const DataEntryLanguageModal: React.FC<{
  ...
  const DATA_ENTRY_OPTIONS = [
    {
      code: "ne-phonetic",
      label: "Nepali — Phonetic",
      mode: "phonetic",
      description: 'Type "Namaste" → नमस्ते',
    },
    {
      code: "en",
      label: "English",
      mode: "direct",
      description: "Direct keyboard input (default)",
    },
    {
      code: "hi-phonetic",
      label: "Hindi — Phonetic",
      mode: "phonetic",
      description: 'Type "Mera Naam" → मेरा नाम',
    },
    {
      code: "hi-inscript",
      label: "Hindi — Inscript",
      mode: "inscript",
      description: "Inscript keyboard layout",
    },
    {
      code: "ta-phonetic",
      label: "Tamil — Phonetic",
      mode: "phonetic",
      description: 'Type "Vanakkam" → வணக்கம்',
    },
    {
      code: "gu-phonetic",
      label: "Gujarati — Phonetic",
      mode: "phonetic",
      description: "Phonetic Gujarati input",
    },
    {
      code: "unicode",
      label: "Unicode Direct",
      mode: "unicode",
      description: "Direct bilingual keyboard input — accepts any Unicode character",
    },
  ];
  ...
  useEffect(() => {
    if (selectedLang.includes("phonetic")) {
      if (selectedLang.startsWith("ne")) {
        setPreviewOutput('e.g. type "Namaste" → नमस्ते (Nepali)');
    ...
    >
      ...
      >
        ...
        <div className="px-4 py-2 bg-blue-50 border-b border-blue-100 flex items-start gap-2">
          ...
          <div className="text-[12px] text-blue-800 leading-snug">
            Data entry language controls what language you TYPE in text fields (ledger names,
            narrations, addresses). It is separate from the display language.
          </div>
        </div>
          ...
          {DATA_ENTRY_OPTIONS.map((option) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors shrink-0 ${
                    isSelected ? "border-[var(--ds-action-primary)] bg-[var(--ds-action-primary)]" : "border-gray-300 bg-white"
                  }`}
                >
                  {isSelected && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`text-[13px] truncate ${
                      isSelected ? "font-semibold text-[var(--ds-action-primary)]" : "font-medium text-gray-800"
                    }`}
                  >
                    {option.label}
                  </div>
                ...
                {option.mode === "phonetic" && (
                  <div className="text-[12px] bg-[var(--ds-surface-muted)] border border-[var(--ds-border-default)] rounded px-1.5 py-0.5 text-[var(--ds-text-muted)] font-semibold uppercase tracking-wider">
                    Phonetic
                  </div>
                )}
              </div>
        ...
        {selectedLang.includes("phonetic") && (
          <div className="px-4 py-3 bg-gray-50 border-t border-gray-200">
            <div className="text-[12px] font-semibold text-gray-700 uppercase tracking-wide mb-1.5">
            ...
            <input
              ...
              className="h-8 w-full px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[var(--ds-action-primary)]/20 focus:border-[var(--ds-action-primary)] mb-2 shadow-sm"
            />

            <div
              className={`bg-white border border-gray-300 rounded-md px-2.5 py-1.5 min-h-[32px] text-[13px] flex items-center ${
        ...
        <div className="px-4 py-2 bg-amber-50 border-t border-amber-100 flex items-start gap-2">
          ...
          <div className="text-[12px] text-amber-800 leading-snug">
            <span className="font-semibold">Applies to:</span> Ledger Name, Narration, Address, Item
            Description, Notes. <span className="font-semibold">Does NOT apply to:</span> Amounts,
            Dates, GSTIN, PAN (must be in English).
          </div>
        </div>
        ...
        <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 flex justify-end gap-2">
          ...
          <button
            ...
            className="h-8 px-3 bg-white border border-gray-300 text-[var(--ds-action-primary)] text-[12px] font-medium rounded-md hover:bg-blue-50 transition-colors"
          >
