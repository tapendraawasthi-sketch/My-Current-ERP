...
export default function VatAnnexExport() {
  ...
  function validate() {
    ...
    [...annexA, ...annexB].forEach((r: any) => {
      ...
      const date = r["Invoice Date (BS)"] || r["Invoice Date"] || r["Date"] || "";
      ...
      <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4">
        <div className="flex items-center justify-between mb-3 border-b border-gray-100 pb-2">
          ...
          <span className="text-[11px] text-gray-500">
            Showing {Math.min(20, rows.length)} of {rows.length}
          </span>
        </div>

        {rows.length === 0 ? (
          <div className="text-[12px] py-6 text-center text-gray-500">
                ...
                {cleanRows.slice(0, 20).map((r: any, idx: number) => (
                  <tr key={idx} className="bg-white hover:bg-gray-50 text-[12px] transition-colors">
                    {headers.map((h) => {
                      ...
                      return (
                        <td
                          className={`px-3 py-2 whitespace-nowrap ${isNum ? "text-right" : "text-left"} ${h.includes("Amount") ? "font-medium text-gray-800" : "text-gray-600"}`}
                          key={h}
                        >
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-4 mb-5 border-b border-gray-200 pb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">
            Export VAT Annex A, B and C in IRD-ready Excel format.
          </p>
        </div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-md p-4 mb-5 shadow-sm">
        <div className="flex flex-wrap gap-4 items-center justify-between">
          <div className="flex gap-4 text-[12px]">
            <span className="text-red-700 font-medium flex items-center gap-1">
              Errors:{" "}
              <span className="font-bold bg-white px-1.5 py-0.5 rounded border border-red-200">
                {errorCount}
              </span>
            </span>
            <span className="text-amber-700 font-medium flex items-center gap-1">
              Warnings:{" "}
              <span className="font-bold bg-white px-1.5 py-0.5 rounded border border-amber-200">
                {warningCount}
              </span>
            </span>
            {issues.length === 0 && (
        ...
        {issues.length > 0 && (
          <div className="mt-4 bg-white border border-gray-200 rounded-md p-3">
            <div className="text-[12px] font-bold text-gray-800 mb-2 border-b border-gray-100 pb-1">
