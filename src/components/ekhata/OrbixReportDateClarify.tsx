...
const OrbixReportDateClarify: React.FC<OrbixReportDateClarifyProps> = ({
  ...
  return (
    <div className="mt-2 rounded-lg border border-cyan-500/25 bg-cyan-500/5 p-3">
      ...
      {needsParty && (
        <div className="mb-2">
          <label className="text-[10px] text-slate-500 mb-1 block">Party</label>
          <select
            value={partyName}
            onChange={(e) => setPartyName(e.target.value)}
            className="w-full h-8 rounded-md border border-white/10 bg-white/5 px-2 text-[11px] text-slate-200 focus:border-cyan-500/40 focus:outline-none"
          >
      ...
      <button
        ...
        className="mt-2.5 w-full h-8 rounded-md bg-gradient-to-r from-cyan-600 to-blue-600 text-[11px] font-medium text-white hover:from-cyan-500 hover:to-blue-500 disabled:opacity-40 flex items-center justify-center gap-1.5"
      >
        Generate report
