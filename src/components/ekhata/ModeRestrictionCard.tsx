...
const ModeRestrictionCard: React.FC<ModeRestrictionCardProps> = ({
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 border-b border-[var(--ox-border)] bg-[var(--ox-primary-soft)] px-3.5 py-3">
        <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[var(--ox-surface)] text-[var(--ox-primary)]">
          <Shield className="h-4 w-4" />
        </div>
        <div className="min-w-0">
          <p className="text-[13px] font-semibold text-[var(--ox-text)]">Ask Mode — read only</p>
          <p className="mt-1 text-[13px] leading-relaxed text-[var(--ox-text-muted)]">{friendly}</p>
          {payload?.requested_operation && (
            <p className="mt-1 text-[11px] text-[var(--ox-text-subtle)]">
      ...
      <div className="flex flex-wrap items-center gap-2 px-3.5 py-3">
        <button
          ...
          className="inline-flex h-8 items-center gap-1.5 rounded-[var(--ox-radius-md)] bg-[var(--ox-primary)] px-3 text-[12px] font-medium text-white hover:bg-[var(--ox-primary-hover)]"
        >
          Switch to Accountant Mode
          <ArrowRight className="h-3.5 w-3.5" />
        </button>
        <span className="text-[11px] text-[var(--ox-text-subtle)]">
