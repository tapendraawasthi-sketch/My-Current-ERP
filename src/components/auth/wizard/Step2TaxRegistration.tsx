  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#000000] mt-1">Enter your tax registration details for Nepal IRD compliance</p>
      </div>

      <div className="space-y-4">
        <div>
          <label className="block text-[11px] font-medium text-[#000000] mb-1">
            PAN / VAT Registration Number * <span className="text-[#000000] font-normal">({(data.panNumber || '').length}/9 digits)</span>
          </label>
          <input
            type="text"
            maxLength={9}
          ...
          {!data.panNumber || !/^\d{9}$/.test(data.panNumber) ? (
            <p className="text-red-600 text-[10px] mt-0.5">PAN must be exactly 9 digits (numbers only)</p>
          ) : (
          ...
          <p className="text-[10px] text-[#000000] mt-1 bg-[#EBF5E2] border border-[#9DC07A] rounded px-2 py-1">
            In Nepal, your PAN and VAT Registration Number are the same 9-digit number for VAT-registered businesses.
          </p>
        </div>

        <div className="bg-amber-50 border border-amber-300 rounded p-2 text-[11px] text-[#000000]">
          <strong>Nepal VAT Threshold:</strong> Annual turnover above NPR 50 lakh (NPR 5,000,000) requires mandatory VAT registration under Nepal Income Tax Act. Businesses below this threshold may register voluntarily.
        </div>
        ...
        <div>
          <label className="flex items-center space-x-2 mb-2 cursor-pointer">
            <input
              ...
              onChange={(e) => onChange({ ...data, hasVAT: e.target.checked, vatNumber: e.target.checked ? data.panNumber : '' })}
          ...
          {data.hasVAT && (
            <div className="space-y-3 pl-6">
              <div>
                <label className="block text-[11px] font-medium text-[#000000] mb-1">VAT Registration Number</label>
                <input
                  type="text"
              ...
              <div>
                ...
                <input
                  ...
                  className="h-8 px-2.5 text-[12px] border border-[#9DC07A] rounded-md bg-white text-[#000000] focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                />
                <p className="text-[10px] text-[#000000] mt-0.5">Date when IRD registered your business for VAT</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div>
          <label className="block text-[11px] font-medium text-[#000000] mb-1">IRD Office Province *</label>
          <select
        ...
        <div>
          <label className="block text-[11px] font-medium text-[#000000] mb-1">IRD Tax Office Branch Name</label>
          <input
            type="text"
          ...
          />
          <p className="text-[10px] text-[#000000] mt-0.5">Optional: Specify your local IRD branch for TDS challan references</p>
        </div>
        ...
        <div>
          <label className="flex items-center space-x-2 mb-2 cursor-pointer">
            <input
              ...
              onChange={(e) => onChange({ ...data, hasExcise: e.target.checked, exciseRegNumber: '' })}
              className="rounded border-[#9DC07A] h-4 w-4"
            />
            <span className="text-[12px] font-medium text-[#000000]">Excise Liable Business</span>
          </label>
          <p className="text-[10px] text-[#000000] mb-2">Check if your business deals in alcohol, tobacco, petroleum, or vehicles (subject to Nepal Excise Duty)</p>
          {data.hasExcise && (
            <div>
              <label className="block text-[11px] font-medium text-[#000000] mb-1">Excise Registration Number</label>
              <input
                type="text"
