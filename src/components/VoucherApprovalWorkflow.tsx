...
function getWorkflowPermission(role: string, currentStatus: string): { canAdvance: boolean; nextStatus: string; canReject: boolean; rejectStatus: string } {
  if (role === "accountant") {
    if (currentStatus === "draft") return { canAdvance: true, nextStatus: "submitted", canReject: false, rejectStatus: "" };
    return { canAdvance: false, nextStatus: "", canReject: false, rejectStatus: "" };
  }
  if (role === "manager") {
    if (currentStatus === "submitted") return { canAdvance: true, nextStatus: "under_review", canReject: true, rejectStatus: "draft" };
    if (currentStatus === "under_review") return { canAdvance: true, nextStatus: "approved", canReject: true, rejectStatus: "draft" };
    return { canAdvance: false, nextStatus: "", canReject: false, rejectStatus: "" };
  }
  if (role === "admin") {
    if (currentStatus === "draft") return { canAdvance: true, nextStatus: "submitted", canReject: false, rejectStatus: "" };
    if (currentStatus === "submitted") return { canAdvance: true, nextStatus: "approved", canReject: true, rejectStatus: "draft" };
    if (currentStatus === "under_review") return { canAdvance: true, nextStatus: "approved", canReject: true, rejectStatus: "draft" };
    if (currentStatus === "approved") return { canAdvance: true, nextStatus: "posted", canReject: false, rejectStatus: "" };
    return { canAdvance: false, nextStatus: "", canReject: false, rejectStatus: "" };
  }
  return { canAdvance: false, nextStatus: "", canReject: false, rejectStatus: "" };
}
        ...
        {steps.map((step, index) => (
          <React.Fragment key={step}>
            <div className="flex flex-col items-center relative z-10">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${getStatusClass(index)}`}>
                {getStatusIcon(index)}
              </div>
              <div className={`text-[10px] mt-2 font-medium capitalize tracking-wide ${index <= currentIndex ? 'text-gray-800' : 'text-gray-400'}`}>
                {step.replace("_", " ")}
              </div>
            </div>
            {index < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 -mt-5 rounded-full transition-colors duration-300 ${index < currentIndex ? 'bg-green-500' : 'bg-gray-200'}`}></div>
            )}
          </React.Fragment>
        ))}
      </div>
          ...
          <div className="bg-white rounded-md shadow-xl border border-gray-200 w-full max-w-sm flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5">
              <p className="text-[12px] text-gray-600">
                Are you sure you want to {confirmAction === "advance" ? <span className="font-semibold text-gray-800">advance this to {permissions.nextStatus.replace("_", " ")}</span> : <span className="font-semibold text-red-600">reject</span>} this voucher?
              </p>
            </div>
