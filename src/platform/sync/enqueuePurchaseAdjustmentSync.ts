/**
 * Atomic durable outbox writer for purchase_return_posted / supplier_debit_note_posted (Phase 8).
...
import {
  ...
  buildPurchaseAdjustmentPostedEvent,
  type PurchaseAdjustmentPostedPayload,
...
export interface EnqueuePurchaseAdjustmentSyncInput {
  tenantId: string;
  companyId: string;
...
export type EnqueuePurchaseAdjustmentSyncResult =
  | { syncStatus: "disabled"; eventId: null }
  | { syncStatus: "pending"; eventId: string; localSequence: number };

export async function enqueuePurchaseAdjustmentSyncInTransaction(
  db: SutraERPDatabase,
  input: EnqueuePurchaseAdjustmentSyncInput,
): Promise<EnqueuePurchaseAdjustmentSyncResult> {
  ...
  const fullPayload: PurchaseAdjustmentPostedPayload = {
    ...input.payload,
    device_id: deviceId,
