/**
 * Atomic durable outbox writer for purchase_posted (Phase 5).
 * Must run inside the same Dexie transaction as accounting posting.
 * Caller must resolve company sync policy before the transaction.
 */
...
export interface EnqueuePurchaseSyncInput {
  tenantId: string;
  companyId: string;
  ...
  /** Pre-resolved policy — avoid reading settings inside the Dexie txn. */
...
export type EnqueuePurchaseSyncResult =
  | { syncStatus: "disabled"; eventId: null }
  | { syncStatus: "pending"; eventId: string; localSequence: number };

/**
 * Write domainEvents + eventSyncQueue (+ update local sequence) inside an open txn.
 * Caller must include domainEvents, eventSyncQueue, syncLocalSequences in the txn table list.
 */
export async function enqueuePurchaseSyncInTransaction(
  db: SutraERPDatabase,
  input: EnqueuePurchaseSyncInput,
): Promise<EnqueuePurchaseSyncResult> {
  ...
  const fullPayload: PurchasePostedPayload = {
    ...input.payload,
    device_id: deviceId,
