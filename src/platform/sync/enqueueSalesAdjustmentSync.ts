/**
 * Atomic durable outbox writer for sales_return_posted / sales_credit_note_posted (Phase 7).
...
import {
  ...
  buildSalesAdjustmentPostedEvent,
  type SalesAdjustmentPostedPayload,
...
export interface EnqueueSalesAdjustmentSyncInput {
  tenantId: string;
  companyId: string;
...
export type EnqueueSalesAdjustmentSyncResult =
  | { syncStatus: "disabled"; eventId: null }
  | { syncStatus: "pending"; eventId: string; localSequence: number };

export async function enqueueSalesAdjustmentSyncInTransaction(
  db: SutraERPDatabase,
  input: EnqueueSalesAdjustmentSyncInput,
): Promise<EnqueueSalesAdjustmentSyncResult> {
  ...
  const fullPayload: SalesAdjustmentPostedPayload = {
    ...input.payload,
    device_id: deviceId,
