/**
 * Atomic durable outbox writer for Phase 9 financial / settlement events.
...
import {
  ...
  type FinancialTransactionPostedPayload,
...
export type { FinancialEventType, FinancialTransactionPostedPayload };

export interface EnqueueFinancialSyncInput {
  tenantId: string;
  companyId: string;
...
export type EnqueueFinancialSyncResult =
  | { syncStatus: "disabled"; eventId: null }
  | { syncStatus: "pending"; eventId: string; localSequence: number };

export async function enqueueFinancialSyncInTransaction(
  db: SutraERPDatabase,
  input: EnqueueFinancialSyncInput,
): Promise<EnqueueFinancialSyncResult> {
  ...
  const fullPayload: FinancialTransactionPostedPayload = {
    ...input.payload,
    device_id: deviceId,
