/**
 * Atomic durable outbox writer for Phase 10 bank reconciliation events.
 * Must run inside the same Dexie transaction as treasury posting.
 *
 * Depends on BankReconciliationEventType / BankReconciliationPostedPayload
 * (and buildFinancialPostedEvent accepting them) in accountingSyncContract.
 */
...
import {
  ...
  buildFinancialPostedEvent,
  type BankReconciliationEventType,
  type BankReconciliationPostedPayload,
...
export type { BankReconciliationEventType, BankReconciliationPostedPayload };

export interface EnqueueBankSyncInput {
  tenantId: string;
  companyId: string;
  ...
  eventType: BankReconciliationEventType;
  payload: Omit<
...
export type EnqueueBankSyncResult =
  | { syncStatus: "disabled"; eventId: null }
  | { syncStatus: "pending"; eventId: string; localSequence: number };

export async function enqueueBankSyncInTransaction(
  db: SutraERPDatabase,
  input: EnqueueBankSyncInput,
): Promise<EnqueueBankSyncResult> {
  ...
  } as BankReconciliationPostedPayload;
