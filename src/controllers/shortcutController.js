...
export const getShortcuts = async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      "SELECT * FROM keyboard_shortcuts ORDER BY display_order ASC, id ASC",
    );
  ...
  try {
    ...
    if (key_combo) {
      const { rows: existing } = await pool.query(
        "SELECT id FROM keyboard_shortcuts WHERE key_combo = $1 AND id != $2",
        [key_combo, id],
      );
      if (existing.length > 0) {
        return res
          .status(400)
          .json({ success: false, error: "Key combination is already in use by another shortcut" });
      }
    }
