...
export const updateCompanyFeatures = async (req, res, next) => {
  try {
    const {
      show_more_features, show_all_features, maintain_accounts, enable_bill_wise_entry,
      enable_cost_centres, enable_interest_calculation, maintain_inventory,
      integrate_accounts_with_inventory, enable_multiple_price_levels, enable_batches,
      maintain_expiry_date_for_batches, enable_job_order_processing, enable_cost_tracking,
      use_discount_column_in_invoices, use_separate_actual_billed_qty, enable_gst,
      gst_registration_type, gstin, gst_applicable_from, enable_tds, tan_number,
      tds_applicable_from, enable_tcs, tcs_applicable_from, enable_vat,
      vat_registration_number, vat_applicable_from, enable_excise,
      excise_registration_number, enable_service_tax, service_tax_registration_number,
      enable_browser_access_for_reports, enable_remote_access_sync, maintain_payroll,
      enable_payroll_statutory, pf_registration_number, esi_registration_number,
      enable_multiple_addresses, mark_modified_vouchers, mailing_details_in_local_language
    } = req.body;
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [
      finalShowMoreFeatures, show_all_features, maintain_accounts, enable_bill_wise_entry,
      enable_cost_centres, enable_interest_calculation, maintain_inventory,
      integrate_accounts_with_inventory, enable_multiple_price_levels, enable_batches,
      maintain_expiry_date_for_batches, enable_job_order_processing, enable_cost_tracking,
      use_discount_column_in_invoices, use_separate_actual_billed_qty, enable_gst,
    ...
    if (result.rows.length > 0) {
      ...
      const featureGroups = {
        maintain_accounts: 'Accounting', enable_bill_wise_entry: 'Accounting',
        enable_cost_centres: 'Accounting', enable_interest_calculation: 'Accounting',
        maintain_inventory: 'Inventory', integrate_accounts_with_inventory: 'Inventory',
        enable_multiple_price_levels: 'Inventory', enable_batches: 'Inventory',
        maintain_expiry_date_for_batches: 'Inventory', enable_job_order_processing: 'Inventory',
        enable_cost_tracking: 'Inventory', use_discount_column_in_invoices: 'Inventory',
        use_separate_actual_billed_qty: 'Inventory',
        enable_gst: 'Taxation', enable_tds: 'Taxation', enable_tcs: 'Taxation',
        enable_vat: 'Taxation', enable_excise: 'Taxation', enable_service_tax: 'Taxation',
        enable_browser_access_for_reports: 'Online Access', enable_remote_access_sync: 'Online Access',
        maintain_payroll: 'Payroll', enable_payroll_statutory: 'Payroll',
        enable_multiple_addresses: 'Others', mark_modified_vouchers: 'Others',
...
export const getCompanyAddresses = async (req, res, next) => {
  try {
    ...
    const tableCheck = await pool.query(
      "SELECT to_regclass('public.company_addresses')"
    );
    ...
    for (const address of addresses) {
      if (address.id) {
        ...
        await pool.query(`
          ...
          WHERE id = $20
        `, [
          address.address_type, address.mailing_name, address.address_line1, address.address_line2,
          address.address_line3, address.country, address.state, address.city, address.pin_code,
          address.phone, address.mobile, address.email, address.website, address.gstin,
          address.contact_person, address.is_primary, address.is_default_for_invoice,
          address.is_default_for_reports, address.is_active, address.id
        ]);
      } else {
        // Insert new
        await pool.query(`
        ...
        `, [
          ...
          address.phone, address.mobile, address.email, address.website, address.gstin,
          address.contact_person, address.is_primary, address.is_default_for_invoice,
          address.is_default_for_reports, address.is_active
        ]);
      }
    }
