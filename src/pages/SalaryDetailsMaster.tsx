...
const SalaryDetailsMaster: React.FC = () => {
  const { salaryDetails, addSalaryDetail, updateSalaryDetail, deleteSalaryDetail, employees, employeeGroups, payHeads } = useStore();
  ...
  const [form, setForm] = useState({
    ...
    employeeGroupId: "",
    effectiveFromDate: "",
    payHeadAllocations: [] as { payHeadId: string; calculationType: string; amount: number; percentage: number }[],
    isActive: true,
  });
  ...
  const loadFormForEdit = (detail: any) => {
    setForm({
      ...
      payHeadAllocations: Array.isArray(detail.payHeadAllocations) ? [...detail.payHeadAllocations] : [],
  ...
  const addPayHeadRow = () => {
    setForm(prev => ({
      ...prev,
      payHeadAllocations: [...prev.payHeadAllocations, { payHeadId: "", calculationType: "flat_rate", amount: 0, percentage: 0 }]
    }));
  };
      ...
      <div className={`flex-1 flex flex-col ${showForm ? "hidden lg:flex lg:w-1/2 xl:w-2/3 border-r border-gray-200" : "w-full"}`}>
        <div className="p-4 flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-[15px] font-semibold text-gray-800">Salary Details / Structure Master</h1>
              <p className="text-[11px] text-gray-500 mt-0.5">Manage pay structures for employees and groups</p>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#f5f6fa] border-b border-gray-200 sticky top-0 z-10">
                  ...
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Applies To</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Employee / Group Name</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Effective From</th>
                  <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Pay Heads</th>
                  ...
                  filteredDetails.map((detail, index) => {
                    ...
                    return (
                      <tr key={detail.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2.5 text-[12px] text-gray-700">{detail.appliesTo}</td>
                        <td className="px-3 py-2.5 text-[12px] text-gray-700 font-medium">
                          {detail.appliesTo === "Employee" ? emp?.name || "-" : grp?.name || "-"}
                        </td>
                        <td className="px-3 py-2.5 text-[12px] text-gray-700">{detail.effectiveFromDate}</td>
                        ...
                        <td className="px-3 py-2.5 text-[12px] text-center">
                          <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded-full ${detail.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
        ...
        <div className="w-full lg:w-[450px] bg-white flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-20">
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-1.5 text-[12px] text-gray-700 cursor-pointer">
                  <input
                    ...
                    onChange={() => setForm({ ...form, appliesTo: "Employee", employeeId: "", employeeGroupId: "" })}
                  />
                  Employee
                </label>
                <label className="flex items-center gap-1.5 text-[12px] text-gray-700 cursor-pointer">
                  <input
                    ...
                    onChange={() => setForm({ ...form, appliesTo: "Employee Group", employeeId: "", employeeGroupId: "" })}
                  />
                  Employee Group
                </label>
              </div>
            ...
            {form.appliesTo === "Employee" && (
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Employee <span className="text-red-500">*</span></label>
                <select
            ...
            {form.appliesTo === "Employee Group" && (
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Employee Group <span className="text-red-500">*</span></label>
                <select
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
            ...
            <div>
              <label className="text-[11px] font-medium text-gray-600 mb-1 block">Effective From (AD) <span className="text-red-500">*</span></label>
              <input
                type="date"
                className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
            ...
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center">
                <label className="text-[11px] font-medium text-gray-600">Pay Head Allocations <span className="text-red-500">*</span></label>
                <button
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#f5f6fa] border-b border-gray-200">
                      <th className="px-2 py-1.5 text-[10px] font-semibold text-gray-500 uppercase w-[40%]">Pay Head</th>
                      <th className="px-2 py-1.5 text-[10px] font-semibold text-gray-500 uppercase w-[25%]">Calc Type</th>
                      <th className="px-2 py-1.5 text-[10px] font-semibold text-gray-500 uppercase w-[20%] text-right">Amt / %</th>
                      <th className="px-2 py-1.5 text-[10px] font-semibold text-gray-500 uppercase w-[15%] text-center">Act</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-100">
                    {form.payHeadAllocations.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="px-2 py-4 text-center text-[11px] text-gray-500 italic">
                          ...
                          <tr key={idx}>
                            ...
                            <td className="px-2 py-1">
                              <select
                                ...
                                onChange={(e) => updatePayHeadRow(idx, 'calculationType', e.target.value)}
                                disabled
                              >
                            ...
                            <td className="px-2 py-1">
                              {isFlatRate ? (
                                <input
                                  ...
                                  value={alloc.amount}
                                  onChange={(e) => updatePayHeadRow(idx, 'amount', parseFloat(e.target.value) || 0)}
                                  min="0"
                                  step="any"
                                />
                              ) : isPct ? (
                                <div className="relative">
                                  <input
                                    ...
                                    onChange={(e) => updatePayHeadRow(idx, 'percentage', parseFloat(e.target.value) || 0)}
                                    min="0"
                                    step="0.01"
                                  />
                                  <span className="absolute right-1 top-1.5 text-[10px] text-gray-500">%</span>
                                </div>
                              ) : (
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-md p-3 flex justify-between items-center">
              <span className="text-[12px] font-medium text-blue-800">Estimated Gross Earning:</span>
