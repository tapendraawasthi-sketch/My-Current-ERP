...
import { ChevronDown, ChevronRight, Download, Package, TrendingUp, TrendingDown, Clock, Warehouse } from "lucide-react";
...
const InventoryAnalysis: React.FC = () => {
  const { items, vouchers, stockMovements, itemGroups, warehouses, batches, accounts, setCurrentPage } = useStore();
  ...
  const itemProfitabilityData = useMemo(() => {
    ...
    vouchers.forEach(voucher => {
      if ((voucher.type === "sales-invoice" || voucher.type === "sales") && voucher.status !== "cancelled") {
          ...
          if (line.itemId) {
            ...
            if (!itemMap[itemId]) {
              itemMap[itemId] = {
                ...
                groupName: item?.groupId ? itemGroups.find(g => g.id === item.groupId)?.name || "N/A" : "N/A",
                salesQty: 0,
                salesValue: 0,
                purchaseQty: 0,
                purchaseCost: 0
    ...
    // Process purchases
    vouchers.forEach(voucher => {
      if ((voucher.type === "purchase-invoice" || voucher.type === "purchase") && voucher.status !== "cancelled") {
        voucher.lines?.forEach(line => {
          if (line.itemId) {
            const itemId = line.itemId;
            const item = getItemById(itemId);
  ...
  const tab1Totals = useMemo(() => {
    return itemProfitabilityData.reduce((acc, item) => {
  ...
  const movementAnalysisData = useMemo(() => {
    ...
    stockMovements.forEach(movement => {
      if (new Date(movement.date) >= periodStart) {
        ...
        if (movement.type === "out" || movement.type === "sale") {
          ...
          if (!itemMap[itemId].lastSaleDate || new Date(movement.date) > new Date(itemMap[itemId].lastSaleDate)) {
    ...
    stockMovements.forEach(movement => {
      if (new Date(movement.date) >= twelveMonthsAgo && (movement.type === "out" || movement.type === "sale")) {
        const itemId = movement.itemId;
  ...
  const exportTab1ToExcel = () => {
    const headers = [
      "Item Code", "Item Name", "Group", "Sales Qty", 
      "Sales Value NPR", "COGS NPR", "Gross Profit NPR", "Margin %"
    ];
  ...
  const exportTab2ToExcel = () => {
    const headers = [
      "Item Code", "Item Name", "Sales in Period", 
      "Last Sale Date", "Days Since Last Sale", "Classification"
    ];
  ...
  const exportTab4ToExcel = () => {
    ...
    const headers = [
      "Item Code", "Item Name", "Opening Qty", "Received", 
      "Issued", "Closing Qty", "Unit", "Value"
    ];
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">Comprehensive insights into stock and movement</p>
          </div>
        </div>
        
        {/* Tab Navigation */}
        <div className="flex border-b border-gray-200 mb-4 bg-white px-2 pt-2 rounded-t-md shadow-sm">
          {["Item Profitability", "Movement Analysis", "Stock Aging", "Godown-wise Stock", "Category-wise Valuation"].map((tab, index) => (
            <button
              key={index}
              ...
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Item Code</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Item Name</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Group</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Sales Qty</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Sales Value</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">COGS</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Gross Profit</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Margin %</th>
                  </tr>
                </thead>
                <tbody>
                  {itemProfitabilityData.map((item, index) => (
                    <tr key={item.itemId} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 font-mono">{item.itemCode}</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 font-medium">{item.itemName}</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700">{item.groupName}</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.salesQty}</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right font-mono">{money(item.salesValue)}</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right font-mono">{money(item.purchaseCost)}</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right font-mono">{money(item.grossProfit)}</td>
                      <td className={`px-3 py-2.5 text-[12px] font-medium text-right ${item.marginPct > 20 ? 'text-[#059669]' : item.marginPct >= 10 ? 'text-[#d97706]' : 'text-[#dc2626]'}`}>
                        {item.marginPct.toFixed(2)}%
                      </td>
                    </tr>
                  ))}
                  <tr className="bg-[#eef2ff] border-t-2 border-[#c7d2fe]">
                    ...
                    <td className="px-3 py-2.5"></td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right">{tab1Totals.salesQty}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right font-mono">{money(tab1Totals.salesValue)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right font-mono">{money(tab1Totals.purchaseCost)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right font-mono">{money(tab1Totals.grossProfit)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right">
                      {tab1Totals.salesValue > 0 ? ((tab1Totals.grossProfit / tab1Totals.salesValue) * 100).toFixed(2) + "%" : "0.00%"}
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div className="overflow-x-auto">
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500">
                      {expandedGroups.has('fast') ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                    </span>
                  ...
                  <span className="text-[11px] font-semibold text-gray-500">
                    Total Value: {money(fastMoving.reduce((sum, item) => sum + (item.salesInPeriod * (getItemById(item.itemId)?.purchaseRate || 0)), 0))}
                  </span>
                </div>
                  ...
                  <table className="w-full min-w-max border-collapse">
                    <thead>
                      <tr className="bg-white border-y border-gray-200">
                        <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Item Code</th>
                        <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Item Name</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Sales in Period</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Last Sale Date</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Days Since Last Sale</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Classification</th>
                        <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {fastMoving.map(item => (
                        <tr key={item.itemId} className="border-b border-gray-100 hover:bg-gray-50 bg-green-50/30">
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 font-mono">{item.itemCode}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 font-medium">{item.itemName}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.salesInPeriod}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.lastSaleDate || "Never"}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.daysNoSale === Infinity ? "Never" : item.daysNoSale}</td>
                          <td className="px-3 py-2.5 text-[12px] text-[#059669] font-medium text-right">{item.classification}</td>
              ...
              <div className="mb-2 border-b border-gray-200">
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500">
                      {expandedGroups.has('slow') ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                    </span>
                    <span className="text-[11px] font-semibold text-gray-700 uppercase tracking-wide">
                  ...
                  <span className="text-[11px] font-semibold text-gray-500">
                    Total Value: {money(slowMoving.reduce((sum, item) => sum + (item.salesInPeriod * (getItemById(item.itemId)?.purchaseRate || 0)), 0))}
                  </span>
                </div>
                {expandedGroups.has('slow') && (
                  <table className="w-full min-w-max border-collapse">
                    ...
                    <tbody>
                      {slowMoving.map(item => (
                        <tr key={item.itemId} className="border-b border-gray-100 hover:bg-yellow-50 bg-amber-50/40">
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 font-mono">{item.itemCode}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 font-medium">{item.itemName}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.salesInPeriod}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.lastSaleDate || "Never"}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.daysNoSale === Infinity ? "Never" : item.daysNoSale}</td>
                          <td className="px-3 py-2.5 text-[12px] text-[#d97706] font-medium text-right">{item.classification}</td>
                          <td className="px-3 py-2.5 text-center">
                            <button 
                              className={`h-6 px-2 text-[10px] font-semibold uppercase rounded ${reviewFlags.has(item.itemId) ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                              onClick={() => toggleReviewFlag(item.itemId)}
              ...
              <div>
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500">
                      {expandedGroups.has('nonmoving') ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                    </span>
                    <span className="text-[11px] font-semibold text-gray-700 uppercase tracking-wide">
                  ...
                  <span className="text-[11px] font-semibold text-gray-500">
                    Total Value: {money(nonMoving.reduce((sum, item) => sum + (item.salesInPeriod * (getItemById(item.itemId)?.purchaseRate || 0)), 0))}
                  </span>
                </div>
                {expandedGroups.has('nonmoving') && (
                  <table className="w-full min-w-max border-collapse">
                    ...
                    <tbody>
                      {nonMoving.map(item => (
                        <tr key={item.itemId} className="border-b border-gray-100 hover:bg-red-50 bg-red-50/40">
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 font-mono">{item.itemCode}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 font-medium">{item.itemName}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.salesInPeriod}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.lastSaleDate || "Never"}</td>
                          <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{item.daysNoSale === Infinity ? "Never" : item.daysNoSale}</td>
                          <td className="px-3 py-2.5 text-[12px] text-[#dc2626] font-medium text-right">{item.classification}</td>
                          <td className="px-3 py-2.5 text-center">
                            <button 
                              className={`h-6 px-2 text-[10px] font-semibold uppercase rounded ${reviewFlags.has(item.itemId) ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                              onClick={() => toggleReviewFlag(item.itemId)}
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden max-w-full">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide" rowSpan={2}>Item Name</th>
                    <th className="px-3 py-2 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide" rowSpan={2}>Unit</th>
                    {["0-30 days", "31-60 days", "61-90 days", "91-180 days", "181-365 days", "365+ days"].map(label => (
                      <th key={label} className="px-3 py-1.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-l border-gray-200" colSpan={2}>{label}</th>
                    ))}
                    <th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-l border-gray-200" rowSpan={2}>Total Qty</th>
                    <th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide" rowSpan={2}>Total Value</th>
                  </tr>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    {Array(6).fill(0).map((_, i) => (
                      <React.Fragment key={i}>
                        <th className="px-2 py-1.5 text-right text-[9px] font-semibold text-gray-400 uppercase tracking-wide border-l border-gray-200">Qty</th>
                        <th className="px-2 py-1.5 text-right text-[9px] font-semibold text-gray-400 uppercase tracking-wide">Value</th>
                      </React.Fragment>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {stockAgingData.map((item, i) => (
                    <tr key={item.itemId} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="px-3 py-2 text-[12px] text-gray-700 font-medium">{item.itemName}</td>
                      <td className="px-3 py-2 text-[12px] text-gray-500 text-center">{item.unit}</td>
                      {item.buckets.map((bucket, idx) => (
                        <React.Fragment key={idx}>
                          <td className={`px-2 py-2 text-[11px] text-right border-l border-gray-100 ${bucket.qty > 0 ? 'text-gray-700' : 'text-gray-300'}`}>{bucket.qty || '-'}</td>
                          <td className={`px-2 py-2 text-[11px] text-right font-mono ${bucket.value > 0 ? 'text-gray-700' : 'text-gray-300'}`}>{bucket.value > 0 ? money(bucket.value) : '-'}</td>
                        </React.Fragment>
                      ))}
                      <td className="px-3 py-2 text-[12px] font-medium text-gray-700 text-right border-l border-gray-200">{item.totalQty}</td>
                      <td className="px-3 py-2 text-[12px] font-mono font-medium text-gray-700 text-right">{money(item.totalValue)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden flex min-h-[400px]">
            ...
            <div className="w-1/4 max-w-[250px] border-r border-gray-200 bg-gray-50 flex flex-col">
              <div className="p-3 border-b border-gray-200">
                <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Warehouses</h3>
              </div>
              <div className="flex-1 overflow-y-auto p-2 space-y-1">
                {warehouses.length === 0 ? (
                  <div className="text-[11px] text-gray-500 p-2 text-center">No warehouses configured</div>
                ) : (
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              {selectedWarehouseId ? (
                  ...
                  <div className="p-3 border-b border-gray-200 flex justify-between items-center bg-white">
                    ...
                    <div className="flex gap-4 items-center">
                      <div className="text-[12px]">
                        ...
                        <span className="font-mono font-bold text-gray-800">{money(godownTotalValue)}</span>
                      </div>
                      <button
                        className="h-8 px-3 bg-white border border-gray-300 text-gray-700 text-[12px] font-medium rounded-md hover:bg-gray-50 flex items-center gap-1.5"
                  ...
                  <div className="flex-1 overflow-auto">
                    <table className="w-full min-w-max border-collapse">
                      <thead className="sticky top-0 bg-[#f5f6fa] shadow-sm z-10">
                        <tr>
                          <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200">Item Code</th>
                          <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200">Item Name</th>
                          <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200">Opening Qty</th>
                          <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200 text-[#059669]">Received</th>
                          <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200 text-[#dc2626]">Issued</th>
                          <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200">Closing Qty</th>
                          <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200">Unit</th>
                          <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-200">Value</th>
                        </tr>
                      </thead>
                      <tbody>
                        {godownStockData.map(item => (
                          <tr key={item.itemId} className="border-b border-gray-100 hover:bg-gray-50">
                            <td className="px-3 py-2.5 text-[12px] text-gray-700 font-mono">{item.itemCode}</td>
                            <td className="px-3 py-2.5 text-[12px] text-gray-700 font-medium">{item.itemName}</td>
                            <td className="px-3 py-2.5 text-[12px] text-gray-500 text-right">{item.openingQty}</td>
                            <td className="px-3 py-2.5 text-[12px] text-[#059669] font-medium text-right">{item.received}</td>
                            <td className="px-3 py-2.5 text-[12px] text-[#dc2626] font-medium text-right">{item.issued}</td>
                            <td className="px-3 py-2.5 text-[12px] text-gray-800 font-semibold text-right">{item.closingQty}</td>
                            <td className="px-3 py-2.5 text-[11px] text-gray-500 text-center">{item.unit}</td>
                            <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right font-mono">{money(item.value)}</td>
                          </tr>
                        ))}
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8">
                  ...
                  <p className="text-[13px] font-medium">Please select a warehouse from the sidebar</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden max-w-full">
            <div className="overflow-x-auto">
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Category / Item</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Items Count</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Total Qty</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Total Value</th>
                  </tr>
                </thead>
                <tbody>
                        ...
                        <td className="px-3 py-2.5 text-[12px] font-semibold text-gray-800">
                          <div className="flex items-center gap-2">
                            <span className="text-gray-400">
                              {expandedGroups.has(group.groupId) ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                            </span>
                            {group.groupName}
                          </div>
                        </td>
                        <td className="px-3 py-2.5 text-[12px] font-medium text-gray-600 text-right">{group.itemCount}</td>
                        <td className="px-3 py-2.5 text-[12px] font-semibold text-gray-800 text-right">{group.totalQty}</td>
                        <td className="px-3 py-2.5 text-[12px] font-mono font-bold text-[#1557b0] text-right">{money(group.totalValue)}</td>
                      </tr>
                      
                      {expandedGroups.has(group.groupId) && group.items.map(item => (
                        <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                          <td className="pl-9 pr-3 py-2 text-[12px] text-gray-700">{item.name}</td>
                          <td className="px-3 py-2"></td>
                          <td className="px-3 py-2 text-[12px] text-gray-600 text-right">{item.qty}</td>
                          <td className="px-3 py-2 text-[12px] text-gray-600 text-right font-mono">{money(item.value)}</td>
                        </tr>
                      ))}
                    </React.Fragment>
                  ))}
                  ...
                  {categoryValuationData.length > 0 && (
                    <tr className="bg-[#eef2ff] border-t-2 border-[#c7d2fe]">
                      <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800">GRAND TOTAL</td>
                      <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right">
                      ...
                      <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right font-mono">
                        {money(categoryValuationData.reduce((sum, group) => sum + group.totalValue, 0))}
                      </td>
                    </tr>
                  )}
                </tbody>
