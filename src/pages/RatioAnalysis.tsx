...
const RatioAnalysis: React.FC = () => {
  ...
  const ratiosData = useMemo(() => {
    ...
    accounts.forEach(acc => {
      const balance = (ledgerTotals[acc.id]?.balanceDr || 0) - (ledgerTotals[acc.id]?.balanceCr || 0);
      
      // Determine if account is current asset
      const isCurrentAsset = acc.type === "asset" && (
      ...
      const isCurrentLiability = acc.type === "liability" && (
      ...
      if (acc.name.toLowerCase().includes("debtor") || acc.name.toLowerCase().includes("sundry debtor")) {
        totalDebtors += balance;
      }
      
      if (acc.name.toLowerCase().includes("creditor") || acc.name.toLowerCase().includes("sundry creditor")) {
      ...
      if (acc.name.toLowerCase().includes("stock") || acc.name.toLowerCase().includes("inventory")) {
        totalStock += balance;
      }
    });
    ...
    const directExpenses = accounts.filter(acc => acc.type === "expense").reduce((sum, acc) => {
      const balance = (ledgerTotals[acc.id]?.balanceDr || 0) - (ledgerTotals[acc.id]?.balanceCr || 0);
      return sum + balance;
    }, 0);
    ...
    const expenses = accounts.filter(acc => acc.type === "expense").reduce((sum, acc) => {
      const balance = (ledgerTotals[acc.id]?.balanceDr || 0) - (ledgerTotals[acc.id]?.balanceCr || 0);
      return sum + balance;
    }, 0);
    const incomes = accounts.filter(acc => acc.type === "income").reduce((sum, acc) => {
      const balance = (ledgerTotals[acc.id]?.balanceDr || 0) - (ledgerTotals[acc.id]?.balanceCr || 0);
      return sum + balance;
    }, 0);
    ...
    // Calculate ratios
    const currentRatio = totalCurrentLiabilities !== 0 ? totalCurrentAssets / totalCurrentLiabilities : NaN;
    const quickRatio = totalCurrentLiabilities !== 0 ? (totalCurrentAssets - totalStock) / totalCurrentLiabilities : NaN;
    ...
    const getStatus = (value: number, good: number, caution: number, poor: number, isHigherBetter: boolean) => {
  ...
  const renderCell = (columnKey: string, value: any, row: any) => {
    ...
    if (columnKey === "status") {
      return (
        <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded-md ${value.className}`}>
          {value.label}
        </span>
      );
    }
  ...
  return (
    ...
    >
      <div className="grid gap-8 grid-cols-1 xl:grid-cols-2 mb-8">
        ...
        {/* Liquidity Ratios */}
        <section>
          <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-3">Liquidity Ratios</h3>
          <div className="bg-white border border-gray-200 rounded-md overflow-hidden shadow-sm">
            <ReportGrid 
              columns={columns} 
              data={ratiosData.liquidity} 
        ...
        {/* Profitability Ratios */}
        <section>
          <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-3">Profitability Ratios</h3>
          <div className="bg-white border border-gray-200 rounded-md overflow-hidden shadow-sm">
            <ReportGrid 
              columns={columns} 
              data={ratiosData.profitability} 
              renderCell={renderCell}
            />
          </div>
        </section>
        ...
        {/* Efficiency Ratios */}
        <section>
          <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-3">Efficiency Ratios</h3>
          <div className="bg-white border border-gray-200 rounded-md overflow-hidden shadow-sm">
            <ReportGrid 
              columns={columns} 
              data={ratiosData.efficiency} 
              renderCell={renderCell}
            />
          </div>
        </section>
        ...
        {/* Solvency Ratios */}
        <section>
          <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-3">Solvency Ratios</h3>
          <div className="bg-white border border-gray-200 rounded-md overflow-hidden shadow-sm">
            <ReportGrid 
              columns={columns} 
              data={ratiosData.solvency} 
              renderCell={renderCell}
            />
          </div>
        </section>
