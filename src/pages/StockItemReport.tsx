...
const StockItemReport: React.FC = () => {
  ...
  const reportData = useMemo(() => {
    ...
    let filteredMovements = (stockMovements || []).filter(m => 
      m.itemId === selectedItemId && 
      m.date >= startDate && 
      m.date <= endDate
    );
    ...
    result.push({
      ...
      inwardQty: "",
      outwardQty: "",
      rate: runningBalance > 0 ? ((result.reduce((sum, r) => sum + r.value, 0) - result[0].value) / runningBalance) : 0,
      value: "",
      balanceQty: runningBalance,
      isClosing: true
    });
  ...
  const renderCell = (columnKey: string, value: any, row: any) => {
    if (row.isOpening || row.isClosing) {
      ...
      if (["inwardQty", "outwardQty", "rate", "value", "balanceQty"].includes(columnKey) && value !== "") {
    ...
    if (["inwardQty", "outwardQty", "rate", "value", "balanceQty"].includes(columnKey)) {
      ...
      return <span className={`font-mono ${columnKey === "balanceQty" && value < 0 ? "text-red-600 font-semibold" : ""}`}>{formatNumber(value)}</span>;
    }
    ...
    >
      ...
      <div className="grid grid-cols-4 gap-4 text-[12px] mb-4">
        <div className="bg-white border border-gray-200 rounded-md p-3 shadow-sm flex flex-col justify-center">
          <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Total Inward Qty</div>
          <div className="text-[16px] font-mono font-bold text-[#1557b0]">{formatNumber(reportData.summary.totalInwardQty)}</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-md p-3 shadow-sm flex flex-col justify-center">
          <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Total Outward Qty</div>
          <div className="text-[16px] font-mono font-bold text-[#d97706]">{formatNumber(reportData.summary.totalOutwardQty)}</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-md p-3 shadow-sm flex flex-col justify-center">
          <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Closing Stock Qty</div>
          <div className={`text-[16px] font-mono font-bold ${reportData.summary.closingQty < 0 ? "text-red-600" : "text-[#059669]"}`}>
            {formatNumber(reportData.summary.closingQty)}
          </div>
        </div>
        <div className="bg-white border border-gray-200 rounded-md p-3 shadow-sm flex flex-col justify-center">
          <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Closing Stock Value</div>
          <div className="text-[16px] font-mono font-bold text-gray-800">Rs. {formatNumber(reportData.summary.closingValue)}</div>
        </div>
      </div>
