...
const DEFAULT_ROLES = {
  ...
  viewer: ["VIEW_VOUCHERS", "VIEW_REPORTS", "VIEW_MASTERS", "VIEW_INVENTORY", "VIEW_BANKING", "VIEW_TAX"],
};
...
const MODULE_ACCESS_GROUPS = {
  Vouchers: ["VIEW_VOUCHERS", "CREATE_VOUCHERS", "EDIT_VOUCHERS", "DELETE_VOUCHERS", "APPROVE_VOUCHERS"],
...
export function filterByUserBranch(data: any[], currentUser: any): any[] {
  ...
  return (data || []).filter((item) => !item.costCenterId || item.costCenterId === currentUser.costCenterId);
}
...
export default function UsersManagement() {
  ...
  async function loadUsers() {
    const db = getDB();
    const dbUsers = await db.table("users").toArray().catch(() => []);
  ...
  async function loadRoles() {
    const db = getDB();
    const dbRoles = await db.table("roles").toArray().catch(() => []);
    ...
    if (!merged || merged.length === 0) {
      ...
      for (const role of merged) {
        await db.table("roles").put(role).catch(() => {});
      }
    }
  ...
  async function loadSecuritySettings() {
    const db = getDB();
    const s = await db.table("securitySettings").get("global").catch(() => null);
  ...
  async function loadAuditEvents() {
    ...
    if (rows.length === 0) {
      ...
      setAuditEvents(seed);
      for (const e of seed) {
        await db.table("loginAudit").put(e).catch(() => {});
      }
      return;
    }
  ...
  function validateUserForm() {
    ...
    if (!/^[a-zA-Z0-9_]+$/.test(userForm.username)) return "Username must be alphanumeric or underscore only";
    if (!userForm.name.trim()) return "Full name is required";
    if (userForm.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(userForm.email)) return "Invalid email address";
  ...
  async function removeUser(user: any) {
    ...
    const db = getDB();
    await db.table("users").delete(user.id).catch(() => {});
  ...
  async function createCustomRole() {
    ...
    if (roles.some((r) => String(r.name || r.id).toLowerCase() === name)) return toast.error("Role already exists");

    const role = { id: generateId(), name, permissions: [], isDefault: false };
    await getDB().table("roles").put(role).catch(() => {});
    setRoles((r) => [...r, role]);
    setNewRoleName("");
  ...
  async function deleteCustomRole(role: any) {
    ...
    if (["admin", "manager", "accountant", "viewer"].includes(roleName)) return toast.error("Default roles cannot be deleted");
    if (!confirm(`Delete custom role ${roleName}?`)) return;

    await getDB().table("roles").delete(role.id).catch(() => {});
  ...
  const auditSummary = useMemo(() => {
    ...
    const todayRows = (auditEvents || []).filter((e) => String(e.timestamp || "").startsWith(today));
    return {
    ...
    <div className="min-h-screen bg-[#f5f6fa] p-4 text-gray-800">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">Field-level permissions, branch isolation, login audit and security policies</p>
        </div>
      </div>
      ...
      {activeTab === "Users" && (
        <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4">
          <div className="flex flex-col md:flex-row gap-3 justify-between mb-4">
            <input
              className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full md:max-w-sm shadow-sm"
              placeholder="Search users by name, username, email, role..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            <button 
              className="h-8 px-3 bg-[#1557b0] text-white text-[12px] font-medium rounded-md hover:bg-[#0f4a96] transition-colors flex items-center gap-1.5 shadow-sm whitespace-nowrap" 
              onClick={openAddUser}
            >
              ...
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    {["Username", "Full Name", "Role Badge", "Assigned Branch", "Last Login", "Status", "Actions"].map(
                      (h) => (
                        <th key={h} className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">
                          {h}
                        </th>
                      ),
                    )}
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((u) => {
                      ...
                      const lastLogin = auditEvents.find((e) => e.username === u.username && e.action === "login");
                      return (
                        <tr key={u.id} className="bg-white hover:bg-gray-50 transition-colors text-[12px]">
                          <td className="px-3 py-2.5 font-medium text-gray-800">{u.username}</td>
                          <td className="px-3 py-2.5 text-gray-600">{u.name}</td>
                          <td className="px-3 py-2.5">
                            <span className={`${roleBadgeClass(u.role)} rounded px-2 py-0.5 text-[10px] uppercase font-semibold tracking-wide border`}>
                              {u.role || "custom"}
                            </span>
                          </td>
                          <td className="px-3 py-2.5 text-gray-600">{branch?.name || "All Branches"}</td>
                          <td className="px-3 py-2.5 text-gray-600">{lastLogin ? new Date(lastLogin.timestamp).toLocaleString() : "Never"}</td>
                          <td className="px-3 py-2.5">
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex gap-2.5 items-center">
                              <button onClick={() => openEditUser(u)} title="Edit" className="text-gray-500 hover:text-[#1557b0] transition-colors">
                                <Edit2 size={14} />
                              </button>
                              <button onClick={() => removeUser(u)} title="Delete" className="text-gray-500 hover:text-red-600 transition-colors">
                                <Trash2 size={14} />
                              </button>
                              <button onClick={() => resetPassword(u)} title="Reset Password" className="text-gray-500 hover:text-[#1557b0] transition-colors">
                                <Lock size={14} />
                              </button>
                            </div>
                          </td>
        ...
        <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4">
          <div className="flex flex-col md:flex-row gap-3 justify-between mb-4">
            <div className="flex gap-2">
              <input
                className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] max-w-xs shadow-sm"
            ...
            <button 
              className="h-8 px-4 bg-[#1557b0] text-white text-[12px] font-medium rounded-md hover:bg-[#0f4a96] transition-colors flex items-center gap-1.5 shadow-sm" 
          ...
          <div className="border border-gray-200 rounded-md overflow-hidden relative">
            <div className="overflow-x-auto">
              <table className="border-collapse min-w-[1000px] w-full">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-gray-600 uppercase tracking-wide sticky left-0 z-10 bg-[#f5f6fa] border-r border-gray-200 shadow-[2px_0_4px_rgba(0,0,0,0.02)]">Permission</th>
                    {roles.map((r) => (
                      <th key={r.id} className="px-3 py-3 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide border-r border-gray-200 last:border-r-0">
                        <div className="flex items-center justify-center gap-2">
                          <span>{r.name}</span>
                          {!["admin", "manager", "accountant", "viewer"].includes(String(r.name).toLowerCase()) && (
                            <button className="text-red-500 hover:text-red-700 transition-colors" onClick={() => deleteCustomRole(r)}>
                              <Trash2 size={12} />
                            </button>
                          )}
                        </div>
                    ...
                    <React.Fragment key={group}>
                      ...
                      {permissions.map((p) => (
                        <tr key={p} className="bg-white hover:bg-gray-50 transition-colors">
                          <td className="px-4 py-2 text-[12px] text-gray-600 font-mono sticky left-0 z-10 bg-white border-r border-gray-200 shadow-[2px_0_4px_rgba(0,0,0,0.02)]">{p}</td>
                          {roles.map((r) => {
                            const name = String(r.name || r.id).toLowerCase();
                            const checked = name === "admin" || (r.permissions || []).includes(p);
                            return (
                              <td key={r.id + p} className="px-3 py-2 text-center border-r border-gray-100 last:border-r-0">
                                <input
                                  type="checkbox"
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1.5">Min Password Length</label>
                <input
              ...
              <div className="flex items-center pt-5">
                <label className="text-[12px] flex items-center gap-2 text-gray-700 cursor-pointer">
                  <input
                    ...
                    onChange={(e) => setSecuritySettings({ ...securitySettings, requireSpecial: e.target.checked })}
                  />
                  Require Special
                </label>
              </div>
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1.5">Password Expires (Days)</label>
                <input
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm"
                  type="number"
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1.5">Lock After Failed Attempts</label>
                <input
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm"
                  type="number"
                  value={securitySettings.failedAttempts}
                  onChange={(e) => setSecuritySettings({ ...securitySettings, failedAttempts: Number(e.target.value) })}
                />
              </div>
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1.5">Lockout Duration (Mins)</label>
                <input
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm"
                  type="number"
                  value={securitySettings.lockoutMinutes}
                  onChange={(e) => setSecuritySettings({ ...securitySettings, lockoutMinutes: Number(e.target.value) })}
                />
              </div>
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1.5">Session Timeout (Mins)</label>
                <input
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm"
                  type="number"
              ...
              <div className="flex flex-col justify-end">
                <label className="text-[12px] flex items-center gap-2 text-gray-700 cursor-pointer h-8">
                  <input
                    ...
                    onChange={(e) => setSecuritySettings({ ...securitySettings, require2FAAdmin: e.target.checked })}
                  />
                  <span>Require 2FA for Admin <span className="block text-[10px] text-amber-600">Via email OTP</span></span>
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-5">
            <h2 className="text-[14px] font-bold text-gray-800 mb-4 border-b border-gray-100 pb-2">Allowed IP Ranges</h2>
            <textarea
              className="px-3 py-2 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full min-h-[100px] shadow-sm font-mono"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <label className="text-[12px] flex items-center gap-2 text-gray-700 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setSecuritySettings({ ...securitySettings, maskBankAccounts: e.target.checked })}
                />
              ...
              <label className="text-[12px] flex items-center gap-2 text-gray-700 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setSecuritySettings({ ...securitySettings, maskPanReports: e.target.checked })}
                />
              ...
              <label className="text-[12px] flex items-center gap-2 text-gray-700 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setSecuritySettings({ ...securitySettings, watermarkPrints: e.target.checked })}
                />
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 flex flex-col justify-center">
              <div className="text-[11px] text-gray-500 uppercase font-semibold tracking-wide mb-1">Total Logins Today</div>
              <div className="text-[28px] font-bold text-gray-800">{auditSummary.totalLoginsToday}</div>
            </div>
            <div className={`border rounded-md shadow-sm p-4 flex flex-col justify-center ${auditSummary.failedAttemptsToday > 5 ? "bg-red-50 border-red-200" : "bg-white border-gray-200"}`}>
              <div className={`text-[11px] uppercase font-semibold tracking-wide flex items-center gap-1.5 mb-1 ${auditSummary.failedAttemptsToday > 5 ? "text-red-700" : "text-gray-500"}`}>
                {auditSummary.failedAttemptsToday > 5 && <AlertTriangle size={14} />}
                Failed Attempts Today
              </div>
              <div className={`text-[28px] font-bold ${auditSummary.failedAttemptsToday > 5 ? "text-red-600" : "text-gray-800"}`}>{auditSummary.failedAttemptsToday}</div>
            </div>
            <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 flex flex-col justify-center">
              <div className="text-[11px] text-gray-500 uppercase font-semibold tracking-wide mb-1">Active Sessions</div>
              <div className="text-[28px] font-bold text-[#1557b0]">{auditSummary.activeSessions}</div>
            </div>
          </div>

          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-4">
              <select className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] shadow-sm" value={auditUser} onChange={(e) => setAuditUser(e.target.value)}>
              ...
              <input className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] shadow-sm" type="date" value={auditFrom} onChange={(e) => setAuditFrom(e.target.value)} />
              <input className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] shadow-sm" type="date" value={auditTo} onChange={(e) => setAuditTo(e.target.value)} />
              <select className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] shadow-sm" value={auditAction} onChange={(e) => setAuditAction(e.target.value)}>
            ...
            <div className="border border-gray-200 rounded-md overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full min-w-max border-collapse">
                  <thead>
                    <tr className="bg-[#f5f6fa] border-b border-gray-200">
                      {["Timestamp", "Username", "Action", "IP Address", "Browser/Device", "Status"].map((h) => (
                        <th key={h} className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredAuditEvents.map((e) => (
                      <tr key={e.id} className="bg-white hover:bg-gray-50 text-[12px] transition-colors">
                        <td className="px-3 py-2 text-gray-600">{new Date(e.timestamp).toLocaleString()}</td>
                        <td className="px-3 py-2 font-medium text-gray-800">{e.username}</td>
                        <td className="px-3 py-2">
                          <span className={`${actionBadgeClass(e.action)} rounded px-2 py-0.5 text-[10px] uppercase font-semibold tracking-wide border`}>
                            {e.action}
                          </span>
                        </td>
                        <td className="px-3 py-2 text-gray-600 font-mono">{e.ipAddress}</td>
                        <td className="px-3 py-2 text-gray-500 truncate max-w-[200px]" title={e.userAgent}>{e.userAgent}</td>
                        <td className="px-3 py-2">
                          <span className={`inline-flex items-center gap-1.5 font-medium ${e.action === "failed" || e.action === "locked" ? "text-amber-600" : "text-green-600"}`}>
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-xl w-full max-w-5xl max-h-[92vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h2 className="text-[15px] font-bold text-gray-800 flex items-center gap-2">
                <User size={16} className="text-[#1557b0]" />
                {editingUser ? "Edit User Account" : "Create New User"}
              </h2>
              <button className="text-gray-400 hover:text-gray-600 transition-colors" onClick={() => setUserModalOpen(false)}>
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-4 overflow-y-auto space-y-6">
              <div className="space-y-4">
                <h3 className="text-[13px] font-bold text-gray-800 border-b border-gray-100 pb-1 flex items-center gap-2">
                  <span className="bg-[#1557b0] text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px]">1</span> Basic Info
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Username <span className="text-red-500">*</span></label>
                    <input 
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm" 
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Full Name <span className="text-red-500">*</span></label>
                    <input 
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm" 
                      value={userForm.name} 
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Role Type</label>
                    <select 
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm" 
                      value={userForm.role} 
                    ...
                    >
                      ...
                      {roles
                        .filter((r) => !["admin", "manager", "accountant", "viewer"].includes(String(r.name).toLowerCase()))
                        .map((r) => (
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Branch/Department Assignment</label>
                    <select 
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm" 
              ...
              <div className="space-y-4">
                <h3 className="text-[13px] font-bold text-gray-800 border-b border-gray-100 pb-1 flex items-center gap-2">
                  <span className="bg-[#1557b0] text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px]">2</span> Login Restrictions
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Allowed Login From Time</label>
                    <input 
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm" 
                      type="time" 
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Allowed Login To Time</label>
                    <input 
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm" 
                      type="time" 
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Max Concurrent Sessions</label>
                    <input
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm"
                      type="number"
                      min={1}
                      ...
                      onChange={(e) => setUserForm({ ...userForm, maxSessions: Number(e.target.value) })}
                    />
                  </div>
                  <div className="md:col-span-3">
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-[11px] font-medium text-gray-600">Allowed IP Addresses</label>
                      <button className="text-[10px] text-[#1557b0] hover:underline flex items-center gap-1" onClick={() => setShowIpHelp(!showIpHelp)}>
                        <HelpCircle size={12} /> Help
                      </button>
                    </div>
                    {showIpHelp && (
                      <div className="bg-blue-50 text-blue-800 text-[11px] p-2 rounded mb-2 border border-blue-100">
                        Enter IP addresses like 192.168.1.100. Leave blank to allow from anywhere. One IP per line.
                      </div>
                    )}
                    <textarea
                      className="px-3 py-2 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full min-h-[80px] shadow-sm font-mono"
              ...
              <div className={`space-y-4 ${userForm.role !== "custom" && userForm.role !== "admin" ? "opacity-75 pointer-events-none" : ""}`}>
                <h3 className="text-[13px] font-bold text-gray-800 border-b border-gray-100 pb-1 flex items-center gap-2">
                  <span className="bg-[#1557b0] text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px]">3</span> Module Access
                  {userForm.role !== "custom" && userForm.role !== "admin" && (
                    <span className="text-[10px] font-normal text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-200">Set role to 'custom' to edit</span>
                  )}
                </h3>
                  ...
                  {Object.entries(MODULE_ACCESS_GROUPS).map(([group, perms]) => (
                    <div key={group} className="border border-gray-200 rounded-md bg-gray-50/50">
                      <div className="flex items-center justify-between p-2.5 border-b border-gray-200 bg-gray-50 rounded-t-md">
                        <div className="text-[11px] font-bold text-gray-700 uppercase tracking-wide">{group}</div>
                        <div className="flex gap-2">
                          <button className="text-[10px] text-[#1557b0] hover:underline" onClick={() => setGroupPermissions(group, "all")}>
                            All
                          </button>
                          <button className="text-[10px] text-red-600 hover:underline" onClick={() => setGroupPermissions(group, "clear")}>
                            Clear
                          </button>
                        </div>
                      </div>
                      <div className="p-2 space-y-1">
                        {perms.map((p) => (
                          <label key={p} className="flex items-center gap-2 text-[11px] text-gray-600 hover:bg-gray-100 p-1 rounded cursor-pointer transition-colors">
                            <input 
                              type="checkbox" 
                              className="w-3.5 h-3.5 text-[#1557b0] border-gray-300 rounded focus:ring-[#1557b0]"
                              checked={userForm.role === "admin" || (userForm.permissions || []).includes(p)} 
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-2 rounded-b-md">
              ...
              <button 
                className="h-8 px-5 bg-[#1557b0] text-white text-[12px] font-medium rounded-md hover:bg-[#0f4a96] transition-colors shadow-sm" 
                onClick={saveUser}
              >
