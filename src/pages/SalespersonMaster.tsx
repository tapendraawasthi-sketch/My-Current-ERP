...
import { User, MapPin, Target, TrendingUp, Plus, Edit2, Trash2, Download, ChevronDown, ChevronRight } from "lucide-react";
...
export default function SalespersonMaster() {
  ...
  const handleSave = async () => {
    ...
    try {
      const db = getDB();
      await db.salespersons.put({ id: editingId || generateId(), ...form, updatedAt: new Date().toISOString() });
  ...
  const performanceData = useMemo(() => {
    ...
    const startDate = selectedPeriod === "Full Year" ? fy.startDate : new Date().toISOString().split('T')[0]; // Simplified for example
    const endDate = selectedPeriod === "Full Year" ? fy.endDate : new Date().toISOString().split('T')[0];
    ...
    const reportData = salespersons.map(sp => {
      ...
      const collections = vouchers.filter(v => 
  ...
  const exportPerformanceExcel = () => {
    const ws = XLSX.utils.json_to_sheet(performanceData.map(p => ({
  ...
  return (
    <div style={{ backgroundColor: BG, minHeight: '100vh', padding: '20px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#000000', marginBottom: '20px' }}>Salesperson Master</h1>
        ...
        <div style={{ display: 'flex', gap: '20px' }}>
          ...
          <div style={{ width: '260px', backgroundColor: BG_CARD, border: BORDER, padding: '15px', borderRadius: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000' }}>Salespersons</h2>
              <button
                onClick={clearForm}
                style={{
            ...
            <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
              {filteredSalespersons.map(sp => (
                ...
                >
                  <div style={{ fontWeight: 'bold', fontSize: '12px' }}>{sp.code} - {sp.name}</div>
                  <div style={{ fontSize: '11px', color: '#666' }}>{sp.assignedArea || sp.territory}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ flex: 1, backgroundColor: BG_CARD, border: BORDER, padding: '20px', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Code*</label>
                <input
                  type="text"
                  value={form.code}
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Assigned Area/Route</label>
                <input
                  type="text"
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Territory</label>
                <input
                  type="text"
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Commission Type</label>
                <select
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Commission Rate (%)</label>
                <input
                  type="number"
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Monthly Target (Rs.)</label>
                <input
                  type="number"
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Linked Employee</label>
                <select
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Joining Date</label>
                <input
                  type="date"
              ...
              <div style={{ gridColumn: 'span 2' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Notes</label>
                <textarea
                  value={form.notes}
      ...
      {activeTab === 'performance' && (
        <div style={{ backgroundColor: BG_CARD, border: BORDER, padding: '20px', borderRadius: '8px' }}>
          <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'end' }}>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Period</label>
              <select
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Salesperson</label>
              <select
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
              <thead>
                <tr style={{ backgroundColor: BG_HEADER }}>
                  ...
                  <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Actual Sales</th>
                  <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Achievement %</th>
                  <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Collection</th>
                  <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Outstanding</th>
              ...
              <tbody>
                {performanceData.map((p, idx) => {
                  const achievementColor = 
                    p.achievementPct >= 100 ? '#059669' :
                    p.achievementPct >= 75 ? '#d97706' : '#dc2626';
                  
                  return (
                    <tr key={p.sp.id} style={{ backgroundColor: idx % 2 === 0 ? BG_DEEP : 'transparent' }}>
                      <td style={{ border: BORDER, padding: '8px' }}>{p.sp.name}</td>
                      <td style={{ border: BORDER, padding: '8px' }}>{p.sp.assignedArea}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(p.target)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(p.totalSales)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right', color: achievementColor, fontWeight: 'bold' }}>
                        {p.achievementPct.toFixed(2)}%
                      </td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(p.totalCollection)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(p.outstanding)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{p.invoiceCount}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(p.commissionEarned)}</td>
                    </tr>
                  );
                })}
                {/* Total Row */}
                <tr style={{ backgroundColor: BG_HEADER }}>
                  ...
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right', fontWeight: 'bold' }}>
                  ...
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right', fontWeight: 'bold' }}>
                    {performanceData.length > 0 ? (performanceData.reduce((sum, p) => sum + p.achievementPct, 0) / performanceData.length).toFixed(2) : '0.00'}%
                  </td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right', fontWeight: 'bold' }}>
                ...
                >
                  <div style={{ fontWeight: 'bold' }}>
                    <span style={{ marginRight: '5px' }}>
                      {expandedAreas[province] ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                    </span>
                    {province}
                  </div>
                  <div style={{ fontWeight: 'bold' }}>
                    {money(Object.values(areaWiseData[province]).reduce((sum, dist) => 
                      sum + Object.values(dist).reduce((s, sp) => s + (sp.totalSales || 0), 0), 0))}
                  </div>
                </div>
                
                {expandedAreas[province] && Object.keys(areaWiseData[province]).map(district => (
                  <div key={district} style={{ marginLeft: '20px' }}>
                    ...
                    >
                      <div>
                        <span style={{ marginRight: '5px' }}>
                          {expandedDistricts[`${province}-${district}`] ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                        </span>
                        {district}
                      </div>
                      <div>
                        {money(Object.values(areaWiseData[province][district]).reduce((sum, sp) => sum + (sp.totalSales || 0), 0))}
                      </div>
                    </div>
                    
                    {expandedDistricts[`${province}-${district}`] && Object.keys(areaWiseData[province][district]).map(spId => {
                      ...
                      return (
                        <div key={spId} style={{ marginLeft: '40px' }}>
                          <div 
                            ...
                            onClick={() => toggleExpand('sp', `${province}-${district}-${spId}`)}
                          >
                            <div>
                              <span style={{ marginRight: '5px' }}>
                                {expandedSalespersons[`${province}-${district}-${spId}`] ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                              </span>
