...
import { Shield, AlertTriangle, CheckCircle, TrendingDown, Plus, Edit2, Trash2, Lock, Unlock, Download, Search, Filter } from "lucide-react";
...
export default function PricePolicyManager() {
  ...
  const exceptions = useMemo(() => {
    ...
    invoices.forEach(inv => {
      ...
      (inv.lines || []).forEach(line => {
        ...
        const compliance = checkPriceCompliance(line.itemId, sellingRate, discountPct, pricePolicies);
    ...
    return showOnlyUnapproved ? allExceptions.filter(exc => exc.status === "Pending") : allExceptions;
  ...
  const handleBulkSet = async () => {
    ...
    try {
      ...
      toast.success(`Updated ${itemsToProcess.length} items with ${bulkForm.marginPercent}% margin`);
    ...
    <div style={{ padding: '20px' }}>
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'end' }}>
        <div style={{ flex: 1, minWidth: '200px' }}>
          <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Search Items</label>
          <input
            type="text"
        ...
        <div>
          <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Item Group</label>
          <select
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            <thead>
              <tr style={{ backgroundColor: BG_HEADER }}>
                ...
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Min Selling Price</th>
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Max Discount %</th>
              ...
              {filteredItems.map(item => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td style={{ border: BORDER, padding: '8px' }}>{itemGroups.find(g => g.id === item.groupId)?.name || "N/A"}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(purchaseRate)}</td>
                    <td style={{ border: BORDER, padding: '8px' }}>
                      <input
                        type="number"
                        value={policy?.minSellingPrice || 0}
                        onChange={(e) => {
                          const updatedPolicy = { ...policy, minSellingPrice: parseFloat(e.target.value) || 0 };
                    ...
                    <td style={{ border: BORDER, padding: '8px' }}>
                      <input
                        ...
                        onChange={(e) => {
                          const updatedPolicy = { ...policy, maxDiscountPct: parseFloat(e.target.value) || 0 };
                          handleSavePolicy(item, updatedPolicy);
                        }}
                        style={{ width: '100%', padding: '4px', border: BORDER, borderRadius: '4px', fontSize: '11px' }}
                      />
  ...
  const renderExceptionsLog = () => (
    <div style={{ padding: '20px' }}>
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'end' }}>
        ...
        <div>
          <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Item</label>
          <select
            value={itemFilter}
        ...
        <div>
          <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Party</label>
          <select
            value={partyFilter}
      ...
      <div style={{ backgroundColor: BG_CARD, padding: '20px', borderRadius: '8px', border: BORDER }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            ...
            <tbody>
              {exceptions.map(exc => (
                <tr key={exc.id} style={{ backgroundColor: exc.status === "Pending" ? WARNING_BG : OK_BG }}>
                  ...
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(exc.sellingRate)}</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(exc.floorRate)}</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{exc.discountPct.toFixed(2)}%</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{exc.maxDiscountPct.toFixed(2)}%</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(exc.variance)}</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'center' }}>
                    <span style={{
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px', marginBottom: '20px' }}>
        <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '6px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Total Below Floor Revenue</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#dc2626' }}>{money(marginDashboard.totalBelowFloorRevenue)}</div>
        </div>
        <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '6px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Violation Count</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{marginDashboard.violations.length}</div>
        </div>
      </div>
      
      <div style={{ backgroundColor: BG_CARD, padding: '20px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
        <h3 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Biggest Margin Violators</h3>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            <thead>
              <tr style={{ backgroundColor: BG_HEADER }}>
                ...
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Avg Sale Rate</th>
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Floor Price</th>
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>% Below Floor</th>
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Revenue Impact</th>
              </tr>
            </thead>
            <tbody>
              {marginDashboard.topViolators.map(violator => (
                <tr key={violator.itemId}>
                  ...
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{violator.totalQty}</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(violator.avgSaleRate)}</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(violator.floorPrice)}</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{violator.percentBelow.toFixed(2)}%</td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(violator.totalRevenueImpact)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ backgroundColor: WARNING_BG, padding: '15px', borderRadius: '6px', border: BORDER, marginBottom: '20px' }}>
        <h3 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>Price Correction Suggestions</h3>
        <ul style={{ margin: 0, padding: '0 0 0 20px' }}>
          {marginDashboard.topViolators.slice(0, 3).map(violator => (
            <li key={violator.itemId} style={{ marginBottom: '5px', color: '#000000' }}>
              Consider revising floor price for <strong>{violator.itemName}</strong> (currently {violator.percentBelow.toFixed(2)}% below floor) OR negotiate better purchase rates to improve margins.
            </li>
          ))}
        </ul>
      </div>
  ...
  return (
    <div style={{ backgroundColor: BG, minHeight: '100vh' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#000000', padding: '20px', marginBottom: '0' }}>Price Policy Manager</h1>
      ...
      {bulkModalOpen && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Bulk Set from Margin</h2>
            ...
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Margin Percent (Above Purchase Rate)</label>
              <input
                type="number"
                value={bulkForm.marginPercent}
                onChange={(e) => setBulkForm({...bulkForm, marginPercent: parseFloat(e.target.value) || 0})}
                style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px' }}
              />
            </div>
