...
import { Plus, Edit, Trash2, Eye, Download, Printer, Calendar, Filter, AlertTriangle, TrendingUp, TrendingDown, RefreshCw, Save, X } from "lucide-react";
...
const FISCAL_MONTH_LABELS = ["Shrawan","Bhadra","Ashwin","Kartik","Mangsir","Poush","Magh","Falgun","Chaitra","Baisakh","Jestha","Ashadh"];

const Budgets: React.FC = () => {
  const { accounts, vouchers, currentFiscalYear, currentUser, costCenters, fiscalYears } = useStore();
  ...
  useEffect(() => {
    if (budgetForm.type === "Income Budget") {
      ...
      setBudgetLines(incomeAccounts.map(acc => ({ id: generateId(), accountId: acc.id, accountName: acc.name, annualAmount: 0 })));
    } else if (budgetForm.type === "Expense Budget") {
      const expenseAccounts = accounts.filter(a => a.type === "expense" && !a.isGroup);
      setBudgetLines(expenseAccounts.map(acc => ({ id: generateId(), accountId: acc.id, accountName: acc.name, annualAmount: 0 })));
    } else if (budgetForm.type === "Combined Budget") {
      const combinedAccounts = accounts.filter(a => (a.type === "income" || a.type === "expense") && !a.isGroup);
      setBudgetLines(combinedAccounts.map(acc => ({ id: generateId(), accountId: acc.id, accountName: acc.name, annualAmount: 0 })));
    }
  ...
  const saveBudget = async () => {
    ...
    try {
      await db.table("budgets").put(budgetRecord).catch(() => {
  ...
  const deleteBudget = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this budget?")) {
      const db = getDB();
      await db.table("budgets").delete(id).catch(() => {
        const arr = JSON.parse(localStorage.getItem("budgets") || "[]");
  ...
  const approveBudget = async (id: string, status: string) => {
    const db = getDB();
    await db.table("budgets").update(id, { status }).catch(() => {
      const arr = JSON.parse(localStorage.getItem("budgets") || "[]");
      const idx = arr.findIndex((b: any) => b.id === id);
  ...
  const applyDistribution = () => {
    ...
    const db = getDB();
    db.table("budgets").put(updatedBudget).catch(() => {
      const arr = JSON.parse(localStorage.getItem("budgets") || "[]");
  ...
  const summaryData = useMemo(() => {
    if (!budgetVsActualData.length) return { income: { budgeted: 0, actual: 0, variance: 0 }, expenses: { budgeted: 0, actual: 0, variance: 0 }, net: { budgeted: 0, actual: 0, variance: 0 } };
  ...
  const departmentBudgetData = useMemo(() => {
    ...
    const costCenterVouchers = vouchers.filter(v => 
      v.status === "posted" && 
      v.lines.some(l => l.costCenterId === selectedCostCenter)
    );
    
    const costCenterAccounts = accounts.filter(a => 
      (a.type === "expense" || a.type === "income") && 
      !a.isGroup
    );
    
    return costCenterAccounts.map(acc => {
      const budgeted = selectedBudget?.lines?.find(l => l.accountId === acc.id)?.annualAmount || 0;
      const actual = costCenterVouchers
        .filter(v => v.lines.some(l => l.accountId === acc.id && l.costCenterId === selectedCostCenter))
        .reduce((sum, v) => {
          const line = v.lines.find(l => l.accountId === acc.id && l.costCenterId === selectedCostCenter);
  ...
  const departmentSummary = useMemo(() => {
    if (!departmentBudgetData.length) return { totalBudgeted: 0, totalActual: 0, utilization: 0, status: "" };
    ...
    const status = utilization < 80 ? "Under Budget" : utilization < 100 ? "On Track" : "Over Budget";
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">Plan, distribute, and track your financial budgets</p>
          </div>
        </div>
        ...
        <div className="flex border-b border-gray-200 mb-4 bg-white px-2 pt-2 rounded-t-md shadow-sm overflow-x-auto hide-scrollbar">
          {["Budget Master", "Monthly Distribution", "Budget vs Actual", "Department Budget"].map((tab, index) => (
            <button
              key={index}
                  ...
                  <table className="w-full min-w-max border-collapse">
                    <thead>
                      <tr className="bg-[#f5f6fa] border-b border-gray-200">
                        <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Budget Name</th>
                        <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Fiscal Year</th>
                        ...
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Total Budgeted</th>
                        <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Created Date</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {budgets.map(budget => {
                        const totalBudgeted = budget.lines?.reduce((sum: number, line: any) => sum + (line.annualAmount || 0), 0) || 0;
                        return (
                          <tr key={budget.id} className="bg-white hover:bg-gray-50 border-b border-gray-100 text-[12px] transition-colors">
                            ...
                            <td className="px-3 py-2.5">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide ${
                                budget.status === "Draft" ? "bg-gray-100 text-gray-700 border border-gray-200" : 
                                budget.status === "Submitted" ? "bg-amber-100 text-amber-700 border border-amber-200" : 
                                "bg-green-100 text-green-700 border border-green-200"
                              }`}>
                                {budget.status}
                              </span>
                            </td>
                            <td className="px-3 py-2.5 text-right font-medium text-gray-800">{money(totalBudgeted)}</td>
                            <td className="px-3 py-2.5 text-gray-500">{new Date(budget.createdAt).toLocaleDateString()}</td>
                            <td className="px-3 py-2.5 text-right">
                              <div className="flex items-center justify-end gap-3">
                                ...
                                <button 
                                  ...
                                  onClick={() => {
                                    if (budget.status === "Draft" && currentUser.role === "accountant") {
                                      approveBudget(budget.id, "Submitted");
                                    } else if (currentUser.role === "admin" || (currentUser.role === "manager" && budget.type === "Department Budget")) {
                                      approveBudget(budget.id, "Approved");
                                    }
                                  }}
                                  disabled={!(budget.status === "Draft" || budget.status === "Submitted")}
                                >
                                  {budget.status === "Draft" ? "Submit" : budget.status === "Submitted" ? "Approve" : "Approved"}
                                </button>
                              </div>
                            </td>
                          </tr>
              ...
              <div className="bg-white">
                <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-3">
                  ...
                  <button 
                    ...
                    onClick={() => { setShowBudgetForm(false); setEditingBudget(null); }}
                  >
                    <X size={18} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Budget Name <span className="text-red-500">*</span></label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Budget Type</label>
                    <select
                  ...
                  <table className="w-full min-w-max border-collapse">
                    <thead className="sticky top-0 z-10 bg-[#f5f6fa]">
                      <tr className="border-b border-gray-200">
                        <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Code</th>
                        <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Account Name</th>
                        <th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-48">Annual Allocation (NPR)</th>
                        <th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Monthly Avg.</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {budgetLines.map((line, index) => (
                        <tr key={line.id} className="bg-white hover:bg-gray-50 text-[12px] transition-colors">
                          <td className="px-3 py-2 text-gray-500">{accounts.find(a => a.id === line.accountId)?.code || "-"}</td>
                          <td className="px-3 py-2 font-medium text-gray-800">{line.accountName}</td>
                          <td className="px-3 py-2 text-right">
                            <input
                              type="number"
                              step="0.01"
                              value={line.annualAmount || ""}
                              onChange={(e) => handleLineChange(index, 'annualAmount', e.target.value)}
                    ...
                    <tfoot className="sticky bottom-0 bg-gray-50 border-t-2 border-gray-200">
                      <tr>
                        <td colSpan={2} className="px-3 py-2.5 text-right font-bold text-[11px] uppercase tracking-wide text-gray-600">Total Budget Allocation:</td>
                        <td className="px-3 py-2.5 text-right font-bold text-[13px] text-[#1557b0]">
                          NPR {money(budgetLines.reduce((sum, l) => sum + (l.annualAmount || 0), 0))}
                        </td>
        ...
        {activeTab === 1 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-5 mb-4 max-w-full overflow-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
              <div>
                <h2 className="text-[15px] font-semibold text-gray-800">Monthly Distribution Model</h2>
                <p className="text-[11px] text-gray-500 mt-0.5">Allocate annual budgets across fiscal months</p>
              </div>
              <div className="flex flex-col sm:flex-row items-center gap-3">
                <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">Target Budget:</label>
                <select
            ...
            {selectedBudget ? (
              <>
                <div className="mb-6 bg-gray-50 p-4 rounded-md border border-gray-200">
                  <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6">
                    <div>
                      <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">Distribution Algorithm</label>
                    ...
                    <div className="ml-auto">
                      <button
                        ...
                        disabled={distributionMethod === "seasonal" && Math.abs(seasonalPercentages.reduce((sum, pct) => sum + pct, 0) - 100) > 0.01}
                      >
                        <Save size={14} />
                ...
                {distributionMethod === "seasonal" && (
                  <div className="mb-6 bg-white border border-gray-200 rounded-md p-4 shadow-sm">
                    <h4 className="text-[13px] font-semibold text-gray-800 mb-3 border-b border-gray-100 pb-2">Seasonal Allocation Parameters</h4>
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-12 gap-3 mb-3">
                      {FISCAL_MONTH_LABELS.map((month, idx) => (
                        <div key={idx} className="flex flex-col">
                          <label className="text-[10px] text-gray-500 font-medium mb-1 text-center">{month.substring(0,3)}</label>
                          <div className="relative">
                            ...
                            />
                            <span className="absolute right-1 top-1/2 transform -translate-y-1/2 text-[9px] text-gray-400">%</span>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between bg-gray-50 p-2 rounded border border-gray-200 text-[12px] font-medium">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-[14px] font-bold text-gray-800">{seasonalPercentages.reduce((sum, pct) => sum + pct, 0).toFixed(2)}%</span>
                        <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wide ${
                          Math.abs(seasonalPercentages.reduce((sum, pct) => sum + pct, 0) - 100) < 0.01 
                            ? 'bg-green-100 text-green-700 border border-green-200' 
                            : 'bg-red-100 text-red-700 border border-red-200'
                        }`}>
                          {Math.abs(seasonalPercentages.reduce((sum, pct) => sum + pct, 0) - 100) < 0.01 ? '✓ Valid (100%)' : '✗ Invalid (Must be 100%)'}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <h4 className="text-[13px] font-semibold text-gray-800 mb-3 mt-2">Preview of Monthly Distribution</h4>
                  ...
                  <table className="w-full min-w-max border-collapse text-[11px]">
                    <thead>
                      <tr className="bg-[#f5f6fa] border-b border-gray-200 text-gray-500 uppercase tracking-wide">
                        ...
                        <th className="px-3 py-2.5 text-right font-semibold bg-gray-100">Annual Budget</th>
                        {FISCAL_MONTH_LABELS.map(month => (
                          <th key={month} className="px-2 py-2.5 text-right font-semibold">{month.substring(0,3)}</th>
                        ))}
                        <th className="px-3 py-2.5 text-right font-semibold bg-gray-100">Check Total</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {selectedBudget.lines?.map((line: any, idx: number) => {
                        ...
                        return (
                          <tr key={idx} className="bg-white hover:bg-gray-50 transition-colors">
                            <td className="px-3 py-2 font-medium text-gray-800 truncate max-w-[150px]" title={line.accountName}>{line.accountName}</td>
                            <td className="px-3 py-2 text-right font-bold text-[#1557b0] bg-blue-50/30">{money(line.annualAmount)}</td>
                            {monthlyAmounts.map((amt, i) => (
                              <td key={i} className="px-2 py-2 text-right text-gray-600">{money(amt)}</td>
                            ))}
                            <td className={`px-3 py-2 text-right font-bold bg-gray-50 ${isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                              {money(rowTotal)}
                            </td>
                          </tr>
                        );
                      ...
                      {(!selectedBudget.lines || selectedBudget.lines.length === 0) && (
                        <tr>
                          <td colSpan={15} className="px-3 py-8 text-center text-[12px] text-gray-400">
              ...
              <div className="flex flex-col items-center justify-center py-16 text-gray-400 bg-gray-50/50 rounded-md border border-gray-200 border-dashed">
                ...
                <p className="text-[13px] font-medium">Please select a budget from the dropdown above to manage its monthly distribution.</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 2 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6 border-b border-gray-100 pb-4">
              <div>
                <h2 className="text-[15px] font-semibold text-gray-800">Budget vs Actual Performance</h2>
                <p className="text-[11px] text-gray-500 mt-0.5">Real-time variance analysis of financial actuals against targets</p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 bg-gray-50 p-2 rounded-md border border-gray-200">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1.5 bg-white border border-gray-300 rounded-md px-2 h-8">
                    ...
                    />
                    <label htmlFor="overBudget" className="text-[10px] font-medium text-gray-600 cursor-pointer">Has Budget Only</label>
                  </div>
                </div>
              </div>
            </div>
            ...
            {selectedBudget ? (
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
                  <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm hover:border-blue-200 transition-colors relative overflow-hidden">
                    ...
                    <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Total Income</div>
                    <div className="text-2xl font-bold text-gray-800 mb-2">NPR {money(summaryData.income.actual)}</div>
                    <div className="flex items-center gap-2 text-[12px] bg-gray-50 p-2 rounded">
                      <span className="text-gray-500">Target: NPR {money(summaryData.income.budgeted)}</span>
                    </div>
                    <div className={`mt-2 text-[12px] font-bold flex items-center gap-1 ${summaryData.income.variance >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                      {summaryData.income.variance >= 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                  ...
                  <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm hover:border-blue-200 transition-colors relative overflow-hidden">
                    ...
                    <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Total Expenses</div>
                    <div className="text-2xl font-bold text-gray-800 mb-2">NPR {money(summaryData.expenses.actual)}</div>
                    <div className="flex items-center gap-2 text-[12px] bg-gray-50 p-2 rounded">
                      <span className="text-gray-500">Target: NPR {money(summaryData.expenses.budgeted)}</span>
                    </div>
                    <div className={`mt-2 text-[12px] font-bold flex items-center gap-1 ${summaryData.expenses.variance <= 0 ? 'text-green-600' : 'text-red-500'}`}>
                      {summaryData.expenses.variance <= 0 ? <TrendingDown size={14} /> : <TrendingUp size={14} />}
                  ...
                  <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm hover:border-blue-200 transition-colors relative overflow-hidden">
                    ...
                    <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Net Performance</div>
                    <div className="text-2xl font-bold text-[#1557b0] mb-2">NPR {money(summaryData.net.actual)}</div>
                    <div className="flex items-center gap-2 text-[12px] bg-gray-50 p-2 rounded">
                      <span className="text-gray-500">Target: NPR {money(summaryData.net.budgeted)}</span>
                    </div>
                    <div className={`mt-2 text-[12px] font-bold flex items-center gap-1 ${summaryData.net.variance >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                      {summaryData.net.variance >= 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                ...
                <div className="bg-white border border-gray-200 rounded-md p-5 shadow-sm mb-6">
                  <h3 className="text-[13px] font-semibold text-gray-800 mb-4 pb-2 border-b border-gray-100">Top Variances (Budget vs Actual)</h3>
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={topVarianceData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E5E7EB" />
                        <XAxis dataKey="name" tick={{fontSize: 10, fill: '#6B7280'}} axisLine={false} tickLine={false} dy={10} />
                        <YAxis tick={{fontSize: 10, fill: '#6B7280'}} axisLine={false} tickLine={false} dx={-10} tickFormatter={(value) => `₹${value >= 1000 ? (value/1000).toFixed(0)+'k' : value}`} />
                        <Tooltip cursor={{fill: '#F3F4F6'}} contentStyle={{borderRadius: '6px', border: '1px solid #E5E7EB', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)'}} />
                        <Bar dataKey="budgeted" name="Budgeted" fill="#1557b0" radius={[4, 4, 0, 0]} maxBarSize={40} />
                        <Bar dataKey="actual" name="Actual" fill="#0ea5e9" radius={[4, 4, 0, 0]} maxBarSize={40} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-end mb-3">
                  <h3 className="text-[13px] font-semibold text-gray-800">Detailed Variance Analysis</h3>
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => {
                        const ws = XLSX.utils.json_to_sheet(budgetVsActualData.map(item => ({
                          "Account Code": accounts.find(a => a.id === item.accountId)?.code || "N/A",
                  ...
                  <table className="w-full min-w-max border-collapse">
                    <thead>
                      <tr className="bg-[#f5f6fa] border-b border-gray-200">
                        ...
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide bg-gray-50">Annual Target</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Period Budget</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Actuals</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Variance</th>
                        <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">% Diff</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {budgetVsActualData.map(item => {
                        ...
                        return (
                          <tr key={item.id} className="bg-white hover:bg-gray-50 text-[12px] transition-colors">
                            <td className="px-3 py-2.5">
                              <div className="font-medium text-gray-800">{item.accountName}</div>
                              <div className="text-[10px] text-gray-400">{account?.code || "-"}</div>
                            </td>
                            <td className="px-3 py-2.5 text-right font-medium text-gray-600 bg-gray-50">{money(item.budgeted)}</td>
                            <td className="px-3 py-2.5 text-right text-[#1557b0] font-medium">{money(item.budgeted)}</td>
                            <td className="px-3 py-2.5 text-right font-bold text-gray-800">{money(item.actual)}</td>
                            <td className={`px-3 py-2.5 text-right font-medium ${isAdverse ? 'text-red-600 bg-red-50/50' : 'text-green-600 bg-green-50/50'}`}>
                              {money(Math.abs(item.variance))} {isAdverse ? '(Adv)' : '(Fav)'}
                            </td>
                            <td className={`px-3 py-2.5 text-right font-bold ${isAdverse ? 'text-red-600' : 'text-green-600'}`}>
              ...
              <div className="flex flex-col items-center justify-center py-16 text-gray-400 bg-gray-50/50 rounded-md border border-gray-200 border-dashed">
                ...
                <p className="text-[13px] font-medium">Select a budget to view performance analysis.</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 3 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pb-4 border-b border-gray-100 gap-4">
              <div>
                <h2 className="text-[15px] font-semibold text-gray-800">Departmental Cost Control</h2>
                <p className="text-[11px] text-gray-500 mt-0.5">Track expense utilization across cost centers</p>
              </div>
              <div className="flex items-center gap-3">
                <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Target Cost Center:</label>
                <select
            ...
            {costCenters.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-gray-400 bg-gray-50/50 rounded-md border border-gray-200 border-dashed">
                ...
                <div className="text-[14px] font-bold text-gray-600 mb-1">No cost centers configured</div>
                <p className="text-[12px]">You need to set up cost centers in the Masters section first.</p>
              </div>
            ) : selectedCostCenter ? (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                  <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm hover:border-[#1557b0] transition-colors">
                    <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Total Allocation</div>
                    <div className="text-xl font-bold text-gray-800">NPR {money(departmentSummary.totalBudgeted)}</div>
                  </div>
                  <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm hover:border-[#1557b0] transition-colors">
                    <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Actual Spent</div>
                    <div className="text-xl font-bold text-gray-800">NPR {money(departmentSummary.totalActual)}</div>
                  </div>
                  <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm hover:border-[#1557b0] transition-colors">
                    <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Utilization Rate</div>
                    <div className="text-xl font-bold text-[#1557b0]">{departmentSummary.utilization.toFixed(1)}%</div>
                  </div>
                  <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm hover:border-[#1557b0] transition-colors">
                    <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Status</div>
                    <div className="flex items-center gap-1.5 mt-1">
                      <div className={`w-2 h-2 rounded-full ${
                        departmentSummary.utilization < 80 ? 'bg-green-500' : 
                        departmentSummary.utilization < 100 ? 'bg-amber-500' : 'bg-red-500'
                      }`}></div>
                      <div className={`text-lg font-bold ${
                        departmentSummary.utilization < 80 ? 'text-green-600' : 
                        departmentSummary.utilization < 100 ? 'text-amber-600' : 'text-red-600'
                      }`}>
                ...
                <h3 className="text-[13px] font-semibold text-gray-800 mb-4 border-b border-gray-100 pb-2">Line Item Utilization</h3>
                    ...
                    return (
                      <div key={item.accountId} className="mb-5 last:mb-0 group">
                        <div className="flex justify-between items-end text-[12px] mb-1.5">
                          <span className="font-medium text-gray-800 group-hover:text-[#1557b0] transition-colors">{item.accountName}</span>
                          <div className="text-right">
                            <span className="font-bold text-gray-800">NPR {money(item.actual)}</span>
                            <span className="text-gray-400 mx-1">/</span>
                            <span className="text-gray-500">NPR {money(item.budgeted)}</span>
                            <span className="ml-2 font-mono text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">{pct.toFixed(0)}%</span>
                          </div>
                        </div>
              ...
              <div className="flex flex-col items-center justify-center py-16 text-gray-400 bg-gray-50/50 rounded-md border border-gray-200 border-dashed">
                <AlertTriangle size={48} className="mb-3 opacity-20" />
                <p className="text-[13px] font-medium">Select a cost center above to view utilization metrics.</p>
              </div>
            )}
          </div>
        )}
