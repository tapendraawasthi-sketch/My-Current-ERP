...
import { Printer, Mail, FileText, CheckCircle, Plus, CheckSquare, Bell, Clock, Search, AlertTriangle } from "lucide-react";
...
const OutstandingManagement: React.FC = () => {
  ...
  const getPartyOutstanding = (partyId: string, invoicesList: any[], vouchersList: any[]) => {
    ...
    return partyInvoices.map(inv => {
      const paid = (vouchersList || []).filter(v => 
      ...
      const daysOverdue = Math.max(0, Math.floor((new Date().getTime() - dueDate.getTime()) / 86400000));
      const interestRate = party?.interestRate || 0;
      const interest = balance * (interestRate / 100 / 365) * daysOverdue;
      
      return { ...inv, paid, balance, dueDate: dueDate.toISOString().split("T")[0], daysOverdue, interest };
    });
  };
  ...
  const receivables = useMemo(() => {
    ...
    return filtered.map(inv => getPartyOutstanding(inv.partyId, [inv], vouchers)[0]).filter(i => i.balance > 0);
  ...
  const receivableTotals = useMemo(() => {
    return receivables.reduce((acc, inv) => {
  ...
  const payableTotals = useMemo(() => {
    return payables.reduce((acc, inv) => {
      acc.totalOutstanding += inv.balance;
      acc.totalInterest += inv.interest;
      return acc;
    }, { totalOutstanding: 0, totalInterest: 0 });
  ...
  const markDeposited = (id: string) => {
    const db = getDB();
    db.table("pdcRecords").update(id, { status: "deposited", depositDate: new Date().toISOString() });
    setPdcs(prev => prev.map(p => p.id === id ? { ...p, status: "deposited", depositDate: new Date().toISOString() } : p));
  ...
  const executeBounce = async () => {
    ...
    try {
      const partyAccount = accounts.find(a => a.name === parties.find(p => p.id === selectedPdc.partyId)?.name);
      ...
      await addVoucher({
        ...
        lines: [
          { accountId: partyAccount.id, accountName: partyAccount.name, debit: selectedPdc.amount, credit: 0 },
          { accountId: bankAccount.id, accountName: bankAccount.name, debit: 0, credit: selectedPdc.amount }
        ],
      ...
      if (bankChargeAccount && bounceForm.bankCharge > 0) {
        await addVoucher({
          ...
          lines: [
            { accountId: bankChargeAccount.id, accountName: bankChargeAccount.name, debit: bounceForm.bankCharge, credit: 0 },
            { accountId: bankAccount.id, accountName: bankAccount.name, debit: 0, credit: bounceForm.bankCharge }
          ],
      ...
      const db = getDB();
      await db.table("pdcRecords").update(selectedPdc.id, { status: "bounced", bouncedDate: new Date().toISOString() });
      setPdcs(prev => prev.map(p => p.id === selectedPdc.id ? { ...p, status: "bounced", bouncedDate: new Date().toISOString() } : p));
  ...
  const generateInterestJournal = async () => {
    ...
    for (const inv of overdueInvoices) {
      ...
      const interestIncomeAccount = accounts.find(a => a.name.toLowerCase().includes("interest income"));
      ...
      await addVoucher({
        ...
        lines: [
          { accountId: partyAccount.id, accountName: partyAccount.name, debit: inv.interest, credit: 0 },
          { accountId: interestIncomeAccount.id, accountName: interestIncomeAccount.name, debit: 0, credit: inv.interest }
        ],
  ...
  const generateStatement = () => {
    ...
    const partyTransactions = vouchers.filter(v => 
      v.status === 'posted' && 
    ...
    const statementRows = partyTransactions.map(v => {
      const line = v.lines?.find((l:any) => l.accountId === party.accountId);
      if (line) {
  ...
  const generateReminderLetters = () => {
    ...
    const letters = selectedPartyList.map(partyId => {
      const party = parties.find(p => p.id === partyId);
      const partyInvoices = receivables.filter(inv => inv.partyId === partyId && inv.daysOverdue >= Number(agingBucket));
  ...
  const exportToPdf = (data: any[], title: string) => {
    ...
    if(data.length > 0) {
      (doc as any).autoTable({
        startY: 20,
        head: [Object.keys(data[0]).map(k => k.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()))],
  ...
  const todaysDueCheques = pdcs.filter(pdc => pdc.chequeDate === new Date().toISOString().split('T')[0] && pdc.status === "held");
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">Manage receivables, payables, PDCs, statements, and reminders</p>
          </div>
        </div>
        ...
        <div className="flex border-b border-gray-200 mb-4 bg-white px-2 pt-2 rounded-t-md shadow-sm overflow-x-auto hide-scrollbar">
          {["Bills Receivable", "Bills Payable", "PDC Register", "Statement of Account", "Reminder Letters"].map((tab, index) => (
            <button
              key={index}
        ...
        {activeTab === 0 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            <div className="flex flex-wrap items-end gap-3 mb-4 bg-gray-50 p-3 rounded-md border border-gray-200">
              ...
              <div className="w-32">
                <label className="block text-[11px] font-medium text-gray-600 mb-1">Min Amount</label>
                <input
                  type="number"
                  value={minAmount || ""}
                  placeholder="0.00"
              ...
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    ...
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Inv Date</th>
                    ...
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Received</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Balance</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Days Due</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Interest</th>
                  </tr>
                </thead>
                <tbody>
                  {receivables.map(inv => {
                    const party = parties.find(p => p.id === inv.partyId);
                    return (
                      <tr key={inv.id} className={`${getAgingBgColor(inv.daysOverdue)} hover:bg-gray-50 border-b border-gray-100 text-[12px] transition-colors`}>
                        ...
                        <td className="px-3 py-2.5 text-gray-700 text-right">{money(inv.grandTotal)}</td>
                        <td className="px-3 py-2.5 text-green-600 text-right">{money(inv.paid)}</td>
                        <td className="px-3 py-2.5 text-red-600 font-medium text-right">{money(inv.balance)}</td>
                        <td className="px-3 py-2.5 text-gray-700 text-right font-medium">{inv.daysOverdue}</td>
                        <td className="px-3 py-2.5 text-amber-600 text-right">{money(inv.interest)}</td>
                      </tr>
                    );
                  })}
                  {receivables.length > 0 && (
                    <tr className="bg-[#f5f6fa] border-t-2 border-gray-200">
                      <td className="px-3 py-2.5 text-[11px] font-bold text-gray-700 uppercase" colSpan={4}>Totals</td>
                      <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right" colSpan={3}>{money(receivableTotals.totalOutstanding)}</td>
                      <td className="px-3 py-2.5" colSpan={1}></td>
                      <td className="px-3 py-2.5 text-[12px] font-bold text-amber-600 text-right">{money(receivableTotals.totalInterest)}</td>
                    </tr>
                  )}
                  {receivables.length === 0 && (
                    <tr>
              ...
              <table className="w-full min-w-max border-collapse">
                ...
                <tbody>
                  ...
                  {payables.length > 0 && (
                    <tr className="bg-[#f5f6fa] border-t-2 border-gray-200">
                      <td className="px-3 py-2.5 text-[11px] font-bold text-gray-700 uppercase" colSpan={4}>Totals</td>
                      <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right" colSpan={3}>{money(payableTotals.totalOutstanding)}</td>
                      <td className="px-3 py-2.5" colSpan={1}></td>
                      <td className="px-3 py-2.5 text-[12px] font-bold text-amber-600 text-right">{money(payableTotals.totalInterest)}</td>
                    </tr>
                  )}
                  {payables.length === 0 && (
                    <tr>
        ...
        {activeTab === 2 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-[14px] font-semibold text-gray-800">Post-Dated Cheque Register</h2>
              <button
            ...
            {todaysDueCheques.length > 0 && (
              <div className="bg-amber-50 text-amber-800 p-3 rounded-md border border-amber-200 mb-4 flex items-center gap-2 text-[13px] font-medium shadow-sm">
                ...
                {todaysDueCheques.length} cheque(s) are due TODAY for deposit! Total: NPR {money(todaysDueTotal)}
              </div>
            )}
            
            {showPdcForm && (
              <div className="mb-6 bg-white border border-gray-200 shadow-sm rounded-md p-4">
                <h3 className="text-[14px] font-semibold text-gray-800 mb-4 pb-2 border-b border-gray-100">Add New PDC</h3>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Party <span className="text-red-500">*</span></label>
                    <select
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Cheque Number <span className="text-red-500">*</span></label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Cheque Date <span className="text-red-500">*</span></label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Amount <span className="text-red-500">*</span></label>
                    <input
                      type="number"
                      step="0.01"
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Linked Invoice</label>
                    <select
                    ...
                    >
                      ...
                      {invoices.filter(inv => inv.partyId === pdcForm.partyId).map(inv => (
                        <option key={inv.id} value={inv.id}>{inv.invoiceNo} - NPR {money(inv.grandTotal)}</option>
                      ))}
                    </select>
                  </div>
                </div>
            ...
            {showBounceForm && selectedPdc && (
              <div className="mb-6 bg-red-50 border border-red-200 shadow-sm rounded-md p-4">
                ...
                <div className="text-[12px] text-red-700 mb-4">
                  Cheque <strong>{selectedPdc.chequeNo}</strong> from <strong>{parties.find(p=>p.id===selectedPdc.partyId)?.name}</strong> for NPR <strong>{money(selectedPdc.amount)}</strong>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-[11px] font-medium text-red-700 mb-1">Bank Charges (NPR)</label>
                    <input
                      type="number"
                      step="0.01"
                      value={bounceForm.bankCharge}
                      onChange={(e) => setBounceForm(prev => ({...prev, bankCharge: Number(e.target.value)}))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setBounceForm(prev => ({...prev, bankChargeDate: e.target.value}))}
                      className="h-8 px-2.5 text-[12px] border border-red-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 w-full"
                    />
                  </div>
                </div>
                <div className="flex justify-end gap-2 pt-4 border-t border-red-100">
                  <button
                    className="h-8 px-4 bg-white border border-gray-300 text-gray-700 text-[12px] font-medium rounded-md hover:bg-gray-50 transition-colors"
                    onClick={() => { setShowBounceForm(false); setSelectedPdc(null); }}
                  >
                    Cancel
                  </button>
                  <button
            ...
            <div className="border border-gray-200 rounded-md overflow-hidden">
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Cheque No</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Bank</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Party</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Cheque Date</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Amount</th>
                    <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Status</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Days to Maturity</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Linked Invoice</th>
                ...
                <tbody>
                  {pdcs.map(pdc => {
                    ...
                    const daysToMaturity = Math.ceil((chequeDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
                    
                    const isPastDue = chequeDate < today && pdc.status === 'held';
                    
                    return (
                      <tr key={pdc.id} className={`${isPastDue ? 'bg-red-50' : 'bg-white'} hover:bg-gray-50 border-b border-gray-100 text-[12px] transition-colors`}>
                        ...
                        <td className="px-3 py-2.5 text-gray-800 text-right font-medium">{money(pdc.amount)}</td>
                        <td className="px-3 py-2.5 text-center">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide ${
                            pdc.status === "held" ? "bg-amber-100 text-amber-700" :
                            pdc.status === "deposited" ? "bg-blue-100 text-blue-700" :
                            pdc.status === "cleared" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                          }`}>
                            {pdc.status}
                          </span>
                        </td>
                        <td className="px-3 py-2.5 text-gray-700 text-right">{pdc.status === 'held' ? daysToMaturity : '-'}</td>
        ...
        {activeTab === 3 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            <div className="flex flex-wrap items-end gap-3 mb-6 bg-gray-50 p-3 rounded-md border border-gray-200">
              <div className="flex-1 min-w-[200px]">
                <label className="block text-[11px] font-medium text-gray-600 mb-1">Select Party <span className="text-red-500">*</span></label>
                <select
            ...
            {statementData.length > 0 && (
              <div className="border border-gray-200 rounded-md p-6 bg-white shadow-sm mb-4">
                <div className="text-center mb-6">
                  <h2 className="text-[18px] font-bold text-gray-800">{companySettings?.name || "Company Name"}</h2>
                  <div className="text-[14px] font-semibold text-gray-600 mt-1 uppercase tracking-widest">Statement of Account</div>
                  <div className="text-[12px] text-gray-500 mt-2">Party: <span className="font-semibold text-gray-700">{parties.find(p => p.id === statementParty)?.name}</span></div>
                  <div className="text-[12px] text-gray-500">Period: {statementFrom || 'Start'} to {statementTo}</div>
                </div>
                  ...
                  <table className="w-full min-w-max border-collapse">
                    <thead>
                      <tr className="bg-[#f5f6fa] border-b border-gray-200">
                        ...
                        <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-1/3">Description</th>
                    ...
                    <tbody>
                      {statementData.map((row, idx) => (
                        <tr key={idx} className="hover:bg-gray-50 border-b border-gray-100 text-[12px] transition-colors">
                          <td className="px-3 py-2 text-gray-700">{row.date}</td>
                          <td className="px-3 py-2 text-gray-700">{row.voucherNo}</td>
                          <td className="px-3 py-2 text-gray-700 truncate max-w-xs" title={row.description}>{row.description}</td>
                          <td className="px-3 py-2 text-gray-700 text-right">{row.dr > 0 ? money(row.dr) : ''}</td>
                          <td className="px-3 py-2 text-gray-700 text-right">{row.cr > 0 ? money(row.cr) : ''}</td>
                          <td className="px-3 py-2 text-gray-800 font-medium text-right">{money(row.balance)} {row.balance >= 0 ? 'Dr' : 'Cr'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="mt-4 p-3 bg-gray-50 rounded-md border border-gray-200 text-right">
                  ...
                  <span className="text-[16px] font-bold text-[#1557b0]">
                    NPR {money(Math.abs(statementData[statementData.length - 1]?.balance || 0))} {statementData[statementData.length - 1]?.balance >= 0 ? 'Dr' : 'Cr'}
                  </span>
                </div>
                ...
                <div className="mt-6 flex gap-2 justify-end">
                  ...
                  <button
                    className="h-8 px-4 bg-white border border-gray-300 text-gray-700 text-[12px] font-medium rounded-md hover:bg-gray-50 flex items-center gap-1.5 transition-colors shadow-sm"
                    onClick={() => exportToPdf(statementData, `Statement_${parties.find(p => p.id === statementParty)?.name || 'Unknown'}`)}
                  >
                    <FileText size={14} />
                    Export PDF
                  </button>
        ...
        {activeTab === 4 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            <div className="flex flex-wrap items-end gap-3 mb-6 bg-gray-50 p-3 rounded-md border border-gray-200">
              <div className="w-48">
                <label className="block text-[11px] font-medium text-gray-600 mb-1">Aging Bucket</label>
                <select
                  value={agingBucket}
                  ...
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-10">
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            const allSelected = new Set(parties.filter(p => 
                              getPartyOutstanding(p.id, invoices, vouchers).some(inv => inv.daysOverdue >= Number(agingBucket))
                            ).map(p => p.id));
                        ...
                        checked={parties.filter(p => 
                          getPartyOutstanding(p.id, invoices, vouchers).some(inv => inv.daysOverdue >= Number(agingBucket))
                        ).length > 0 && selectedParties.size === parties.filter(p => 
                          getPartyOutstanding(p.id, invoices, vouchers).some(inv => inv.daysOverdue >= Number(agingBucket))
                        ).length}
                    ...
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Outstanding Amount</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Max Days Overdue</th>
                  </tr>
                </thead>
                <tbody>
                  {parties.map(party => {
                    const partyOutstandings = getPartyOutstanding(party.id, invoices, vouchers).filter(inv => inv.daysOverdue >= Number(agingBucket));
                    if (partyOutstandings.length === 0) return null;
                    
                    const totalOutstanding = partyOutstandings.reduce((sum, inv) => sum + inv.balance, 0);
                    const maxDaysOverdue = Math.max(...partyOutstandings.map(inv => inv.daysOverdue));
                    
                    return (
                      <tr key={party.id} className="bg-white hover:bg-gray-50 border-b border-gray-100 text-[12px] transition-colors">
                        ...
                        <td className="px-3 py-2.5 text-gray-700">{party.email || party.phone || "N/A"}</td>
                        <td className="px-3 py-2.5 text-gray-800 font-medium text-right">{money(totalOutstanding)}</td>
                        <td className="px-3 py-2.5 text-red-600 font-medium text-right">{maxDaysOverdue}</td>
                      </tr>
                    );
                  })}
                  {parties.filter(p => getPartyOutstanding(p.id, invoices, vouchers).some(inv => inv.daysOverdue >= Number(agingBucket))).length === 0 && (
                    <tr>
            ...
            {reminderLetters.length > 0 && (
              <div className="mt-6 border-t border-gray-200 pt-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-[14px] font-semibold text-gray-800">Generated Letters ({reminderLetters.length})</h3>
                  <button
                    className="h-8 px-4 bg-[#1557b0] hover:bg-[#0f4a96] text-white text-[12px] font-medium rounded-md flex items-center gap-1.5 transition-colors shadow-sm"
                    onClick={() => window.print()}
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {reminderLetters.map((letter, index) => (
                    <div key={index} className="bg-white border border-gray-200 p-6 rounded-md shadow-sm whitespace-pre-wrap text-[13px] font-serif leading-relaxed text-gray-800">
                      {letter}
                    </div>
                  ))}
                </div>
