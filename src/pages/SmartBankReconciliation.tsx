...
const parseBankCSV = (csvText: string): BankStatementEntry[] => {
  ...
  const refIndex = headers.findIndex(h => h.includes('ref') || h.includes('chq') || h.includes('cheque'));
...
): { matched: { ledgerId: string; statementId: string; matchScore: number }[]; unmatched: string[] } => {
  ...
  for (const stmtEntry of statementEntries) {
    ...
    for (const ledgerEntry of ledgerEntries) {
      ...
      if (Math.abs(stmtAmount - Math.abs(ledgerAmount)) < 0.01) {
        ...
        if (stmtEntry.reference && ledgerEntry.chequeNo && stmtEntry.reference.includes(ledgerEntry.chequeNo)) {
          score += 50;
        }
...
const computeReconciliation = (
  ...
  const reconciledBalance = ledgerBalance - outstandingCheques + depositInTransit + bankCharges - bankInterest;
...
const SmartBankReconciliation: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedStatement) {
      ...
      const sortedEntries = [...selectedStatement.entries].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
      const calculatedOpening = sortedEntries[0]?.balance - sortedEntries[0].debitAmount + sortedEntries[0].creditAmount || 0;
  ...
  const bankAccounts = accounts.filter(account => account.group && account.group.toLowerCase().includes('bank'));
  ...
  const markAsException = (entryId: string, type: "bank_charge" | "bank_interest" | "other") => {
    ...
    const updatedEntries = selectedStatement.entries.map(entry =>
      entry.id === entryId
        ? { ...entry, isException: true, exceptionType: type }
        : entry
    );
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-[#f5f6fa]">
                        <tr>
                          ...
                          <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Description</th>
                          <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Reference</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Debit</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Credit</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Balance</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
                        {selectedStatement.entries.slice(0, 5).map((entry, idx) => (
                          <tr key={idx}>
                            <td className="px-4 py-2 whitespace-nowrap text-[12px] text-gray-700">{entry.date}</td>
                            <td className="px-4 py-2 text-[12px] text-gray-700">{entry.description}</td>
                            <td className="px-4 py-2 text-[12px] text-gray-700">{entry.reference}</td>
                            <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">{entry.debitAmount > 0 ? formatCurrency(entry.debitAmount) : '-'}</td>
                            <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">{entry.creditAmount > 0 ? formatCurrency(entry.creditAmount) : '-'}</td>
                            <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">{formatCurrency(entry.balance)}</td>
                          </tr>
                        ))}
                        {selectedStatement.entries.length > 5 && (
                          <tr>
                            <td colSpan={6} className="px-4 py-2 text-center text-[12px] text-gray-500">
          ...
          <div className="space-y-6">
            <div className="bg-white p-4 rounded-lg shadow-sm">
              ...
              <div className="text-sm text-gray-600 mb-4">
                Matched: {selectedStatement.entries.filter(e => e.isMatched).length}, 
                Unmatched: {selectedStatement.entries.filter(e => !e.isMatched).length}, 
                Exceptions: {selectedStatement.entries.filter(e => e.isException).length}
              </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {selectedStatement.entries.map(entry => {
                          ...
                          return (
                            <tr key={entry.id} className={isMatched || isException ? 'bg-green-50' : ''}>
                              <td className="px-4 py-2 whitespace-nowrap text-[12px] text-gray-700">{entry.date}</td>
                              <td className="px-4 py-2 text-[12px] text-gray-700">{entry.description}</td>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-[#f5f6fa] sticky top-0">
                        <tr>
                          ...
                          <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Description</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Amount</th>
                          <th className="px-4 py-2 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Type</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm print:shadow-none print:border">
              <div className="flex justify-between items-start mb-6 print:mb-4">
                <div>
                  ...
                  <p className="text-gray-600 print:text-sm">
                    {selectedStatement?.bankAccountName || 'Selected Account'} - {selectedStatement?.period || 'N/A'}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 print:hidden">
                <div className="bg-blue-50 border border-blue-200 rounded p-4">
                  ...
                  <p className="text-lg font-bold text-blue-900">{formatCurrency(reconciliation.bankStatementBalance)}</p>
                </div>
                ...
                <table className="min-w-full divide-y divide-gray-200 border rounded">
                  <thead className="bg-[#f5f6fa]">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Amount (NPR)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    <tr>
                      <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-700">Balance as per Bank Statement</td>
                      <td className="px-4 py-3 text-right text-[12px] text-gray-700 font-mono">{formatCurrency(reconciliation.bankStatementBalance)}</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-700">Add: Outstanding Deposits (Deposits in Transit)</td>
                      <td className="px-4 py-3 text-right text-[12px] text-gray-700 font-mono">+ {formatCurrency(reconciliation.adjustments.depositInTransit)}</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-700">Less: Outstanding Cheques</td>
                      <td className="px-4 py-3 text-right text-[12px] text-gray-700 font-mono">- {formatCurrency(reconciliation.adjustments.outstandingCheques)}</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-700">Add: Bank Interest Earned</td>
                      <td className="px-4 py-3 text-right text-[12px] text-gray-700 font-mono">+ {formatCurrency(reconciliation.adjustments.bankInterest)}</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-700">Less: Bank Charges</td>
                      <td className="px-4 py-3 text-right text-[12px] text-gray-700 font-mono">- {formatCurrency(reconciliation.adjustments.bankCharges)}</td>
                    </tr>
                    <tr className="bg-gray-50 font-bold">
                      <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-900">Balance as per Books (Adjusted)</td>
                      <td className="px-4 py-3 text-right text-[12px] text-gray-900 font-mono">{formatCurrency(reconciliation.reconciledBalance)}</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-700">Balance as per Ledger</td>
                      <td className="px-4 py-3 text-right text-[12px] text-gray-700 font-mono">{formatCurrency(reconciliation.ledgerBalance)}</td>
                    </tr>
                    <tr className={Math.abs(reconciliation.difference) < 0.01 ? "bg-green-100 font-bold" : "bg-red-100 font-bold"}>
                      <td className="px-4 py-3 whitespace-nowrap text-[12px] text-gray-900">DIFFERENCE</td>
                      <td className="px-4 py-3 text-right text-[12px] text-gray-900 font-mono">{formatCurrency(reconciliation.difference)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div className={`mt-6 p-4 rounded-lg text-center ${
                ...
                {reconciliation.isBalanced ? (
                  <p className="font-bold">✓ RECONCILED — Balance as per Bank matches adjusted book balance</p>
                ) : (
                  <p className="font-bold">✗ UNRECONCILED — Difference of {formatCurrency(reconciliation.difference)}. Please review unmatched entries.</p>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-[#f5f6fa]">
                    <tr>
                      <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Period</th>
                      <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Account</th>
                      <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Opening Balance</th>
                      <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Closing Balance</th>
                      <th className="px-4 py-2 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Entries</th>
                      <th className="px-4 py-2 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                      <th className="px-4 py-2 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {statements
                      ...
                      .map(stmt => {
                        ...
                        return (
                          <tr key={stmt.id}>
                            <td className="px-4 py-2 whitespace-nowrap text-[12px] text-gray-700">{stmt.period}</td>
                            <td className="px-4 py-2 text-[12px] text-gray-700">{stmt.bankAccountName}</td>
                            <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">{formatCurrency(stmt.openingBalance)}</td>
