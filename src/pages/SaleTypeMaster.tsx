...
export default function SaleTypeMaster() {
  ...
  const filtered = (saleTypes || []).filter((s: any) =>
    s.name.toLowerCase().includes(search.toLowerCase()),
  );
  const salesAccounts = (accounts || []).filter(
    (a: any) => !a.isGroup && (a.type === "income" || a.name?.toLowerCase().includes("sales")),
  );
  ...
  const handleEdit = (s: any) => {
    ...
    setForm({
      ...
      salesAccountId: s.salesAccountId || "",
  ...
  const handleSubmit = async () => {
    ...
    if (selected) {
      await updateSaleType(selected.id, form);
      alert("Sale Type updated.");
    } else {
      await addSaleType(form);
      alert("Sale Type saved.");
    }
    resetForm();
  };
  ...
  return (
    <div style={{ display: "flex", height: "100%", gap: 0 }}>
      ...
      >
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ textAlign: "center", padding: 24, color: "#666" }}>
                    No sale types defined. Examples: VAT/13%, VAT/Exempt, Services.
                  </td>
                </tr>
              ) : (
                    ...
                    <td style={{ padding: "5px 10px", borderBottom: BORDER }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            if (confirm("Delete?")) await deleteSaleType(s.id);
                          }}
                        >
        ...
        >
          Total: {filtered.length} sale type(s)
        </div>
      </div>
          ...
          >
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                style={INPUT_STYLE}
                placeholder="e.g. VAT/13%, Exempt, Services"
              />
            </label>
            <label style={{ fontSize: 12 }}>
