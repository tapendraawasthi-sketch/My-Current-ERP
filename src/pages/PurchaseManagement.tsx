...
const PurchaseManagement: React.FC = () => {
  ...
  useEffect(() => {
    const db = getDB();
    db.table("rfqs").toArray().then(setRfqs).catch(() => setRfqs([]));
    db.table("requisitions").toArray().then(setRequisitions).catch(() => setRequisitions([]));
  }, []);
  ...
  const addRfqItemRow = () => {
    setRfqForm(prev => ({
      ...prev,
      items: [...prev.items, { id: generateId(), itemId: "", description: "", quantity: 0, unit: "", remarks: "" }]
    }));
  };
  ...
  const saveRfq = async () => {
    if (!rfqForm.date || !rfqForm.requiredByDate || rfqForm.suppliers.length === 0 || rfqForm.items.length === 0) {
  ...
  const saveRequisition = async () => {
    if (!reqForm.requesterName || !reqForm.department || !reqForm.dateRequired || reqForm.items.length === 0) {
      toast.error("Please fill all required fields");
      return;
    }
  ...
  const convertRfqToPo = (rfq: any) => {
    ...
    if (addPurchaseOrder) {
      ...
      const db = getDB();
      db.table("purchaseOrders").add(po).catch(() => {});
    }

    const db = getDB();
    db.table("rfqs").update(rfq.id, { status: "converted" }).then(() => {
  ...
  const vendorScores = useMemo(() => {
    ...
    return supplierList.map(supplier => {
      ...
      supplierOrders.forEach(order => {
        const grn = vouchers.find(v => v.purchaseOrderId === order.id && v.type === "goods-receipt-note");
        if (grn) {
          totalDeliveries++;
      ...
      const onTimeDelivery = totalDeliveries > 0 ? (onTimeDeliveries / totalDeliveries) * 100 : (totalOrders > 0 ? 50 : 0);
      
      const returnVouchers = vouchers.filter(v => v.partyId === supplier.id && v.type === "purchase-return");
      const qualityScore = Math.max(0, 100 - (returnVouchers.length * 10));
      const priceCompetitiveness = 80;
      const overallScore = totalOrders === 0 ? 0 : (onTimeDelivery * 0.4) + (qualityScore * 0.35) + (priceCompetitiveness * 0.25);
    ...
    }).filter(s => s.totalOrders > 0).sort((a,b) => b.overallScore - a.overallScore);
  ...
  const getStatusBadge = (status: string) => {
    switch(status.toLowerCase()) {
      ...
      case "approved":
      case "converted":
        return <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide">{status}</span>;
      case "price variance": 
      case "qty variance": 
      case "submitted":
        return <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide">{status}</span>;
      case "missing grn":
      case "missing po":
      case "rejected":
        return <span className="bg-red-100 text-red-700 px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide">{status}</span>;
      default: 
        return <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide">{status}</span>;
    }
  };
      ...
      <div className="w-full">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">Manage RFQs, Requisitions, Three-Way Matching, and Vendor Scoring</p>
          </div>
        </div>
        
        {/* Tab Navigation */}
        <div className="flex border-b border-gray-200 mb-4 bg-white px-2 pt-2 rounded-t-md shadow-sm">
          {["RFQ", "Purchase Requisition", "Three-Way Matching", "Vendor Scorecard"].map((tab, index) => (
            <button
              key={index}
        ...
        {activeTab === 0 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-[14px] font-semibold text-gray-800">Request for Quotation (RFQ)</h2>
              <button
            ...
            {showRfqForm && (
              <div className="mb-6 bg-white border border-gray-200 shadow-sm rounded-md p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">RFQ Number</label>
                    <input
                      type="text"
                      value={rfqForm.rfqNo}
                      readOnly
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Required By Date</label>
                    <input
                      type="date"
                ...
                <div className="mb-6">
                  <label className="block text-[11px] font-medium text-gray-600 mb-2">Select Suppliers</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-3 bg-gray-50 border border-gray-200 rounded-md max-h-40 overflow-y-auto">
                    {suppliers.map(supplier => (
                      <label key={supplier.id} className="flex items-center gap-2 cursor-pointer hover:bg-gray-100 p-1 rounded">
                        <input
                          type="checkbox"
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              ...
                              handleRfqFormChange('suppliers', rfqForm.suppliers.filter((id: string) => id !== supplier.id));
                            }
                          }}
                    ...
                    {suppliers.length === 0 && <span className="text-[12px] text-gray-500">No suppliers available.</span>}
                  </div>
                </div>
                    ...
                    <table className="w-full min-w-max border-collapse">
                      <thead>
                        <tr className="bg-[#f5f6fa] border-b border-gray-200">
                          ...
                          <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-24">Quantity</th>
                          <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-24">Unit</th>
                          <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Remarks</th>
                        ...
                        {rfqForm.items.map((item, index) => (
                          <tr key={item.id} className="bg-white border-b border-gray-100">
                            <td className="px-3 py-2 align-top">
                              <select
                                value={item.itemId}
                                onChange={(e) => handleRfqItemChange(index, 'itemId', e.target.value)}
                                className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                              >
                            ...
                            <td className="px-3 py-2 align-top">
                              <input
                                type="number"
                                value={item.quantity || ""}
                                onChange={(e) => handleRfqItemChange(index, 'quantity', Number(e.target.value) || 0)}
                            ...
                            <td className="px-3 py-2 align-top">
                              <input
                                type="text"
                                value={item.remarks}
                                onChange={(e) => handleRfqItemChange(index, 'remarks', e.target.value)}
                                className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                              />
                            </td>
            ...
            <div className="border border-gray-200 rounded-md overflow-hidden">
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">RFQ No</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Date</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Items</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Suppliers Invited</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Responses</th>
                  ...
                  {rfqs.map(rfq => (
                    <React.Fragment key={rfq.id}>
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 text-gray-700 text-right">{rfq.suppliers.length}</td>
                        <td className="px-3 py-2.5 text-gray-700 text-right">{rfq.responses?.length || 0}</td>
                      ...
                      {expandedRfq === rfq.id && (
                        <tr className="bg-gray-50 border-b border-gray-200">
                          <td colSpan={7} className="p-4">
                            <h4 className="text-[12px] font-semibold text-gray-800 mb-3">Comparative Statement</h4>
                                ...
                                <thead>
                                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                                    ...
                                    {rfq.suppliers.map((supplierId:string) => {
                                      ...
                                      return (
                                        <th key={supplierId} className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">
                                          {supplier?.name || "Supplier"}
                                        </th>
                                      );
                                    })}
                                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Lowest Rate</th>
                                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-48">Selected Supplier</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {rfq.items.map((item:any, idx:number) => (
                                    <tr key={idx} className="border-b border-gray-100">
                                      <td className="px-3 py-2 text-[12px] text-gray-700">{item.description}</td>
                                      <td className="px-3 py-2 text-[12px] text-gray-700 text-right">{item.quantity}</td>
                                      {rfq.suppliers.map((supplierId:string) => {
                                        return (
                                      ...
                                      <td className="px-3 py-2 text-[12px] text-gray-700 text-right">—</td>
                                      <td className="px-3 py-2 align-top">
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            ...
            {showReqForm && (
              <div className="mb-6 bg-white border border-gray-200 shadow-sm rounded-md p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Requester Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">Date Required</label>
                    <input
                      type="date"
                  ...
                  <div className="border border-gray-200 rounded-md overflow-hidden">
                    <table className="w-full min-w-max border-collapse">
                      <thead>
                        <tr className="bg-[#f5f6fa] border-b border-gray-200">
                          <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Item</th>
                          <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-32">Quantity</th>
                          <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Purpose</th>
                          <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-12">Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {reqForm.items.map((item, index) => (
                          <tr key={item.id} className="bg-white border-b border-gray-100">
                            <td className="px-3 py-2 align-top">
                              <select
                                value={item.itemId}
                                onChange={(e) => handleReqItemChange(index, 'itemId', e.target.value)}
                                className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                              >
                                <option value="">Select Item</option>
                                {itemOptions.map(opt => (
                            ...
                            <td className="px-3 py-2 align-top">
                              <input
                                type="number"
                                value={item.quantity || ""}
                                onChange={(e) => handleReqItemChange(index, 'quantity', Number(e.target.value) || 0)}
                                className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white text-right focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                              />
                            </td>
                            <td className="px-3 py-2 align-top">
                              <input
                                type="text"
                                value={item.purpose}
                                onChange={(e) => handleReqItemChange(index, 'purpose', e.target.value)}
                                className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                              />
                            </td>
                            <td className="px-3 py-2 align-top text-center">
            ...
            <div className="border border-gray-200 rounded-md overflow-hidden">
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Req No</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Date</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Department</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Requester</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Items</th>
                    <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Status</th>
                    <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Actions</th>
                  </tr>
                ...
                <tbody>
                  {requisitions.map(req => (
                    <tr key={req.id} className="bg-white hover:bg-gray-50 border-b border-gray-100 text-[12px] transition-colors">
                      <td className="px-3 py-2.5 text-gray-800 font-medium">{req.id.substring(0, 8)}</td>
                      <td className="px-3 py-2.5 text-gray-700">{req.createdAt ? new Date(req.createdAt).toISOString().split('T')[0] : ''}</td>
              ...
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    ...
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Supplier</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Date</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Invoice Amt</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">PO No</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">PO Amt</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">GRN No</th>
                    <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {threeWayMatches.map(match => {
                    ...
                    return (
                      <React.Fragment key={match.invoice.id}>
                        <tr 
                          className="bg-white hover:bg-gray-50 border-b border-gray-100 text-[12px] cursor-pointer transition-colors"
                          onClick={() => setExpandedInvoice(expandedInvoice === match.invoice.id ? null : match.invoice.id)}
                        >
                          <td className="px-3 py-2.5 text-gray-800 font-medium">{match.invoice.voucherNo}</td>
                          <td className="px-3 py-2.5 text-gray-700">{supplier?.name || "Unknown"}</td>
                          <td className="px-3 py-2.5 text-gray-700">{match.invoice.date}</td>
                          <td className="px-3 py-2.5 text-gray-700 text-right">{money(match.invoice.totalAmount || match.invoice.grandTotal || 0)}</td>
                          <td className="px-3 py-2.5 text-gray-700">{match.po?.poNo || "N/A"}</td>
                          <td className="px-3 py-2.5 text-gray-700 text-right">{match.po ? money(match.po.totalAmount) : "N/A"}</td>
                          <td className="px-3 py-2.5 text-gray-700">{match.grn?.voucherNo || "N/A"}</td>
                          <td className="px-3 py-2.5 text-center">
                            ...
                            <td colSpan={8} className="p-4">
                              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                <div className="border border-gray-200 rounded-md bg-white p-3">
                                  <h4 className="text-[12px] font-semibold text-gray-800 mb-3 border-b border-gray-100 pb-2">Purchase Order (PO)</h4>
                                  {match.po ? (
                                    <table className="w-full min-w-max border-collapse">
                                      <thead>
                                        <tr className="bg-[#f5f6fa] border-b border-gray-200">
                                          <th className="px-2 py-1.5 text-left text-[10px] font-semibold text-gray-500 uppercase">Item</th>
                                      ...
                                      <tbody>
                                        {match.po.items?.map((item:any, idx:number) => (
                                          <tr key={idx} className="border-b border-gray-100">
                                            <td className="px-2 py-1.5 text-[11px] text-gray-700">{item.name || item.description}</td>
                                            <td className="px-2 py-1.5 text-[11px] text-gray-700 text-right">{item.quantity}</td>
                                            <td className="px-2 py-1.5 text-[11px] text-gray-700 text-right">{money(item.rate)}</td>
                                          </tr>
                                        ))}
                                      </tbody>
                                    </table>
                                  ) : (
                                    <div className="text-[12px] text-gray-500 p-2 text-center">No PO data</div>
                                  )}
                                </div>
                                
                                <div className="border border-gray-200 rounded-md bg-white p-3">
                                  <h4 className="text-[12px] font-semibold text-gray-800 mb-3 border-b border-gray-100 pb-2">Goods Receipt (GRN)</h4>
                                  {match.grn ? (
                                    <table className="w-full min-w-max border-collapse">
                                      <thead>
                                        <tr className="bg-[#f5f6fa] border-b border-gray-200">
                                          <th className="px-2 py-1.5 text-left text-[10px] font-semibold text-gray-500 uppercase">Item</th>
                                          <th className="px-2 py-1.5 text-right text-[10px] font-semibold text-gray-500 uppercase">Qty Received</th>
                                        </tr>
                                      </thead>
                                      <tbody>
                                        {match.grn.lines?.filter(l=>l.itemId).map((line:any, idx:number) => (
                                          <tr key={idx} className="border-b border-gray-100">
                                            <td className="px-2 py-1.5 text-[11px] text-gray-700">{line.itemName || 'Item'}</td>
                                            <td className="px-2 py-1.5 text-[11px] text-gray-700 text-right">{line.quantity || line.debit || 0}</td>
                                          </tr>
                                        ))}
                                      </tbody>
                                    </table>
                                  ) : (
                                    <div className="text-[12px] text-gray-500 p-2 text-center">No GRN data</div>
                                  )}
                                </div>
                                
                                <div className="border border-gray-200 rounded-md bg-white p-3">
                                  <h4 className="text-[12px] font-semibold text-gray-800 mb-3 border-b border-gray-100 pb-2">Purchase Invoice</h4>
                                  <table className="w-full min-w-max border-collapse">
                                    <thead>
                                      <tr className="bg-[#f5f6fa] border-b border-gray-200">
                                        <th className="px-2 py-1.5 text-left text-[10px] font-semibold text-gray-500 uppercase">Item</th>
                                        <th className="px-2 py-1.5 text-right text-[10px] font-semibold text-gray-500 uppercase">Qty Billed</th>
                                        <th className="px-2 py-1.5 text-right text-[10px] font-semibold text-gray-500 uppercase">Rate Billed</th>
                                      </tr>
                                    </thead>
                                    <tbody>
                                      {match.invoice.lines?.filter(l=>l.itemId).map((line:any, idx:number) => (
                                        <tr key={idx} className="border-b border-gray-100">
                                          <td className="px-2 py-1.5 text-[11px] text-gray-700">{line.itemName || 'Item'}</td>
                                          <td className="px-2 py-1.5 text-[11px] text-gray-700 text-right">{line.quantity || 0}</td>
                                          <td className="px-2 py-1.5 text-[11px] text-gray-700 text-right">{money(line.rate || 0)}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            ...
            <div className="border border-gray-200 rounded-md overflow-hidden">
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Vendor Name</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Total Orders</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">On-Time Delivery (%)</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Quality Score</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Price Competitiveness</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Overall Score</th>
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-24">Rating</th>
                  </tr>
                </thead>
                <tbody>
                  {vendorScores.map(score => (
                    <tr key={score.supplier.id} className="bg-white hover:bg-gray-50 border-b border-gray-100 text-[12px]">
                      <td className="px-3 py-2.5 text-gray-800 font-medium">{score.supplier.name}</td>
                      <td className="px-3 py-2.5 text-gray-700 text-right">{score.totalOrders}</td>
                      <td className="px-3 py-2.5 text-gray-700 text-right">{score.onTimeDelivery.toFixed(1)}%</td>
                      <td className="px-3 py-2.5 text-gray-700 text-right">{score.qualityScore.toFixed(1)}</td>
                      <td className="px-3 py-2.5 text-gray-700 text-right">{score.priceCompetitiveness.toFixed(1)}</td>
                      <td className="px-3 py-2.5 text-[#1557b0] font-semibold text-right">{score.overallScore.toFixed(1)}</td>
                        ...
                        <div className="flex items-center gap-0.5">
                          {[1, 2, 3, 4, 5].map(star => (
                            <Star 
                              key={star} 
                              size={12} 
                              className={star <= Math.round(score.overallScore / 20) ? "fill-amber-400 text-amber-400" : "text-gray-200 fill-gray-100"} 
                            />
                          ))}
                        </div>
                      </td>
