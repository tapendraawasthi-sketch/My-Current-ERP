...
const computePriceHistory = (invoices: any[]): ItemPriceHistory[] => {
  ...
  Object.values(grouped).forEach(item => {
    ...
    item.priceEntries.sort((a: PriceEntry, b: PriceEntry) => new Date(b.date).getTime() - new Date(a.date).getTime());
    item.sales.sort((a: PriceEntry, b: PriceEntry) => new Date(b.date).getTime() - new Date(a.date).getTime());
    item.purchases.sort((a: PriceEntry, b: PriceEntry) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    // Calculate averages
    const avgSaleRate = item.sales.length > 0 
      ? item.sales.reduce((sum: number, entry: PriceEntry) => sum + entry.rate, 0) / item.sales.length
      : 0;
    
    const avgPurchaseRate = item.purchases.length > 0 
      ? item.purchases.reduce((sum: number, entry: PriceEntry) => sum + entry.rate, 0) / item.purchases.length
      : 0;
...
const PriceHistory: React.FC = () => {
  ...
  const allInvoices = [
    ...(invoices ?? []), 
    ...(vouchers ?? []).filter(v => ["SALES", "PURCHASE", "sales", "purchase"].includes(v.type ?? ""))
  ];
    ...
    return allInvoices
      ...
      .flatMap(inv => 
        (inv.lines || []).map((line: any) => ({
          ...
          netRate: (line.rate ?? line.unitPrice ?? line.price ?? 0) * (1 - (line.discount ?? line.discountPct ?? 0)/100),
          type: inv.type.includes('SALES') || inv.type.includes('sales') ? 'sale' : 'purchase'
        }))
      )
  ...
  const rateRanges = useMemo(() => {
    ...
    return {
      ...
      purchases: {
        ...
        avg: purchasesRates.length > 0 ? purchasesRates.reduce((a, b) => a + b, 0) / purchasesRates.length : 0
      }
    };
  }, [selectedItemData]);
    ...
    return (
      <div className="flex flex-col h-full bg-[#f5f6fa]">
        <div className="p-4 bg-white border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600">View item-wise and party-wise sale/purchase rate history</p>
        </div>
        ...
        {activeTab === "item_view" ? (
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-[#f5f6fa]">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Last Sale</th>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Date</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Last Purchase</th>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Date</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {filteredHistory.map(item => (
                        ...
                        >
                          ...
                          <td className="px-4 py-2 text-[12px] text-gray-700">{item.lastSaleDate}</td>
                          <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">
                            {item.lastPurchaseRate > 0 ? `NPR ${item.lastPurchaseRate.toFixed(2)}` : '-'}
                          </td>
                          <td className="px-4 py-2 text-[12px] text-gray-700">{item.lastPurchaseDate}</td>
                          <td className="px-4 py-2 text-right text-[12px]">
                            <span 
                              className={
                                item.marginPct > 15 ? 'text-green-600' : 
                                item.marginPct > 5 ? 'text-amber-600' : 
                                'text-red-600'
                              }
                            >
                              {item.marginPct > 0 ? `+${item.marginPct.toFixed(2)}` : item.marginPct.toFixed(2)}%
                            </span>
                          </td>
                        </tr>
                      ))}
              ...
              {selectedItemData && (
                <div className="lg:col-span-1">
                  <div className="bg-white rounded-lg border border-gray-200 p-4">
                    <h3 className="text-md font-semibold mb-4">Price History for: {selectedItemData.itemName}</h3>
                    
                    {rateRanges && (
                      <div className="mb-4 space-y-1">
                        <p className="text-sm font-medium">Sale Rate: Min NPR {rateRanges.sales.min.toFixed(2)} / Max NPR {rateRanges.sales.max.toFixed(2)} / Avg NPR {rateRanges.sales.avg.toFixed(2)}</p>
                        <p className="text-sm font-medium">Purchase Rate: Min NPR {rateRanges.purchases.min.toFixed(2)} / Max NPR {rateRanges.purchases.max.toFixed(2)} / Avg NPR {rateRanges.purchases.avg.toFixed(2)}</p>
                      </div>
                    )}
                    ...
                    <div className="mb-4">
                      ...
                      ) : getTrendDirection(selectedItemData.priceEntries, 'purchase') === 'down' ? (
                        <div className="flex items-center gap-1">
                          <TrendingDown className="h-4 w-4 text-red-600" />
                          <span className="text-red-600 text-sm">Decreasing</span>
                        </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-[#f5f6fa] sticky top-0">
                            <tr>
                              ...
                              <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Party</th>
                              <th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Rate</th>
                              <th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Net</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-200">
                            {selectedItemData.priceEntries
                              ...
                              .map((pe, idx) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                  <td className="px-3 py-2 text-[12px] text-gray-700">{pe.dateNepali || pe.date}</td>
                                  <td className="px-3 py-2 text-[12px] text-gray-700">{pe.partyName}</td>
                                  <td className="px-3 py-2 text-right text-[12px] text-gray-700 font-mono">{pe.rate.toFixed(2)}</td>
                                  <td className="px-3 py-2 text-right text-[12px] text-blue-600 font-mono">{pe.netRate.toFixed(2)}</td>
                                </tr>
                              ))}
                          </tbody>
                        </table>
                            ...
                            {selectedItemData.priceEntries
                              ...
                              .map((pe, idx) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                  ...
                                  <td className="px-3 py-2 text-[12px] text-gray-700">{pe.partyName}</td>
                                  <td className="px-3 py-2 text-right text-[12px] text-gray-700 font-mono">{pe.rate.toFixed(2)}</td>
                                  <td className="px-3 py-2 text-right text-[12px] text-orange-600 font-mono">{pe.netRate.toFixed(2)}</td>
                                </tr>
                              ))}
                          </tbody>
                        </table>
          ...
          <div className="space-y-6">
            ...
            {selectedParty && (
              <>
                <div className="bg-white rounded-lg border border-gray-200 p-4">
                  <h3 className="text-md font-semibold mb-2">Last 5 Transactions with {selectedParty}</h3>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-[#f5f6fa]">
                        <tr>
                          <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Date</th>
                          <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Item</th>
                          <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Type</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Rate</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Net Rate</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
                        {partyPriceHistory.slice(0, 5).map((entry, idx) => (
                          <tr key={idx} className="hover:bg-gray-50">
                            <td className="px-4 py-2 text-[12px] text-gray-700">{entry.dateNepali || entry.date}</td>
                            <td className="px-4 py-2 text-[12px] text-gray-700">{entry.itemName}</td>
                            ...
                            <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">{entry.rate.toFixed(2)}</td>
                  ...
                  {partyPriceHistory.length > 0 ? (
                    <div className="p-3 bg-blue-50 rounded border border-blue-200">
                      <p className="text-sm">
                        ...
                        <span className="font-semibold">{partyPriceHistory[0].rate.toFixed(2)}</span>{' '}
                        for {partyPriceHistory[0].itemName}
                      </p>
                    </div>
                  ) : (
                    <p className="text-sm text-gray-500">No previous transactions found for this party.</p>
                  )}
                </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-[#f5f6fa]">
                        <tr>
                          ...
                          <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Type</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Qty</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Rate</th>
                          <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Net Rate</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
                        {partyPriceHistory.map((entry, idx) => (
                          <tr key={idx} className="hover:bg-gray-50">
                            <td className="px-4 py-2 text-[12px] text-gray-700">{entry.dateNepali || entry.date}</td>
                            <td className="px-4 py-2 text-[12px] text-gray-700">{entry.itemName}</td>
                            <td className="px-4 py-2 text-[12px] text-gray-700">{entry.invoiceNo}</td>
                            <td className="px-4 py-2 text-[12px] text-gray-700">
                              <span className={`px-2 py-1 rounded text-xs ${
                                entry.type === 'sale' 
                                  ? 'bg-blue-100 text-blue-800' 
                            ...
                            <td className="px-4 py-2 text-right text-[12px] text-gray-700">{entry.qty}</td>
                            <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">{entry.rate.toFixed(2)}</td>
                            <td className="px-4 py-2 text-right text-[12px] font-mono">
                              {entry.type === 'sale' ? (
                                <span className="text-blue-600">{entry.netRate.toFixed(2)}</span>
