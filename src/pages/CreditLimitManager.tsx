...
import { Edit2, Save, AlertTriangle, CheckCircle, Download, ChevronDown, ChevronUp, ShieldAlert, FileText } from "lucide-react";
...
function statusBadge(status: string) {
  if (status === "Normal") return <span className="px-2 py-0.5 bg-green-50 text-green-700 border border-green-200 rounded text-[10px] font-semibold uppercase tracking-wide">Normal</span>;
  if (status === "At Risk") return <span className="px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded text-[10px] font-semibold uppercase tracking-wide">At Risk</span>;
  if (status === "Over Limit") return <span className="px-2 py-0.5 bg-red-50 text-red-700 border border-red-200 rounded text-[10px] font-semibold uppercase tracking-wide">Over Limit</span>;
  if (status === "Blocked") return <span className="px-2 py-0.5 bg-gray-800 text-white border border-gray-900 rounded text-[10px] font-semibold uppercase tracking-wide">Blocked</span>;
  return <span className="px-2 py-0.5 bg-gray-100 text-gray-700 border border-gray-200 rounded text-[10px] font-semibold uppercase tracking-wide">{status}</span>;
}
...
export default function CreditLimitManager() {
  ...
  const rows = useMemo(() => {
    return (parties || []).map((party) => {
      ...
      const billsSales = getPartyBillwiseOutstanding(party.id, "sales", invoices, vouchers, parties);
      const billsPurchase = getPartyBillwiseOutstanding(party.id, "purchase", invoices, vouchers, parties);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (typeFilter !== "All") {
        ...
        if (typeFilter === "Customer" && !(pt.includes("customer") || pt.includes("debtor") || pt === "")) {
          return false;
        }
  ...
  const stats = useMemo(() => {
    ...
    const overLimit = filteredRows.filter((r) => r.status === "Over Limit" || r.status === "Blocked").length;
  ...
  async function saveSettings() {
    ...
    const db = getDB();
    await db.table("partyCreditSettings").put(row).catch(() => {});
  ...
  const bulkAffected = useMemo(() => {
    ...
    return parties.filter((p) => String(p.type || "").toLowerCase().includes(type));
  }, [parties, bulkType]);
    ...
    <div className="min-h-screen bg-[#f5f6fa] p-4 text-gray-800">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">
            Manage party-wise credit limits, credit days, overdue interest and invoice blocking controls.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col justify-center">
          <div className="text-[11px] font-medium text-gray-500 uppercase tracking-wide mb-1">Total Limit Extended</div>
          <div className="text-[20px] font-bold text-gray-800">NPR {money(stats.totalLimit)}</div>
        </div>

        <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col justify-center">
          <div className="text-[11px] font-medium text-gray-500 uppercase tracking-wide mb-1">Total Outstanding</div>
          <div className="text-[20px] font-bold text-gray-800">NPR {money(stats.totalOutstanding)}</div>
        </div>

        <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col justify-center">
          <div className="text-[11px] font-medium text-gray-500 uppercase tracking-wide mb-1">Overall Utilization</div>
        ...
        <div className="bg-red-50/50 border border-red-200 rounded-md p-4 shadow-sm flex flex-col justify-center">
          <div className="text-[11px] font-medium text-red-700 uppercase tracking-wide flex items-center gap-1.5 mb-1"><AlertTriangle size={14}/> Over Limit / Blocked</div>
      ...
      <div className={`${cardClass} mb-6`}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div>
             <input className={inputClass} placeholder="Search party name..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
             <button className={outlineBtn} onClick={() => setBulkOpen(!bulkOpen)}>
               {bulkOpen ? <ChevronUp size={14} className="text-gray-400" /> : <ChevronDown size={14} className="text-gray-400" />}
               Bulk Update
             </button>
          </div>
        </div>
        ...
        {bulkOpen && (
          <div className="mt-4 border border-indigo-200 rounded-md p-4 bg-indigo-50/30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
              <div>
                <label className="block text-[11px] font-medium text-indigo-800 mb-1">Party Type</label>
                <select className={inputClass} value={bulkType} onChange={(e) => setBulkType(e.target.value)}>
              ...
              <div>
                <label className="block text-[11px] font-medium text-indigo-800 mb-1">Credit Limit (NPR)</label>
                <input className={inputClass} type="number" placeholder="0.00" value={bulkLimit} onChange={(e) => setBulkLimit(e.target.value)} />
              </div>

              <div>
                <label className="block text-[11px] font-medium text-indigo-800 mb-1">Credit Days</label>
                <input className={inputClass} type="number" value={bulkDays} onChange={(e) => setBulkDays(e.target.value)} />
              </div>
        ...
        <div className="overflow-x-auto rounded-md border border-gray-200">
          <table className="w-full border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                return (
                  <tr key={r.party.id} className="bg-white hover:bg-gray-50">
                    ...
                    <td className={tableCellClass}>
                       <span className="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded uppercase tracking-wide">{r.party.type || "Other"}</span>
                    </td>
                    <td className={tableCellClass}>{r.creditLimit > 0 ? `NPR ${money(r.creditLimit)}` : <span className="text-gray-400 font-medium italic">Unlimited</span>}</td>
                    <td className={`${tableCellClass} font-semibold ${r.outstanding > 0 ? "text-gray-900" : "text-gray-500"}`}>NPR {money(r.outstanding)}</td>
                      ...
                      {r.creditLimit > 0 ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[11px] font-semibold w-12 ${
                             pct < 70 ? "text-green-700" : pct < 90 ? "text-amber-700" : "text-red-700"
                          }`}>{money(pct)}%</span>
                        </div>
                      ) : (
                        <span className="text-[10px] text-gray-400 uppercase tracking-wide">N/A</span>
                      )}
                    </td>
                    ...
                    <td className={tableCellClass}>
                      <button className="text-[11px] font-medium text-[#1557b0] hover:underline flex items-center gap-1" onClick={() => openEdit(r)}>
                        <Edit2 size={12}/> Edit
                      </button>
                    </td>
                  </tr>
              ...
              {!filteredRows.length && (
                <tr>
                  <td colSpan={9} className="text-center p-8 text-gray-500 text-[12px] bg-gray-50/50">
        ...
        {editingParty && (
          <div className="space-y-4">
            <div className="bg-gray-50 border border-gray-200 rounded-md p-3 text-[12px]">
              <div className="font-semibold text-gray-800 text-[14px] mb-1">{editingParty.name}</div>
              <div className="text-gray-500">
                Type: <span className="font-medium text-gray-700 uppercase tracking-wide">{editingParty.type || "Other"}</span> • 
                PAN: <span className="font-medium text-gray-700">{editingParty.panNumber || editingParty.vatNumber || "N/A"}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                 ...
                 <input className={inputClass} type="number" placeholder="Enter limit (0 for unlimited)" value={form.creditLimit} onChange={(e) => setForm({ ...form, creditLimit: e.target.value })} />
               </div>
   
               <div>
                 <label className="block text-[11px] font-medium text-gray-600 mb-1">Credit Days</label>
                 <input className={inputClass} type="number" value={form.creditDays} onChange={(e) => setForm({ ...form, creditDays: e.target.value })} />
               </div>
   
               <div>
                 <label className="block text-[11px] font-medium text-gray-600 mb-1">Interest Rate % (Per Annum)</label>
                 <input className={inputClass} type="number" value={form.interestRate} onChange={(e) => setForm({ ...form, interestRate: e.target.value })} />
               </div>
            </div>
            ...
            <div className="mt-2 p-3 bg-red-50/30 border border-red-100 rounded-md">
               <label className="flex items-center gap-2 cursor-pointer mb-1 text-red-900 font-semibold">
                 <input type="checkbox" className="rounded border-red-300 text-red-600 focus:ring-red-500" checked={form.isCreditBlocked} onChange={(e) => setForm({ ...form, isCreditBlocked: e.target.checked })} />
            ...
            <div>
              <label className="block text-[11px] font-medium text-gray-600 mb-1">Notes / Reason (Internal)</label>
              <textarea className={`${inputClass} h-auto py-2`} rows={3} placeholder="Reason for updating limit or blocking credit..." value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
            </div>

            <div className="flex justify-end gap-3 pt-3 border-t border-gray-100">
              <button className={outlineBtn} onClick={() => setEditModal(false)}>Cancel</button>
