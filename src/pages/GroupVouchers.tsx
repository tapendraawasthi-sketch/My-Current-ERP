...
const GroupVouchers: React.FC = () => {
  ...
  const reportData = useMemo(() => {
    if (!selectedGroupId || !vouchers || !accounts) return { rows: [], totalDebit: 0, totalCredit: 0 };
    ...
    filteredVouchers.forEach(voucher => {
      ...
      matchingLines.forEach(line => {
        ...
        const contraNames = otherLines.map(l => {
  ...
  const groups = useMemo(() => {
    return (accounts || []).filter(acc => acc.isGroup).sort((a, b) => a.name.localeCompare(b.name));
  }, [accounts]);
  ...
  const renderCell = (columnKey: string, value: any, row: any) => {
    if (row.isTotal) {
      ...
      if (columnKey === "debit" || columnKey === "credit") {
        return <span className="font-bold font-mono text-gray-800">{value > 0 ? formatNumber(value) : "—"}</span>;
      }
      return "";
    }
    ...
    if (columnKey === "debit" || columnKey === "credit") {
      ...
      return <span className={`font-mono ${columnKey === 'debit' ? 'text-[#1557b0]' : 'text-gray-700'}`}>{formatNumber(value)}</span>;
    }
