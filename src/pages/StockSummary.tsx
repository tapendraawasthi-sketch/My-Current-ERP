      ...
      <div className="grid grid-cols-4 gap-3 mb-3">
        {[
          { label: "Total Items", value: totals.items, color: "#1557b0" },
          {
            ...
            value: `Rs. ${formatNumber(totals.value)}`,
            color: "#15803d",
          },
          ...
          >
            ...
            <div className="text-[16px] font-bold text-gray-800 mt-0.5" style={{ color }}>
              {value}
            </div>
          </div>
        ))}
      ...
      >
        <table className="data-table">
              ...
              paginatedRows.map((row) => {
                ...
                return (
                  <tr key={row.itemId} style={bgStyle} className="hover:bg-[#e8eeff]">
                    ...
                    <td className="px-3 py-[7px] text-[12px] text-right font-mono amt amt-positive">
                      {formatNumber(row.inQty)}
                    </td>
                    <td className="px-3 py-[7px] text-[12px] text-right font-mono amt amt-negative">
                      {formatNumber(row.outQty)}
                    </td>
                    <td className="px-3 py-[7px] text-[12px] text-right font-mono amt font-bold">
                      <div className="flex items-center justify-end gap-1.5">
                        {isOut ? (
                          <span className="badge bg-red-100 text-red-700 rounded px-1.5 py-0.5 text-[10px] font-bold">
                            OUT
                          </span>
                        ) : isLow ? (
                          <span className="badge bg-amber-100 text-amber-700 rounded px-1.5 py-0.5 text-[10px] font-bold">
                            LOW
                          </span>
                        ) : null}
                    ...
                    >
                      Rs. {formatNumber(row.closingValue)}
                    </td>
                  </tr>
                );
              })
          ...
          <tfoot className="bg-[#eef1f8] border-t-2 border-[#c5cad8] font-bold">
            <tr>
              <td colSpan={3} className="px-3 py-2 text-[12px] text-gray-700">
                Total
              </td>
              <td colSpan={5} />
              <td
