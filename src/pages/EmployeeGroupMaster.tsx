...
const EmployeeGroupMaster: React.FC = () => {
  ...
  const [form, setForm] = useState({
    ...
    underGroupId: "",
    defaultCostCentreId: "",
    defaultAttendanceTypeId: "",
    isActive: true,
  });

  const filteredGroups = (employeeGroups || []).filter(
    (group) =>
      matchBranch((group as any).branchId) &&
  ...
  const loadFormForEdit = (group: any) => {
    setForm({
      name: group.name || "",
      alias: group.alias || "",
      underGroupId: group.underGroupId || "",
      defaultCostCentreId: group.defaultCostCentreId || "",
      defaultAttendanceTypeId: group.defaultAttendanceTypeId || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selected) {
        await updateEmployeeGroup(selected.id, payload);
        toast.success("Updated successfully");
      } else {
        await addEmployeeGroup(payload);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Delete this employee group?")) return;
    const row = (employeeGroups || []).find((g) => g.id === id);
    if (!row) return;
    const snapshot = { ...row };
    try {
      ...
      toast.undo(`"${row.name}" deleted`, async () => {
        try {
          await addEmployeeGroup({ ...snapshot });
        } catch (error) {
      ...
      >
        <div className="p-4 flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-[15px] font-semibold text-gray-900">Employee Group Master</h1>
                ...
                {filteredGroups.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-3 py-8 text-center text-[12px] text-gray-500">
                      No employee groups found. Click "Add New" to create one.
                    </td>
                  </tr>
                ) : (
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Enter group name (e.g. Management, Sales Staff)"
                autoFocus
              />
            </div>
