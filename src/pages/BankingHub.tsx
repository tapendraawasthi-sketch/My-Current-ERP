...
const BankingHub: React.FC = () => {
  const { accounts, vouchers, parties, companySettings, currentFiscalYear, addVoucher } =
    useStore();
  ...
  const bankAccounts = useMemo(() => {
    return (
      ...
      accounts.filter(
        (a) =>
          a.type === "asset" &&
          (a.name.toLowerCase().includes("bank") || a.name.toLowerCase().includes("account")),
      )
    );
  }, [accounts]);
  ...
  useEffect(() => {
    ...
    db.table("chequeBooks")
      .toArray()
      .then(setChequeBooks)
      .catch(() => setChequeBooks([]));

    db.table("bankGuarantees")
      .toArray()
      .then(setBankGuarantees)
      .catch(() => setBankGuarantees([]));

    db.table("fixedDeposits")
      .toArray()
  ...
  const saveChequeBook = async () => {
    if (
      !editingChequeBook.bankAccountId ||
      !editingChequeBook.prefix ||
      !editingChequeBook.fromNumber ||
      !editingChequeBook.toNumber
    ) {
  ...
  const saveFd = async () => {
    if (
      ...
      !editingFd.bankName ||
      !editingFd.amount ||
      !editingFd.interestRate ||
      !editingFd.startDate ||
      !editingFd.maturityDate
    ) {
      toast.error("Please fill all required fields");
      return;
    }
  ...
  const bgStatus = (expiryDate: string) => {
    ...
    if (daysLeft < 0)
      return { label: "Expired", color: "bg-red-100 text-red-700 border-red-200", days: daysLeft };
    if (daysLeft <= 30)
      return {
        label: "Expiring Soon",
  ...
  const fdStatus = (maturityDate: string) => {
    ...
    if (daysLeft < 0)
      return { label: "Matured", color: "bg-red-100 text-red-700 border-red-200", days: daysLeft };
    if (daysLeft <= 30)
      return {
        label: "Maturing Soon",
        color: "bg-amber-100 text-amber-700 border-amber-200",
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">
              Manage cheque books, bank guarantees, fixed deposits, and cash flow
            </p>
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleChequeBookFormChange("fromNumber", Number(e.target.value))
                      }
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleChequeBookFormChange("toNumber", Number(e.target.value))
                      }
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleChequeBookFormChange("currentNumber", Number(e.target.value))
                      }
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                    />
                  </div>
                  ...
                  {chequeBooks.map((cb) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className={`px-3 py-2.5 text-right font-medium ${isLow ? "text-red-600 bg-red-50" : "text-green-600"}`}
                        >
                          {remaining}
                        </td>
                        <td className="px-3 py-2.5 text-right font-bold text-[#1557b0]">
                          {cb.currentNumber + 1}
                        </td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setChequeForm((prev) => ({ ...prev, voucherId: e.target.value }))
                      }
                    ...
                    >
                      ...
                      {vouchers
                        .filter((v) => v.type === "payment" && v.status === "posted" && !v.chequeNo)
                        .map((v) => (
                          <option key={v.id} value={v.id}>
                            {v.voucherNo} - {v.partyName || "N/A"}
                          </option>
                        ))}
                    </select>
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setChequeForm((prev) => ({ ...prev, chequeBookId: e.target.value }))
                      }
                      className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full shadow-sm"
                    >
                  ...
                  <div className="flex items-end">
                    <button
                      ...
                      onClick={() => {
                        ...
                        const chequeBook = chequeBooks.find(
                          (cb) => cb.id === chequeForm.chequeBookId,
                        );
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4">
            ...
            {bankGuarantees.some((bg) => bgStatus(bg.expiryDate).label === "Expired") && (
              <div className="bg-red-50 text-red-700 p-3 rounded-md border border-red-200 mb-4 flex items-center gap-2 text-[12px] font-medium">
                ...
                {
                  bankGuarantees.filter((bg) => bgStatus(bg.expiryDate).label === "Expired").length
                }{" "}
                Bank Guarantee(s) have expired! Total exposure: NPR{" "}
                {money(
                  bankGuarantees
                    .filter((bg) => bgStatus(bg.expiryDate).label === "Expired")
                    .reduce((sum, bg) => sum + bg.amount, 0),
                )}
              </div>
            )}
            ...
            {bankGuarantees.some(
              (bg) =>
                bgStatus(bg.expiryDate).days <= 30 && bgStatus(bg.expiryDate).label !== "Expired",
            ) && (
              <div className="bg-amber-50 text-amber-700 p-3 rounded-md border border-amber-200 mb-4 flex items-center gap-2 text-[12px] font-medium">
                <Clock size={16} className="text-amber-600" />
                {
                  bankGuarantees.filter(
                    (bg) =>
                      bgStatus(bg.expiryDate).days <= 30 &&
                      bgStatus(bg.expiryDate).label !== "Expired",
                  ).length
                }{" "}
                Bank Guarantee(s) expiring within 30 days!
              </div>
            )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5">
                          ...
                          <div className="text-gray-500 truncate max-w-[200px]" title={bg.purpose}>
                            {bg.purpose}
                          </div>
                        </td>
                        <td className="px-3 py-2.5 text-gray-600">
                          <div>
                            <span className="text-gray-400">Iss:</span> {bg.issueDate}
                          </div>
                          <div>
                            <span className="text-gray-400">Exp:</span>{" "}
                            <span
                              className={statusInfo.days <= 30 ? "font-medium text-red-600" : ""}
                            >
                              {bg.expiryDate}
                            </span>
                          </div>
                        ...
                        <td className="px-3 py-2.5 text-center">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide border ${statusInfo.color}`}
                          >
                            {statusInfo.label}
                          </span>
                        </td>
                        <td
                          className={`px-3 py-2.5 text-right font-medium ${statusInfo.days < 0 ? "text-red-600" : statusInfo.days <= 30 ? "text-amber-600" : "text-gray-600"}`}
                        >
        ...
        {activeTab === 2 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4">
            ...
            {fixedDeposits.some(
              (fd) =>
                fdStatus(fd.maturityDate).days <= 30 &&
                fdStatus(fd.maturityDate).label !== "Matured",
            ) && (
              <div className="bg-amber-50 text-amber-700 p-3 rounded-md border border-amber-200 mb-4 flex items-center gap-2 text-[12px] font-medium">
                ...
                FD(s) maturing within 30 days — Total: NPR{" "}
                {money(
                  fixedDeposits
                    .filter(
                      (fd) =>
            ...
            {showFdForm && (
              <div className="mb-6 bg-gray-50 border border-gray-200 rounded-md p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                  <div>
                    <label className="block text-[11px] font-medium text-gray-600 mb-1">
                      FD Certificate No <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
              ...
              <table className="w-full min-w-max border-collapse">
                ...
                <tbody className="divide-y divide-gray-100">
                  {fixedDeposits.map((fd) => {
                    ...
                    const tenor = Math.ceil(
                      (new Date(fd.maturityDate).getTime() - new Date(fd.startDate).getTime()) /
                        (1000 * 60 * 60 * 24),
                    );
                    const daysToMaturity = Math.ceil(
                      (new Date(fd.maturityDate).getTime() - new Date().getTime()) /
                        (1000 * 60 * 60 * 24),
                    );
                    ...
                    return (
                      ...
                      >
                        <td className="px-3 py-2.5">
                          ...
                          {fd.autoRenewal && (
                            <div className="text-[9px] bg-blue-50 text-blue-600 inline-block px-1 rounded mt-0.5">
                              Auto-Renewal
                            </div>
                          )}
                        </td>
                        ...
                        <td className="px-3 py-2.5 text-gray-600">
                          <div>
                            <span className="text-gray-400">Start:</span> {fd.startDate}
                          </div>
                          <div>
                            <span className="text-gray-400">Mat:</span>{" "}
                            <span
                              className={statusInfo.days <= 30 ? "font-medium text-amber-600" : ""}
                            >
                              {fd.maturityDate}
                            </span>
                          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 mb-4 max-w-full overflow-auto">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-[11px] text-gray-500 mt-0.5">
                  Projected inflows, outflows, and net balances based on vouchers and PDCs
                </p>
              </div>
              <button
