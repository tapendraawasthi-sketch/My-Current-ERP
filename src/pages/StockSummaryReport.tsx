    ...
    <div className="p-4 bg-[#f5f6fa] min-h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        {[
          ...
          { label: "Stock Value", value: `Rs. ${formatNumber(totals.totalValue)}`, color: "text-[#1557b0]" },
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-3 mb-4 flex items-center gap-3 flex-wrap">
        <div className="flex gap-1">
          {([["alphabetical", "Alphabetical"], ["group_wise", "Group-wise"], ["critical_level", "Critical Level"]] as const).map(([key, label]) => (
            <button key={key} onClick={() => setViewMode(key)}
              ...
              {key === "critical_level" && totals.criticalItems > 0 && (
                <span className={`ml-1.5 text-[10px] px-1.5 py-0.5 rounded ${viewMode === key ? "bg-white/20" : "bg-red-100 text-red-700"}`}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <input type="date" value={asOnDate} onChange={e => setAsOnDate(e.target.value)} className={inputCls} />
        </div>
        <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search items..." className={`${inputCls} w-44`} />
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          ...
          Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([group, groupItems]) => (
            <div key={group}>
              <div className="px-4 py-2 bg-[#e8f0fe] border-b border-blue-200 font-semibold text-[12px] text-[#1557b0] flex justify-between">
                <span>{group} — {groupItems.length} items</span>
                <span className="font-mono">Rs. {formatNumber(groupItems.reduce((s, i) => s + i.value, 0))}</span>
              </div>
          ...
          <>
            {viewMode === "critical_level" && criticalItems.length === 0 && (
              <div className="py-12 text-center">
                ...
                <p className="text-[12px] text-gray-500">No items at critical stock levels. All stock levels are normal.</p>
              </div>
            )}
        ...
        <div className="px-4 py-2.5 border-t-2 border-[#c7d2fe] bg-[#eef2ff] flex justify-between text-[12px] font-bold text-gray-800">
          <span>TOTAL — {viewMode === "critical_level" ? criticalItems.length : sorted.length} items</span>
          <span className="font-mono">
            Rs. {formatNumber((viewMode === "critical_level" ? criticalItems : sorted).reduce((s, i) => s + i.value, 0))}
          </span>
        </div>
      </div>
    </div>
