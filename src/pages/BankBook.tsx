...
const BankBook: React.FC = () => {
  ...
  const bankAccounts = useMemo(() => {
    return (accounts || []).filter(acc => 
      !acc.isGroup && 
      (acc.name.toLowerCase().includes("bank") || acc.type === "asset")
    );
  }, [accounts]);
  ...
  const data = useMemo(() => {
    ...
    // Sort by date
    relevantVouchers = relevantVouchers.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    ...
    relevantVouchers.forEach(voucher => {
      ...
      const particularNames = otherLines.map(line => {
      ...
      switch(voucher.type) {
        ...
        case "sales-invoice": voucherTypeLabel = "Sales"; break;
        case "purchase-invoice": voucherTypeLabel = "Purchase"; break;
        ...
        default: voucherTypeLabel = voucher.type;
      }
    ...
    return result;
  }, [selectedAccountId, vouchers, accounts, startDate, endDate, showNarration, showOnlyReceipts, showOnlyPayments]);
  ...
  const renderCell = (columnKey: string, value: any, row: any) => {
    ...
    if (columnKey === "balance" && row.balance !== undefined && row.balance !== "") {
      ...
      const formattedValue = isDr ? `${formatNumber(Math.abs(row.balance))} Dr` : `${formatNumber(Math.abs(row.balance))} Cr`;
