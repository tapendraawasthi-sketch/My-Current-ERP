 ...
 *
 * Bank Book page showing bank account transactions and cheque details.
 */
...
const BankBook: React.FC = () => {
  ...
  const bankAccounts = useMemo(
    () => accounts.filter((acc) => acc.type === "asset" && acc.name.toLowerCase().includes("bank")),
    [accounts],
  );
  ...
  }, [vouchers, bankAccountId, startDate, endDate]);
  ...
  const handleImport = () => {
    toast("Bank statement import is not implemented yet.");
  };

  const handleExport = () => {
    toast("Bank Book export not implemented yet.");
  };

  const handlePrint = () => {
    toast("Bank Book print not implemented yet.");
  };
  ...
  return (
    <div className="flex flex-col gap-6 animate-fadeIn select-none text-xs">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-[15px] font-semibold text-gray-800">Bank Book</h1>
          <p className="text-[11px] text-gray-500 mt-0.5">Bank receipts and payments</p>
        </div>
        ...
        <Card border padding="md">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="text-xs text-slate-500">
              Total:{" "}
              {formatNumber(
                rows.filter((row) => row.isReceipt).reduce((sum, row) => sum + row.amount, 0),
              )}
            </div>
          </div>
          <Table
            ...
            emptyMessage="No bank receipts in selected range."
            stickyHeader
          />
        </Card>
        ...
        <Card border padding="md">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="text-xs text-slate-500">
              Total:{" "}
              {formatNumber(
                rows.filter((row) => row.isPayment).reduce((sum, row) => sum + row.amount, 0),
              )}
            </div>
          </div>
          <Table
            ...
            emptyMessage="No bank payments in selected range."
            stickyHeader
          />
        </Card>
      </div>
