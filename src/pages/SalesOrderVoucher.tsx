...
const SalesOrderVoucher: React.FC = () => {
  ...
  function emptyLine() {
    return {
      ...
      discount: 0,
      discountAmount: 0,
      taxRate: 0, // For sales orders, tax is estimated
      taxAmount: 0,
      amount: 0,
      totalAmount: 0,
      godownId: "",
  ...
  const handleSave = async () => {
    ...
    if (new Date(orderDueDate) < new Date(date)) {
      toast.error("Order due date cannot be before order date");
      return;
    }
    ...
    if (grandTotal <= 0) {
      toast.error("Order amount cannot be zero");
      return;
    }
    ...
    try {
      ...
      const voucher = {
        ...
        isOptional: false, // Orders are never optional
        isPostDated: false,
        subTotal: subTotal,
    ...
    <div className="p-4 pr-32">
      ...
      <ActionToolbar className="mb-4 sticky top-0 z-10 bg-white shadow-sm">
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold text-blue-700">Sales Order</h1>
      ...
      {/* Party Section with additional order fields */}
      <Card className="mb-4">
      ...
      <Card className="mb-4 w-80 ml-auto">
        <div className="p-4 space-y-2">
          ...
          <div className="flex justify-between pt-2 border-t border-gray-200">
            ...
            <span className="font-bold text-lg text-blue-600">{formatNumber(grandTotal)}</span>
          </div>
        </div>
      </Card>
