...
import { Phone, CheckCircle, Clock, AlertTriangle, Plus, User, Download, Search, Calendar, MessageSquare, Mail, MapPin, Home } from "lucide-react";
...
export default function FollowUpTracker() {
  ...
  const todayTasks = useMemo(() => {
    ...
    const upcoming = notes.filter(note => note.followUpDate > today && new Date(note.followUpDate) <= new Date(new Date().setDate(new Date().getDate() + 7)));
  ...
  const outstandingParties = useMemo(() => {
    ...
    followUpNotes.forEach(note => {
      ...
      if (existing && (!existing.lastFollowUpDate || new Date(note.createdAt) > new Date(existing.lastFollowUpDate))) {
  ...
  const handleMarkResolved = async (noteId) => {
    try {
      const db = getDB();
      await db.followUpNotes.update(noteId, { isResolved: true, resolvedAt: new Date().toISOString() });
  ...
  const renderTodayTasks = () => (
    <div style={{ padding: '20px' }}>
      <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', marginBottom: '20px' }}>Today's Follow-Up Tasks</h2>
      
      {/* Overdue Section */}
      {todayTasks.overdue.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#dc2626', color: 'white', padding: '10px', borderRadius: '4px', marginBottom: '10px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: 'bold', margin: 0, display: 'flex', alignItems: 'center', gap: '5px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '15px' }}>
            {todayTasks.overdue.map(note => {
              ...
              return (
                <div key={note.id} style={{ backgroundColor: OVERDUE_BG, border: BORDER, borderRadius: '8px', padding: '15px' }}>
                  <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>{party?.name || 'N/A'}</div>
                  <div style={{ fontSize: '12px', marginBottom: '5px' }}>
                    Invoice: {invoice?.invoiceNo || 'N/A'} | Amount: {money(invoice?.grandTotal || 0)}
                  </div>
                  <div style={{ fontSize: '12px', marginBottom: '5px' }}>
      ...
      {todayTasks.dueToday.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#d97706', color: 'white', padding: '10px', borderRadius: '4px', marginBottom: '10px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: 'bold', margin: 0, display: 'flex', alignItems: 'center', gap: '5px' }}>
              <Clock size={16} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '15px' }}>
            {todayTasks.dueToday.map(note => {
              ...
              return (
                <div key={note.id} style={{ backgroundColor: DUE_TODAY_BG, border: BORDER, borderRadius: '8px', padding: '15px' }}>
                  <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>{party?.name || 'N/A'}</div>
                  <div style={{ fontSize: '12px', marginBottom: '5px' }}>
                    Invoice: {invoice?.invoiceNo || 'N/A'} | Amount: {money(invoice?.grandTotal || 0)}
                  </div>
      ...
      {todayTasks.upcoming.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: BG_HEADER, padding: '10px', borderRadius: '4px', marginBottom: '10px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: 'bold', margin: 0, display: 'flex', alignItems: 'center', gap: '5px' }}>
              <Calendar size={16} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '15px' }}>
            {todayTasks.upcoming.map(note => {
              ...
              return (
                <div key={note.id} style={{ backgroundColor: BG_CARD, border: BORDER, borderRadius: '8px', padding: '15px' }}>
                  <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>{party?.name || 'N/A'}</div>
                  <div style={{ fontSize: '12px', marginBottom: '5px' }}>
                    Invoice: {invoice?.invoiceNo || 'N/A'} | Amount: {money(invoice?.grandTotal || 0)}
                  </div>
  ...
  const renderAddViewNotes = () => (
    <div style={{ display: 'flex', gap: '20px', padding: '20px' }}>
      ...
      <div style={{ width: '260px', backgroundColor: BG_CARD, border: BORDER, padding: '15px', borderRadius: '8px' }}>
        ...
        <input
          ...
          style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', marginBottom: '10px', fontSize: '12px' }}
        />
        ...
        <div style={{ maxHeight: '500px', overflowY: 'auto' }}>
          {outstandingParties.map(p => (
            ...
            >
              ...
              <div style={{ fontSize: '10px', color: '#888' }}>Since: {p.oldestDate} | Last: {p.daysSinceLastFollowUp}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, backgroundColor: BG_CARD, border: BORDER, padding: '20px', borderRadius: '8px' }}>
        {selectedParty ? (
          <div>
            <div style={{ marginBottom: '20px', paddingBottom: '15px', borderBottom: BORDER }}>
              <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', marginBottom: '5px' }}>{selectedParty.name}</h2>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px' }}>
                <div style={{ backgroundColor: BG_HEADER, padding: '10px', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000' }}>{money(outstandingParties.find(p => p.party.id === selectedParty.id)?.totalOutstanding || 0)}</div>
                </div>
                <div style={{ backgroundColor: BG_HEADER, padding: '10px', borderRadius: '6px' }}>
                  <div style={{ fontSize: '12px', color: '#000000' }}>Oldest Invoice</div>
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000' }}>
                    {outstandingParties.find(p => p.party.id === selectedParty.id)?.oldestDate || 'N/A'}
                  </div>
                </div>
                <div style={{ backgroundColor: BG_HEADER, padding: '10px', borderRadius: '6px' }}>
            ...
            <h3 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Follow-up History</h3>
                  ...
                  <div key={note.id} style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                      <div style={{ fontWeight: 'bold', fontSize: '13px' }}>
                        {formatADToBS(note.createdAt.split('T')[0])} | {note.createdAt.split('T')[1].substring(0, 5)}
                      </div>
                      <div>
                        <span style={{
                          backgroundColor: note.priority === 'High' ? '#dc2626' : 
                                          note.priority === 'Urgent' ? '#d97706' : '#059669',
                          color: 'white',
                          padding: '2px 6px',
                          borderRadius: '12px',
                          fontSize: '10px',
                    ...
                    <div style={{ fontSize: '12px', marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                      ...
                      <span><strong>Contact:</strong> {note.contactMethod}</span>
                    </div>
                    <div style={{ fontSize: '12px', marginBottom: '5px' }}>
                    ...
                    {note.promisedAmount > 0 && (
                      <div style={{ fontSize: '12px', marginBottom: '5px' }}>
                        <strong>Promised Payment:</strong> Rs. {money(note.promisedAmount)} by {formatADToBS(note.promisedDate)}
                      </div>
                    )}
            ...
            {/* Add Note Form */}
            <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '8px', border: BORDER }}>
              <h3 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Add New Follow-up Note</h3>
              
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Contact Method</label>
                  <select
                    value={addNoteForm.contactMethod}
                    onChange={(e) => setAddNoteForm({...addNoteForm, contactMethod: e.target.value})}
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Contact Person</label>
                  <input
                    type="text"
                    value={addNoteForm.contactPerson}
                    onChange={(e) => setAddNoteForm({...addNoteForm, contactPerson: e.target.value})}
                    style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px' }}
                  />
                </div>
                <div style={{ gridColumn: 'span 2' }}>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Note/Summary</label>
                  <textarea
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Promised Amount</label>
                  <input
                    type="number"
                    value={addNoteForm.promisedAmount}
                    onChange={(e) => setAddNoteForm({...addNoteForm, promisedAmount: e.target.value})}
                    style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px' }}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Promised Date</label>
                  <input
                    type="date"
                    value={addNoteForm.promisedDate}
                    onChange={(e) => setAddNoteForm({...addNoteForm, promisedDate: e.target.value})}
                    style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px' }}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Next Follow-up Date</label>
                  <input
                    type="date"
                    value={addNoteForm.followUpDate}
                    onChange={(e) => setAddNoteForm({...addNoteForm, followUpDate: e.target.value})}
                    style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px' }}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Priority</label>
                  <select
  ...
  const renderCollectionPerformance = () => {
    const performanceData = useMemo(() => {
      ...
      const collectionEfficiency = amountsPromised > 0 ? (amountsCollected / amountsPromised) * 100 : 0;
      
      return { totalOutstanding, followUpsMade, amountsPromised, amountsCollected, collectionEfficiency };
    }, [followUpNotes, outstandingParties, fromDate, toDate, userFilter]);
    
    return (
      <div style={{ padding: '20px' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', marginBottom: '20px' }}>Collection Performance</h2>
        
        <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
          <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '8px', border: BORDER, minWidth: '150px' }}>
            <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Total Outstanding</div>
            <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(performanceData.totalOutstanding)}</div>
          </div>
          <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '8px', border: BORDER, minWidth: '150px' }}>
            <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Follow-ups Made</div>
            <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{performanceData.followUpsMade}</div>
          </div>
          <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '8px', border: BORDER, minWidth: '150px' }}>
            <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Amounts Promised</div>
            <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(performanceData.amountsPromised)}</div>
          </div>
          <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '8px', border: BORDER, minWidth: '150px' }}>
            <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Collection Efficiency</div>
            <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{performanceData.collectionEfficiency.toFixed(2)}%</div>
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            <thead>
              <tr style={{ backgroundColor: BG_HEADER }}>
                ...
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Total Outstanding</th>
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>No. of Follow-ups</th>
                <th style={{ border: BORDER, padding: '8px' }}>Last Follow-up Date</th>
                <th style={{ border: BORDER, padding: '8px' }}>Last Promised Date</th>
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Promised Amount</th>
              ...
              {outstandingParties.map(p => {
                ...
                if (lastNote) {
                  if (lastNote.isResolved) {
                    status = "Resolved";
                  } else if (lastNote.promisedDate && new Date(lastNote.promisedDate) < new Date()) {
                ...
                return (
                  <tr key={p.party.id} style={{ backgroundColor: status === "Broken Promise" ? OVERDUE_BG : (status === "Resolved" ? RESOLVED_BG : 'transparent') }}>
                    <td style={{ border: BORDER, padding: '8px' }}>{p.party.name}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(p.totalOutstanding)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{partyNotes.length}</td>
                    <td style={{ border: BORDER, padding: '8px' }}>{lastNote ? formatADToBS(lastNote.createdAt.split('T')[0]) : 'N/A'}</td>
                    <td style={{ border: BORDER, padding: '8px' }}>{lastNote?.promisedDate ? formatADToBS(lastNote.promisedDate) : 'N/A'}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(lastNote?.promisedAmount || 0)}</td>
                    <td style={{ border: BORDER, padding: '8px' }}>
                      <span style={{
                        backgroundColor: status === "On Track" ? '#059669' : 
                                        status === "Broken Promise" ? '#dc2626' : 
                                        status === "Resolved" ? '#10b981' : '#d97706',
                        color: 'white',
                        padding: '2px 6px',
                        borderRadius: '12px',
                        fontSize: '11px',
  ...
  return (
    <div style={{ backgroundColor: BG, minHeight: '100vh' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#000000', padding: '20px', marginBottom: '0' }}>Follow-Up Tracker</h1>
