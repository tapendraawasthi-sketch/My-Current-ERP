...
const OptionalVouchers: React.FC = () => {
  ...
  const optionalVouchers = vouchers.filter(v => v.isOptional || v.status === "optional" || v.convertedDate);
  ...
  const handleConvert = async (voucherId: string) => {
    if (window.confirm("Are you sure you want to convert this optional voucher to an actual voucher?")) {
      try {
        const db = getDB();
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">Manage non-accounting vouchers and forecasting scenarios</p>
          </div>
        </div>
              ...
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    ...
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Scenario Name</th>
                  ...
                  {optionalVouchers.map(voucher => {
                    ...
                    return (
                      <tr key={voucher.id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="px-3 py-2.5 text-[12px] text-gray-700 font-mono font-medium">{voucher.voucherNo || "Draft"}</td>
                        <td className="px-3 py-2.5 text-[12px] text-gray-700">{voucher.date}</td>
                        <td className="px-3 py-2.5 text-[12px] text-gray-700 capitalize">{voucher.type}</td>
                        <td className="px-3 py-2.5 text-[12px] text-gray-700 truncate max-w-xs" title={voucher.narration}>{voucher.narration}</td>
                        <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{money(voucher.totalDebit)}</td>
                        <td className="px-3 py-2.5 text-[12px] text-gray-700">{scenario?.name || "N/A"}</td>
                        <td className="px-3 py-2.5 text-center">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide ${isConverted ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
        ...
        {(activeTab === 1 || showForm) && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-4 max-w-4xl">
            ...
            <div className="flex items-center mb-6 p-2 bg-blue-50 border border-blue-100 rounded-md">
              ...
              />
              <label htmlFor="isOptional" className="text-[12px] text-blue-800 font-medium">This is an Optional Voucher (will not affect regular accounting books)</label>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1">Scenario <span className="text-red-500">*</span></label>
                <select
                ...
                {errors.scenarioId && <div className="text-[10px] text-red-500 mt-1">{errors.scenarioId}</div>}
              </div>
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1">Reference/Quote No</label>
                <input
                  type="text"
