...
export default function QuotationPage({ type }: Props) {
  ...
  const partyList = (parties || []).filter((p: any) =>
    isSales ? p.type === "customer" || p.type === "both" : p.type === "supplier" || p.type === "both"
  );
  ...
  const addLine = () => setLines(prev => [...prev, { itemId: "", itemName: "", quantity: 1, unit: "Pcs", rate: 0, discountPercent: 0, amount: 0 }]);
  ...
  const openAdd = () => {
    ...
    setLines([{ itemId: "", itemName: "", quantity: 1, unit: "Pcs", rate: 0, discountPercent: 0, amount: 0 }]);
    setShowForm(true);
  };

  const handleSave = () => {
    if (!partyId) { toast.error("Select a party"); return; }
    if (lines.some(l => !l.itemId)) { toast.error("All rows must have an item selected"); return; }
    ...
    setQuotations(prev =>
      editQuotation
        ? prev.map(q => q.id === editQuotation.id ? { ...newQuotation, id: editQuotation.id, quotationNo: editQuotation.quotationNo } : q)
        : [...prev, newQuotation]
    );
  ...
  const statusBadge = (status: Quotation["status"]) => {
    ...
    return <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${map[status]}`}>{status.toUpperCase()}</span>;
  };
    ...
    return (
      <div className="p-4 bg-[#f5f6fa] min-h-screen">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-[15px] font-semibold text-gray-800">{editQuotation ? "Modify" : "Add"} {title}</h1>
            <p className="text-[11px] text-gray-500 mt-0.5">
              Quotation No: <strong>{prefix}{String(quotations.length + 1).padStart(4, "0")}</strong>
            </p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setShowForm(false)} className="h-8 px-3 bg-white border border-gray-300 text-gray-700 text-[12px] rounded-md hover:bg-gray-50">← Back (Esc)</button>
            <button onClick={handleSave} className="h-8 px-3 bg-[#1557b0] text-white text-[12px] font-medium rounded-md hover:bg-[#0f4a96]">Save (F2)</button>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <input type="date" value={validUpto} onChange={e => setValidUpto(e.target.value)} className={`${inputCls} w-full`} />
            </div>
            <div className="md:col-span-2">
              <label className={labelCls}>{isSales ? "Customer" : "Supplier"} *</label>
              <select value={partyId} onChange={e => setPartyId(e.target.value)} className={`${inputCls} w-full`}>
        ...
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden mb-4">
                ...
                {lines.map((line, idx) => (
                  <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="px-2 py-1">
                      <select value={line.itemId} onChange={e => handleLineChange(idx, "itemId", e.target.value)} className={`${inputCls} w-full`}>
                    ...
                    <td className="px-2 py-1">
                      <input type="number" value={line.quantity} onChange={e => handleLineChange(idx, "quantity", e.target.value)} className={`${inputCls} w-20 text-right`} min={0.01} step={0.01} />
                    </td>
                    <td className="px-2 py-1">
                      <input value={line.unit} onChange={e => handleLineChange(idx, "unit", e.target.value)} className={`${inputCls} w-20`} />
                    </td>
                    <td className="px-2 py-1">
                      <input type="number" value={line.rate} onChange={e => handleLineChange(idx, "rate", e.target.value)} className={`${inputCls} w-28 text-right`} min={0} step={0.01} />
                    </td>
                    <td className="px-2 py-1">
                      <input type="number" value={line.discountPercent} onChange={e => handleLineChange(idx, "discountPercent", e.target.value)} className={`${inputCls} w-20 text-right`} min={0} max={100} step={0.1} />
                    </td>
                    <td className="px-3 py-2 text-[12px] font-mono text-right">{formatNumber(line.amount)}</td>
                    <td className="px-2 py-1">
                      <button onClick={() => removeLine(idx)} className="p-1 text-red-400 hover:bg-red-50 rounded"><X className="h-3.5 w-3.5" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="px-4 py-3 border-t border-gray-200 bg-[#eef2ff] flex justify-end">
            <div className="text-right">
              <div className="text-[11px] text-gray-500 uppercase tracking-wide">Quotation Total</div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          <div>
            ...
            <textarea
              ...
              rows={2} placeholder="e.g. Being quotation submitted as per request"
            />
          </div>
          <div>
    ...
    <div className="p-4 bg-[#f5f6fa] min-h-screen">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">
            Manage quotations — convert accepted quotations to {isSales ? "sales orders / invoices" : "purchase orders"} via F11
          </p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[#f5f6fa] border-b border-gray-200">
              {["Quotation No.", "Date", "Valid Upto", isSales ? "Customer" : "Supplier", "Items", "Total", "Status", "Actions"].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(q => (
              <tr key={q.id} className="hover:bg-gray-50">
                <td className="px-3 py-2.5 text-[12px] font-mono font-bold text-[#1557b0]">{q.quotationNo}</td>
                <td className="px-3 py-2.5 text-[12px] text-gray-600">{q.date}</td>
                <td className="px-3 py-2.5 text-[12px] text-gray-600">
                  <span className={new Date(q.validUpto) < new Date() && q.status === "open" ? "text-red-500 font-semibold" : ""}>
                    {q.validUpto}
                  </span>
                </td>
                ...
                <td className="px-3 py-2.5 text-[12px] font-mono text-right">Rs. {formatNumber(q.totalAmount)}</td>
                ...
                <td className="px-3 py-2.5">
                  <div className="flex gap-1 flex-wrap">
                    ...
                    {q.status === "open" && (
                      <>
                        <button onClick={() => updateStatus(q.id, "accepted")} className="h-7 px-2 text-[11px] border border-green-200 text-green-700 rounded hover:bg-green-50">Accept</button>
                        <button onClick={() => updateStatus(q.id, "rejected")} className="h-7 px-2 text-[11px] border border-red-200 text-red-600 rounded hover:bg-red-50">Reject</button>
                      </>
                    )}
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="py-12 text-center text-[12px] text-gray-500">
