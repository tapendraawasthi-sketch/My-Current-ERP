...
export default function PhysicalStockPage2() {
  ...
  const getBookStock = (itemId: string): number => {
    return (stockMovements || [])
      ...
      .reduce((sum: number, m: any) => {
        ...
        return (t.includes("in") || t.includes("purchase") || t.includes("opening") || t.includes("received"))
          ? sum + qty
          : sum - qty;
      }, 0);
  };
  ...
  const loadItems = () => {
    const stockLines = (items || []).filter((i: any) => i.isActive !== false).map((item: any) => {
  ...
  const exportCSV = () => {
    ...
    const rows = lines.map(l => [l.itemName, l.unit, l.bookStock, l.physicalStock, l.variance, l.varianceType].join(","));
    ...
    <div className="p-4 bg-[#f5f6fa] min-h-screen">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">Record actual physical stock count — system auto-generates Stock Journal for variances</p>
        </div>
        <div className="flex gap-2">
          {loaded && (
            <button onClick={exportCSV} className="h-8 px-3 bg-white border border-gray-300 text-gray-700 text-[12px] rounded-md hover:bg-gray-50 flex items-center gap-1.5">
              <Download className="h-3.5 w-3.5" /> Export CSV
            </button>
          )}
          {loaded && !saved && (
            <button onClick={handleSave} className="h-8 px-3 bg-[#1557b0] hover:bg-[#0f4a96] text-white text-[12px] font-medium rounded-md">
              Save (F2)
            </button>
          )}
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <input value={materialCentre} onChange={e => setMaterialCentre(e.target.value)} className={`${inputCls} w-full`} />
          </div>
          <div className="md:col-span-2">
            <label className={labelCls}>Narration</label>
            <input value={narration} onChange={e => setNarration(e.target.value)} className={`${inputCls} w-full`} />
          </div>
        </div>
        <div className="mt-3 flex gap-2 items-center">
          <button onClick={loadItems} className="h-8 px-3 bg-[#059669] hover:bg-[#047857] text-white text-[12px] font-medium rounded-md flex items-center gap-1.5">
            <Package className="h-3.5 w-3.5" /> Load Items
          </button>
          <span className="text-[11px] text-gray-400">Click "Load Items" to populate all active inventory items from the system</span>
        </div>
      </div>
      ...
      {loaded && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
          {[
            { label: "Total Items", value: summary.total, color: "text-gray-800", bg: "bg-gray-50" },
            { label: "Matched", value: summary.matched, color: "text-green-700", bg: "bg-green-50" },
        ...
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            ...
            <table className="w-full">
              <thead className="sticky top-0 bg-[#f5f6fa] z-10">
                <tr className="border-b border-gray-200">
                  {["#", "Item Name", "Unit", "Book Stock", "Physical Stock", "Variance", "Status"].map(h => (
              ...
              <tbody className="divide-y divide-gray-100">
                {lines.map((line, idx) => (
                  <tr key={line.itemId} className={`hover:bg-gray-50 ${line.variance !== 0 ? (line.varianceType === "excess" ? "bg-blue-50/30" : "bg-red-50/30") : ""}`}>
                    <td className="px-3 py-2 text-[11px] text-gray-400">{idx + 1}</td>
                    <td className="px-3 py-2 text-[12px] font-medium text-gray-800">{line.itemName}</td>
                    <td className="px-3 py-2 text-[12px] text-gray-500">{line.unit}</td>
                    <td className="px-3 py-2 text-[12px] font-mono text-gray-600 text-right">{formatNumber(line.bookStock)}</td>
                    ...
                    <td className={`px-3 py-2 text-[12px] font-mono text-right font-bold ${line.variance > 0 ? "text-blue-700" : line.variance < 0 ? "text-red-600" : "text-gray-400"}`}>
                      {line.variance > 0 ? "+" : ""}{formatNumber(line.variance)}
                    </td>
                    <td className="px-3 py-2">
                      {line.varianceType === "match" ? (
                        <span className="flex items-center gap-1 text-[10px] text-green-600 font-semibold"><CheckCircle className="h-3 w-3" /> Match</span>
                      ) : line.varianceType === "excess" ? (
                        <span className="flex items-center gap-1 text-[10px] text-blue-600 font-semibold"><AlertTriangle className="h-3 w-3" /> Excess</span>
                      ) : (
                        <span className="flex items-center gap-1 text-[10px] text-red-600 font-semibold"><AlertTriangle className="h-3 w-3" /> Shortage</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          {saved && (
            <div className="px-4 py-3 border-t border-gray-200 bg-green-50 text-[12px] text-green-700 font-semibold flex items-center gap-2">
              ...
              Physical Stock Voucher saved. Stock Journal auto-generated for {summary.totalVarianceItems} variance items.
            </div>
          )}
        </div>
      )}
