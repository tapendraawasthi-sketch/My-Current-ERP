...
const BatchMaster: React.FC = () => {
  ...
  const openingValue = (parseFloat(form.openingQty as any) || 0) * (parseFloat(form.purchaseRate as any) || 0);
        ...
        <div className="p-4 flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">Manage item batches, manufacturing and expiry dates</p>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#f5f6fa] border-b border-gray-200 sticky top-0 z-10">
                  ...
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Batch Name</th>
                  ...
                  <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">MRP (NPR)</th>
                  ...
                  filteredBatches.map((batch, index) => {
                    ...
                    return (
                      <tr key={batch.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2.5 text-[12px] text-gray-700 font-medium">{batch.batchName}</td>
                        ...
                        <td className="px-3 py-2.5 text-[12px] text-center">
                          <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded-full ${batch.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
      ...
      {showForm && (
        <div className="w-full lg:w-[400px] xl:w-[450px] bg-white flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-20">
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            <div>
              <label className="text-[11px] font-medium text-gray-600 mb-1 block">Batch Name / Number <span className="text-red-500">*</span></label>
              <input
                type="text"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Mfg Date (AD)</label>
                <input
                  type="date"
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                  value={form.mfgDate}
              ...
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Expiry Date (AD)</label>
                <input
                  type="date"
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Purchase Rate (NPR)</label>
                <input
                  type="number"
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Sales Rate (NPR)</label>
                <input
                  type="number"
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Opening Quantity</label>
                <input
                  type="number"
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                  value={form.openingQty}
                  onChange={(e) => setForm({ ...form, openingQty: parseFloat(e.target.value) || 0 })}
                  min="0"
                  step="any"
                />
              </div>
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Opening Value (NPR) <span className="text-red-500">*</span></label>
