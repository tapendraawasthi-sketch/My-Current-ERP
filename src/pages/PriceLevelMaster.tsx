...
const PriceLevelMaster: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selected) {
        await updatePriceLevel(selected.id, form);
        toast.success("Updated successfully");
      } else {
        await addPriceLevel(form);
        ...
        <div className="p-4 flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">Manage item price levels for different customers</p>
            </div>
                ...
                {filteredLevels.length === 0 ? (
                  ...
                  filteredLevels.map((level, index) => (
                    <tr key={level.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2.5 text-[12px] text-gray-700">{level.description || "-"}</td>
      ...
      {showForm && (
        <div className="w-full lg:w-96 bg-white flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-20">
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            <div>
              <label className="text-[11px] font-medium text-gray-600 mb-1 block">Price Level Name <span className="text-red-500">*</span></label>
              <input
                type="text"
