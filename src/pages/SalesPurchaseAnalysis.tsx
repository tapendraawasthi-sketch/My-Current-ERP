...
// Constants
const CHART_COLORS = ["#1557b0","#059669","#d97706","#dc2626","#0284c7","#7c3aed","#db2777","#ea580c"];
const NEPALI_MONTHS = ["Baisakh","Jestha","Ashadh","Shrawan","Bhadra","Ashwin","Kartik","Mangsir","Poush","Magh","Falgun","Chaitra"];

// Compute functions
const computeTopParties = (invoices: any[], partyType: "customer"|"vendor", topN: number): PartyAnalysis[] => {
  ...
  const grouped = relevantInvoices.reduce((acc, inv) => {
    ...
    if (new Date(inv.date || inv.invoiceDate || '') > new Date(acc[partyKey].lastTransactionDate)) {
...
const computeTopItems = (invoices: any[], itemType: "sales"|"purchase", topN: number): ItemAnalysis[] => {
  if (!invoices || invoices.length === 0) return [];
...
const SalesPurchaseAnalysis: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"customers" | "vendors" | "items" | "trends">("customers");
  const [topN, setTopN] = useState<number>(10);
  const [trendMonths, setTrendMonths] = useState<number>(6);
  const [itemType, setItemType] = useState<"sales" | "purchase">("sales");
  const [filterPeriod, setFilterPeriod] = useState<"thisYear" | "thisQuarter" | "thisMonth" | "custom">("thisYear");
  
  const { invoices, vouchers } = useStore();
  const allInvoices = [...(invoices ?? []), ...(vouchers?.filter(v => v.type === "SALES" || v.type === "PURCHASE") ?? [])];
  
  const topCustomers = useMemo(() => computeTopParties(allInvoices, "customer", topN), [allInvoices, topN]);
  const topVendors = useMemo(() => computeTopParties(allInvoices, "vendor", topN), [allInvoices, topN]);
  const topItems = useMemo(() => computeTopItems(allInvoices, itemType, topN), [allInvoices, itemType, topN]);
  const monthlyTrend = useMemo(() => computeMonthlyTrend(allInvoices, trendMonths), [allInvoices, trendMonths]);
    ...
    <div className="flex flex-col h-full bg-[#f5f6fa] pb-20">
      <div className="p-4 bg-white border-b border-gray-200">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600">Top customers, vendors, products and business trends</p>
          </div>
      ...
      <div className="grid grid-cols-4 gap-4 p-4">
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-lg font-bold text-gray-900">Rs. {totalSales.toLocaleString('en-IN')}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-sm text-gray-600">Total Purchases</p>
          <p className="text-lg font-bold text-gray-900">Rs. {totalPurchases.toLocaleString('en-IN')}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-sm text-gray-600">Top Customer</p>
          <p className="text-lg font-bold text-gray-900">{topCustomer.partyName}</p>
          <p className="text-sm text-gray-600">Rs. {topCustomer.totalAmount.toLocaleString('en-IN')}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-[#f5f6fa]">
                          <tr>
                            <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Rank</th>
                            <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Party Name</th>
                            <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Invoice Count</th>
                            <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Total Amount</th>
                            <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Avg Invoice</th>
                            <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Last Transaction</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                          {topCustomers.map((customer, index) => (
                            <tr key={customer.partyId} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-2 text-[12px] text-gray-700">{customer.partyName}</td>
                              <td className="px-4 py-2 text-right text-[12px] text-gray-700">{customer.invoiceCount}</td>
                              <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">
                                Rs. {customer.totalAmount.toLocaleString('en-IN')}
                              </td>
                              <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">
                                Rs. {customer.avgInvoiceValue.toLocaleString('en-IN', { maximumFractionDigits: 2 })}
                              </td>
                              <td className="px-4 py-2 text-[12px] text-gray-700">{customer.lastTransactionDate}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div>
                    <div className="bg-white rounded-lg border border-gray-200 p-4">
                      ...
                      <div className="mt-4 text-center">
                        <p className="text-sm font-medium">Total: Rs. {topCustomers.reduce((sum, c) => sum + c.totalAmount, 0).toLocaleString('en-IN')}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {topVendors.map((vendor, index) => (
                            <tr key={vendor.partyId} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-2 text-[12px] text-gray-700">{vendor.partyName}</td>
                              <td className="px-4 py-2 text-right text-[12px] text-gray-700">{vendor.invoiceCount}</td>
                              <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">
                                Rs. {vendor.totalAmount.toLocaleString('en-IN')}
                              </td>
                              <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">
                                Rs. {vendor.avgInvoiceValue.toLocaleString('en-IN', { maximumFractionDigits: 2 })}
                              </td>
                              <td className="px-4 py-2 text-[12px] text-gray-700">{vendor.lastTransactionDate}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div>
                    <div className="bg-white rounded-lg border border-gray-200 p-4">
                      ...
                      <div className="mt-4 text-center">
                        <p className="text-sm font-medium">Total: Rs. {topVendors.reduce((sum, v) => sum + v.totalAmount, 0).toLocaleString('en-IN')}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-[#f5f6fa]">
                      <tr>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Rank</th>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Item Name</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Total Qty</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Total Amount</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Avg Rate</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Gross Margin %</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {topItems.map((item, index) => (
                        <tr key={item.itemId} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-2 text-right text-[12px] text-gray-700">{item.totalQty}</td>
                          <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">
              ...
              <>
                <div className="bg-white rounded-lg border border-gray-200 p-4">
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    ...
                    >
                      ...
                      <Tooltip formatter={(value) => [`Rs. ${Number(value).toLocaleString('en-IN')}`, 'Amount']} />
                      <Legend />
                      <Line type="monotone" dataKey="salesAmount" stroke="#1557b0" name="Sales" strokeWidth={2} />
                      <Line type="monotone" dataKey="purchaseAmount" stroke="#d97706" name="Purchases" strokeWidth={2} />
                      <Line type="monotone" dataKey="grossProfit" stroke="#059669" name="Gross Profit" strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-[#f5f6fa]">
                      <tr>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Month</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Sales</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Purchases</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Gross Profit</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Margin %</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {monthlyTrend.map((trend, index) => {
                        const marginPct = trend.salesAmount > 0 
                          ? (trend.grossProfit / trend.salesAmount) * 100 
                          : 0;
                        
                        return (
                          <tr key={index} className="hover:bg-gray-50">
                            <td className="px-4 py-2 whitespace-nowrap text-[12px] text-gray-700">{trend.month}</td>
                            <td className="px-4 py-2 text-right text-[12px] text-gray-700 font-mono">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-green-600">
                                +{(
                                  ((monthlyTrend[monthlyTrend.length - 1].salesAmount - 
                                   monthlyTrend[monthlyTrend.length - 2].salesAmount) / 
                                   monthlyTrend[monthlyTrend.length - 2].salesAmount) * 100
                                ).toFixed(2)}%
                              </span>
                            </div>
                          ) : (
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-green-600">
                                +{(
                                  ((monthlyTrend[monthlyTrend.length - 1].purchaseAmount - 
                                   monthlyTrend[monthlyTrend.length - 2].purchaseAmount) / 
                                   monthlyTrend[monthlyTrend.length - 2].purchaseAmount) * 100
                                ).toFixed(2)}%
                              </span>
                            </div>
                          ) : (
