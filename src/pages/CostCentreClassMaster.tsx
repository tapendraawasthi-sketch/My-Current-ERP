...
const CostCentreClassMaster: React.FC = () => {
  ...
  const loadFormForEdit = (cls: any) => {
    setForm({
      name: cls.name || "",
      applicableVoucherTypes: Array.isArray(cls.applicableVoucherTypes)
        ? [...cls.applicableVoucherTypes]
        : [],
                  ...
                  filteredClasses.map((cls, index) => (
                    <tr key={cls.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2.5 text-[12px] text-gray-700">
                        {getVoucherTypeLabels(cls.applicableVoucherTypes || [])}
                      </td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700">
                      ...
                      <td className="px-3 py-2.5 text-[12px]">
                        <span
                          className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded-full ${cls.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"}`}
                        >
