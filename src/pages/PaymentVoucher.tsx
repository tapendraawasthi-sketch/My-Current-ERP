...
import { Plus, Eye, Search, Download, Wallet } from "lucide-react";
...
const PaymentVoucher: React.FC = () => {
  ...
  const payments = useMemo(
    () => vouchers.filter((v) => v.type === VoucherType.PAYMENT),
    [vouchers],
  );

  // Deep link: /app/payment/new | /app/payment/:id
  useEffect(() => {
    ...
    if (route.entityId) {
      const row = payments.find((v) => v.id === route.entityId);
      if (row) {
        setActiveId(row.id);
        setMode("edit");
      }
  ...
  }, [route.pageId, route.entityId, payments]);
  ...
  }, [payments, statusFilter, branchFilter, fromDate, toDate]);
  ...
  const exportCsv = () => {
    const headers = ["Voucher No", "Date", "Paid To", "Narration", "Amount", "Status"];
    const rows = searched.map((row) =>
      [row.voucherNo, row.date, row.partyName, row.narration, row.totalCredit, row.status]
    ...
    link.href = url;
    link.download = "payment-vouchers.csv";
    link.click();
        ...
        {searched.length === 0 ? (
            ...
            <EmptyState
              ...
              title={
                payments.length === 0
                  ...
                  : "No payment vouchers match your filters"
              }
              description={
                payments.length === 0
                  ? "Record a payment to a party or expense ledger to start this register."
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody>
                  {displayed.map((row) => (
                    ...
                    >
                      ...
                      <td className={`${td} font-mono text-right text-red-600 font-medium`}>
            ...
            <div className="px-3 py-2 border-t border-gray-200 bg-[var(--ds-surface-muted)] text-[12px] text-gray-500">
              Showing {displayed.length} of {searched.length} payment vouchers
            </div>
          </div>
        )}
      </div>
