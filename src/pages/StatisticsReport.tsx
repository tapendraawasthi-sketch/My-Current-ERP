...
const StatisticsReport: React.FC = () => {
  ...
  const { vouchers, accounts, items, parties, costCenters, employees, warehouses, companySettings, currentFiscalYear } = useStore();
  ...
  const statsData = useMemo(() => {
    ...
    const voucherStats = [
      { type: "Sales Invoices", key: "sales-invoice", count: voucherCounts["sales-invoice"].count, amount: voucherCounts["sales-invoice"].amount },
      { type: "Purchase Bills", key: "purchase-invoice", count: voucherCounts["purchase-invoice"].count, amount: voucherCounts["purchase-invoice"].amount },
      { type: "Receipt Vouchers", key: "receipt", count: voucherCounts["receipt"].count, amount: voucherCounts["receipt"].amount },
      { type: "Payment Vouchers", key: "payment", count: voucherCounts["payment"].count, amount: voucherCounts["payment"].amount },
      { type: "Journal Vouchers", key: "journal", count: voucherCounts["journal"].count, amount: voucherCounts["journal"].amount },
      { type: "Contra Vouchers", key: "contra", count: voucherCounts["contra"].count, amount: voucherCounts["contra"].amount },
      { type: "Credit Notes", key: "credit-note", count: voucherCounts["credit-note"].count, amount: voucherCounts["credit-note"].amount },
      { type: "Debit Notes", key: "debit-note", count: voucherCounts["debit-note"].count, amount: voucherCounts["debit-note"].amount },
      { type: "Sales Returns", key: "sales-return", count: voucherCounts["sales-return"].count, amount: voucherCounts["sales-return"].amount },
      { type: "Purchase Returns", key: "purchase-return", count: voucherCounts["purchase-return"].count, amount: voucherCounts["purchase-return"].amount },
      { type: "Stock Journals", key: "stock-journal", count: voucherCounts["stock-journal"].count, amount: voucherCounts["stock-journal"].amount }
    ];
    ...
    // Master statistics
    const masterStats = [
      { label: "Total Ledger Accounts", count: (accounts || []).filter(a => !a.isGroup && (a.isActive !== false)).length },
    ...
    const transactionSummary = {
      ...
      totalAmount: filteredVouchers.filter(v => v.status === "posted").reduce((sum, v) => sum + (v.totalDebit || v.totalCredit || 0), 0)
    };
  ...
  }, [vouchers, accounts, items, parties, costCenters, employees, warehouses, startDate, endDate]);
  ...
  const renderCell = (columnKey: string, value: any, row: any) => {
    ...
    if (columnKey === "amount") {
      return <span className="font-mono text-[#1557b0]">{formatNumber(value)}</span>;
    }
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-8 pb-8">
        ...
        {/* Masters Summary */}
        <section>
          <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-3">Masters Summary</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            {statsData.masterStats.map((stat, index) => (
              <div key={index} className="bg-white border border-gray-200 rounded-md p-3 shadow-sm flex flex-col justify-center text-center">
                <span className="text-[18px] font-mono font-bold text-[#1557b0] mb-1">{stat.count}</span>
                <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide leading-tight">{stat.label}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-3">Transaction Status Summary</h3>
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col">
              <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Posted Vouchers</div>
              <div className="text-[24px] font-mono font-bold text-[#059669]">{formatNumber(statsData.transactionSummary.posted)}</div>
            </div>
            <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col">
              <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Draft Vouchers</div>
              <div className="text-[24px] font-mono font-bold text-[#d97706]">{formatNumber(statsData.transactionSummary.draft)}</div>
            </div>
            <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col">
              <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Cancelled Vouchers</div>
              <div className="text-[24px] font-mono font-bold text-[#dc2626]">{formatNumber(statsData.transactionSummary.cancelled)}</div>
            </div>
          </div>
        </section>
        ...
        <section>
          <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-3">Voucher-wise Statistics</h3>
