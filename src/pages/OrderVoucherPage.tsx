...
export default function OrderVoucherPage({ type }: Props) {
  ...
  const [orderType, setOrderType] = useState<string>(isSalesOrder ? SaleType.LOCAL_GST_18 : PurchaseType.LOCAL_GST_18);
  ...
  const addLine = () => setLines(prev => [...prev, { itemId: "", itemName: "", quantity: 1, unit: "Pcs", rate: 0, amount: 0 }]);
  ...
  const handleSave = () => {
    ...
    if (lines.some(l => !l.itemId)) { toast.error("All item rows must have an item selected"); return; }
    ...
    const newOrder: Order = {
      ...
      orderType,
      lines: lines.map((l, i) => ({ ...l, id: `ol-${i}`, pendingQty: l.quantity, deliveredQty: 0 })),
      narration,
      totalAmount,
      status: "open",
    ...
    setOrders(prev =>
      editOrder
        ? prev.map(o => o.id === editOrder.id ? { ...newOrder, id: editOrder.id, orderNo: editOrder.orderNo } : o)
        : [...prev, newOrder]
    );
    ...
    return (
      <div className="p-4 bg-[#f5f6fa] min-h-screen">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-[15px] font-semibold text-gray-800">{editOrder ? "Modify" : "Add"} {title}</h1>
            <p className="text-[11px] text-gray-500 mt-0.5">Order No: <strong>{prefix}{String(orders.length + 1).padStart(4, "0")}</strong></p>
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <select value={orderType} onChange={e => setOrderType(e.target.value)} className={`${inputCls} w-full`}>
                {(isSalesOrder ? SALE_TYPES : PURCHASE_TYPES).map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
                  ...
                  <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="px-2 py-1">
                      <select value={line.itemId} onChange={e => handleLineChange(idx, "itemId", e.target.value)} className={`${inputCls} w-full`}>
                        ...
                        {(items || []).filter((i: any) => i.isActive !== false).map((i: any) => <option key={i.id} value={i.id}>{i.name}</option>)}
                      </select>
                    </td>
                    <td className="px-2 py-1">
                    ...
                    <td className="px-2 py-1">
                      <input type="date" value={(line as any).deliveryDate || ""} onChange={e => handleLineChange(idx, "deliveryDate", e.target.value)} className={`${inputCls} w-36`} />
                    </td>
                    <td className="px-2 py-1">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">
            Manage {title.toLowerCase()}s — pick into {isSalesOrder ? "sales/delivery challans" : "purchase/GRNs"} via F11
          </p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[#f5f6fa] border-b border-gray-200">
              {["Order No.", "Date", isSalesOrder ? "Customer" : "Supplier", "Items", "Total Amount", "Status", "Actions"].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(order => (
              <tr key={order.id} className="hover:bg-gray-50">
                <td className="px-3 py-2.5 text-[12px] font-mono font-bold text-[#1557b0]">{order.orderNo}</td>
                <td className="px-3 py-2.5 text-[12px] text-gray-600">{order.date}</td>
                <td className="px-3 py-2.5 text-[12px] font-medium text-gray-800">{order.partyName}</td>
                <td className="px-3 py-2.5 text-[12px] text-gray-600">{order.lines.length} items</td>
                <td className="px-3 py-2.5 text-[12px] font-mono text-right">Rs. {formatNumber(order.totalAmount)}</td>
                ...
                <td className="px-3 py-2.5">
                  <div className="flex gap-1">
                    <button
                      onClick={() => {
                        setEditOrder(order);
                        setLines(order.lines.map(({ id, pendingQty, deliveredQty, ...rest }) => rest));
                        setDate(order.date);
                    ...
                    >Edit</button>
                    {order.status === "open" && (
                      <button onClick={() => cancelOrder(order.id)} className="h-7 px-2 text-[11px] border border-red-200 text-red-600 rounded hover:bg-red-50">Cancel</button>
                    )}
                  </div>
                </td>
              </tr>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={7} className="py-12 text-center text-[12px] text-gray-500">
