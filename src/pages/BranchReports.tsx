...
import { Building2, Download, BarChart2, RefreshCcw, ChevronDown, ChevronRight } from "lucide-react";
...
export default function BranchReports() {
  ...
  useEffect(() => {
    if (fiscalYears.length > 0 && !selectedFiscalYear) {
      const currentFY = fiscalYears.find(fy => fy.status === 'open') || fiscalYears[fiscalYears.length - 1];
  ...
  const stockByBranch = useMemo(() => {
    ...
    branches.forEach(branch => {
      ...
      // Get item details
      const detailedStock = Object.entries(branchStock).map(([itemId, data]) => {
  ...
  return (
    <div style={{ backgroundColor: BG, minHeight: '100vh', padding: '20px' }}>
      <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
        <h1 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', margin: 0 }}>Branch-wise Reports</h1>
        
        <div style={{ display: 'flex', gap: '15px', marginTop: '15px', flexWrap: 'wrap', alignItems: 'end' }}>
          <div>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Branch</label>
            <select
              value={branchFilter}
      ...
      {activeTab === 'sales' && (
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Sales Summary by Branch</h2>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
              <thead>
                <tr style={{ backgroundColor: BG_HEADER }}>
                  ...
                  <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Total Sales</th>
                  <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Total Purchase</th>
              ...
              <tbody>
                {Object.values(salesSummary).map((s, idx) => (
                  <tr key={s.branch.id} style={{ backgroundColor: idx % 2 === 0 ? BG_DEEP : 'transparent' }}>
                    <td style={{ border: BORDER, padding: '8px' }}>{s.branch.name}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(s.sales)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(s.purchases)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right', color: s.sales - s.purchases >= 0 ? '#059669' : '#dc2626' }}>
                      {money(s.sales - s.purchases)}
                    </td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(s.collection)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(s.outstanding)}</td>
                  </tr>
                ))}
                {/* Total Row */}
                <tr style={{ backgroundColor: BG_HEADER }}>
                  ...
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right', fontWeight: 'bold' }}>
                    {money(Object.values(salesSummary).reduce((sum, s) => sum + (s.sales - s.purchases), 0))}
                  </td>
                  <td style={{ border: BORDER, padding: '8px', textAlign: 'right', fontWeight: 'bold' }}>
      ...
      {activeTab === 'pl' && (
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>P&L by Branch</h2>
          
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            {Object.values(plByBranch).map(pl => (
              <div key={pl.branch.id} style={{ border: BORDER, borderRadius: '8px', padding: '15px' }}>
                <h3 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>{pl.branch.name}</h3>
                ...
                <div style={{ marginBottom: '10px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', borderTop: BORDER, paddingTop: '5px' }}>
                    <span>Net Sales</span>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', borderTop: BORDER, paddingTop: '5px' }}>
                    ...
                    <span style={{ color: pl.grossProfit >= 0 ? '#059669' : '#dc2626', fontWeight: 'bold' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: BORDER, paddingTop: '5px' }}>
                    <span>Net Profit</span>
                    <span style={{ color: pl.netProfit >= 0 ? '#059669' : '#dc2626', fontWeight: 'bold' }}>
                      {money(pl.netProfit)}
                    </span>
                  </div>
                </div>
      ...
      {activeTab === 'stock' && (
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Stock by Branch</h2>
            ...
            return (
              <div key={branch.id} style={{ marginBottom: '20px' }}>
                ...
                >
                  ...
                  {expandedSections[`stock-${branch.id}`] ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                </div>
                
                {expandedSections[`stock-${branch.id}`] && (
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER, marginTop: '5px' }}>
                      <thead>
                        <tr style={{ backgroundColor: BG_HEADER }}>
                          <th style={{ border: BORDER, padding: '8px' }}>Item Name</th>
                          <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Qty</th>
                          <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Avg Rate</th>
                          <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Total Value</th>
                        </tr>
                      </thead>
                      <tbody>
                        {stock.map((item, idx) => (
                          <tr key={item.item.id} style={{ backgroundColor: idx % 2 === 0 ? BG_DEEP : 'transparent' }}>
                            <td style={{ border: BORDER, padding: '8px' }}>{item.item.name}</td>
                            <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{item.qty}</td>
                            <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.avgRate)}</td>
                            <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.totalValue)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {activeTab === 'consolidated' && (
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Consolidated View</h2>
          
          <div style={{ backgroundColor: '#fef3c7', padding: '10px', borderRadius: '6px', marginBottom: '15px', border: '1px solid #f59e0b' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
            <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '6px', border: BORDER }}>
              ...
              <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(consolidatedData.totalRevenue)}</div>
            </div>
            <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '6px', border: BORDER }}>
              <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Total COGS</div>
              <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(consolidatedData.totalCogs)}</div>
            </div>
            <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '6px', border: BORDER }}>
              <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Gross Profit</div>
              <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(consolidatedData.grossProfit)}</div>
            </div>
            <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '6px', border: BORDER }}>
              <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Net Profit</div>
              <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(consolidatedData.netProfit)}</div>
            </div>
          </div>
        </div>
      )}
