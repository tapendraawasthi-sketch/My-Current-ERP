...
export default function VoucherTypeMaster() {
  ...
  const renderFeatureBadges = (item: VoucherSeriesItem) => {
    ...
    if (item.autoRoundOff)
      badges.push({ label: "Round-off", cls: "bg-green-100 text-green-700" });
    ...
    if (item.enableAdvancedPOS)
      badges.push({ label: "POS", cls: "bg-gray-100 text-gray-700" });
  ...
  return (
    <div className="flex h-full min-h-0 bg-[#f5f6fa]">
      ...
      {/* Series list */}
      <div
        className={`flex flex-1 flex-col min-w-0 ${showForm ? "border-r border-gray-200" : ""}`}
      >
