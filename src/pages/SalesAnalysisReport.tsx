...
export default function SalesAnalysisReport() {
  ...
  const salesInvoices = useMemo(() =>
    (invoices || []).filter((inv: any) => {
      ...
      const matchType = voucherType === "both" ? true
        : voucherType === "sales"
          ? (inv.type === "sales-invoice" || inv.type === "SALES_INVOICE" || inv.voucherType === "Sales")
          : (inv.type === "purchase-invoice" || inv.type === "PURCHASE_INVOICE" || inv.voucherType === "Purchase");
      return inRange && matchType && inv.status !== "cancelled";
    }), [invoices, fromDate, toDate, voucherType]);

  const analysisData = useMemo(() => {
    const grouped: Record<string, { key: string; label: string; totalSales: number; totalQty: number; invoiceCount: number }> = {};
    ...
    salesInvoices.forEach((inv: any) => {
      if (groupBy === "item") {
        ...
        lineItems.forEach(line => {
          ...
          if (!grouped[iKey]) grouped[iKey] = { key: iKey, label: iLabel, totalSales: 0, totalQty: 0, invoiceCount: 0 };
          grouped[iKey].totalSales += Number(line.lineTotal || line.totalAmount || line.amount || 0);
      ...
      grouped[key].totalQty += (inv.lines || inv.items || []).reduce((s: number, l: any) => s + Number(l.qty || l.quantity || 0), 0);
    ...
    <div className="p-4 bg-[#f5f6fa] min-h-screen">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">Party turnover, item-wise sales, month-wise trends and purchase analysis</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <div>
            ...
            <select value={voucherType} onChange={e => setVoucherType(e.target.value as any)} className={`${inputCls} w-full`}>
          ...
          <div>
            ...
            <select value={groupBy} onChange={e => setGroupBy(e.target.value as GroupBy)} className={`${inputCls} w-full`}>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          ...
          <thead>
            <tr className="bg-[#f5f6fa] border-b border-gray-200">
              ...
              <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">
                {groupBy === "party" ? "Party / Account" : groupBy === "item" ? "Item Name" : "Month"}
              </th>
              <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Invoices</th>
              <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Total Qty</th>
              <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Avg/Invoice</th>
              <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Total Amount</th>
              <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-36">Share</th>
            </tr>
          </thead>
            ...
            {analysisData.map((row, idx) => {
              ...
              return (
                <tr key={row.key} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2.5 text-[12px] font-mono text-right text-gray-600">{row.invoiceCount}</td>
                  <td className="px-3 py-2.5 text-[12px] font-mono text-right text-gray-600">{formatNumber(row.totalQty)}</td>
                  <td className="px-3 py-2.5 text-[12px] font-mono text-right text-gray-600">Rs. {formatNumber(row.avgPerInvoice)}</td>
                  <td className="px-3 py-2.5 text-[12px] font-mono text-right font-bold text-gray-800">Rs. {formatNumber(row.totalSales)}</td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-gray-100 rounded-full h-1.5 overflow-hidden">
                        <div className="bg-[#1557b0] h-full rounded-full" style={{ width: `${barWidth}%` }} />
                      </div>
                      <span className="text-[10px] text-gray-500 w-10 text-right">{share.toFixed(1)}%</span>
                    </div>
                  </td>
                </tr>
              );
            ...
            {analysisData.length === 0 && (
              <tr>
                <td colSpan={7} className="py-12 text-center">
                  ...
                  <p className="text-[12px] text-gray-500">No data found for selected period and filters</p>
                </td>
              </tr>
            )}
          </tbody>
        ...
        {analysisData.length > 0 && (
          <div className="px-4 py-2.5 border-t-2 border-[#c7d2fe] bg-[#eef2ff] flex justify-between text-[12px] font-bold text-gray-800">
            <span>GRAND TOTAL — {analysisData.length} {groupBy === "party" ? "parties" : groupBy === "item" ? "items" : "months"}</span>
