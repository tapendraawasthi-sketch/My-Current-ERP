...
> = {
  ...
  "Global IME": {
    dateCol: "Date",
    narrationCol: "Particular",
    debitCol: "Withdrawal Amount",
    creditCol: "Deposit Amount",
    balanceCol: "Balance Amount",
    dateFormat: "DD/MM/YYYY",
  },
  Siddhartha: {
    dateCol: "Trans Date",
...
export default function CommunicationHub() {
  ...
  function buildReminderMessage(row: any) {
    return templateText
      ...
      .join(row.party.name || "")
      .split("{amount}")
      .join(money(row.total))
      .split("{days}")
      .join(String(row.oldestDays));
  }
  ...
  function sendBulkWhatsApp() {
    ...
    selected.forEach((row: any, i) => {
      setTimeout(() => {
        ...
        if (phone)
          window.open(
            `https://wa.me/${phone}?text=${encodeURIComponent(buildReminderMessage(row))}`,
            "_blank",
          );
      }, i * 1500);
    });
  ...
  function handleTallyXML(file: File) {
    ...
    reader.onload = (e) => {
      ...
      const vouchersParsed = Array.from(xmlDoc.querySelectorAll("VOUCHER")).map((v: any) => ({
        voucherNo:
          v.querySelector("VOUCHERNUMBER")?.textContent || v.getAttribute("VCHKEY") || generateId(),
  ...
  function downloadJournalTemplate() {
    downloadTemplate("Journal_Entry_Template.xlsx", [
      {
        Date: "2025-01-01",
        "Dr Account Name": "Cash",
        "Cr Account Name": "Sales",
        "Amount NPR": 1000,
        Narration: "Sample",
      },
    ]);
  ...
  function validateVoucherImport() {
    ...
    voucherImportRows.forEach((row, idx) => {
      ...
      if (dr && !accountNames.has(String(dr).toLowerCase()))
        errors.push(`Row ${rowNo}: Account '${dr}' not found`);
      if (cr && !accountNames.has(String(cr).toLowerCase()))
        errors.push(`Row ${rowNo}: Account '${cr}' not found`);

      if (
        date &&
        amount &&
        (!dr || accountNames.has(String(dr).toLowerCase())) &&
        (!cr || accountNames.has(String(cr).toLowerCase()))
      ) {
        valid.push(row);
      }
    });
    ...
    for (const row of validVoucherRows) {
      ...
      const voucher = {
        ...
        lines: [
          {
            id: generateId(),
            accountName: row["Dr Account Name"] || row["Account Name"],
            debit: amount,
            credit: 0,
          },
          {
            id: generateId(),
            accountName: row["Cr Account Name"] || "Suspense",
            debit: 0,
            credit: amount,
          },
        ],
  ...
  function autoMatchBank(rows: any[]) {
    return rows.map((row) => {
      ...
      if (candidates.length === 1)
        return { rowId: row.id, status: "Matched", voucher: candidates[0] };
      if (candidates.length > 1)
        return { rowId: row.id, status: "Possible Match", voucher: candidates[0] };
  ...
  const tallyConflictCounts = useMemo(() => {
    ...
    const ledgerConflicts = tallyPreview.ledgers.filter((l) =>
      names.has(String(l.name).toLowerCase()),
    ).length;
      ...
      {activeTab === "Invoice Delivery" && (
        <div className="space-y-4">
          <div className={cardClass}>
            <label className="block text-[11px] font-medium text-gray-600 mb-2 uppercase tracking-wide">
              Select Invoice to Share
            </label>
            <select
              ...
              <div className={`${cardClass} border-t-4 border-t-blue-500`}>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <textarea
                      className={`${inputClass} min-h-[160px] py-2`}
                      value={emailBody}
                  ...
                  <div className="pt-2">
                    <button className={`${primaryBtn} w-full`} onClick={sendEmail}>
                      <Mail size={14} /> Open in Default Mail Client
                    </button>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 mt-1">
                        Include country code without '+'. Assumes 977 if 10 digits starting with 9.
                      </p>
                    </div>
                ...
                <div className={`${cardClass} border-t-4 border-t-indigo-500 bg-indigo-50/10`}>
                  ...
                  <div className="flex justify-between items-center">
                    <p className="text-[10px] text-gray-500 italic max-w-[60%]">
                      Configure your merchant ID in settings to activate real payments.
                    </p>
          ...
          <div className={`${cardClass} lg:col-span-1 space-y-4`}>
            ...
            <div>
              ...
              <select
                className={inputClass}
                value={reminderGroup}
                onChange={(e) => setReminderGroup(e.target.value)}
              >
                <option>All</option>
            ...
            <div>
              ...
              <textarea
                className={`${inputClass} min-h-[140px] py-2 leading-relaxed text-[11px] text-gray-600`}
                value={templateText}
                  ...
                  <tr>
                    <th className={`${tableHeadClass} w-10 text-center`}>
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked)
                            setSelectedReminderParties(
                              overdueReminderRows.map((r: any) => r.party.id),
                            );
                          else setSelectedReminderParties([]);
                        }}
                      />
                    </th>
                    {["Party Name", "Contact", "Total Overdue", "Oldest Due", "Status"].map((h) => (
                ...
                <tbody className="divide-y divide-gray-100">
                        ...
                        <div className="flex flex-col gap-0.5 text-[11px]">
                          {row.party.phone && (
                            <span className="flex items-center gap-1 text-gray-600">
                              <MessageSquare size={10} /> {row.party.phone}
                            </span>
                          )}
                          {row.party.email && (
                            <span className="flex items-center gap-1 text-gray-500">
                              <Mail size={10} /> {row.party.email}
                            </span>
                          )}
                  ...
                  {!overdueReminderRows.length && (
                    <tr>
                      ...
                      >
                        No overdue invoices match the current filters. Your receivables are looking
                        good!
                      </td>
                    </tr>
                  )}
          ...
          <div className={cardClass}>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50 hover:bg-gray-100 transition-colors relative cursor-pointer group">
              ...
              <p className="text-[11px] text-gray-500 mt-1">
                Supports standard Tally XML export format
              </p>
            </div>
          </div>
          ...
          <div className={cardClass}>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-md p-3 text-center">
                ...
                {tallyConflictCounts.ledgerConflicts > 0 && (
                  <div className="text-[10px] text-red-600 font-medium mt-1">
                    {tallyConflictCounts.ledgerConflicts} conflicts
                  </div>
                )}
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-md p-3 text-center">
            ...
            {tallyIssues.length > 0 && (
              <div className="border border-red-200 rounded-md p-3 bg-red-50 mb-6 max-h-40 overflow-y-auto">
                <div className="text-[12px] font-bold text-red-800 mb-2 flex items-center gap-1.5">
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                onClick={validateTallyData}
                disabled={!tallyPreview.ledgers.length && !tallyPreview.vouchers.length}
              >
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  onClick={importTallyAccounts}
                  disabled={!tallyPreview.ledgers.length}
                >
                  Import Ledgers Only
                </button>
                <button
                  className={outlineBtn}
                  onClick={importTallyVouchers}
                  disabled={!tallyPreview.vouchers.length}
                >
                  Import Vouchers Only
                </button>
              </div>
              <button
                className={`${primaryBtn} w-full`}
                onClick={importTallyAll}
                disabled={!tallyPreview.ledgers.length && !tallyPreview.vouchers.length}
              >
          ...
          <div className={`${cardClass} md:col-span-1 space-y-6`}>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center bg-gray-50 relative cursor-pointer group">
                <input
                  ...
                  accept=".xlsx,.xls,.csv"
                  onChange={(e) =>
                    e.target.files?.[0] && parseExcelFile(e.target.files[0], setVoucherImportRows)
                  }
                />
                <Upload
                  size={24}
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                <button
                  className={`${outlineBtn} w-full`}
                  onClick={validateVoucherImport}
                  disabled={!voucherImportRows.length}
                >
                  Validate Data
                </button>
                <button
                  ...
                  disabled={!validVoucherRows.length}
                >
              ...
              <div className="flex justify-between items-center text-[11px] mt-2 px-1">
                ...
                <span className="text-red-600 font-medium">
                  {voucherImportErrors.length} Errors
                </span>
              </div>
            </div>
          </div>
            ...
            {voucherImportRows.length > 0 ? (
              <div className="overflow-x-auto border border-gray-200 rounded-md">
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      {Object.keys(voucherImportRows[0])
                        .slice(0, 6)
                        .map((h) => (
                          <th key={h} className={tableHeadClass}>
                            {h}
                      ...
                      {Object.keys(voucherImportRows[0]).length > 6 && (
                    ...
                    {voucherImportRows.slice(0, 12).map((row, idx) => (
                      <tr key={idx} className="bg-white hover:bg-gray-50">
                        {Object.keys(voucherImportRows[0])
                          .slice(0, 6)
                          .map((h) => (
                            <td key={h} className={`${tableCellClass} truncate max-w-[150px]`}>
                              {String(row[h] || "")}
                            </td>
                          ))}
                        {Object.keys(voucherImportRows[0]).length > 6 && (
                ...
                <div className="mt-4 p-3 bg-blue-50 border border-blue-100 rounded-md">
                  ...
                  <div className="flex justify-between items-center text-[11px] mb-1">
                    ...
                    <span className="font-bold text-green-600">
                      {bankMatches.filter((m) => m.status === "Matched").length}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-[11px]">
                    <span className="text-gray-600">Unmatched:</span>
                    <span className="font-bold text-red-600">
                      {bankMatches.filter((m) => m.status === "Unmatched").length}
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className={`${cardClass} lg:col-span-3`}>
            ...
            <div className="overflow-x-auto border border-gray-200 rounded-md">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    {["Date", "Description", "Debit", "Credit", "Match Status", "Action"].map(
                      (h) => (
                        <th
                          key={h}
                          className={
                            h === "Debit" || h === "Credit"
                              ? `${tableHeadClass} text-right`
                              : tableHeadClass
                          }
                        >
                          {h}
                  ...
                  {bankRows.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className="bg-white hover:bg-gray-50">
                        ...
                        <td className={`${tableCellClass} max-w-[250px] truncate`}>
                          {row.description}
                        </td>
                        <td className={`${tableCellClass} text-right text-red-600`}>
                        ...
                        <td className={tableCellClass}>
                          ...
                          {match?.voucher && (
                            <div className="text-[10px] text-gray-400 mt-1">
                              {match.voucher.voucherNo || match.voucher.id}
                            </div>
                          )}
                        </td>
                        <td className={tableCellClass}>
                  ...
                  {!bankRows.length && (
                    <tr>
                      ...
                      >
                        ...
                        <div className="mt-1">
                          Select a bank format and upload an excel/csv file to begin reconciliation.
                        </div>
                      </td>
                    </tr>
                  )}
