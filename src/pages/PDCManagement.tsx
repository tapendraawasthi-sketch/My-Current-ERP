...
export default function PDCManagement() {
  ...
  const bankAccounts = useMemo(
    () => accounts.filter((a: any) =>
      !a.isGroup && (a.name || "").toLowerCase().includes("bank")
    ),
    [accounts]
  );
  ...
  const filtered = useMemo(() => {
    return pdcRegister.filter((p: any) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      pendingIssuedAmt:  issued.filter((p: any) => p.status === "pending").reduce((s: number, p: any) => s + p.amount, 0),
      pendingReceivedAmt: received.filter((p: any) => p.status === "pending").reduce((s: number, p: any) => s + p.amount, 0),
      dueSoon:           pdcRegister.filter((p: any) =>
        p.status === "pending" && p.chequeDate >= todayStr && p.chequeDate <= in7DaysStr
      ).length,
      overdue:           pdcRegister.filter((p: any) =>
        p.status === "pending" && p.chequeDate < todayStr
      ).length,
  ...
  const handleSave = async () => {
    ...
    if (!form.chequeNo.trim()) { toast.error("Cheque number required"); return; }
    if (!form.chequeDate) { toast.error("Cheque date required"); return; }
    ...
    <div className="p-4 md:p-6 bg-[#f5f6fa] min-h-screen">
      ...
      {stats.overdue > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3 flex items-center gap-2 text-[12px] text-red-800">
          ...
          <strong>{stats.overdue} PDC(s) are overdue</strong> — cheque date has passed but status is still Pending.
        </div>
      )}
      {stats.dueSoon > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-3 flex items-center gap-2 text-[12px] text-amber-800">
          ...
          <strong>{stats.dueSoon} PDC(s) due within 7 days.</strong> Prepare to deposit or make payment.
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
        {[
          { label: "PDCs to Collect",   value: "Rs. " + fmt(stats.pendingReceivedAmt), color: "text-green-700" },
          { label: "PDCs to Pay",       value: "Rs. " + fmt(stats.pendingIssuedAmt),   color: "text-red-600"   },
            ...
            {["ALL", "pending", "deposited", "dishonoured", "cancelled", "returned"].map((s) => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`h-8 px-2.5 text-[11px] font-medium capitalize transition-colors ${
                  statusFilter === s ? "bg-[#1557b0] text-white" : "bg-white text-gray-600 hover:bg-gray-50"
                }`}>
                {s === "ALL" ? "All" : s}
              </button>
            ))}
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full" style={{ minWidth: 1050 }}>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={9} className="px-4 py-10 text-center text-[12px] text-gray-400">
                ...
                return (
                  <tr key={pdc.id}
                    ...
                    <td className={tdCls}>
                      ...
                      {pdc.branchName && <div className="text-[10px] text-gray-400">{pdc.branchName}</div>}
                    </td>
                    <td className={`${tdCls} ${isOverdue ? "text-red-600 font-semibold" : isDueSoon ? "text-amber-700 font-semibold" : ""}`}>
                      {pdc.chequeDate}
                    </td>
                    ...
                    <td className={`${amtCls} font-semibold text-gray-800`}>
                      {fmt(pdc.amount)}
                    </td>
                    ...
                    <td className="px-3 py-2.5 border-b border-gray-100">
                      <span className={`px-2 py-0.5 text-[10px] font-semibold rounded uppercase flex items-center gap-1 w-fit ${statusColors[pdc.status] || "bg-gray-100 text-gray-700"}`}>
                    ...
                    <td className="px-3 py-2.5 border-b border-gray-100">
                      <div className="flex gap-1 flex-wrap">
                        {pdc.status === "pending" && (
                          <>
                            <button
                              onClick={() => quickUpdateStatus(pdc.id, "deposited", { depositDate: today() })}
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (reason) quickUpdateStatus(pdc.id, "dishonoured", { dishonourDate: today(), dishonourReason: reason });
                              }}
                        ...
                        <button
                          onClick={() => { setForm({ ...pdc }); setEditingId(pdc.id); setShowModal(true); }}
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 bg-[#f5f6fa] border-b border-gray-200">
              <h3 className="text-[14px] font-semibold text-gray-800">
                {editingId ? "Edit PDC" : `Add PDC — ${form.type === "received" ? "Received from Customer" : "Issued to Supplier"}`}
              </h3>
              <button onClick={() => setShowModal(false)} className="p-1 rounded hover:bg-gray-200 text-gray-500">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="p-4 overflow-y-auto space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select className={inputCls} value={form.partyId}
                    onChange={(e) => {
                      const party = parties.find((p: any) => p.id === e.target.value);
                      setForm((f: any) => ({ ...f, partyId: e.target.value, partyName: party?.name || "", partyPan: party?.pan || "" }));
                    }}>
                ...
                <div>
                  ...
                  <input type="number" className={inputCls} value={form.amount || ""}
                    onChange={(e) => setForm((f: any) => ({ ...f, amount: Number(e.target.value) || 0 }))}
                ...
                {form.type === "received" && (
                  <div>
                    ...
                    <select className={inputCls} value={form.bankAccountId}
                      onChange={(e) => setForm((f: any) => ({ ...f, bankAccountId: e.target.value }))}>
                ...
                {form.status === "deposited" && (
                  <div>
                    ...
                    <input type="date" className={inputCls} value={form.depositDate || ""}
                ...
                {form.status === "dishonoured" && (
                  <>
                    <div>
                      ...
                      <input type="date" className={inputCls} value={form.dishonourDate || ""}
                        onChange={(e) => setForm((f: any) => ({ ...f, dishonourDate: e.target.value }))} />
                    </div>
                    <div>
                      <label className={labelCls}>Reason</label>
                      <input className={inputCls} value={form.dishonourReason || ""}
                        onChange={(e) => setForm((f: any) => ({ ...f, dishonourReason: e.target.value }))}
                ...
                <div>
                  ...
                  <input className={inputCls} value={form.linkedInvoiceNo || ""}
                    onChange={(e) => setForm((f: any) => ({ ...f, linkedInvoiceNo: e.target.value }))}
                    placeholder="Invoice or bill reference" />
                </div>
                <div className="col-span-2">
                  <label className={labelCls}>Narration / Notes</label>
                  <input className={inputCls} value={form.narration || ""}
