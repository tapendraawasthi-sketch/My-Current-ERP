...
export default function DataExportImport() {
  ...
  const handleExport = () => {
    ...
    if (selectedMasters.includes("Account"))
      dataToExport["accounts"] = accounts.filter(
        (a: any) => !a.isGroup && matchBranch(a.branchId),
      );
    if (selectedMasters.includes("Item"))
      dataToExport["items"] = items.filter((i: any) => matchBranch(i.branchId));
    if (selectedMasters.includes("Warehouse"))
      dataToExport["warehouses"] = ((useStore.getState() as any).warehouses || []).filter(
        (w: any) => matchBranch(w.branchId),
      );
    if (exportBalances) {
      dataToExport["account_balances"] = accounts
        .filter((a: any) => !a.isGroup && matchBranch(a.branchId))
        .map((a: any) => ({
          code: a.code,
          name: a.name,
          balance: a.balance || 0,
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
        {activeTab === "export-masters" && (
          <div>
            ...
            <button style={BTN("var(--ds-action-primary-hover)")} onClick={handleExport}>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 480 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button style={BTN("var(--ds-action-primary-hover)")} onClick={handleImportExcel}>
                  Import Now
                </button>
                <button
                  style={BTN("#fff")}
