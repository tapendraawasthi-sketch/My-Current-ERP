...
const DebitNoteVoucher: React.FC = () => {
  ...
  useEffect(() => {
    const generateNumber = async () => {
      try {
        const number = await generateSerialNumber(VoucherType.DEBIT_NOTE, undefined, currentFiscalYear?.fiscalYearBS);
        setVoucherNumber(number);
      } catch (error) {
        setVoucherNumber("DN-" + Date.now());
      }
    };
    generateNumber();
  }, []);
  ...
  const handleSave = async () => {
    ...
    if (!creditReason) {
      toast.error("Please select a reason for debit");
      return;
    }
    ...
    if (grandTotal <= 0) {
      toast.error("Debit note amount cannot be zero");
      return;
    }
    ...
    try {
      ...
      toast.success(`Debit note saved successfully — ${voucherNumber}`);
      
      // Reset form
      resetForm();
      
      // Generate new voucher number
      const newNumber = await generateSerialNumber(VoucherType.DEBIT_NOTE, undefined, currentFiscalYear?.fiscalYearBS);
    ...
    <div className="p-4 pr-32">
      ...
      <Card className="mb-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Debit Reason *</label>
            <Select
              value={creditReason}
          ...
          <div className="md:col-span-2">
            <div className="flex items-center">
              ...
              <label className="ml-2 block text-sm text-gray-700">
                Goods physically returned to supplier
              </label>
            </div>
          </div>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {lines.map((line, index) => {
                  ...
                  return (
                    <tr key={line.key} className={`hover:bg-gray-50 ${isOverReturn ? 'bg-amber-50' : ''}`}>
