...
const GodownSummary: React.FC = () => {
  ...
  const summaryData = useMemo(() => {
    ...
    const warehouseItemMap = new Map<string, Map<string, { qty: number, value: number, totalQty: number, totalValue: number }>>();
  ...
  }, [warehouses, items, stockMovements, asOnDate, selectedWarehouseId, selectedItemId, showZeroQty, expandedWarehouses]);
      ...
      <div className="bg-white border border-gray-200 rounded-md overflow-hidden mb-6">
        <table className="w-full text-[12px] border-collapse bg-white">
          <thead className="bg-[#f5f6fa] border-b border-gray-200">
            <tr>
              <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-2/5">Item / Stock Item</th>
              <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Unit</th>
              <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Quantity</th>
              <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Avg Rate</th>
          ...
          <tbody className="divide-y divide-gray-100">
              ...
              if (row.isWarehouse) {
                ...
                return (
                  <tr key={row.id} className="bg-gray-50 font-semibold text-gray-700">
                    <td className="px-3 py-2.5 pl-9 text-gray-500 text-[11px] uppercase tracking-wide">{row.itemName}</td>
                    <td className="px-3 py-2.5"></td>
                    <td className="px-3 py-2.5"></td>
                    <td className="px-3 py-2.5"></td>
                    <td className="px-3 py-2.5 text-right font-mono text-[#1557b0]">{formatNumber(row.value)}</td>
                  </tr>
                );
              } else if (row.isItem) {
                return (
                  <tr key={row.id} className={`hover:bg-gray-50 transition-colors ${row.isNegative ? "bg-red-50 hover:bg-red-100" : ""}`}>
                    <td className="px-3 py-2.5 pl-9 text-gray-700">
                      {row.isNegative && <span className="text-red-600 font-bold mr-1" title="Negative Stock">⚠️</span>}
                      {row.itemName}
                    </td>
                    <td className="px-3 py-2.5 text-gray-600">{row.unit}</td>
                    <td className={`px-3 py-2.5 text-right font-mono ${row.isNegative ? "text-red-600 font-semibold" : "text-gray-700"}`}>{formatNumber(row.qty)}</td>
                    <td className="px-3 py-2.5 text-right font-mono text-gray-600">{row.rate === 0 ? "—" : formatNumber(row.rate)}</td>
                    <td className={`px-3 py-2.5 text-right font-mono ${row.isNegative ? "text-red-600 font-semibold" : "text-gray-700"}`}>{formatNumber(row.value)}</td>
                  </tr>
                );
              }
              return null;
            ...
            {summaryData.groupedData.length > 0 && (
              <tr className="bg-[#eef2ff] font-bold border-t-2 border-[#c7d2fe]">
                ...
                <td className="px-3 py-3"></td>
                <td className="px-3 py-3"></td>
                <td className="px-3 py-3 text-right font-mono text-[#1557b0] text-[14px]">{formatNumber(summaryData.grandTotalValue)}</td>
              </tr>
            )}
            
            {summaryData.groupedData.length === 0 && (
              <tr>
                <td colSpan={5} className="px-3 py-8 text-center text-gray-500 italic">No warehouse stock data found for the selected criteria.</td>
              </tr>
            )}
          </tbody>
        </table>
