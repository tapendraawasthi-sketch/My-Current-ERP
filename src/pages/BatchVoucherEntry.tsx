...
const BatchVoucherEntry: React.FC = () => {
  ...
  const [rows, setRows] = useState<BatchRow[]>(() => Array.from({length: 20}, () => createEmptyRow()));
  const [postingProgress, setPostingProgress] = useState<{ current: number, total: number } | null>(null);
  const [validationResult, setValidationResult] = useState<{ validCount: number, errorCount: number }>({ validCount: 0, errorCount: 0 });

  const accountOptions = useMemo(() => {
    return accounts.filter(a => a.isActive).map(acc => ({
      id: acc.id,
      code: acc.code,
      name: acc.name
    }));
  ...
  const handlePostAll = async () => {
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      try {
        await addVoucher({
          ...
          lines: [
            { id: generateId(), accountId: row.drAccountId, debit: row.amount, credit: 0, narration: row.narration },
            { id: generateId(), accountId: row.crAccountId, debit: 0, credit: row.amount, narration: row.narration }
          ],
    ...
    if (errors.length === 0) {
      ...
      setValidationResult({ validCount: 0, errorCount: validatedRows.filter(r => r.isValid === false).length });
    } else {
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">Rapid data entry for multiple journal vouchers</p>
          </div>
        </div>
            ...
            <table className="w-full min-w-max border-collapse">
              <thead>
                <tr className="bg-[#f5f6fa] border-b border-gray-200">
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-4">#</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-36">Date</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Dr Account</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Cr Account</th>
                  ...
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-48">Validation</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, index) => {
                    ...
                    <tr key={row.id} className={`${rowClass} hover:opacity-90`}>
                      ...
                      <td className="px-3 py-2 align-top">
                        <input
                          ...
                          value={row.amount || ""}
                          placeholder="0.00"
                          onChange={(e) => handleCellChange(index, 'amount', Number(e.target.value) || 0)}
                      ...
                      <td className="px-3 py-2 align-top pt-3.5">
                        {row.error && <div className="text-[10px] font-medium text-red-600">{row.error}</div>}
                        {row.isValid === true && <div className="text-[10px] font-medium text-green-600 flex items-center gap-1"><CheckCircle size={10} /> Valid</div>}
                      </td>
                    </tr>
                  );
                })}
