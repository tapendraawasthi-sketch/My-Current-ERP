...
const TDSNatureOfPaymentMaster: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Delete this TDS Nature of Payment?")) {
      try {
      ...
      <div className={`flex-1 flex flex-col ${showForm ? "hidden lg:flex lg:w-2/3 xl:w-3/4 border-r border-gray-200" : "w-full"}`}>
        <div className="p-4 flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between mb-2">
            <div>
              <h1 className="text-[15px] font-semibold text-gray-800">TDS Nature of Payment Master (Nepal ITA 2058)</h1>
              <p className="text-[11px] text-gray-500 mt-0.5">Manage TDS rates, sections, and thresholds</p>
            </div>
          ...
          <div className="mb-4">
            <p className="text-[11px] italic text-gray-600 bg-gray-100 py-1.5 px-3 rounded inline-block">
              Nepal TDS is governed by Income Tax Act 2058. Rates are set by IRD (Inland Revenue Department). Section 88: resident payments, Section 88K: bank interest, Section 107: non-resident payments.
            </p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#f5f6fa] border-b border-gray-200 sticky top-0 z-10">
                  ...
                  <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">TDS Rate %</th>
                  <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">PAN N/A Rate</th>
                  <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Threshold (NPR)</th>
                ...
                {filteredPayments.length === 0 ? (
                  ...
                  filteredPayments.map((payment, index) => (
                    <tr key={payment.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 font-medium">{payment.name}</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700">{payment.sectionCode}</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{payment.tdsRate}%</td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 text-right">{payment.panNotAvailableRate}%</td>
                      ...
                      <td className="px-3 py-2.5 text-[12px] text-center">
                        <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded-full ${payment.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
      ...
      {showForm && (
        <div className="w-full lg:w-[350px] xl:w-[400px] bg-white flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-20">
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            ...
            <div>
              <label className="text-[11px] font-medium text-gray-600 mb-1 block">Section Code <span className="text-red-500">*</span></label>
              <input
                type="text"
                className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">TDS Rate % <span className="text-red-500">*</span></label>
                <input
                  type="number"
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                  value={form.tdsRate}
              ...
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">PAN N/A Rate %</label>
                <input
                  type="number"
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
                  value={form.panNotAvailableRate}
                  onChange={(e) => setForm({ ...form, panNotAvailableRate: parseFloat(e.target.value) || 0 })}
                  min="0"
                  step="0.01"
                  placeholder="e.g. 3.0"
                />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] font-medium text-gray-600 mb-1 block">Threshold (NPR)</label>
                <input
                  type="number"
                  className="h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] w-full"
