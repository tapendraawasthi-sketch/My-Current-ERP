...
const CostCategoryMaster: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Delete this cost category?")) return;
    const row = (costCategories || []).find((c) => c.id === id);
    if (!row) return;
    const snapshot = { ...row };
    try {
      ...
      toast.undo(`"${row.name}" deleted`, async () => {
        try {
          await addCostCategory({ ...snapshot });
        } catch (error) {
      ...
      >
        <div className="p-4 flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-[15px] font-semibold text-gray-900">Cost Category Master</h1>
              <p className="text-[11px] text-gray-500 mt-0.5">
                Manage cost categories and allocation rules
              </p>
            </div>
                ...
                {filteredCategories.length === 0 ? (
                  ...
                  filteredCategories.map((category, index) => (
                    <tr key={category.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2.5 text-[12px] text-gray-700">
                        {category.allocateRevenueItems ? "Yes" : "No"}
                      </td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700">
                        {category.allocateNonRevenueItems ? "Yes" : "No"}
                      </td>
                      <td className="px-3 py-2.5 text-[12px] text-gray-700">
                        {category.allowParallelAllocation ? "Yes" : "No"}
                      </td>
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            <div>
              <label className="text-[11px] font-medium text-gray-600 mb-1 block">
                Cost Category Name <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
