...
import { Users, Calculator, FileText, Download, Calendar, DollarSign, CheckCircle, UserCheck, Clock } from "lucide-react";
...
const PayrollRun: React.FC = () => {
  const { employees, payHeads, companySettings, currentFiscalYear, addVoucher, accounts } = useStore();
  ...
  const handleLoadEmployees = () => {
    ...
    employees.filter(emp => emp.isActive).forEach(emp => {
  ...
  function computeEmployeeSalary(emp, payHeads, presentDays, workingDays, otHrs, advDeduct, otherDeduct, arrears, bonus) {
    const basic = Number(emp.salaryDetails?.basicSalary || emp.basicSalary || 0);
    
    // Compute each payHead
    let earnTotal = basic;
    (payHeads || []).filter(ph => ph.payHeadType === "earnings" || ph.category === "earnings").forEach(ph => {
    ...
    const empSSFEmployer = emp.isSSFContributor ? basic * 0.20 : 0;
    
    // Nepal PF: employee 10%, employer 10% of basic
    const empPF = emp.isPFContributor ? basic * 0.10 : 0;
    const empPFEmployer = emp.isPFContributor ? basic * 0.10 : 0;
    
    // CIT (Citizen Investment Trust): optional 10% of basic
    const cit = emp.isCITContributor ? basic * 0.10 : 0;
    ...
    function computeAnnualTax(income, isWoman) {
      ...
      else if (income <= 700000) tax = 5000 + (income - 500000) * 0.10;
      else if (income <= 1000000) tax = 25000 + (income - 700000) * 0.20;
      else if (income <= 2000000) tax = 85000 + (income - 1000000) * 0.30;
      else tax = 385000 + (income - 2000000) * 0.36;
      if (isWoman) tax = tax * 0.90;
  ...
  const handleComputeSalary = () => {
    const results = Object.values(attendance).map(record => {
  ...
  const handlePostToAccounts = async () => {
    try {
      ...
      const salaryExpenseAccount = accounts.find(a => a.name.toLowerCase().includes("salary") && a.type === "expense");
      const netSalaryPayableAccount = accounts.find(a => a.name.toLowerCase().includes("net salary payable"));
      ...
      const incomeTaxPayableAccount = accounts.find(a => a.name.toLowerCase().includes("tax payable"));
      const employerSSFAccount = accounts.find(a => a.name.toLowerCase().includes("employer ssf"));
      ...
      const db = getDB();
      await db.payrollRecords.bulkAdd(payrollResults.map(result => ({
        id: generateId(),
        ...result,
        month: selectedMonth,
        year: selectedYear,
  ...
  const handleExportExcel = () => {
    const headers = [
      "Employee Name", "Gross", "SSF Emp", "PF Emp", "Tax TDS", "CIT", "OT Pay", 
      "Advance", "Other", "Total Deductions", "Net Salary", "Employer Cost"
    ];
  ...
  const handleExportBank = () => {
    ...
    link.setAttribute("download", `bank_transfer_${selectedYear}_${nepaliMonths[selectedMonth - 1]}.csv`);
  ...
  const handleExportTaxCertificate = () => {
    payrollResults.forEach(result => {
      ...
      doc.save(`tax_certificate_${result.employeeName}_${selectedYear}_${nepaliMonths[selectedMonth - 1]}.pdf`);
    });
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
          {[1, 2, 3, 4, 5].map(num => (
            <div key={num} className="flex flex-col items-center">
              <div className={`w-9 h-9 rounded-full flex items-center justify-center font-semibold shadow-sm transition-colors text-[13px] ${
                step >= num ? 'bg-[#1557b0] text-white border-transparent' : 'bg-gray-100 text-gray-500 border border-gray-200'
              }`}>
                {num}
              </div>
              <div className={`text-[10px] mt-1.5 font-medium uppercase tracking-wide ${step >= num ? 'text-[#1557b0]' : 'text-gray-400'}`}>
                {num === 1 && "Setup"}
                {num === 2 && "Attendance"}
                {num === 3 && "Compute"}
                {num === 4 && "Summary"}
        ...
        {step === 1 && (
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-5">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1">Payroll Date</label>
                <input
                  type="date"
                  value={payrollDate}
              ...
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Emp Code</th>
                    ...
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Present Days</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-red-600">Absent Days</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-amber-600">OT Hours</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Advance Deduct</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Other Deduct</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Arrears</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-green-600">Bonus</th>
                    <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-12">Edit</th>
                  </tr>
                </thead>
                <tbody>
                  {Object.values(attendance).map((record: any) => (
                    <tr key={record.empId} className="border-b border-gray-100 hover:bg-gray-50 group">
                      <td className="px-3 py-2 text-[12px] text-gray-700 font-mono">
                        {editingCell?.empId === record.empId && editingCell?.field === 'empCode' ? (
                          ...
                          <span onClick={() => startEditing(record.empId, 'empCode', record.empCode)} className="cursor-pointer hover:text-[#1557b0] block w-full">
                            {record.empCode || "-"}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-gray-800 font-medium">{record.name}</td>
                      <td className="px-3 py-2 text-[12px] text-gray-600">
                        {editingCell?.empId === record.empId && editingCell?.field === 'department' ? (
                          <input
                            type="text"
                            value={editValue}
                            onChange={(e) => setEditValue(e.target.value)}
                          ...
                          <span onClick={() => startEditing(record.empId, 'department', record.department)} className="cursor-pointer hover:text-[#1557b0] block w-full">
                            {record.department || "-"}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-gray-700 text-right">
                        {editingCell?.empId === record.empId && editingCell?.field === 'basicSalary' ? (
                          <input
                            type="number"
                            value={editValue}
                            onChange={(e) => setEditValue(e.target.value)}
                          ...
                          <span onClick={() => startEditing(record.empId, 'basicSalary', record.basicSalary)} className="cursor-pointer hover:text-[#1557b0] block w-full">
                            {money(record.basicSalary)}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-gray-800 font-medium text-right bg-blue-50/30">
                        {editingCell?.empId === record.empId && editingCell?.field === 'presentDays' ? (
                          <input
                            type="number"
                            value={editValue}
                            onChange={(e) => setEditValue(e.target.value)}
                          ...
                          <span onClick={() => startEditing(record.empId, 'presentDays', record.presentDays)} className="cursor-pointer hover:text-[#1557b0] block w-full">
                            {record.presentDays}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-red-600 font-medium text-right bg-red-50/30">
                        {editingCell?.empId === record.empId && editingCell?.field === 'absentDays' ? (
                          <input
                            type="number"
                            value={editValue}
                            onChange={(e) => setEditValue(e.target.value)}
                          ...
                          <span onClick={() => startEditing(record.empId, 'absentDays', record.absentDays)} className="cursor-pointer hover:text-[#1557b0] block w-full">
                            {record.absentDays}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-amber-700 font-medium text-right bg-amber-50/30">
                        {editingCell?.empId === record.empId && editingCell?.field === 'otHours' ? (
                          ...
                          <span onClick={() => startEditing(record.empId, 'otHours', record.otHours)} className="cursor-pointer hover:text-[#1557b0] block w-full">
                            {record.otHours}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-gray-700 text-right">
                        {editingCell?.empId === record.empId && editingCell?.field === 'advanceDeduct' ? (
                          <input
                            type="number"
                            value={editValue}
                            onChange={(e) => setEditValue(e.target.value)}
                          ...
                          <span onClick={() => startEditing(record.empId, 'advanceDeduct', record.advanceDeduct)} className="cursor-pointer hover:text-[#1557b0] block w-full text-red-600">
                            {money(record.advanceDeduct)}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-gray-700 text-right">
                        {editingCell?.empId === record.empId && editingCell?.field === 'otherDeduct' ? (
                          <input
                            type="number"
                            value={editValue}
                            onChange={(e) => setEditValue(e.target.value)}
                          ...
                          <span onClick={() => startEditing(record.empId, 'otherDeduct', record.otherDeduct)} className="cursor-pointer hover:text-[#1557b0] block w-full text-red-600">
                      ...
                      <td className="px-3 py-2 text-[12px] text-gray-700 text-right">
                        {editingCell?.empId === record.empId && editingCell?.field === 'arrears' ? (
                          ...
                          <span onClick={() => startEditing(record.empId, 'arrears', record.arrears)} className="cursor-pointer hover:text-[#1557b0] block w-full text-green-600">
                      ...
                      <td className="px-3 py-2 text-[12px] text-gray-700 text-right">
                        {editingCell?.empId === record.empId && editingCell?.field === 'bonus' ? (
                          ...
                          <span onClick={() => startEditing(record.empId, 'bonus', record.bonus)} className="cursor-pointer hover:text-[#1557b0] block w-full text-green-600">
                            {money(record.bonus)}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-center text-gray-400 group-hover:text-blue-500 transition-colors">
                        <button 
                          ...
                          onClick={() => startEditing(record.empId, 'basicSalary', record.basicSalary)}
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-max border-collapse">
                <thead>
                  <tr className="bg-[#f5f6fa] border-b border-gray-200">
                    <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Employee Name</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Gross</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">SSF Emp</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">PF Emp</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Tax TDS</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">CIT</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">OT Pay</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Advance</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Other</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Total Deductions</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-[#1557b0]">Net Salary</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Employer Cost</th>
                  </tr>
                </thead>
                <tbody>
                  {payrollResults.map(result => (
                    <tr key={result.employeeId} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="px-3 py-2 text-[12px] text-gray-800 font-medium">{result.employeeName}</td>
                      <td className="px-3 py-2 text-[12px] text-gray-700 text-right">{money(result.grossEarnings)}</td>
                      <td className="px-3 py-2 text-[12px] text-red-600 text-right">{money(result.empSSF)}</td>
                      <td className="px-3 py-2 text-[12px] text-red-600 text-right">{money(result.empPF)}</td>
                      <td className="px-3 py-2 text-[12px] text-red-600 text-right">{money(result.monthlyTax)}</td>
                      <td className="px-3 py-2 text-[12px] text-red-600 text-right">{money(result.cit)}</td>
                      <td className="px-3 py-2 text-[12px] text-green-600 text-right">{money(result.otPay)}</td>
                      <td className="px-3 py-2 text-[12px] text-red-600 text-right">{money(result.advDeduct)}</td>
                      <td className="px-3 py-2 text-[12px] text-red-600 text-right">{money(result.otherDeduct)}</td>
                      <td className="px-3 py-2 text-[12px] text-gray-800 font-medium text-right bg-gray-50/50">{money(result.totalDeductions)}</td>
                      <td className={`px-3 py-2 text-[12px] font-semibold text-right bg-blue-50/30 ${result.netSalary >= 0 ? 'text-[#1557b0]' : 'text-red-600'}`}>
                  ...
                  <tr className="bg-[#eef2ff] border-t-2 border-[#c7d2fe]">
                    ...
                    <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right">{money(totals.grossEarnings)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-red-700 text-right">{money(totals.empSSF)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-red-700 text-right">{money(totals.empPF)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-red-700 text-right">{money(totals.monthlyTax)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-red-700 text-right">{money(totals.cit)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-green-700 text-right">{money(totals.otPay)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-red-700 text-right">{money(totals.advDeduct)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-red-700 text-right">{money(totals.otherDeduct)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right">{money(totals.totalDeductions)}</td>
                    <td className="px-3 py-2.5 text-[13px] font-bold text-[#1557b0] text-right">{money(totals.netSalary)}</td>
                    <td className="px-3 py-2.5 text-[12px] font-bold text-gray-800 text-right">{money(totals.employerCost)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-md shadow-sm p-6 max-w-4xl mx-auto">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-200">
              ...
              <div>
                ...
                <p className="text-[12px] text-gray-500">Journal vouchers have been created. You can now export the required documents.</p>
              </div>
            </div>
