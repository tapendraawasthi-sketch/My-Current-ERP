  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow space-y-4">
          ...
          <div className="border-t pt-4">
            <button
              ...
              className="w-full flex items-center justify-center space-x-2 h-9 px-3 bg-[#3D6B25] hover:bg-[#2D5A1A] text-white text-[12px] font-medium rounded-md disabled:opacity-50 transition-colors"
            >
              {isExporting ? <Spinner size="sm" /> : <Download className="w-4 h-4" />}
              <span>{isExporting ? "Exporting…" : "Download Backup Now"}</span>
            </button>
            <div className="text-[11px] text-[#000000] mt-2 flex items-center gap-1">
        ...
        <div className="bg-white p-6 rounded-lg shadow space-y-4">
          ...
          <div className="border-t pt-4 space-y-4">
            <div>
              <label className="block text-sm font-medium text-[#000000] mb-2">Upload Backup File (.json)</label>
              <input
                type="file"
                accept=".json,application/json"
                onChange={handleFileUpload}
                className="w-full text-[12px] text-[#000000] file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:bg-[#3D6B25] file:text-white file:text-[11px] file:cursor-pointer"
              />
            </div>
 
            {backupPreview && (
              <div className="bg-[#D4EABD] p-4 rounded space-y-2 border border-[#9DC07A]">
                <h3 className="font-semibold text-[13px] text-[#000000]">Backup Preview</h3>
                {backupPreview.meta && (
                ...
                <div className="text-[12px] space-y-1 text-[#000000]">
            ...
            <button
              ...
              className="w-full flex items-center justify-center space-x-2 h-9 px-3 bg-orange-600 hover:bg-orange-700 text-[#000000] text-[12px] font-medium rounded-md disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
      ...
      <div className="bg-white p-6 rounded-lg shadow space-y-4">
        ...
        {loadingHistory ? (
          <div className="text-center py-6 text-[#000000] text-sm">Loading history…</div>
        ) : history.length === 0 ? (
          <div className="text-center py-6 text-[#000000] text-sm">No backup records yet.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-[12px]">
              <thead className="bg-[#f5f6fa] border-b border-[#9DC07A]">
                <tr>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#000000] uppercase">Date (BS)</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#000000] uppercase">Filename</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#000000] uppercase">Type</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#000000] uppercase">Size</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#000000] uppercase">Rows</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-[#000000] uppercase">Status</th>
                </tr>
              </thead>
              <tbody>
                {history.map((rec) => (
                  <tr key={rec.id} className="border-b border-[#9DC07A] hover:bg-[#EBF5E2]">
                    <td className="px-3 py-2 text-[#000000]">{formatBSDate(rec.created_at)}</td>
                    <td className="px-3 py-2 text-[#000000] max-w-[200px] truncate font-mono text-[10px]" title={rec.filename}>{rec.filename}</td>
                    <td className="px-3 py-2">
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase ${
                        rec.backup_type === "restore" ? "bg-orange-100 text-orange-700" : "bg-[#D4EABD] text-[#000000]"
                      }`}>{rec.backup_type}</span>
                    </td>
                    <td className="px-3 py-2 text-[#000000]">{formatFileSize(rec.file_size_bytes)}</td>
                    <td className="px-3 py-2 text-[#000000]">{rec.rows_backed_up?.toLocaleString() || "—"}</td>
                    <td className="px-3 py-2">
      ...
      <div className="bg-white p-6 rounded-lg shadow opacity-60">
        <div className="flex items-center space-x-3 mb-3">
          <Cloud className="w-6 h-6 text-[#000000]" />
        ...
        <p className="text-sm text-[#000000]">Automatically backup to Google Drive or AWS S3.</p>
      </div>
