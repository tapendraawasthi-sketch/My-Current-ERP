...
const VouchersRegister: React.FC = () => {
  ...
  const paginatedRows = useMemo(() => {
    ...
    const hasTotal =
      registerData.rows.length > 0 && registerData.rows[registerData.rows.length - 1].isTotal;
          ...
          <ReportGrid
            ...
            getRowClassName={(row) => {
              ...
              if (row.status === "cancelled")
                return "bg-red-50 opacity-75 hover:opacity-100 hover:bg-red-100";
              return "";
            }}
          ...
          <div className="flex justify-between items-center px-4 py-3 bg-gray-50 border-t border-gray-200 sm:px-6 mt-auto">
            <div className="text-[12px] text-gray-700">
              ...
              <span className="font-semibold text-gray-900">
                {Math.min((page - 1) * pageSize + 1, registerData.filteredCount)}
              </span>{" "}
              to{" "}
              <span className="font-semibold text-gray-900">
                {Math.min(page * pageSize, registerData.filteredCount)}
              </span>{" "}
              of <span className="font-semibold text-gray-900">{registerData.filteredCount}</span>{" "}
              entries
            </div>
