...
export default function ItemProfitabilityReport() {
  ...
  }, [items, invoices, fromDate, toDate, groupFilter, salespersonFilter, viewMode, searchTerm, itemGroups]);
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(profitData.map((item, idx) => ({
      'Rank': idx + 1,
  ...
  return (
    <div style={{ backgroundColor: BG, minHeight: '100vh', padding: '20px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#000000', marginBottom: '20px' }}>Item Profitability Report</h1>
      ...
      <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'end' }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>View</label>
            <select
              value={viewMode}
          ...
          <div style={{ flex: 1 }}>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Search</label>
            <input
              type="text"
              placeholder="Search items..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              style={{ padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px', width: '100%' }}
            />
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '20px' }}>
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Total Revenue</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(summary.totalRevenue)}</div>
        </div>
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Total COGS</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(summary.totalCOGS)}</div>
        </div>
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Total Gross Profit</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(summary.totalGP)}</div>
        </div>
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Overall Margin %</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{summary.overallMargin.toFixed(2)}%</div>
        </div>
      </div>
      
      {/* Chart */}
      <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
        <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Gross Margin by Item</h2>
              ...
              <Bar dataKey="margin" name="Gross Margin %">
                {chartData.map((entry, index) => (
                  <Cell 
                    key={`cell-${index}`} 
                    fill={
                      entry.margin >= 20 ? '#059669' : 
                      entry.margin >= 10 ? '#d97706' : 
                      '#dc2626'
                    } 
                  />
                ))}
      ...
      <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
        <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>
          {viewMode === 'top20' ? 'Top 20 Profitable Items' : 
           viewMode === 'bottom20' ? 'Bottom 20 Profitable Items' : 
           'All Items'}
        </h2>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            <thead>
              <tr style={{ backgroundColor: BG_HEADER }}>
                ...
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Avg Selling Rate</th>
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Total Revenue</th>
                <th style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>Avg Purchase Rate</th>
            ...
            <tbody>
              {profitData.length > 0 ? (
                profitData.map((item, idx) => {
                  const marginColor = 
                    item.marginPct >= 20 ? '#059669' : 
                    item.marginPct >= 10 ? '#d97706' : 
                    '#dc2626';
                  
                  const rowBgColor = item.marginPct < 0 ? '#fee2e2' : (idx % 2 === 0 ? BG_DEEP : 'transparent');
                  ...
                  return (
                    <tr key={item.item.id} style={{ backgroundColor: rowBgColor }}>
                      ...
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{item.unitsSold}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.avgSaleRate)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.revenue)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.avgPurchRate)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.cogs)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.grossProfit)}</td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'right', color: marginColor, fontWeight: 'bold' }}>
                        {item.marginPct.toFixed(2)}%
                      </td>
                      <td style={{ border: BORDER, padding: '8px', textAlign: 'center' }}>
                        {item.marginPct > 0 ? <TrendingUp size={16} style={{ color: '#059669' }} /> : <TrendingDown size={16} style={{ color: '#dc2626' }} />}
                      </td>
                    </tr>
                  );
                })
      ...
      {lossMakingItems.length > 0 && (
        <div style={{ backgroundColor: '#fee2e2', padding: '15px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#dc2626', marginBottom: '10px' }}>Items Selling at a Loss</h2>
          <div style={{ backgroundColor: '#fecaca', padding: '10px', borderRadius: '6px', marginBottom: '15px', color: '#991b1b', fontWeight: 'bold' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
              <thead>
                <tr style={{ backgroundColor: '#f87171', color: '#fff' }}>
                  ...
                  <th style={{ border: BORDER, padding: '8px', color: '#fff', textAlign: 'right' }}>Revenue</th>
                  <th style={{ border: BORDER, padding: '8px', color: '#fff', textAlign: 'right' }}>COGS</th>
                  <th style={{ border: BORDER, padding: '8px', color: '#fff', textAlign: 'right' }}>Gross Profit</th>
                  <th style={{ border: BORDER, padding: '8px', color: '#fff', textAlign: 'right' }}>Margin %</th>
                </tr>
              </thead>
              <tbody>
                {lossMakingItems.map(item => (
                  <tr key={`loss-${item.item.id}`}>
                    ...
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.revenue)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(item.cogs)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right', color: '#dc2626', fontWeight: 'bold' }}>{money(item.grossProfit)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right', color: '#dc2626', fontWeight: 'bold' }}>{item.marginPct.toFixed(2)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
