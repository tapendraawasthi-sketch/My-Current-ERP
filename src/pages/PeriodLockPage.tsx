...
const dangerBtn = "h-8 px-3 bg-red-600 hover:bg-red-700 text-white text-[12px] font-medium rounded-md flex items-center justify-center gap-1.5 transition-colors shadow-sm disabled:opacity-50";
const inputClass = "h-8 px-2.5 text-[12px] border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-[#1557b0]/20 focus:border-[#1557b0] transition-shadow w-full";
...
export default function PeriodLockPage() {
  ...
  async function runAutoLock() {
    ...
    for (const p of periods) {
      ...
      if (daysPast > autoLockDays && !alreadyLocked) {
        ...
        await db.table("periodLocks").put(row).catch(() => {});
  ...
  async function confirmPeriodAction() {
    ...
    if (actionType === "unlock") {
      ...
      if (lock) {
        await db.table("periodLocks").delete(lock.id).catch(() => {});
  ...
  async function savePin() {
    ...
    const db = getDB();
    const existing = await db.table("securitySettings").get("global").catch(() => null);
  ...
  function runHealthCheck() {
    ...
    const invoicePartyIds = Array.from(new Set((invoices || []).map((i) => i.partyId).filter(Boolean)));
    ...
    const invoicesNoParty = (invoices || []).filter((i) => !i.partyId && i.type === "sales-invoice");
  ...
  async function autoFix(check: any) {
    if (check.issue === "Items with no unit of measure") {
      ...
      for (const item of rows) {
        await db.table("items").update(item.id, { unit: "PCS", uom: "PCS" }).catch(() => {});
      }
  ...
  return (
    <div className="min-h-screen bg-[#f5f6fa] p-4 text-gray-800">
      ...
      {activeTab === "Period Lock" && (
        <div className={cardClass}>
          <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-2">
            <h2 className="text-[14px] font-semibold text-gray-800">Accounting Periods - {currentFiscalYear?.name}</h2>
          </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {[
                    ...
                    <th key={h} className={h === 'Vouchers' ? `${tableHeadClass} text-right` : tableHeadClass}>{h}</th>
                  ))}
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-gray-100">
                {periods.map((p) => {
                  ...
                  return (
                    <tr key={p.periodKey} className={isLocked ? "bg-red-50/50 hover:bg-red-50" : "bg-white hover:bg-gray-50"}>
                      ...
                      <td className={tableCellClass}>{lock?.lockedAt ? new Date(lock.lockedAt).toLocaleString() : ""}</td>
                      <td className={tableCellClass}>
                        {isLocked ? (
                          <button 
      ...
      {activeTab === "Lock Settings" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className={cardClass}>
            ...
            <div className={`p-3 rounded-md border text-[12px] mb-4 flex items-center gap-2 ${requiredPin ? "bg-green-50 border-green-200 text-green-800" : "bg-amber-50 border-amber-200 text-amber-800"}`}>
               {requiredPin ? <CheckCircle size={16}/> : <AlertTriangle size={16}/>}
               Current Period Lock PIN is: <b className="uppercase">{requiredPin ? "Set" : "Not Set"}</b>
            </div>

            <div className="space-y-4">
               <div>
                  <label className="block text-[11px] font-medium text-gray-600 mb-1">Set New PIN (4 digits)</label>
                  <input
               ...
               <div>
                  <label className="block text-[11px] font-medium text-gray-600 mb-1">Confirm PIN</label>
                  <input
                    className={inputClass}
                    type="password"
                    maxLength={4}
               ...
               <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                  <button className="text-[11px] text-red-600 hover:text-red-800 font-medium transition-colors" onClick={clearPin}>
                    Clear PIN (Disable Protection)
                  </button>
                  <button className={primaryBtn} onClick={savePin}>Save PIN</button>
               </div>
            </div>
          </div>
          ...
          <div className={cardClass}>
            ...
            <p className="text-[11px] text-gray-500 mb-4">Automatically lock accounting periods after a specified number of days past the period end date to ensure compliance.</p>
            ...
            <div className="space-y-4">
               <label className="flex items-center gap-2 cursor-pointer p-3 border border-gray-200 rounded-md bg-gray-50 hover:bg-gray-100 transition-colors">
                 ...
                 />
                 <div className="text-[12px] font-medium text-gray-700">Enable Auto-Lock for periods</div>
               </label>

               <div className={autoLockEnabled ? "opacity-100" : "opacity-50 pointer-events-none"}>
                  <label className="block text-[11px] font-medium text-gray-600 mb-1">Grace Period (Days past period end)</label>
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4 items-center">
              <div>
                <h2 className="text-[14px] font-semibold text-gray-800 flex items-center gap-2 mb-1">
                  <Activity size={16} className={healthScore >= 90 ? "text-green-600" : healthScore >= 60 ? "text-amber-500" : "text-red-600"}/> 
                  Data Health Monitor
                </h2>
                <p className="text-[11px] text-gray-500">Scan your accounting, inventory, and party data for structural integrity issues.</p>
              </div>

              <div className="flex items-center gap-6">
                 <div className="text-center">
                   <div className="text-[10px] font-bold uppercase tracking-wide text-gray-500 mb-0.5">Health Score</div>
                   <div className={`text-[24px] font-black leading-none ${healthScore >= 90 ? "text-green-600" : healthScore >= 60 ? "text-amber-500" : "text-red-600"}`}>
                     {healthScore}%
                   </div>
                 </div>
          ...
          <div className={cardClass}>
            <div className="overflow-x-auto rounded-md border border-gray-200">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    {["Category", "Issue Description", "Issues Found", "Severity", "Auto-Fix", "Action"].map((h) => (
                ...
                <tbody className="divide-y divide-gray-100">
                  {healthChecks.map((c, idx) => (
                    <React.Fragment key={idx}>
                      <tr className="bg-white hover:bg-gray-50 transition-colors">
                        ...
                        <td className={tableCellClass}>
                          <span className={`${severityClass(c.severity)} px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide`}>
                            {c.severity}
                          </span>
                        </td>
                        <td className={tableCellClass}>
                           {c.canAutoFix ? (
                              <span className="text-green-600 font-medium text-[11px]">Available</span>
                           ) : (
                        ...
                        <td className={tableCellClass}>
                          <div className="flex items-center gap-2">
                             ...
                             {c.canAutoFix && c.count > 0 && (
                               <button className="text-[11px] font-medium text-amber-600 hover:underline flex items-center gap-1" onClick={() => autoFix(c)}>
                      ...
                      {expandedHealthIndex === idx && (
                        <tr>
                          <td colSpan={6} className="bg-gray-50 p-4 border-b border-gray-200 shadow-inner">
                            <div className="font-semibold text-[12px] mb-3 text-gray-700 flex items-center gap-2">
                               <AlertCircle size={14} className="text-amber-500"/> Sample of Affected Records
                            </div>
                            {(c.records || []).length === 0 ? (
                              <div className="text-[11px] text-gray-500 italic">No record sample available.</div>
                            ) : (
                                ...
                                <table className="w-full border-collapse">
                                  <thead className="bg-gray-100 sticky top-0">
                                    <tr>
                                      ...
                                      <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">Name / Reference</th>
                                      <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase">Details</th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y divide-gray-100">
                                    {(c.records || []).map((r, ri) => (
                                      <tr key={ri} className="hover:bg-gray-50">
                                        <td className="px-3 py-1.5 text-[11px] text-gray-500 font-mono">{r.id?.substring(0,8) || "..."}</td>
                                        <td className="px-3 py-1.5 text-[11px] font-medium text-gray-800">{r.name || r.invoiceNo || r.voucherNo || "N/A"}</td>
                                        <td className="px-3 py-1.5 text-[11px] text-gray-600">
                                          {r.date ? <span className="mr-2">Date: {r.date}</span> : ""}
                                          {r.type ? <span className="mr-2">Type: <span className="bg-gray-100 px-1 rounded">{r.type}</span></span> : ""}
                                          {r.status ? <span className="mr-2">Status: <span className="bg-gray-100 px-1 rounded">{r.status}</span></span> : ""}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
                  ...
                  {!healthChecks.length && (
                    <tr>
                      <td colSpan={6} className="text-center p-10 text-gray-500 text-[12px] bg-gray-50">
                        <Activity size={24} className="mx-auto text-gray-400 mb-2 opacity-50"/>
                        <p>No health scan results available.</p>
                        <p className="mt-1">Click "Run Scan" to check your books for structural integrity issues.</p>
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      >
        {actionPeriod && (
          <div className="space-y-4">
            <div className={`p-3 rounded-md border text-[12px] flex items-start gap-2 ${actionType === 'lock' ? 'bg-amber-50 border-amber-200 text-amber-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
              <AlertTriangle size={16} className="flex-shrink-0 mt-0.5"/>
              <div>
                You are about to <b>{actionType}</b> the accounting period for <b>{actionPeriod.label}</b>.
                {actionType === 'lock' && <p className="mt-1">This will prevent any new vouchers from being posted or edited in this date range.</p>}
                {actionType === 'unlock' && <p className="mt-1">This will allow users to modify historical records, potentially affecting financial reports.</p>}
              </div>
            </div>

            {requiredPin && (
              <div>
                <label className="block text-[11px] font-medium text-gray-600 mb-1">Enter Admin PIN</label>
                <input
                  className={inputClass}
                  type="password"
                  maxLength={4}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
              <button className={outlineBtn} onClick={() => setConfirmModal(false)}>Cancel</button>
              <button
