    ...
    <div className="page-wrapper">
      ...
      <div className="flex flex-wrap items-center gap-3 px-4 py-2 bg-[#EBF5E2] border-b border-black">
        ...
        <div className="flex flex-col gap-0.5">
          <label className="text-[9px] font-bold uppercase text-gray-600 tracking-wide">Stock Item *</label>
          <select
        ...
        {/* Warehouse */}
        <div className="flex flex-col gap-0.5">
          <label className="text-[9px] font-bold uppercase text-gray-600 tracking-wide">Warehouse</label>
          <select
        ...
        {/* From date */}
        <div className="flex flex-col gap-0.5">
          <label className="text-[9px] font-bold uppercase text-gray-600 tracking-wide">From Date</label>
          <input
            type="date"
            value={fromDate}
        ...
        {/* To date */}
        <div className="flex flex-col gap-0.5">
          <label className="text-[9px] font-bold uppercase text-gray-600 tracking-wide">To Date</label>
          <input
            type="date"
            value={toDate}
      ...
      {result && (
        <div className="grid grid-cols-5 gap-0 border-b border-black bg-[#D4EABD]">
          {[
            { label: 'Opening Qty',    val: result.openingQty,  rate: result.openingRate,   value: result.openingValue },
            { label: 'Total Inward',   val: result.totalInQty,  rate: null,                 value: result.totalInValue },
            { label: 'Total Outward',  val: result.totalOutQty, rate: null,                 value: result.totalOutValue },
            { label: 'Closing Qty',    val: result.closingQty,  rate: result.weightedAvgRate, value: result.closingValue },
          ].map((k, i) => (
            <div key={i} className="px-4 py-2 border-r border-black last:border-r-0">
              ...
              {k.rate !== null && (
                <div className="text-[10px] text-gray-600">@ {formatNumber(k.rate)} = <strong>Rs. {formatNumber(k.value)}</strong></div>
              )}
              {k.rate === null && (
                <div className="text-[10px] text-gray-600">Value: <strong>Rs. {formatNumber(k.value)}</strong></div>
              )}
            </div>
          ))}
        ...
        {!selectedItemId ? (
          ...
          <div className="overflow-x-auto">
            <table className="data-table w-full" style={{ minWidth: 1100 }}>
              <thead>
                <tr>
                  <th className="px-3 py-2 text-left whitespace-nowrap" style={{ width: 100 }}>Date (BS)</th>
                  <th className="px-3 py-2 text-left whitespace-nowrap" style={{ width: 110 }}>Voucher No.</th>
                  <th className="px-3 py-2 text-left whitespace-nowrap" style={{ width: 130 }}>Voucher Type</th>
              ...
              <tbody>
                {filteredRows.map((row, idx) => (
                  <tr
                    key={row.movementId}
                    className={idx % 2 === 0 ? 'bg-[#EBF5E2]' : 'bg-white'}
                  >
                    ...
                    <td className="px-3 py-1.5 text-[11px]">
                      <span className={`badge ${
                        row.type === 'opening' ? 'badge-info' :
                        row.inQty > 0 ? 'badge-success' : 'badge-warning'
                      }`}>
                        {row.voucherType}
                      </span>
                    </td>
      ...
      {result && (
        <div className="border-t border-black bg-[#EBF5E2] px-4 py-3">
          <div className="flex flex-wrap gap-6 items-center text-[12px]">
            <div>
              <span className="text-gray-600 font-semibold uppercase text-[10px] tracking-wide">Item: </span>
              <strong>{selectedItem?.name}</strong>
              <span className="ml-2 text-gray-500">({selectedItem?.unit})</span>
            </div>
            <div>
              <span className="text-gray-600 font-semibold uppercase text-[10px] tracking-wide">Closing Qty: </span>
            ...
            <div>
              <span className="text-gray-600 font-semibold uppercase text-[10px] tracking-wide">Closing Value: </span>
              <strong className="font-mono text-[#1557b0]">Rs. {formatNumber(result.closingValue)}</strong>
            </div>
