  ...
  const dashData = useMemo(() => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const mSales = (invoices || [])
        .filter(
          (inv) => isSalesInvoice(inv) && inv.date >= mStart && inv.date <= mEnd && isPosted(inv),
        )
      ...
      const mExp = (invoices || [])
        .filter(
          (inv) =>
            isPurchaseInvoice(inv) && inv.date >= mStart && inv.date <= mEnd && isPosted(inv),
        )
        .reduce((s, inv) => s + Number(inv.grandTotal || inv.amount || 0), 0);
    ...
    <div className="min-h-screen bg-[#f5f6fa] p-4 text-gray-800">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-[12px] text-gray-500 mt-1">
            ...
            Fiscal Year:{" "}
            {currentFiscalYear?.name || currentFiscalYear?.fiscalYearBS || "Current FY"}
            <span className="mx-2 text-gray-300">|</span>
            Today:{" "}
      ...
      {pendingApprovals > 0 && (
        <div className="bg-amber-50 text-amber-800 p-3 rounded-md border border-amber-200 shadow-sm mb-5 flex items-center justify-between">
          <div className="flex items-center gap-2 text-[12px] font-medium">
            ...
            <span>
              <span className="font-bold">{pendingApprovals}</span> voucher(s) pending your
              approval.
            </span>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
          ...
          <div className="flex items-end gap-2 h-[200px] mt-auto relative border-b border-gray-200 pb-1">
            ...
            {dashData.monthlyData.map((m, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2 z-10 group">
                <div
                  className="w-full flex gap-1 justify-center items-end"
                  style={{ height: "170px" }}
                >
                  <div
                    style={{
                      height: m.sales > 0 ? (m.sales / maxVal) * 165 + "px" : "2px",
                      minHeight: 2,
                    }}
        ...
        <div className={cardClass}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              className="text-[11px] text-[#1557b0] font-medium hover:underline"
              onClick={() => navigateToPage("banking")}
            >
              View All
            </button>
          </div>
          ...
          <div className="space-y-1">
            {dashData.cashBankAccounts.length === 0 ? (
              <div className="text-[12px] py-8 text-center text-gray-400 border border-dashed border-gray-200 rounded-md">
              ...
              <>
                {dashData.cashBankAccounts.slice(0, 6).map((acc) => (
                  <div
                    key={acc.id || acc.name}
                    className="flex justify-between items-center p-2 hover:bg-gray-50 rounded-md transition-colors border-b border-gray-50 last:border-0"
                  >
                ...
                {dashData.cashBankAccounts.length > 6 && (
                  <div
                    className="text-[11px] text-center text-gray-500 pt-3 pb-1 font-medium bg-gray-50 rounded-md mt-2 border border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors"
                    onClick={() => navigateToPage("banking")}
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="rounded-md p-3 text-center bg-red-50 border border-red-200 shadow-sm transition-transform hover:-translate-y-0.5 relative overflow-hidden">
              ...
              <div className="font-bold text-[14px] text-red-900 relative z-10">
                {money(dashData.aging.d90plus)}
              </div>
              {dashData.aging.d90plus > 0 && (
                <div className="absolute top-0 right-0 w-8 h-8 bg-red-100 rounded-bl-full -mr-2 -mt-2"></div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className={cardClass}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[14px] font-bold text-gray-800 flex items-center gap-2">
              <Star size={16} className="text-amber-500 fill-amber-500/20" /> Top Customers This
              Month
            </h2>
          </div>

          {dashData.topCustomers.length === 0 ? (
            <div className="text-center text-[12px] text-gray-500 py-8 border border-dashed border-gray-200 rounded-md">
                  ...
                  {dashData.topCustomers.slice(0, 5).map((row, idx) => (
                    <tr key={idx} className="bg-white hover:bg-gray-50 transition-colors">
                      <td className="px-3 py-2.5 text-[12px] text-gray-500 font-medium">
                        <span
                          className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] ${idx === 0 ? "bg-amber-100 text-amber-700 font-bold" : idx === 1 ? "bg-gray-200 text-gray-700 font-bold" : idx === 2 ? "bg-orange-100 text-orange-800 font-bold" : "bg-gray-50"}`}
                        >
                          {idx + 1}
                        </span>
                      </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                    ...
                    <tr
                      ...
                      className={
                        item.computedStock <= 0
                          ...
                          : "bg-orange-50/50 hover:bg-orange-50 transition-colors"
                      }
                    >
                      ...
                      <td className="px-3 py-2 text-[12px] text-center font-bold text-gray-700">
                        {item.computedStock}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-center text-gray-500">
                        {item.reorderLevelValue}
                      </td>
        ...
        <div className={cardClass}>
          ...
          {dashData.expenseBreakdown.length === 0 ? (
            <div className="text-center text-[12px] py-8 text-gray-500 border border-dashed border-gray-200 rounded-md">
            ...
            <div className="space-y-3">
              {dashData.expenseBreakdown.map((acc) => (
                <div key={acc.id || acc.name} className="flex items-center gap-3 group">
                  <span
                    className="text-[11px] w-28 font-medium text-gray-700 truncate"
                    title={acc.name}
                  >
                    {acc.name}
                  </span>
