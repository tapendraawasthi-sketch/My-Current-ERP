            ...
            {schemes.map((scheme) => (
              <tr key={scheme.id} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2.5 text-[12px] font-mono">
                  {scheme.type === "discount_percent"
                    ? `${scheme.discountPercent}%`
                    : scheme.type === "discount_amount"
                      ? `₹${scheme.discountAmount}`
                      : scheme.type === "qty_bonus"
                        ? `Buy ${scheme.minQty} Get ${scheme.bonusQty}`
                        : "—"}
                </td>
                ...
                <td className="px-3 py-2.5">
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-semibold ${scheme.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                  >
                ...
                <td className="px-3 py-2.5">
                  <div className="flex gap-1">
                    ...
                    <button
                      onClick={() => setSchemes((prev) => prev.filter((s) => s.id !== scheme.id))}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((p) => ({ ...p, applicableItems: e.target.value as any }))
                    }
                    className={`${inputCls} w-full`}
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((p) => ({ ...p, applicableParties: e.target.value as any }))
                    }
                    className={`${inputCls} w-full`}
                  >
