        ...
        <table className="w-full text-left whitespace-nowrap">
          <thead>
            <tr className="bg-[#f5f6fa] border-b border-gray-200">
              ...
              <th className="px-3 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Particulars</th>
              <th className="px-3 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-right">Debit (Rs.)</th>
              <th className="px-3 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-right">Credit (Rs.)</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            ...
            {registerData.rows.map((row, index) => {
              ...
              if (row.type === "narration") {
                return (
                  <tr key={row.id} className={`bg-white hover:bg-gray-50 transition-colors ${cancelClass}`}>
                    <td className="px-3 py-1"></td>
                    <td className="px-3 py-1"></td>
                    <td className="px-3 py-1 text-left text-[11px] italic text-gray-500 whitespace-normal break-words max-w-lg">{row.particulars}</td>
              ...
              return (
                <tr key={row.id} className={`bg-white hover:bg-gray-50 transition-colors ${isFirstLine ? "font-medium" : ""} ${cancelClass}`}>
                  <td className="px-3 py-1.5 text-[12px]">{row.date}</td>
                  <td className="px-3 py-1.5 text-[12px]">{row.voucherNo}</td>
                  <td className={`px-3 py-1.5 text-[12px] text-left ${row.particulars.startsWith("  To ") ? "pl-8" : ""}`}>
                    {row.particulars}
                  </td>
                  <td className="px-3 py-1.5 text-[12px] text-right font-mono" style={{ color: !isCancelled && row.debit > 0 ? "#1557b0" : "inherit" }}>
                    {row.debit > 0 ? formatNumber(row.debit) : ""}
                  </td>
                  <td className="px-3 py-1.5 text-[12px] text-right font-mono" style={{ color: !isCancelled && row.credit > 0 ? "#dc2626" : "inherit" }}>
            ...
            {registerData.rows.length > 0 && (
              <tr className="bg-[#eef2ff] border-t-2 border-[#c7d2fe]">
                ...
                <td className="px-3 py-2.5"></td>
                <td className="px-3 py-2.5 text-[12px] font-bold font-mono text-gray-800 text-right">{formatNumber(registerData.grandTotalDebit)}</td>
                <td className="px-3 py-2.5 text-[12px] font-bold font-mono text-gray-800 text-right">{formatNumber(registerData.grandTotalCredit)}</td>
              </tr>
            )}
            
            {/* Verification row */}
            {registerData.rows.length > 0 && (
              <tr className={`border-t border-gray-200 ${Math.abs(registerData.grandTotalDebit - registerData.grandTotalCredit) < 0.01 ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>
                <td colSpan={5} className="px-3 py-3 text-[12px] font-bold text-center border-t border-gray-200">
