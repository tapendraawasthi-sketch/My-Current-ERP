...
const TrialBalance: React.FC = () => {
  ...
  const previousRows = useMemo(() => {
    return computeTrialBalance(accounts, vouchers, previousRange.startDate, previousRange.endDate);
  }, [accounts, vouchers, previousRange.startDate, previousRange.endDate]);
      ...
      <Card border padding="md" className="bg-slate-50">
        ...
        >
          {isBalanced
            ...
            : `⚠ Out of Balance — Difference: ${formatNumber(Math.abs(totals.closingDr - totals.closingCr))}`}
        </div>
      </Card>
            ...
            <table className="data-table sticky-thead">
              ...
              <tbody>
                {(displayMode === "condensed" ? condensedRows : filteredRows).map((row) => (
                  <tr key={row.accountId} onClick={() => handleRowClick(row)} className="cursor-pointer">
                    ...
                    <td className="amt amt-dr">{row.closingDr ? formatNumber(row.closingDr) : "—"}</td>
                    <td className="amt amt-cr">{row.closingCr ? formatNumber(row.closingCr) : "—"}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background: "#16213e", color: "white" }}>
                  <td colSpan={2} className="px-3 py-2.5 font-bold text-[12px] uppercase">Grand Total</td>
                  ...
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(grandTotalDebit)}</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(grandTotalCredit)}</td>
                </tr>
                <tr style={{ background: "#111827", color: "white" }}>
                  <td colSpan={6} className="px-3 py-1.5 font-bold text-[10px] uppercase">Balanced Status</td>
            ...
            <Card key={group.key} border padding="none">
              ...
              <div className="overflow-x-auto">
                <table className="data-table sticky-thead">
                  ...
                  <tbody>
                    {group.rows.map((row) => (
                      <tr key={row.accountId} onClick={() => handleRowClick(row as any)} className="cursor-pointer">
              ...
              <tbody>
                {comparisonRows.map((row) => (
                  <tr key={row.accountId} onClick={() => handleRowClick(row)} className="cursor-pointer">
                    ...
                    <td className="amt amt-dr">{row.closingDr ? formatNumber(row.closingDr) : "—"}</td>
                    <td className="amt amt-cr">{row.closingCr ? formatNumber(row.closingCr) : "—"}</td>
                    <td className="amt amt-dr">{row.prevDebit ? formatNumber(row.prevDebit) : "—"}</td>
                    <td className="amt amt-cr">{row.prevCredit ? formatNumber(row.prevCredit) : "—"}</td>
                    <td className="amt amt-dr">{row.prevClosingDr ? formatNumber(row.prevClosingDr) : "—"}</td>
                    <td className="amt amt-cr">{row.prevClosingCr ? formatNumber(row.prevClosingCr) : "—"}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background: "#16213e", color: "white" }}>
                  <td colSpan={2} className="px-3 py-2.5 font-bold text-[12px] uppercase">Grand Total</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(comparisonTotals.debit)}</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(comparisonTotals.credit)}</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(comparisonTotals.closingDr)}</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(comparisonTotals.closingCr)}</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(comparisonTotals.prevDebit)}</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(comparisonTotals.prevCredit)}</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(comparisonTotals.prevClosingDr)}</td>
                  <td className="text-right px-3 font-mono font-bold text-[13px]">{formatNumber(comparisonTotals.prevClosingCr)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
