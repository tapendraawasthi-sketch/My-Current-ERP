  ...
  const reportData = useMemo(() => {
    ...
    if (typeFilter === "receivables") {
      filteredInvoices = filteredInvoices.filter(
        (inv) => inv.type === "sales-invoice" || inv.type === "sales-return",
      );
    } else if (typeFilter === "payables") {
      filteredInvoices = filteredInvoices.filter(
        (inv) => inv.type === "purchase-invoice" || inv.type === "purchase-return",
      );
    }
    ...
    if (groupedData.length > 0) {
      const grandTotalPaid = groupedData
        .filter((row) => row.isInvoice)
        .reduce((sum, row) => sum + (row.paidAmount || 0), 0);
      const grandTotalPending = groupedData
        .filter((row) => row.isInvoice)
        .reduce((sum, row) => sum + (row.pendingAmount || 0), 0);
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white border border-gray-200 rounded-md p-4 shadow-sm flex flex-col">
          ...
          <span
            className={`text-2xl font-bold mt-1 font-mono tracking-tight ${reportData.summary.net >= 0 ? "text-[#059669]" : "text-[#dc2626]"}`}
          >
              ...
              if (row.isParty) {
                ...
                return (
                  <tr key={row.id} className={bgColor}>
                    <td className="px-3 py-2.5 pl-9 text-gray-700 font-medium">
                      {row.partyOrInvoice}
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-right font-mono text-[#d97706] font-medium">
                      {formatNumber(row.pendingAmount)}
                    </td>
            ...
            {reportData.groupedData.length === 0 && (
              <tr>
                <td colSpan={8} className="px-3 py-8 text-center text-gray-500 italic">
                  No pending bills found for the selected criteria.
                </td>
              </tr>
            )}
          </tbody>
