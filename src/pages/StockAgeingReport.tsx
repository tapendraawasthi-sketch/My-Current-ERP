...
import { Package, TrendingDown, AlertTriangle, Download, Search, Filter, RefreshCw } from "lucide-react";
...
export default function StockAgeingReport() {
  ...
  const ageingData = useMemo(() => {
    ...
    return items
      ...
      .map(item => {
        ...
        // FIFO consumption
        let outQtyRemaining = outMovements.reduce((s, m) => s + Number(m.quantity || m.qty || 0), 0);
        ...
        for (const inLot of inMovements) {
          ...
          const days = Math.floor((refDate.getTime() - new Date(inLot.date).getTime()) / (1000 * 60 * 60 * 24));
          const val = inLot.remainingQty * inLot.rate;
          if (days <= 30) { buckets.b0_30 += inLot.remainingQty; bucketValues.b0_30 += val; }
          else if (days <= 60) { buckets.b31_60 += inLot.remainingQty; bucketValues.b31_60 += val; }
          else if (days <= 90) { buckets.b61_90 += inLot.remainingQty; bucketValues.b61_90 += val; }
          else if (days <= 180) { buckets.b91_180 += inLot.remainingQty; bucketValues.b91_180 += val; }
          else { buckets.b180plus += inLot.remainingQty; bucketValues.b180plus += val; }
        }
      ...
      .filter(row => {
        ...
        if (categoryFilter === "NEAR-DEAD") return (row.buckets.b180plus / Math.max(row.totalQty, 1)) > 0.5;
  ...
  }, [items, stockMovements, referenceDate, groupFilter, warehouseFilter, categoryFilter, searchTerm, sortBy, sortDirection]);
  ...
  const summary = useMemo(() => {
    ...
    const value91_180plus = ageingData.reduce((sum, row) => sum + row.bucketValues.b91_180 + row.bucketValues.b180plus, 0);
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(ageingData.map(row => ({
    ...
    <div style={{ backgroundColor: BG, minHeight: '100vh', padding: '20px' }}>
      <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          <h1 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', margin: 0 }}>Stock Ageing Report</h1>
          
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'end' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '3px', fontSize: '11px', fontWeight: 'bold' }}>Item Group</label>
              <select
                value={groupFilter}
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '3px', fontSize: '11px', fontWeight: 'bold' }}>Warehouse</label>
              <select
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '3px', fontSize: '11px', fontWeight: 'bold' }}>Reference Date</label>
              <input
                type="date"
                value={referenceDate}
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '3px', fontSize: '11px', fontWeight: 'bold' }}>Category</label>
              <select
        ...
        <div style={{ display: 'flex', gap: '15px', marginTop: '15px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <input
              ...
              style={{ padding: '4px 8px', border: BORDER, borderRadius: '4px', fontSize: '11px', width: '200px' }}
            />
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '20px' }}>
        <div style={{ backgroundColor: BG_CARD, padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Total Stock Value</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{money(summary.totalValue)}</div>
        </div>
        <div style={{ backgroundColor: '#dcfce7', padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>0-30 Days Value</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#166534' }}>{money(summary.value0_30)}</div>
        </div>
        <div style={{ backgroundColor: '#fef9c3', padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Slow Moving (91+ Days)</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#854d0e' }}>{money(summary.value91_180plus)}</div>
        </div>
        <div style={{ backgroundColor: '#dc2626', padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: 'white', marginBottom: '5px' }}>Dead Stock Value</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: 'white' }}>{money(summary.deadStockValue)}</div>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            <thead>
              <tr style={{ backgroundColor: BG_HEADER }}>
                ...
                <th style={{ border: BORDER, padding: '8px', backgroundColor: '#dcfce7', color: '#166534', textAlign: 'center' }}>0-30 Days</th>
                <th style={{ border: BORDER, padding: '8px', backgroundColor: '#fef9c3', color: '#854d0e', textAlign: 'center' }}>31-60 Days</th>
                <th style={{ border: BORDER, padding: '8px', backgroundColor: '#fed7aa', color: '#9a3412', textAlign: 'center' }}>61-90 Days</th>
                <th style={{ border: BORDER, padding: '8px', backgroundColor: '#fecaca', color: '#991b1b', textAlign: 'center' }}>91-180 Days</th>
                <th style={{ border: BORDER, padding: '8px', backgroundColor: '#f87171', color: '#fff', textAlign: 'center' }}>180+ Days</th>
            ...
            <tbody>
              {ageingData.length > 0 ? (
                ageingData.map((row, idx) => (
                  <tr key={row.item.id} style={{ backgroundColor: idx % 2 === 0 ? BG_DEEP : 'transparent' }}>
                    ...
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{row.totalQty}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(row.avgRate)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(row.totalValue)}</td>
                ...
                <tr>
                  <td colSpan="12" style={{ border: BORDER, padding: '16px', textAlign: 'center', color: '#666' }}>
      ...
      {deadStockItems.length > 0 && (
        <div style={{ marginTop: '20px', backgroundColor: '#fee2e2', padding: '15px', borderRadius: '8px', border: BORDER }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <h2 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000' }}>DEAD STOCK SUMMARY</h2>
            <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#dc2626' }}>
              Total Dead Stock Value: {money(deadStockItems.reduce((sum, row) => sum + row.bucketValues.b180plus, 0))}
            </div>
          </div>
          
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: '#fecaca', padding: '10px', borderRadius: '6px', marginBottom: '10px' }}>
            <AlertTriangle size={16} style={{ color: '#991b1b' }} />
            <span style={{ color: '#991b1b', fontWeight: 'bold' }}>RECOMMENDED ACTION: Markdown or Write-Off</span>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
              <thead>
                <tr style={{ backgroundColor: '#f87171', color: '#fff' }}>
                  ...
                  <th style={{ border: BORDER, padding: '8px', color: '#fff', textAlign: 'right' }}>Dead Qty</th>
                  <th style={{ border: BORDER, padding: '8px', color: '#fff', textAlign: 'right' }}>Dead Value</th>
              ...
              <tbody>
                {deadStockItems.map(row => (
                  <tr key={`dead-${row.item.id}`}>
                    <td style={{ border: BORDER, padding: '8px' }}>{row.item.code}</td>
                    <td style={{ border: BORDER, padding: '8px' }}>{row.item.name}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{row.buckets.b180plus}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right', color: '#dc2626', fontWeight: 'bold' }}>{money(row.bucketValues.b180plus)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'center' }}>
                      <button
                        onClick={() => {}}
                        style={{
