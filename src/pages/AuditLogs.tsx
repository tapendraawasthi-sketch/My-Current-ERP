...
const ACTION_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  ...
  PHYSICAL_STOCK_COUNT: { bg: "bg-orange-50", text: "text-orange-600", border: "border-orange-200" },
...
function exportToCsv(logs: DBAuditLog[]): void {
  const headers = [
    "ID", "Timestamp", "User", "User Role", "Action",
    "Entity Type", "Entity ID", "Entity Name", "Change Description",
    "Severity", "Checksum",
  ];
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded border ${actionColor.bg} ${actionColor.text} ${actionColor.border}`}>
              {log.action}
            </span>
            <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded ${SEVERITY_BADGE[log.severity] ?? "bg-gray-100 text-gray-700"}`}>
              {log.severity}
            </span>
          ...
          <div className="grid grid-cols-2 gap-3 text-[12px]">
            <div className="col-span-2">
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-0.5">Timestamp</p>
              <p className="text-gray-800 font-mono">{formatTimestamp(log.timestamp)}</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-0.5">User</p>
              <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-0.5">Role</p>
              <p className="text-gray-700">{log.userRole || "—"}</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-0.5">Entity ID</p>
              <p className="text-gray-700 font-mono text-[11px] break-all">{log.entityId || "—"}</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-0.5">Entity Name</p>
              <p className="text-gray-700">{log.entityName || "—"}</p>
            </div>
            {log.sessionId && (
              <div className="col-span-2">
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-0.5">Session ID</p>
          ...
          {/* Change Description */}
          <div>
            <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Change Description</p>
          ...
          {log.oldValue && (
            <div>
              <button type="button" onClick={() => setShowOld((v) => !v)}
                ...
                <ChevronRight className={`h-3.5 w-3.5 transition-transform ${showOld ? "rotate-90" : ""}`} />
          ...
          {log.newValue && (
            <div>
              <button type="button" onClick={() => setShowNew((v) => !v)}
                className="flex items-center gap-1.5 text-[11px] font-semibold text-gray-600 hover:text-gray-800 mb-1">
                <ChevronRight className={`h-3.5 w-3.5 transition-transform ${showNew ? "rotate-90" : ""}`} />
          ...
          {log.checksum && (
            <div>
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-0.5">Integrity Checksum</p>
...
const AuditLogs: React.FC = () => {
  ...
  const actionOptions = useMemo(() => Array.from(new Set(allLogs.map((l) => l.action))).sort(), [allLogs]);
  const entityTypeOptions = useMemo(() => Array.from(new Set(allLogs.map((l) => l.entityType))).sort(), [allLogs]);
  ...
  const todayCount = useMemo(() => allLogs.filter((l) => l.timestamp?.startsWith(todayISODate())).length, [allLogs]);
  ...
  useEffect(() => { setPage(1); }, [search, actionFilter, entityTypeFilter, severityFilter, fromDate, toDate, pageSize]);

  const clearFilters = () => {
    setSearch(""); setActionFilter(""); setEntityTypeFilter("");
    setSeverityFilter(""); setFromDate(""); setToDate("");
  };

  const hasActiveFilters = !!(search || actionFilter || entityTypeFilter || severityFilter || fromDate || toDate);
    ...
    <div className="p-4 md:p-6 bg-[#f5f6fa] min-h-screen">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">Immutable compliance ledger — all system actions recorded</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => exportToCsv(filteredLogs)} disabled={filteredLogs.length === 0}
      ...
      <div className="flex items-center justify-between mb-3 px-1">
        <p className="text-[11px] text-gray-500 font-medium">
          ...
          <span className="font-semibold text-gray-700">
            {Math.min((page - 1) * pageSize + 1, filteredLogs.length)}–{Math.min(page * pageSize, filteredLogs.length)}
          </span>{" "}
            ...
            <table className="w-full min-w-[900px]">
              <thead>
                <tr className="bg-[#f5f6fa] border-b border-gray-200">
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-36">Timestamp</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-28">User</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-28">Action</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-28">Entity Type</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-32">Entity Name</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Change Description</th>
                  <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-20">Severity</th>
                  <th className="px-3 py-2.5 text-center text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-14">View</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {paginatedLogs.map((log) => {
                  const actionColor = getActionColor(log.action);
                  return (
                    <tr key={log.id} className="hover:bg-[#f5f6fa] transition-colors cursor-pointer"
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[12px] text-gray-700 truncate max-w-[80px]" title={log.userName}>
                            {log.userName || "—"}
                          </span>
                        </div>
                      </td>
                      <td className="px-3 py-2.5">
                        <span className={`inline-block px-2 py-0.5 text-[10px] font-semibold uppercase rounded border ${actionColor.bg} ${actionColor.text} ${actionColor.border}`}>
                          {log.action}
                        </span>
                      </td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="px-3 py-2.5 text-[12px] text-gray-700 max-w-[120px]">
                        <span className="truncate block" title={log.entityName}>{log.entityName || "—"}</span>
                      </td>
                      ...
                      <td className="px-3 py-2.5">
                        <span className={`inline-block px-2 py-0.5 text-[10px] font-semibold uppercase rounded ${SEVERITY_BADGE[log.severity] ?? "bg-gray-100 text-gray-700"}`}>
                          {log.severity}
                        </span>
                      </td>
                      <td className="px-3 py-2.5 text-center" onClick={(e) => { e.stopPropagation(); setSelectedLog(log); }}>
                        <button type="button"
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-[#f5f6fa]">
            ...
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => setPage(1)} disabled={page === 1}
                className="h-7 w-7 flex items-center justify-center rounded border border-gray-300 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="First page">
                <span className="text-[11px] font-bold">«</span>
              </button>
              <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                className="h-7 w-7 flex items-center justify-center rounded border border-gray-300 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="Previous page">
              ...
              <button type="button" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                className="h-7 w-7 flex items-center justify-center rounded border border-gray-300 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="Next page">
                <ChevronRight className="h-3.5 w-3.5" />
              </button>
              <button type="button" onClick={() => setPage(totalPages)} disabled={page === totalPages}
                className="h-7 w-7 flex items-center justify-center rounded border border-gray-300 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" title="Last page">
                <span className="text-[11px] font-bold">»</span>
              </button>
            </div>
          </div>
