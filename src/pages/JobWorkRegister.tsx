...
import { Factory, Package, ArrowRight, ArrowLeft, FileText, Plus, Edit2, Trash2, CheckCircle, Clock, AlertTriangle, Download, Printer } from "lucide-react";
...
export default function JobWorkRegister() {
  ...
  const summaryCounts = useMemo(() => {
    const openOrders = jobWorkOrders.filter(o => o.status === "Sent" || o.status === "In-Process").length;
    const overdueOrders = jobWorkOrders.filter(o => 
      o.expectedReturnDate && 
      new Date(o.expectedReturnDate) < new Date() && 
      o.status !== "Completed"
    ).length;
    const completedThisMonth = jobWorkOrders.filter(o => 
      o.status === "Completed" && 
  ...
  const handleSaveInReceipt = async () => {
    ...
    try {
      ...
      await db.jobWorkOrders.update(inForm.againstOrderId, { status: "Completed", completedAt: new Date().toISOString() });
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(jobWorkOrders.map(order => ({
  ...
  const renderJobWorkOut = () => (
    <div style={{ padding: '20px' }}>
      <div style={{ backgroundColor: BG_CARD, padding: '20px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '15px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Order No</label>
            <input
              type="text"
              value={editingOrder?.orderNo || `JWO-${new Date().toISOString().split('T')[0]}-TEMP`}
              disabled
              style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px', backgroundColor: '#f0f0f0' }}
            />
          </div>
          <div>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Job Worker</label>
            <select
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Expected Return Date</label>
            <input
              type="date"
          ...
          <div style={{ gridColumn: 'span 2' }}>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Description of Work</label>
            <textarea
        ...
        <h3 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>Materials Sent</h3>
                ...
                <tr key={index}>
                  <td style={{ border: BORDER, padding: '4px' }}>
                    <select
                      ...
                      style={{ width: '100%', padding: '4px', border: BORDER, borderRadius: '4px', fontSize: '11px' }}
                    >
                  ...
                  <td style={{ border: BORDER, padding: '4px' }}>
                    <input
                      type="number"
                      value={line.qty}
                      onChange={(e) => updateMaterialLine(index, 'qty', parseFloat(e.target.value) || 0)}
                      style={{ width: '100%', padding: '4px', border: BORDER, borderRadius: '4px', fontSize: '11px' }}
                    />
                  </td>
                  <td style={{ border: BORDER, padding: '4px' }}>
                  ...
                  <td style={{ border: BORDER, padding: '4px' }}>
                    <input
                      type="number"
                      value={line.rate}
                      onChange={(e) => updateMaterialLine(index, 'rate', parseFloat(e.target.value) || 0)}
                      style={{ width: '100%', padding: '4px', border: BORDER, borderRadius: '4px', fontSize: '11px' }}
                    />
                  </td>
                  <td style={{ border: BORDER, padding: '4px', textAlign: 'right' }}>{money(line.amount)}</td>
                  <td style={{ border: BORDER, padding: '4px' }}>
                    <input
                      type="text"
                      value={line.notes}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '15px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Total Material Value</label>
            <input
              type="text"
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Status</label>
            <select
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Delivery Challan No</label>
            <input
              type="text"
      ...
      <div style={{ backgroundColor: BG_CARD, padding: '20px', borderRadius: '8px', border: BORDER }}>
        <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Existing Orders</h2>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            ...
            <tbody>
              {jobWorkOrders.filter(o => o.type === "out").map(order => {
                const isOverdue = order.expectedReturnDate && new Date(order.expectedReturnDate) < new Date() && order.status !== "Completed";
                const daysPending = order.expectedReturnDate ? 
                  Math.ceil((new Date(order.expectedReturnDate) - new Date()) / (1000 * 60 * 60 * 24)) : 0;
                
                return (
                  <tr key={order.id} style={{ backgroundColor: isOverdue ? OVERDUE_BG : (order.status === "Completed" ? COMPLETE_BG : PENDING_BG) }}>
                    ...
                    <td style={{ border: BORDER, padding: '8px' }}>{order.materials?.length || 0}</td>
                    ...
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'center' }}>{daysPending}</td>
  ...
  const renderJobWorkIn = () => (
    <div style={{ padding: '20px' }}>
      <div style={{ backgroundColor: BG_CARD, padding: '20px', borderRadius: '8px', border: BORDER, marginBottom: '20px' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Job Work In Receipt</h2>
        
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '15px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Receipt No</label>
            <input
              type="text"
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Against Job Work Order</label>
            <select
            ...
            >
              ...
              {jobWorkOrders.filter(o => o.type === "out" && o.status !== "Completed").map(order => (
                <option key={order.id} value={order.id}>{order.orderNo} - {order.jobWorkerName}</option>
              ))}
            </select>
          </div>
          <div>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Job Work Charges</label>
            <input
              type="number"
              value={inForm.jobWorkCharges}
              onChange={(e) => setInForm({...inForm, jobWorkCharges: parseFloat(e.target.value) || 0})}
              style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px' }}
            />
          </div>
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Job Work Account</label>
            <select
        ...
        <h3 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>Finished Goods Received</h3>
        <div style={{ overflowX: 'auto', marginBottom: '15px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            <thead>
              <tr style={{ backgroundColor: BG_HEADER }}>
                ...
                <tr key={index}>
                  ...
                  <td style={{ border: BORDER, padding: '4px' }}>
                    <input
                      type="number"
                      value={line.qty}
                      onChange={(e) => updateFinishedGoodLine(index, 'qty', parseFloat(e.target.value) || 0)}
                      style={{ width: '100%', padding: '4px', border: BORDER, borderRadius: '4px', fontSize: '11px' }}
                    />
                  </td>
                  <td style={{ border: BORDER, padding: '4px' }}>
                  ...
                  <td style={{ border: BORDER, padding: '4px' }}>
                    <input
                      type="number"
                      value={line.rate}
                      onChange={(e) => updateFinishedGoodLine(index, 'rate', parseFloat(e.target.value) || 0)}
                      style={{ width: '100%', padding: '4px', border: BORDER, borderRadius: '4px', fontSize: '11px' }}
                    />
                  </td>
                  <td style={{ border: BORDER, padding: '4px', textAlign: 'right' }}>{money(line.amount)}</td>
        ...
        <h3 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>Scrap Materials</h3>
        <div style={{ overflowX: 'auto', marginBottom: '15px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
            <thead>
              <tr style={{ backgroundColor: BG_HEADER }}>
                ...
                <tr key={index}>
                  ...
                  <td style={{ border: BORDER, padding: '4px' }}>
                    <input
                      type="number"
                      value={line.qty}
                      onChange={(e) => updateScrapLine(index, 'qty', parseFloat(e.target.value) || 0)}
                      style={{ width: '100%', padding: '4px', border: BORDER, borderRadius: '4px', fontSize: '11px' }}
                    />
                  </td>
                  <td style={{ border: BORDER, padding: '4px' }}>
                  ...
                  <td style={{ border: BORDER, padding: '4px' }}>
                    <input
                      type="number"
                      value={line.rate}
                      onChange={(e) => updateScrapLine(index, 'rate', parseFloat(e.target.value) || 0)}
                      style={{ width: '100%', padding: '4px', border: BORDER, borderRadius: '4px', fontSize: '11px' }}
                    />
                  </td>
                  <td style={{ border: BORDER, padding: '4px', textAlign: 'right' }}>{money(line.amount)}</td>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '15px' }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Scrap Amount</label>
            <input
              type="text"
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Net Job Work Cost</label>
            <input
              type="text"
              value={money(inForm.jobWorkCharges - inForm.scrapMaterials.reduce((sum, line) => sum + line.amount, 0))}
              disabled
              style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px', backgroundColor: '#f0f0f0' }}
            />
          </div>
    ...
    <div style={{ padding: '20px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '20px' }}>
        <div style={{ backgroundColor: BG_HEADER, padding: '15px', borderRadius: '6px', border: BORDER }}>
          ...
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{summaryCounts.openOrders}</div>
        </div>
        <div style={{ backgroundColor: OVERDUE_BG, padding: '15px', borderRadius: '6px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Overdue Orders</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#dc2626' }}>{summaryCounts.overdueOrders}</div>
        </div>
        <div style={{ backgroundColor: COMPLETE_BG, padding: '15px', borderRadius: '6px', border: BORDER }}>
          <div style={{ fontSize: '12px', color: '#000000', marginBottom: '5px' }}>Completed This Month</div>
          <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000' }}>{summaryCounts.completedThisMonth}</div>
        </div>
      </div>
      ...
      <div style={{ backgroundColor: BG_CARD, padding: '20px', borderRadius: '8px', border: BORDER }}>
        <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Job Work Register</h2>
              ...
              {filteredOrders.map(order => {
                ...
                return (
                  <tr key={order.id} style={{ backgroundColor: isOverdue ? OVERDUE_BG : (order.status === "Completed" ? COMPLETE_BG : PENDING_BG) }}>
                    ...
                    <td style={{ border: BORDER, padding: '8px' }}>{order.date}</td>
                    <td style={{ border: BORDER, padding: '8px' }}>{order.expectedReturnDate}</td>
                    <td style={{ border: BORDER, padding: '8px' }}>{order.finishedGoods?.length || 0}</td>
                    <td style={{ border: BORDER, padding: '8px' }}>
                      <span style={{
                        backgroundColor: order.status === "Completed" ? '#059669' : 
                                        order.status === "Cancelled" ? '#6b7280' : 
                    ...
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'center' }}>{pendingDays}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(order.jobWorkCharges || 0)}</td>
                    <td style={{ border: BORDER, padding: '8px', textAlign: 'right' }}>{money(order.scrapMaterials?.reduce((sum, line) => sum + line.amount, 0) || 0)}</td>
                  </tr>
                );
              })}
            </tbody>
  ...
  const renderJobWorkBill = () => (
    <div style={{ padding: '20px' }}>
      <div style={{ backgroundColor: BG_CARD, padding: '20px', borderRadius: '8px', border: BORDER }}>
        <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#000000', marginBottom: '15px' }}>Job Work Bill</h2>
        
        <div style={{ marginBottom: '15px' }}>
          <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '12px' }}>Select Completed Job Work Order</label>
          <select
            value=""
            onChange={() => {}}
            style={{ width: '100%', padding: '6px', border: BORDER, borderRadius: '4px', fontSize: '12px' }}
        ...
        <div style={{ border: BORDER, padding: '20px', marginBottom: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
            <div>
              <h3 style={{ fontSize: '16px', fontWeight: 'bold', color: '#000000', marginBottom: '5px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <h4 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>Materials Supplied</h4>
            <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
              <thead>
                <tr style={{ backgroundColor: BG_HEADER }}>
                  <th style={{ border: BORDER, padding: '6px' }}>Item</th>
          ...
          <div style={{ marginBottom: '20px' }}>
            <h4 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>Finished Goods Returned</h4>
            <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
              <thead>
                <tr style={{ backgroundColor: BG_HEADER }}>
                  <th style={{ border: BORDER, padding: '6px' }}>Item</th>
          ...
          <div style={{ marginBottom: '20px' }}>
            <h4 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>Scrap Recovered</h4>
            <table style={{ width: '100%', borderCollapse: 'collapse', border: BORDER }}>
              <thead>
                <tr style={{ backgroundColor: BG_HEADER }}>
                  <th style={{ border: BORDER, padding: '6px' }}>Item</th>
          ...
          <div style={{ marginBottom: '20px' }}>
            <h4 style={{ fontSize: '14px', fontWeight: 'bold', color: '#000000', marginBottom: '10px' }}>Services Rendered</h4>
            <div style={{ fontSize: '12px', color: '#000000', marginBottom: '10px' }}>
              Manufacturing and processing of raw materials into finished goods as per specifications provided.
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
            <div style={{ width: '300px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px', borderTop: BORDER, paddingTop: '5px' }}>
                <span><strong>Net Payable:</strong></span>
                <span><strong>Rs. 1,250.00</strong></span>
              </div>
            </div>
          </div>
  ...
  return (
    <div style={{ backgroundColor: BG, minHeight: '100vh' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#000000', padding: '20px', marginBottom: '0' }}>Job Work Register</h1>
      
      {/* Tab Navigation */}
      <div style={{ display: 'flex', gap: '5px', padding: '0 20px', marginBottom: '20px', borderBottom: BORDER }}>
        {[
