...
export default function MiscMasters() {
  ...
  const [mcForm, setMCForm] = useState<Omit<MaterialCentre, "id">>({
    code: "",
    name: "",
    address: "",
    isDefault: false,
  ...
  const [plForm, setPLForm] = useState<Omit<PriceListItem, "id">>({
    name: "",
    category: "A",
    description: "",
    isActive: true,
    ...
    <div className="p-4 bg-[#f5f6fa] min-h-screen">
      ...
      <div className="flex gap-1 mb-4 bg-white border border-gray-200 rounded-lg p-1 w-fit">
        {tabs.map((tab) => (
          ...
          >
            ...
            {tab.label}
            <span
              className={`text-[10px] px-1.5 py-0.5 rounded ${activeTab === tab.key ? "bg-white/20" : "bg-gray-100 text-gray-500"}`}
            >
              {tab.count}
            </span>
          </button>
        ...
        <>
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-[13px] font-semibold text-gray-700">
              Material Centres / Warehouses / Locations
            </h2>
            <button
              onClick={() => {
                setEditMC(null);
                setMCForm({ code: "", name: "", address: "", isDefault: false, isActive: true });
                setShowMCModal(true);
              }}
                ...
                {materialCentres.map((mc) => (
                  <tr key={mc.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-semibold ${mc.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                      >
      ...
      {activeTab === "bom" && (
        <div className="bg-white border border-gray-200 rounded-lg p-8 text-center">
          ...
          <p className="text-[11px] text-gray-400 mt-1">
            Define finished product recipes — what raw materials are consumed during production
          </p>
          <p className="text-[11px] text-gray-400 mt-2">
            Enable Manufacturing Feature in Configuration → Features/Options to use BOM + Production
            Vouchers
          </p>
      ...
      {activeTab === "price-lists" && (
        <>
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-[13px] font-semibold text-gray-700">
              Price Lists (A/B/C Categories)
            </h2>
            <button
              onClick={() => {
                setEditPL(null);
                setPLForm({ name: "", category: "A", description: "", isActive: true });
                setShowPLModal(true);
              }}
              className="h-8 px-3 bg-[#1557b0] hover:bg-[#0f4a96] text-white text-[12px] font-medium rounded-md flex items-center gap-1.5"
            >
              ...
              <tbody className="divide-y divide-gray-100">
                {priceLists.map((pl) => (
                  <tr key={pl.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2.5">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-semibold ${pl.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                      >
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-[15px] font-semibold text-gray-800">
                {editMC ? "Modify" : "Add"} Material Centre
              </h2>
              <button
