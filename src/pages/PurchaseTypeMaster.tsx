...
const DEFAULT_FORM = {
  ...
  taxRate: 13, surcharge: 0, addlCess: 0, purchaseAccountId: "",
...
export default function PurchaseTypeMaster() {
  const { purchaseTypes, accounts, addPurchaseType, updatePurchaseType, deletePurchaseType } = useStore();
  const [showForm, setShowForm] = useState(false);
  const [selected, setSelected] = useState<any>(null);
  const [search, setSearch] = useState("");
  const [form, setForm] = useState(DEFAULT_FORM);

  const filtered = (purchaseTypes || []).filter((s: any) => s.name.toLowerCase().includes(search.toLowerCase()));
  const purchaseAccounts = (accounts || []).filter((a: any) => !a.isGroup && (a.type === "expense" || a.type === "asset" || a.name?.toLowerCase().includes("purchase")));
  ...
  const handleEdit = (s: any) => {
    ...
    setForm({
      ...
      purchaseAccountId: s.purchaseAccountId || "", invoiceHeading: s.invoiceHeading || "",
  ...
  const handleSubmit = async () => {
    ...
    if (selected) { await updatePurchaseType(selected.id, form); alert("Purchase Type updated."); }
    else { await addPurchaseType(form); alert("Purchase Type saved."); }
    resetForm();
  };
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", borderRight: showForm ? BORDER : "none" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderBottom: BORDER, background: BG_HEADER }}>
          ...
          <button style={BTN("#3D6B25")} onClick={() => { resetForm(); setShowForm(true); }}>
            <Plus style={{ width: 12, height: 12, display: "inline", marginRight: 4 }} />Add Purchase Type
          </button>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ background: BG_HEADER, position: "sticky", top: 0 }}>
                {["#", "Purchase Type Name", "Region", "Taxation", "Tax Rate (%)", "Freeze Tax", "Actions"].map((h) => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: "center", padding: 24, color: "#666" }}>No purchase types defined. Examples: VAT/13%, VAT/Exempt, Capital Goods.</td></tr>
              ) : (
                  ...
                  <tr key={s.id} style={{ background: i % 2 === 0 ? "#fff" : BG_ROW_ALT }}>
                    ...
                    <td style={{ padding: "5px 10px", borderBottom: BORDER }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button style={{ ...BTN("#fff"), color: "#c00" }} onClick={async () => { if (confirm("Delete?")) await deletePurchaseType(s.id); }}><Trash2 style={{ width: 12, height: 12 }} /></button>
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <div style={{ padding: "4px 12px", borderTop: BORDER, background: BG_HEADER, fontSize: 11 }}>Total: {filtered.length} purchase type(s)</div>
      </div>
      ...
      {showForm && (
        <div style={{ width: 360, borderLeft: BORDER, display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", padding: "8px 12px", background: BG_HEADER, borderBottom: BORDER }}>
            <span style={{ fontWeight: 700, fontSize: 13 }}>{selected ? "Edit" : "Add"} Purchase Type</span>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
            <label style={{ fontSize: 12 }}>
              ...
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={INPUT_STYLE} placeholder="e.g. VAT/13%, Exempt, Capital Goods" />
            </label>
            <label style={{ fontSize: 12 }}>
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <select value={form.purchaseAccountId} onChange={(e) => setForm({ ...form, purchaseAccountId: e.target.value })} style={INPUT_STYLE}>
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
              <input type="checkbox" checked={form.isCapitalPurchase} onChange={(e) => setForm({ ...form, isCapitalPurchase: e.target.checked })} />
