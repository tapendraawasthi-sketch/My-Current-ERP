...
const DEFAULTS: Omit<PurchaseTypeMasterItem, "id">[] = [
  { name: "Local Purchase GST 13%", purchaseType: PurchaseType.LOCAL_GST_18, defaultTaxRate: 13, rcmApplicable: false, isDefault: true, isActive: true },
  { name: "Import / Central GST", purchaseType: PurchaseType.CENTRAL_GST_18, defaultTaxRate: 18, rcmApplicable: false, isDefault: false, isActive: true },
  { name: "Unregistered (RCM)", purchaseType: PurchaseType.RCM_UNREG, defaultTaxRate: 0, rcmApplicable: true, isDefault: false, isActive: true },
  { name: "Nil Rated Purchase", purchaseType: PurchaseType.NIL_RATED, defaultTaxRate: 0, rcmApplicable: false, isDefault: false, isActive: true },
];
...
export default function PurchaseTypeMaster() {
  ...
  const [form, setForm] = useState<Omit<PurchaseTypeMasterItem, "id">>({ name: "", purchaseType: PurchaseType.LOCAL_GST_18, defaultTaxRate: 13, rcmApplicable: false, isDefault: false, isActive: true });
  ...
  const openAdd = () => {
    setEditItem(null);
    setForm({ name: "", purchaseType: PurchaseType.LOCAL_GST_18, defaultTaxRate: 13, rcmApplicable: false, isDefault: false, isActive: true });
    setShowModal(true);
  };
    ...
    <div className="p-4 bg-[#f5f6fa] min-h-screen">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">Configure purchase types: Local GST, Central GST, RCM/Unregistered, etc.</p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[#f5f6fa] border-b border-gray-200">
              {["Name", "Purchase Type", "Tax Rate %", "RCM", "Default", "Status", "Actions"].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {items.map(item => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2.5 text-[12px] text-center">
                  {item.rcmApplicable ? <span className="px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded text-[10px] font-semibold">RCM</span> : "—"}
                </td>
            ...
            {items.length === 0 && <tr><td colSpan={7} className="py-12 text-center text-[12px] text-gray-500">No purchase types configured</td></tr>}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-[15px] font-semibold text-gray-800">{editItem ? "Modify Purchase Type" : "Add Purchase Type"}</h2>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="p-4 space-y-3">
              <div>
                <label className={labelCls}>Name *</label>
                <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className={`${inputCls} w-full`} placeholder="e.g. Local Purchase GST 18%" />
              </div>
              <div>
                <label className={labelCls}>Purchase Type</label>
                <select value={form.purchaseType} onChange={e => setForm(p => ({ ...p, purchaseType: e.target.value as PurchaseType }))} className={`${inputCls} w-full`}>
