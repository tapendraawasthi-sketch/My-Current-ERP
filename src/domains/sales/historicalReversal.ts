/**
 * Historical VAT and inventory-cost reversal from original sales facts (Phase 7).
...
export function computeHistoricalLineReversal(input: {
  balance: LineRemainingBalance;
  returnQuantity: number;
  taxRuleVersion: string | null;
  valuationMethod: string | null;
  stockCondition?: StockCondition | null;
  /** When true, absorb remaining VAT/revenue/cost rounding on this line */
  ...
  const revenue = proportionMoney(
    balance.original_line_total,
    returnQuantity,
    balance.original_quantity,
    balance.remaining_creditable_total,
    isFinal,
  );
  const taxable = proportionMoney(
    ...
    returnQuantity,
    balance.original_quantity,
    Math.max(0, balance.original_taxable - (balance.previously_credited_total - balance.previously_reversed_vat)),
    isFinal,
  );
  ...
  return {
    original_sales_line_id: balance.original_sales_line_id,
    item_id: balance.item_id,
    return_quantity: returnQuantity,
    revenue_reversal: revenue,
    taxable_reversal: Math.min(taxable, revenue),
    vat_reversal: vat,
    cost_reversal: cost,
...
/** Financial credit note: reverse amount from remaining creditable totals without stock. */
export function computeFinancialCreditReversal(input: {
  balance: LineRemainingBalance;
  creditAmount: number;
  taxRuleVersion: string | null;
}): HistoricalLineReversal {
  const { balance, creditAmount } = input;
  if (!(creditAmount > 0)) {
    throw Object.assign(new Error("Credit amount must be positive."), {
      code: "zero_credit_amount",
    });
  }
  if (creditAmount > balance.remaining_creditable_total + 0.009) {
    throw Object.assign(
      new Error(
        `Over-credit: requested ${creditAmount}, remaining ${balance.remaining_creditable_total}.`,
      ),
      { code: "over_credit_amount" },
    );
  }

  const ratio =
    balance.remaining_creditable_total > 0
      ? creditAmount / balance.remaining_creditable_total
      : 0;
  const vat = Math.min(
    balance.remaining_reversible_vat,
    Math.round(balance.remaining_reversible_vat * ratio * 100) / 100,
  );
  const taxable = Math.max(0, Math.round((creditAmount - vat) * 100) / 100);

  return {
    original_sales_line_id: balance.original_sales_line_id,
    item_id: balance.item_id,
    return_quantity: 0,
    revenue_reversal: creditAmount,
