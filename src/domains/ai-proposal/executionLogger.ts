...
function emit(level: LogLevel, message: string, context?: Record<string, unknown>): void {
  ...
  if (level === "error") console.error("[ai-execution]", entry);
  else if (level === "warn") console.warn("[ai-execution]", entry);
  else if (import.meta.env?.DEV) console.debug("[ai-execution]", entry);
}
